...
export const useMiniAppPagination = <TItem, TData extends PageData<TItem>>(
  ...
  const loadPage = useCallback(async (page: number, append: boolean) => {
    ...
    beginRequest(requestPath, { primary: !append });
    try {
      ...
      if (!append && apiError?.responseMeta) reportMeta(requestPath, apiError.responseMeta);
        ...
        if (apiError?.status === 401 || apiError?.status === 403) clearAvailability(requestPath);
  ...
  }, [beginRequest, clearAvailability, endpoint, finishRequest, pageSize, reportMeta]);
