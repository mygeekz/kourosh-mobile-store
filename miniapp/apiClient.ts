...
export const postMiniAppAction = async <T>(path: string, signal?: AbortSignal): Promise<MiniAppApiResult<T>> => {
  const token = getStoredMiniAppToken();
  if (!token) throw new MiniAppApiError("MINIAPP_AUTH_REQUIRED", "نشست Mini App موجود نیست.", 401);
  const response = await fetch(path, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    credentials: "same-origin",
...
export const authenticateMiniApp = async (
  initData: string,
  signal?: AbortSignal,
  workspaceKind?: "manager" | "customer" | "partner",
): Promise<MiniAppApiResult<MiniAppAuthData>> => {
  const response = await fetch("/api/miniapp/auth", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ initData, ...(workspaceKind ? { workspaceKind } : {}) }),
    credentials: "same-origin",
    signal,
  });
