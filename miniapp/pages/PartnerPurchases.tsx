            ...
            <article key={item.ref} className={`${MINIAPP_PREMIUM.card} p-4`}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1 text-start">
                  ...
                  {isPhone ? (item.identifier ? <MiniAppBidiText className="mt-0.5 block truncate text-[9px] text-premium-muted">{item.identifier}</MiniAppBidiText> : <span className="mt-0.5 block truncate text-[9px] text-premium-muted">شناسه ثبت نشده</span>) : <span className="mt-0.5 block truncate text-[9px] text-premium-muted">{item.quantity.toLocaleString("fa-IR")} {item.unit}</span>}
