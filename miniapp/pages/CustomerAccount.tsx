...
export const CustomerAccount: React.FC = () => {
  ...
  const d = query.data;
  return <CustomerPage id="account-title" title="حساب من" description="وضعیت حساب و گردش‌های ثبت‌شده فروشگاه">
    <CustomerQueryState query={query}>{d && <>
      ...
      <CustomerSection title="گردش‌های اخیر" description="مبالغ هر ردیف مطابق دفتر حساب؛ فهرست کامل سوابق نیست">
        {d.entries.length ? <CustomerList>{d.entries.map(entry => <CustomerRecord key={entry.id} record={entry.id} title={entry.description} detail={`${formatCustomerDate(entry.transactionDate)} · رکورد ${entry.id.toLocaleString("fa-IR")}`}>
        ...
        </CustomerRecord>)}</CustomerList> : <MiniAppDataState empty emptyText="هنوز گردش حسابی در این گزارش ثبت نشده است. ثبت‌های مربوط به حساب شما در این بخش نمایش داده می‌شوند." />}
      </CustomerSection>
    </>}</CustomerQueryState>
  </CustomerPage>;
};
