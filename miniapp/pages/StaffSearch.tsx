    ...
    const timer = window.setTimeout(() => {
      ...
      void fetchMiniAppData<StaffSearchData>(requestPath, controller.signal)
        ...
        .catch((error: unknown) => {
          ...
          else if (apiError?.status === 401 || apiError?.status === 403) clearAvailability(requestPath);
          else finishRequest(requestPath);
          setState({ data: null, loading: false, error: apiError?.message || "جستجو انجام نشد." });
        });
    }, 300);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [beginRequest, clearAvailability, finishRequest, query, reportMeta]);
