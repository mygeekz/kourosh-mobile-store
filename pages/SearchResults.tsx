import { Drawer, TextField } from '@/components/ui';
...
const SearchResultsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await apiFetch(`/api/search?q=${encodeURIComponent(normalizedQuery)}&limit=50`, {
      ...
      {selectedItem ? (() => {
        ...
        return (
          <Drawer
            ...
            title="پیش‌نمایش نتیجه"
            kicker={meta.label}
            ariaDescription="جزئیات، متن تطابق و اقدام‌های سریع نتیجه انتخاب‌شده."
            iconClass={meta.icon}
            size="sm"
            panelAttributes={{ 'data-search-result-preview': 'true' }}
          >
