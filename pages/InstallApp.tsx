...
const InstallApp: React.FC = () => {
  ...
  const readinessLabel = useMemo(() => {
    ...
    if (diagnostics && !diagnostics.productionRuntime) return 'نسخه قابل نصب برنامه فعال نیست.';
    if (diagnostics && !diagnostics.secureContext) return 'مرورگر این اتصال را امن تشخیص نداده است.';
    if (diagnostics && !diagnostics.manifestInstallable) return 'فایل‌ها یا آیکون‌های لازم برای نصب کامل بارگذاری نشده‌اند.';
    if (diagnostics?.serviceWorkerError) return diagnostics.serviceWorkerError;
    if (!workerReady) return 'سرویس اجرای برنامه در حال آماده‌سازی است.';
  ...
  const onPrimaryAction = async () => {
    ...
    if (inAppBrowser) {
      setMessage('از منوی سه‌نقطه بالای صفحه، «باز کردن در Chrome» را انتخاب کنید. نصب برنامه باید در مرورگر اصلی انجام شود.');
      return;
    }
    if (installed) {
    ...
    if (!latest.productionRuntime) {
      setMessage('نسخه قابل نصب برنامه اجرا نشده است. برنامه را ببندید و نسخه جدید را دوباره اجرا کنید.');
      return;
    }
    if (!latest.secureContext) {
      setMessage(`ابتدا از بخش «اعتماد اتصال امن این دستگاه» گواهی ریشه را روی ${platform.label} نصب کنید؛ سپس مرورگر را کامل ببندید و دوباره باز کنید.`);
      return;
    }
    if (!latest.manifestInstallable) {
      setMessage('فایل‌های لازم برای نصب در دسترس نیستند؛ برنامه را یک‌بار ببندید و دوباره اجرا کنید.');
      return;
    }
    if (ios) {
      setShowIOSHelp(true);
    ...
    if (!latest.serviceWorkerActive || !latest.serviceWorkerControlling || latest.serviceWorkerError) {
      ...
      if (outcome === 'controlled' || afterRepair.serviceWorkerControlling) {
        setMessage('سرویس اجرای برنامه با موفقیت فعال شد. چند لحظه صبر کنید تا گزینه نصب Chrome آماده شود.');
      } else if (outcome === 'pending') {
        setMessage('سرویس اجرای برنامه آماده شده و در حال فعال‌شدن است. صفحه را یک‌بار تازه‌سازی کنید.');
      } else if (outcome === 'unsupported') {
        setMessage('این مرورگر از قابلیت نصب برنامه پشتیبانی نمی‌کند؛ صفحه را با نسخه اصلی Chrome باز کنید.');
      } else {
        setMessage(afterRepair.serviceWorkerError || 'آماده‌سازی سرویس نصب کامل نشد؛ داده‌های سایت را در Chrome پاک و صفحه را دوباره باز کنید.');
      }
      return;
    }
  ...
  const diagnosticRows = [
    ...
    },
    {
      ...
      hint: diagnostics?.productionRuntime ? 'نسخه آماده نصب برنامه در حال اجرا است.' : 'این نسخه برای نصب روی دستگاه مناسب نیست.',
    },
    {
      label: 'اتصال امن',
      value: diagnostics?.secureContext === true,
      hint: diagnostics?.secureContext ? 'امکانات امن مرورگر فعال‌اند.' : 'گواهی یا آدرس هنوز مورد اعتماد نیست.',
    },
    {
      label: 'فایل‌های نصب',
      value: diagnostics?.manifestInstallable === true,
      hint: diagnostics?.manifestInstallable ? 'فایل‌ها و آیکون‌های لازم برای نصب آماده‌اند.' : 'فایل‌ها یا آیکون‌های نصب ناقص‌اند.',
    },
    {
      label: 'سرویس نصب',
      value: workerReady,
  ...
  return (
    <AuthPageShell
      ...
      footer="Kourosh Local · شبکه داخلی فروشگاه"
    >
      <div
                ...
                {platform.family === 'mobile' && connectionCoreReady
                  ...
                  : connectionHealth.setupRequired === true
                    ...
                    : connectionCoreReady && !connectionHealth.hostDevice
                      ? 'QR فقط روی رایانه‌ای نمایش داده می‌شود که برنامه کوروش روی آن در حال اجراست.'
