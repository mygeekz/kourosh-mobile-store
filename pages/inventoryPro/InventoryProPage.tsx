import { Table, Button, CheckboxField, DataTableShell, SearchableSelectField, TableActionGroup, TextField, TextareaField } from '@/components/ui';
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button type="button" onClick={() => setOpenCats(true)} variant="secondary" size="sm">مدیریت دسته‌بندی</Button>
          <Button type="button" onClick={() => setOpenSuppliers(true)} variant="secondary" size="sm">مدیریت تأمین‌کننده</Button>
          <Button type="button" onClick={openAddProduct} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-plus" />}>افزودن مورد جدید کالا</Button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-zinc-900/60 backdrop-blur p-3">
        <div className="flex gap-2 flex-wrap items-center justify-between">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <label className="flex items-center gap-2 text-xs font-extrabold opacity-80">
              <CheckboxField controlOnly checked={onlyLowStock} onChange={(e) => setOnlyLowStock(e.target.checked)} />
              فقط کمبود موجودی
            </label>
          </div>
        ...
        <DataTableShell className="mt-3" data-ui-inventory-pro-products-table="true">
          <Table layout="managed" density="comfortable" className="min-w-[980px] w-full text-sm">
            <thead>
                ...
                filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="border-t border-black/5 dark:border-white/10">
                      ...
                      <td className="py-2">
                        <TableActionGroup
                          ariaLabel={`عملیات موجودی ${p.name}`}
                          collapseBelow="lg"
                          actions={[
                            {
                              key: 'edit',
                            ...
                            },
                            {
                              key: 'increase',
                              kind: 'button',
                              label: 'افزایش یک واحد',
                              tooltip: 'افزایش یک واحد موجودی',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <SearchableSelectField
              ...
              onValueChange={(value) => setForm((current: any) => ({ ...current, categoryId: value ? String(value) : '' }))}
              ...
              ariaLabel="جستجو و انتخاب دسته‌بندی کالا"
              clearable
            />
          </div>
          ...
          <div>
            ...
            <SearchableSelectField
              value={form.partnerId ? String(form.partnerId) : null}
              onValueChange={(value) => setForm((current: any) => ({ ...current, partnerId: value ? String(value) : '' }))}
              options={suppliers.map((supplier) => ({
                value: String(supplier.id),
                label: supplier.partnerName,
                searchText: `${supplier.partnerName} ${supplier.phoneNumber || ''} ${supplier.id}`,
              }))}
              placeholder="نام، شماره تماس یا کد تأمین‌کننده را جستجو کنید…"
              noOptionsMessage="تأمین‌کننده مطابق جستجو پیدا نشد"
              ariaLabel="جستجو و انتخاب تأمین‌کننده کالا"
              clearable
            />
          </div>
          ...
          <div className="md:col-span-2 flex justify-end gap-2">
            <Button type="button" onClick={() => setOpenProduct(false)} variant="secondary" size="sm">لغو</Button>
            <Button type="button" onClick={saveProduct} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-floppy-disk" />}>ذخیره تغییرات</Button>
          </div>
        </div>
      </ProModal>
      ...
      <ProModal open={openCats} title="مدیریت دسته‌بندی‌ها" onClose={() => setOpenCats(false)}>
        <div className="flex gap-2 flex-wrap">
          ...
          />
          <Button type="button" onClick={addCategory} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-plus" />}>افزودن مورد جدید</Button>
        </div>
        ...
        <div className="mt-4 space-y-2">
          {categories.map((c) => (
            <div key={c.id} className="flex items-center gap-2 rounded-xl border border-black/10 dark:border-white/10 p-2">
              ...
              />
              <Button type="button" onClick={() => updateCategory(c.id)} variant="secondary" size="xs" leftIcon={<i className="fa-solid fa-floppy-disk" />}>ذخیره تغییرات</Button>
              <Button type="button" onClick={() => deleteCategory(c.id)} variant="danger" size="xs" leftIcon={<i className="fa-solid fa-trash" />}>حذف مورد</Button>
            </div>
          ))}
        </div>
      </ProModal>
      ...
      <ProModal open={openSuppliers} title="مدیریت تأمین‌کنندگان" onClose={() => setOpenSuppliers(false)}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          />
          <Button type="button" onClick={addSupplier} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-plus" />}>افزودن مورد جدید</Button>
        </div>
        ...
        <DataTableShell className="mt-4" data-ui-inventory-pro-suppliers-table="true">
          <Table layout="managed" density="comfortable" className="min-w-[800px] w-full text-sm">
            <thead>
              <tr className="text-xs opacity-70">
          ...
          <div className="px-3 pb-3 text-xs opacity-70 font-bold mt-2">
