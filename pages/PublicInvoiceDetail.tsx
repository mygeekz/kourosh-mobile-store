...
const PublicInvoiceDetail: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    printArea('#public-invoice-print-area', { extraCss: BASE_CSS, title: `فاکتور ${invoice?.invoiceMetadata?.invoiceNumber ?? ""}` });
  };
    ...
    <div className="p-4 md:p-6 space-y-4" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={handlePrint} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-print" />}>
            چاپ
          </Button>
          <Button onClick={downloadPdf} variant="secondary" size="sm" leftIcon={<i className="fa-solid fa-file-pdf" />}>
            دانلود PDF
          </Button>
        </div>
      </div>

      <div id="public-invoice-print-area" ref={invRef} className="bg-white rounded-2xl shadow-sm border p-4 md:p-6">
        <div className="inv">
...
<div className="inv__brandbar" />
          <div className="inv__header">
            <div className="inv__header-inner">
              ...
              <div className="inv__meta">
                ...
                {String((m as any).status || '').toLowerCase() === 'canceled' ? (
                  <>
                    <div className="inv__meta-row"><span className="inv__meta-k">وضعیت</span><span className="inv__meta-v">باطل شده</span></div>
                    {String((m as any).cancelReason || '').trim() ? (
                      <div className="inv__meta-row"><span className="inv__meta-k">دلیل ابطال</span><span className="inv__meta-v">{String((m as any).cancelReason).trim()}</span></div>
                    ) : null}
                  </>
                ) : null}
              </div>
          ...
          {String((m as any).status || '').toLowerCase() === 'canceled' && String((m as any).cancelReason || '').trim() ? (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold leading-7 text-rose-800">
              دلیل ابطال فاکتور: {String((m as any).cancelReason).trim()}
            </div>
          ) : null}
