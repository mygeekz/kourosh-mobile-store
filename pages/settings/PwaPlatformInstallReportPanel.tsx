...
const CHECK_LABELS: Record<string, string> = {
  ...
  noBrowserPageErrors: 'نبود خطای اجرایی مرورگر',
};
...
const PwaPlatformInstallReportPanel: React.FC = () => {
  ...
  const loadReport = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.success) throw new Error(payload.message || 'دریافت گزارش نصب برنامه ناموفق بود.');
      ...
      setError(loadError instanceof Error ? loadError.message : 'گزارش نصب برنامه قابل دریافت نیست.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <section id="pwa-platform-install-quality" className="style-button-report" data-ui-pwa-platform-install-report="true">
      <header className="style-button-report__header">
        <div>
          <strong><i className="fa-solid fa-display" aria-hidden="true" /> نتیجه آخرین بررسی نصب برنامه</strong>
        ...
        <Button
          ...
          loading={loading}
          loadingText="در حال بازخوانی برنامه نصب‌شونده…"
      ...
      {!loading && !error && !report ? (
        <div className="style-button-report__empty">
          ...
          <div>
            <strong>هنوز گزارشی برای بررسی نصب برنامه ثبت نشده است</strong>
            <span>هنوز نتیجه‌ای برای بررسی نصب برنامه ثبت نشده است. پس از اجرای بررسی خودکار سیستم، نتیجه از همین صفحه قابل بازخوانی خواهد بود.</span>
      ...
      {report ? (
        <>
          ...
          <details ref={detailsRef} className="style-control-advanced" data-ui-style-report-details="true">
            <summary>
              <span><i className="fa-solid fa-list-check" aria-hidden="true" />جزئیات و تصاویر گزارش برنامه نصب‌شونده</span>
              <i className="fa-solid fa-chevron-down" aria-hidden="true" />
            </summary>
            <div className="grid gap-3 px-3 pb-3">
              <div className="style-button-report__filters">
                <label><span>وضعیت</span><SelectField controlOnly unstyled showChevron={false} icon={false} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as ReportStatusFilter)} aria-label="فیلتر وضعیت گزارش برنامه نصب‌شونده"><option value="failed">فقط خطاها</option><option value="passed">فقط موفق‌ها</option><option value="all">همه نتایج</option></SelectField></label>
                <label><span>سیستم‌عامل</span><SelectField controlOnly unstyled showChevron={false} icon={false} value={platformFilter} onChange={(event) => setPlatformFilter(event.target.value)} aria-label="فیلتر سیستم‌عامل گزارش برنامه نصب‌شونده"><option value="all">همه سیستم‌عامل‌ها</option>{platformSummaries.map((item) => <option key={item.platform} value={item.platform}>{item.label}</option>)}</SelectField></label>
                <label><span>وضعیت نصب</span><SelectField controlOnly unstyled showChevron={false} icon={false} value={installedFilter} onChange={(event) => setInstalledFilter(event.target.value as InstalledFilter)} aria-label="فیلتر وضعیت نصب گزارش برنامه نصب‌شونده"><option value="all">نصب‌شده و نصب‌نشده</option><option value="installed">نصب‌شده</option><option value="not-installed">نصب‌نشده</option></SelectField></label>
              </div>
                      ...
                      <article key={scenarioKey} className="style-button-report__result" data-status={result.passed ? 'passed' : 'failed'}>
                        ...
                        {!result.passed ? (
                          <div className="style-button-report__failed-checks">
                            ...
                            <strong>{failedChecks.length ? failedChecks.join('، ') : result.error || 'جزئیات این مورد در گزارش بررسی ثبت شده است.'}</strong>
                          </div>
                        ) : null}
                            ...
                            <React.Fragment key={key}>
                              ...
                              {imageUrl ? (
                                <a className="style-button-report__screenshot" href={imageUrl} target="_blank" rel="noreferrer">
                                  <img src={imageUrl} alt={`تصویر بررسی ${kind === 'login' ? 'ورود' : 'نصب'} ${result.platformLabel} ${result.installed ? 'نصب‌شده' : 'نصب‌نشده'}`} loading="lazy" />
