...
export type PricingLearningApiResult = {
  data?: {
    ...
    total?: number;
    generatedAt?: string;
    sourceBreakdown?: { salesOrders?: number; standalonePhones?: number };
  } | PricingLearningItem[];
  items?: PricingLearningItem[];
};
...
const SERVER_DERIVED_PRICING_SOURCES = new Set(['sales-order-phone', 'phone-sales-history', 'pricing-history']);
...
export const loadPricingLearningItems = (): PricingLearningItem[] => {
  ...
  try {
    ...
    return parsed.filter((item) => item && !SERVER_DERIVED_PRICING_SOURCES.has(String(item.source || '')));
  } catch {
    return [];
  }
};
...
export const parsePricingDateTime = (value: PricingDateInput): number => {
  ...
  const sqliteDateTimeMatch = normalized.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2}(?::\d{2})?)$/);
  const candidate = sqliteDateTimeMatch ? `${sqliteDateTimeMatch[1]}T${sqliteDateTimeMatch[2]}` : normalized;
  const direct = new Date(candidate).getTime();
...
export const buildPricingLearningFromPhones = (phones: PhoneEntry[], settings: PricingIntelligenceSettings): PricingLearningItem[] => {
  ...
  return (Array.isArray(phones) ? phones : [])
    .filter((phone) => {
      ...
      const isSold = Boolean(phone?.saleDate) || status.includes('فروخته');
      return isSold && Number(phone?.purchasePrice || 0) > 0 && Number(phone?.salePrice || 0) > 0;
    })
    .map((phone) => {
