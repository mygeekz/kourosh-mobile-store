...
import { Button, CheckboxField, FormGrid, FormGridItem, SearchableSelectField, TextField, TextareaField, requiredTextError } from '@/components/ui';
...
const StoreOwnershipPage: React.FC = () => {
  ...
  const [partnerFormErrors, setPartnerFormErrors] = useState<FormErrors>({});
  ...
  const handleCreateStorePartner = async (e: React.FormEvent) => {
    ...
    const nameError = requiredTextError(normalizedName, 'نام شریک');
    if (nameError) {
      const errors: FormErrors = { newPartnerName: nameError };
      setPartnerFormErrors(errors);
      setError(null);
      focusErrorsSoon(errors, { newPartnerName: 'store-partner-name' });
      return;
    }
    setPartnerFormErrors({});
    setSubmitting(true);
  ...
  const handleCreateProfitProfile = async (event: React.FormEvent) => {
    ...
    const titleError = requiredTextError(newProfitProfileTitle, 'عنوان پروفایل تسهیم سود');
    if (titleError) {
      const errors: FormErrors = { newProfitProfileTitle: titleError };
      setProfileFormErrors((current) => ({ ...current, ...errors }));
      focusErrorsSoon(errors, { newProfitProfileTitle: 'profit-profile-title' });
      return;
    }
    if (!isShareTotalValid) {
      setError(`جمع سهم شرکا باید دقیقاً ۱۰۰٪ باشد؛ مقدار فعلی ${percentFa(totalShareDraft)} است.`);
    ...
    setProfileFormErrors((current) => ({ ...current, newProfitProfileTitle: undefined }));
    setSubmitting(true);
  ...
  const handleCreateOwnershipProfile = async (event: React.FormEvent) => {
    event.preventDefault();
    const titleError = requiredTextError(newOwnershipProfileTitle, 'عنوان پروفایل مالکیت');
    if (titleError) {
      const errors: FormErrors = { newOwnershipProfileTitle: titleError };
      setProfileFormErrors((current) => ({ ...current, ...errors }));
      focusErrorsSoon(errors, { newOwnershipProfileTitle: 'ownership-profile-title' });
      return;
    }
    if (!isShareTotalValid) {
      setError(`جمع سهم شرکا باید دقیقاً ۱۰۰٪ باشد؛ مقدار فعلی ${percentFa(totalShareDraft)} است.`);
    ...
    setProfileFormErrors((current) => ({ ...current, newOwnershipProfileTitle: undefined }));
    setSubmitting(true);
  ...
  const handleAssignReviewItems = async (targetType: 'phones' | 'products') => {
    ...
    const approved = await confirmAction({
      ...
      description: `موارد انتخاب‌شده به پروفایل «${profile?.title || `#${ownershipProfileId}`}» متصل می‌شوند و سابقه این تصمیم برای پیگیری ثبت خواهد شد.`,
                ...
                <div className="flex items-center justify-center">
                  <div className="relative flex h-[260px] w-[260px] items-center justify-center rounded-full border border-slate-200 bg-white shadow-inner dark:border-slate-800 dark:bg-slate-950" style={{ background: donutBackground }}>
                    <div className="flex h-[118px] w-[118px] flex-col items-center justify-center rounded-full border border-slate-200 bg-white text-center dark:border-slate-800 dark:bg-slate-950">
                      ...
                      <div className="mt-2 text-xs font-black tracking-[0.16em] text-slate-500 dark:text-slate-400">پیش‌فرض</div>
    ...
    return (
      <div className="store-ownership-tab-stack">
        ...
        <div className="store-ownership-two-column">
          ...
          >
            <form noValidate className="store-ownership-form" onSubmit={handleCreateStorePartner}>
              <FormErrorSummary errors={partnerFormErrors} labels={{ newPartnerName: 'نام شریک' }} fieldIdMap={{ newPartnerName: 'store-partner-name' }} />
              <FormGrid columns={2}>
                <TextField id="store-partner-name" label="نام شریک" required value={newPartnerName} onChange={(e) => { setNewPartnerName(e.target.value); if (partnerFormErrors.newPartnerName) setPartnerFormErrors((current) => ({ ...current, newPartnerName: undefined })); }} placeholder="مثلاً علیرضا هلیلی" maxLength={120} error={partnerFormErrors.newPartnerName} />
                <TextField label="کد داخلی اختیاری" value={newPartnerCode} onChange={(e) => setNewPartnerCode(e.target.value)} placeholder="مثلاً P-001" maxLength={64} valueKind="code" />
                <FormGridItem span="full">
                  <TextareaField label="یادداشت" value={newPartnerNotes} onChange={(e) => setNewPartnerNotes(e.target.value)} placeholder="توضیح کوتاه درباره نقش یا قرارداد شریک" maxLength={1000} />
                </FormGridItem>
                <FormGridItem span="full" className="store-ownership-field-group">
                ...
                <FormGridItem span="full" className="store-ownership-form__actions">
                  <Button type="submit" variant="primary" size="xs" className={buttonPrimary} disabled={submitting} leftIcon={<i className={`fa-solid ${busyAction?.startsWith('partner-') ? 'fa-spinner fa-spin' : editingPartnerId ? 'fa-floppy-disk' : 'fa-user-plus'}`} />}>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <SectionBlock title="پروفایل‌های تسهیم سود" description="قواعد ثبت‌شده واقعی و یک الگوی جدید بر پایه درصدهای فعلی.">
          <form noValidate className="store-ownership-inline-create" onSubmit={handleCreateProfitProfile}>
            <TextField id="profit-profile-title" label="عنوان پروفایل تسهیم سود" required value={newProfitProfileTitle} onChange={(event) => { setNewProfitProfileTitle(event.target.value); if (profileFormErrors.newProfitProfileTitle) setProfileFormErrors((current) => ({ ...current, newProfitProfileTitle: undefined })); }} placeholder="عنوان پروفایل سود جدید" maxLength={120} error={profileFormErrors.newProfitProfileTitle} />
            <button type="submit" className={buttonSecondary} disabled={submitting || !isShareTotalValid}>
        ...
        <SectionBlock title="پروفایل‌های مالکیت" description="الگوهای شخصی، مشترک و تجمیعی؛ الگوی جدید به‌صورت مالکیت مشترک ساخته می‌شود.">
          <form noValidate className="store-ownership-inline-create" onSubmit={handleCreateOwnershipProfile}>
            <TextField id="ownership-profile-title" label="عنوان پروفایل مالکیت" required value={newOwnershipProfileTitle} onChange={(event) => { setNewOwnershipProfileTitle(event.target.value); if (profileFormErrors.newOwnershipProfileTitle) setProfileFormErrors((current) => ({ ...current, newOwnershipProfileTitle: undefined })); }} placeholder="عنوان مالکیت مشترک جدید" maxLength={120} error={profileFormErrors.newOwnershipProfileTitle} />
            <button type="submit" className={buttonSecondary} disabled={submitting || !isShareTotalValid}>
  ...
  const renderReview = () => (
    <div className="space-y-6">
      ...
      >
        <div className="mb-4">
          <TextField value={reviewNotes} onChange={(e) => setReviewNotes(e.target.value)} placeholder="یادداشت تصمیم برای ثبت در سابقه بررسی؛ حداکثر ۱۰۰۰ کاراکتر" maxLength={1000} />
        </div>
        <div className="grid gap-6 xl:grid-cols-2">
          <div className={softCard}>
      ...
      <section className="store-ownership-hero">
        <div className="store-ownership-hero__main">
          ...
          <div className="store-ownership-hero__meta">
            <span><i className="fa-solid fa-database" /> اطلاعات واقعی فروشگاه</span>
