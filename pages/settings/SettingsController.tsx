...
import { matchingTextError, requiredSelectionError, requiredTextError, textLengthError } from '@/components/ui';
import {
...
export const useSettingsControllerContext = () => {
  ...
  const [telegramFormErrors, setTelegramFormErrors] = useState<Record<string, string | undefined>>({});
  ...
  const handleBusinessInfoChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (telegramFormErrors[name]) setTelegramFormErrors((prev) => ({ ...prev, [name]: undefined }));
  };
  ...
  const runTelegramDiagnostics = async () => {
    ...
    try {
      ...
      const js = await readApiJsonObject(res);
      if (!res.ok || js.success === false) throw new Error(String(js.message || 'خطا در دریافت وضعیت اتصال تلگرام'));
  ...
  const runTelegramAdminAction = async (action: 'enable-polling' | 'reset-bot-menu' | 'send-guest-menu-test' | 'send-real-menu' | 'send-customer-menu' | 'send-partner-menu') => {
    const labels: Record<typeof action, string> = {
      'enable-polling': 'فعال‌سازی دریافت پیام',
      'reset-bot-menu': 'پاک‌سازی منوی تلگرام تلگرام',
      'send-guest-menu-test': 'بررسی منوی اولیه',
  ...
  const handleTelegramSettingsSubmit = async (event?: FormEvent) => {
    ...
    try {
      ...
      const pushFieldError = (key: string, message: string) => {
        errs.push(message);
      ...
      const transportMode = transportRaw === 'cloud_relay' ? 'relay' : ['disabled', 'direct', 'system', 'proxy', 'relay'].includes(transportRaw) ? transportRaw : 'direct';
      ...
      const chatLists: Record<string, { label: string; value: string }> = {
        telegram_chat_ids_reports: { label: 'گزارشات', value: String(telegramInfo.telegram_chat_ids_reports || '').trim() },
        telegram_chat_ids_installments: { label: 'اقساط', value: String(telegramInfo.telegram_chat_ids_installments || '').trim() },
        telegram_chat_ids_sales: { label: 'فروش', value: String(telegramInfo.telegram_chat_ids_sales || '').trim() },
        telegram_chat_ids_notifications: { label: 'اعلان‌ها', value: String(telegramInfo.telegram_chat_ids_notifications || '').trim() },
      };
      const hasAnyTopicChat = Object.values(chatLists).some((entry) => Boolean(entry.value));

      if (transportMode !== 'disabled') {
        if (!botToken) pushFieldError('telegram_bot_token', 'توکن ربات تلگرام را وارد کنید.');
        else if (!/^\d+:[A-Za-z0-9_-]{20,}$/.test(botToken)) pushFieldError('telegram_bot_token', 'فرمت توکن ربات تلگرام نامعتبر است.');
        if (botUsername && !/^[A-Za-z][A-Za-z0-9_]{4,31}$/.test(botUsername)) {
          pushFieldError('telegram_bot_username', 'نام کاربری ربات تلگرام نامعتبر است. مقدار را بدون @ و فقط با حروف انگلیسی، عدد و _ وارد کنید.');
        }
        if (!chatId && !hasAnyTopicChat) pushFieldError('telegram_chat_id', 'حداقل یک Chat ID مقصد عمومی یا تفکیکی وارد کنید.');
      }

      const isValidChatId = (value: string) => /^-?\d+$/.test(value.trim()) || /^@[A-Za-z0-9_]{5,}$/.test(value.trim());
      if (chatId && !isValidChatId(chatId)) pushFieldError('telegram_chat_id', 'شناسه چت اصلی باید عددی یا نام کاربری کانال/گروه باشد.');
      const splitChatIds = (value: string) => value.split(/[\n,؛;\s]+/g).map((item) => item.trim()).filter(Boolean);
      for (const [key, entry] of Object.entries(chatLists)) {
        const invalid = splitChatIds(entry.value).filter((item) => !isValidChatId(item));
        if (invalid.length) pushFieldError(key, `Chat ID نامعتبر در بخش «${entry.label}»: ${invalid.slice(0, 3).join(', ')}${invalid.length > 3 ? '…' : ''}`);
      }

      if (transportMode === 'proxy' && !proxy) pushFieldError('telegram_proxy', 'برای حالت پراکسی، آدرس پراکسی تلگرام را وارد کنید.');
      if (proxy && !/^(socks5|socks|http|https):\/\//i.test(proxy)) {
        pushFieldError('telegram_proxy', 'فرمت پراکسی نامعتبر است. نمونه معتبر: socks5://127.0.0.1:10808');
      }
      if ((miniAppMode === 'self_hosted' || miniAppMode === 'external_tunnel' || miniAppMode === 'stable_tunnel') && !String(telegramInfo.telegram_miniapp_public_url || '').trim()) {
        pushFieldError('telegram_miniapp_public_url', miniAppMode === 'stable_tunnel' ? 'نشانی HTTPS ثابت Mini App را وارد کنید.' : miniAppMode === 'external_tunnel' ? 'نشانی HTTPS اتصال خارجی را وارد کنید.' : 'نشانی HTTPS میزبانی شخصی Mini App را وارد کنید.');
      }
        ...
        if (stablePublicUrl) {
          try {
            ...
            if (parsedStableUrl.protocol !== 'https:' || parsedStableUrl.username || parsedStableUrl.password || parsedStableUrl.hash || parsedStableUrl.search || !['/', '/miniapp.html'].includes(parsedStableUrl.pathname)) {
              pushFieldError('telegram_miniapp_public_url', 'نشانی ثابت Mini App باید با HTTPS، بدون پارامتر اضافی و روی مسیر /miniapp.html ثبت شود.');
            } else if (host === 'trycloudflare.com' || host.endsWith('.trycloudflare.com')) {
              pushFieldError('telegram_miniapp_public_url', 'نشانی موقت trycloudflare برای دسترسی ثابت مجاز نیست.');
            }
          } catch {
            pushFieldError('telegram_miniapp_public_url', 'نشانی ثابت Mini App معتبر نیست.');
          }
        }
        if (!String(telegramInfo.miniapp_live_origin_url || '').trim()) {
          pushFieldError('miniapp_live_origin_url', 'نشانی HTTPS مبدأ زنده فروشگاه را وارد کنید.');
        }
      }
      if ((transportMode === 'relay' || miniAppMode === 'relay') && relayProvider === 'custom') {
        if (!String(telegramInfo.custom_relay_control_url || '').trim()) pushFieldError('custom_relay_control_url', 'نشانی مدیریت رله شخصی را وارد کنید.');
        if (!String(telegramInfo.custom_relay_connector_url || '').trim()) pushFieldError('custom_relay_connector_url', 'نشانی اتصال رله شخصی را وارد کنید.');
      }

      const parseHour = (value: string, key: string, label: string) => {
        if (!value) return null;
        const number = Number(value);
        if (!Number.isInteger(number) || number < 0 || number > 23) {
          pushFieldError(key, `${label} باید عددی بین ۰ تا ۲۳ باشد.`);
          return null;
        }
        return number;
      };
      const quietStart = parseHour(quietStartRaw, 'telegram_quiet_start_hour', 'ساعت شروع سکوت');
      const quietEnd = parseHour(quietEndRaw, 'telegram_quiet_end_hour', 'ساعت پایان سکوت');
      if ((quietStart === null) !== (quietEnd === null)) {
        if (!quietStartRaw) pushFieldError('telegram_quiet_start_hour', 'برای بازه سکوت، ساعت شروع را وارد کنید.');
        if (!quietEndRaw) pushFieldError('telegram_quiet_end_hour', 'برای بازه سکوت، ساعت پایان را وارد کنید.');
      }

      const dailyLimit = Number(dailyLimitRaw);
      if (!Number.isInteger(dailyLimit) || dailyLimit < 0 || dailyLimit > 50) {
        pushFieldError('telegram_max_per_day_per_customer', 'حداکثر پیام روزانه هر مشتری باید عددی بین ۰ تا ۵۰ باشد.');
      }

      if (errs.length) {
        setTelegramFormErrors(fieldErrors);
        focusErrorsSoon(fieldErrors);
        setNotification({ type: 'error', text: errs[0] });
        return;
      }
      setTelegramFormErrors({});
...
const handleBusinessInfoSubmit = async (
  ...
  const handleSaveLocalDomainSettings = async () => {
    ...
    try {
      ...
      setNotification({ type: 'success', text: 'تنظیمات دسترسی محلی و نصب برنامه با موفقیت ذخیره شد.', closeMs: 3600 });
    } catch (error: unknown) {
...
const handleGenerateLocalCertificate = async () => {
  ...
  const handleBackup = async () => {
    ...
    setNotification({ type: 'info', text: 'در حال آماده‌سازی نسخه پشتیبان برای دانلود…' });
    try {
  ...
  const handleNewUserSubmit = async (e: FormEvent) => {
    ...
    const usernameRequiredError = requiredTextError(normalizedUsername, 'نام کاربری');
    const passwordRequiredError = textLengthError(newUser.password, 'کلمه عبور', { min: 6, max: 128 });
    const roleRequiredError = requiredSelectionError(newUser.roleId, 'نقش');
    ...
    const confirmPasswordError = matchingTextError(newUser.confirmPassword, newUser.password, 'تکرار کلمه عبور', { message: 'کلمه عبور و تکرار آن یکسان نیستند.' });
    if (!passwordRequiredError && confirmPasswordError) errors.confirmPassword = confirmPasswordError;
    if (roleRequiredError) errors.roleId = roleRequiredError;
    else if (!Number.isInteger(normalizedRoleId) || !roles.some((role) => role.id === normalizedRoleId)) errors.roleId = 'یک نقش معتبر انتخاب کنید.';
    if (Object.keys(errors).length > 0) {
      setAddUserFormErrors(errors);
      focusErrorsSoon(errors, { username: 'settings-user-username', password: 'settings-user-password', confirmPassword: 'settings-user-confirm-password', roleId: 'settings-user-role' });
      return;
    }
  ...
  const handleEditUserChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setEditUserFormErrors(prev => ({ ...prev, [name]: undefined }));
  };
  const handleEditUserSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!editingUser) return;
    const normalizedRoleId = Number(editingUser.roleId);
    const roleRequiredError = requiredSelectionError(editingUser.roleId, 'نقش');
    if (roleRequiredError || !Number.isInteger(normalizedRoleId) || !roles.some((role) => role.id === normalizedRoleId)) {
      setEditUserFormErrors({ roleId: roleRequiredError || 'یک نقش معتبر انتخاب کنید.' });
      return;
    }
  ...
  const handleResetPasswordSubmit = async (e: FormEvent) => {
    ...
    const passwordRequiredError = textLengthError(resetPasswordData.password, 'کلمه عبور جدید', { min: 6, max: 128 });
    const confirmPasswordError = matchingTextError(resetPasswordData.confirmPassword, resetPasswordData.password, 'تکرار کلمه عبور', { message: 'کلمه‌های عبور یکسان نیستند.' });
    const resetErrors: Partial<typeof resetPasswordData> = {};
    if (passwordRequiredError) resetErrors.password = passwordRequiredError;
    if (!passwordRequiredError && confirmPasswordError) resetErrors.confirmPassword = confirmPasswordError;
    if (Object.keys(resetErrors).length > 0) {
      setResetPasswordErrors(resetErrors);
      focusErrorsSoon(resetErrors, { password: 'settings-reset-password', confirmPassword: 'settings-reset-confirm-password' });
      return;
    }
  ...
  const inputClass =
    'w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-800 text-right shadow-sm transition placeholder:text-slate-400';
