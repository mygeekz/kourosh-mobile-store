...
import { BidiText, FormGrid, ModalField, SelectField, TextField, compactValidationErrors, emailAddressError, requiredTextError, urlAddressError } from '@/components/ui';
...
const BusinessField: React.FC<BusinessFieldProps> = ({ label, icon, required, hint, error, className = '', children }) => (
  <ModalField
    label={label}
...
const SettingsBusinessPanel: React.FC<SettingsBusinessPanelProps> = ({
  ...
  const [businessFormErrors, setBusinessFormErrors] = React.useState<FormErrors>({});
  const businessFieldIds: Record<string, string> = {
  ...
  const businessFieldLabels: Record<string, string> = {
    ...
    app_base_url: 'آدرس پایه Web App عمومی',
    qr_public_base_url: 'آدرس عمومی QR فاکتور',
  };
  const validateBusinessForm = () => compactValidationErrors({
    store_name: requiredTextError(businessInfo.store_name, 'نام فروشگاه'),
    store_email: emailAddressError(businessInfo.store_email, { optional: true, label: 'ایمیل فروشگاه' }),
    app_base_url: urlAddressError(businessInfo.app_base_url, { optional: true, label: 'آدرس پایه Web App عمومی', protocols: ['http:', 'https:'] }),
    qr_public_base_url: urlAddressError(businessInfo.qr_public_base_url, { optional: true, label: 'آدرس عمومی QR فاکتور', protocols: ['http:', 'https:'] }),
  });
  const handleValidatedBusinessInfoChange: React.ChangeEventHandler<HTMLInputElement | HTMLSelectElement> = (event) => {
    handleBusinessInfoChange(event);
    const fieldName = event.target.name;
    if (fieldName && businessFormErrors[fieldName]) {
      setBusinessFormErrors((current) => ({ ...current, [fieldName]: undefined }));
    }
  };
  const handleValidatedBusinessSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const errors = validateBusinessForm();
    setBusinessFormErrors(errors);
    if (Object.keys(errors).length) {
      focusErrorsSoon(errors, businessFieldIds);
      return;
    }
    await handleBusinessInfoSubmit(event);
  };
  ...
  return (
    <form
      id="settings-form"
      noValidate
      onSubmit={handleValidatedBusinessSubmit}
    ...
    >
      <FormErrorSummary errors={businessFormErrors} labels={businessFieldLabels} fieldIdMap={businessFieldIds} />
                ...
                <div key={item.label} className="min-w-0 overflow-hidden rounded-[16px] bg-slate-50 px-3 py-2.5 text-right dark:bg-slate-900/65">
                  ...
                  <div
                    className={`mt-1 min-w-0 text-[12px] font-black leading-5 text-slate-900 dark:text-slate-100 ${
                      isEmailSummary ? 'whitespace-normal break-all text-left [overflow-wrap:anywhere]' : 'truncate'
                    }`}
                    title={summaryValue}
                  >
                    <BidiText kind={isEmailSummary ? 'email' : 'auto'}>{summaryValue}</BidiText>
                  </div>
                </div>
              );
            })}
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]" data-ui-settings-grid="form">
        <section className={`${settingsSectionCard} !rounded-[22px] !p-4 sm:!p-5`} data-ui-settings-card="business-main">
          ...
          <FormGrid columns={2} data-ui-settings-grid="form">
            <BusinessField label="نام فروشگاه" icon="fa-store" required error={businessFormErrors.store_name}>
              <TextField controlOnly type="text" id="store_name" name="store_name" value={businessInfo.store_name || ''} onChange={handleValidatedBusinessInfoChange} placeholder="مثلاً فروشگاه کوروش" autoComplete="organization" required aria-invalid={Boolean(businessFormErrors.store_name)} />
            </BusinessField>

            <BusinessField
              label="نام فروشنده/نماینده قانونی قرارداد"
              icon="fa-user-tie"
              hint="این نام در قراردادهای فروش اقساطی جدید به‌عنوان نام ثبت‌شده فروشگاه ذخیره می‌شود."
            >
              <TextField controlOnly type="text" id="installment_contract_seller_name" name="installment_contract_seller_name" value={businessInfo.installment_contract_seller_name || ''} onChange={handleValidatedBusinessInfoChange} placeholder="نام و نام خانوادگی" autoComplete="name" />
            </BusinessField>
            ...
            >
              <TextField controlOnly type="text" id="installment_contract_seller_national_code" name="installment_contract_seller_national_code" value={businessInfo.installment_contract_seller_national_code || ''} onChange={handleValidatedBusinessInfoChange} valueKind="code" inputMode="numeric" maxLength={10} placeholder="0012345678" autoComplete="off" />
            </BusinessField>

            <BusinessField label="تلفن فروشگاه" icon="fa-phone">
              <TextField controlOnly type="tel" id="store_phone" name="store_phone" value={businessInfo.store_phone || ''} onChange={handleValidatedBusinessInfoChange} valueKind="phone" inputMode="tel" placeholder="021..." autoComplete="tel" />
            </BusinessField>

            <BusinessField label="ایمیل فروشگاه" icon="fa-envelope" error={businessFormErrors.store_email}>
              <TextField controlOnly type="email" id="store_email" name="store_email" value={businessInfo.store_email || ''} onChange={handleValidatedBusinessInfoChange} aria-invalid={Boolean(businessFormErrors.store_email)} valueKind="email" placeholder="store@example.com" autoComplete="email" />
            </BusinessField>

            <BusinessField label="واحد پول نمایشی" icon="fa-coins" hint={`پیش‌نمایش: ${formatCurrencyText(1250000, businessInfo.currency_unit)}`}>
              <SelectField controlOnly icon={false} id="currency_unit" name="currency_unit" value={normalizeCurrencyUnit(businessInfo.currency_unit)} onChange={handleValidatedBusinessInfoChange}>
            ...
            <BusinessField
              ...
              hint="این نشانی فقط برای لینک وب نسخه‌های قدیمی استفاده می‌شود؛ نشانی Mini App تلگرام، QR و دسترسی محلی مستقل هستند."
              error={businessFormErrors.app_base_url}
            >
              <TextField controlOnly type="url" id="app_base_url" name="app_base_url" value={businessInfo.app_base_url || ''} onChange={handleValidatedBusinessInfoChange} aria-invalid={Boolean(businessFormErrors.app_base_url)} valueKind="url" placeholder="https://app.example.com" inputMode="url" />
            </BusinessField>
            ...
            <BusinessField
              ...
              icon="fa-qrcode"
              className="col-span-full"
              hint="این نشانی مستقل است و با نشانی محلی، Mini App یا سرویس ابری به‌صورت خودکار همگام نمی‌شود."
              error={businessFormErrors.qr_public_base_url}
            >
              <TextField controlOnly type="url" id="qr_public_base_url" name="qr_public_base_url" value={businessInfo.qr_public_base_url || ''} onChange={handleValidatedBusinessInfoChange} aria-invalid={Boolean(businessFormErrors.qr_public_base_url)} valueKind="url" placeholder="https://your-public-site.com" inputMode="url" />
            </BusinessField>
          </FormGrid>
        </section>
        ...
        <section className={`${settingsSectionCard} !rounded-[22px] !p-4 sm:!p-5`} data-ui-settings-card="business-address" data-settings-mode="advanced">
          ...
          <FormGrid columns={1}>
            <BusinessField label="آدرس - خط اول" icon="fa-location-crosshairs">
              <TextField controlOnly type="text" id="store_address_line1" name="store_address_line1" value={businessInfo.store_address_line1 || ''} onChange={handleValidatedBusinessInfoChange} autoComplete="address-line1" placeholder="خیابان، کوچه و پلاک" />
            </BusinessField>
            <BusinessField label="آدرس - خط دوم" icon="fa-building">
              <TextField controlOnly type="text" id="store_address_line2" name="store_address_line2" value={businessInfo.store_address_line2 || ''} onChange={handleValidatedBusinessInfoChange} autoComplete="address-line2" placeholder="طبقه، واحد یا توضیح تکمیلی" />
            </BusinessField>
            <BusinessField label="شهر، استان و کدپستی" icon="fa-map-location-dot">
              <TextField controlOnly type="text" id="store_city_state_zip" name="store_city_state_zip" value={businessInfo.store_city_state_zip || ''} onChange={handleValidatedBusinessInfoChange} autoComplete="postal-code" placeholder="تهران، تهران، ۱۲۳۴۵۶۷۸۹۰" />
            </BusinessField>
          </FormGrid>
