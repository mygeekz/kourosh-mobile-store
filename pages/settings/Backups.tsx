...
export default function Backups() {
  ...
  const fetchList = async () => {
    ...
    try {
      const res = await apiFetch('/api/admin/backups', { headers: { Authorization: `Bearer ${token}` } });
      const js = await res.json();
      if (!res.ok || js?.success === false) throw new Error(js?.message || 'خطا در دریافت فهرست نسخه‌های پشتیبان');
      setRows(js.data || []);
    } catch (e: unknown) {
  ...
  const createNow = async () => {
    ...
    setNotification(null);
    try {
      const res = await apiFetch('/api/admin/backups', { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
      const js = await res.json();
      if (!res.ok || js?.success === false) throw new Error(js?.message || 'خطا در ایجاد نسخه پشتیبان');
      setNotification({ text: 'نسخه پشتیبان ایجاد شد.', type: 'success' });
      await fetchList();
    } catch (e: unknown) {
      setNotification({ text: getErrorMessage(e), type: 'error' });
    } finally { setIsLoading(false); }
  ...
  const download = async (fileName: string) => {
    if (!token) return;
    try {
      const res = await apiFetch(`/api/admin/backups/${encodeURIComponent(fileName)}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('دانلود نسخه پشتیبان انجام نشد');
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="overflow-hidden rounded-[32px] border border-slate-200/80 bg-gradient-to-br from-white via-white to-slate-50 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            <div className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-400 dark:text-slate-500">مرکز نسخه‌های پشتیبان</div>
            <div className="text-xl font-black text-slate-900 dark:text-slate-100">پشتیبان‌گیری خودکار اطلاعات</div>
            <div className="text-sm leading-7 text-slate-600 dark:text-slate-300">نسخه پشتیبان اطلاعات به‌صورت خودکار ایجاد می‌شود و از این صفحه می‌توانید نسخه‌ها را مدیریت یا بازیابی کنید.</div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="p-10">
            <div className="mx-auto max-w-2xl rounded-[24px] border border-dashed border-slate-300 bg-slate-50/80 p-8 text-center dark:border-slate-700 dark:bg-slate-950/30">
              ...
              <div className="text-base font-bold text-slate-900 dark:text-slate-100">هنوز نسخه پشتیبان ایجاد نشده است</div>
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400">
        زمان‌بندی خودکار پشتیبان‌گیری توسط سامانه مدیریت می‌شود.
      </div>
    </div>
  );
}
