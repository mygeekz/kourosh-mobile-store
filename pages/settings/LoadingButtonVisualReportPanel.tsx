  ...
  return (
    <section id="loading-button-quality" className="style-button-report" data-ui-loading-button-report="true">
      <header className="style-button-report__header">
        <div>
          <strong><i className="fa-solid fa-chart-simple" aria-hidden="true" /> نتیجه آخرین بررسی کیفیت</strong>
          <small>این گزارش نتیجه آخرین بررسی خودکار دکمه‌های عملیاتی ورود، پشتیبان‌گیری، بازیابی، راه‌اندازی و ثبت گوشی را نشان می‌دهد.</small>
        </div>
        <Button
          type="button"
          size="sm"
      ...
      {!loading && !error && !report ? (
        <div className="style-button-report__empty">
          ...
          <div>
            ...
            <span>هنوز نتیجه‌ای برای کنترل کیفیت این بخش ثبت نشده است. پس از اجرای بررسی خودکار سیستم، نتیجه از همین صفحه قابل بازخوانی خواهد بود.</span>
      ...
      {report ? (
        <>
          ...
          <details ref={detailsRef} className="style-control-advanced" data-ui-style-report-details="true">
            <summary>
              <span><i className="fa-solid fa-list-check" aria-hidden="true" />جزئیات و تصاویر بررسی دکمه‌ها</span>
                      ...
                      <article key={key} className="style-button-report__result" data-status={result.passed ? 'passed' : 'failed'}>
                        ...
                        {!result.passed ? (
                          <div className="style-button-report__failed-checks">
                            ...
                            <strong>{failedChecks.length ? failedChecks.join('، ') : 'جزئیات این مورد در گزارش بررسی ثبت شده است.'}</strong>
                          </div>
                        ) : null}
                        <div className="style-button-report__result-actions">
                          {result.screenshotUrl ? (
                            ...
                            >
                              {imageUrl ? 'تصویر دریافت شد' : 'نمایش تصویر'}
                            </Button>
                          ) : <span>تصویری برای این مورد ذخیره نشده است.</span>}
                        </div>
                        ...
                        {imageUrl ? (
                          <a className="style-button-report__screenshot" href={imageUrl} target="_blank" rel="noreferrer">
                            <img src={imageUrl} alt={`تصویر بررسی ${result.scenarioLabel}`} loading="lazy" />
