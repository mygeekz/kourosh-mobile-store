...
export type SettingsAccountPanelProps = {
  ...
  setProfileFirstName: React.Dispatch<React.SetStateAction<string>>;
  profileLastName: string;
  setProfileLastName: React.Dispatch<React.SetStateAction<string>>;
  accountProfileDirty: boolean;
  isSavingProfile: boolean;
  handleSaveMyProfile: () => void;
};

export type BusinessSummaryItem = { label: string; value: string; icon: string };
export type PartnerShareStatusLite = { state: 'loading' | 'ready' | 'warning' | 'empty' | 'error'; hint: string; label: string };
...
export type SettingsBusinessPanelProps = {
  ...
  handleBusinessInfoSubmit: (event: FormEvent<HTMLFormElement>) => void | Promise<boolean>;
...
export type BackupItem = { fileName: string; size: number; mtime: string; sha256?: string; checksumVerified?: boolean };
...
export type SettingsDataPanelProps = {
  ...
  backupOperationKey: string | null;
  restoreProgress: DatabaseRestoreProgressSnapshot | null;
  restoreHistory: DatabaseRestoreAuditRecord[];
...
export type SettingsLocalPanelProps = {
  ...
  handleDownloadLocalCertificate: () => void;
...
export type PricingDataRuntimeStatus = {
  state: 'loading' | 'ready' | 'degraded' | 'error';
  source: 'server-decision-log' | 'phone-history-fallback' | 'local-cache';
  sourceLabel: string;
  message: string;
  serverCount: number;
  localCount: number;
...
export type SettingsPricingPanelProps = {
  ...
  pricingDataStatus: PricingDataRuntimeStatus;
  refreshPricingLearningData: () => void;
...
export type SettingsSmsPanelProps = {
  ...
  handleBusinessInfoSubmit: (e?: React.FormEvent) => void | Promise<boolean>;
  scrollToSection: (id: string) => void;
  openSmsPatternPreview: (title: string, previewTemplate: string, tokenLabels: string[], bodyId?: string) => void;
  openSmsPatternCheck: (title: string, bodyId: string, tokenLabels: string[], previewTemplate?: string) => void;
  openSmsBulkCheck: (keys: string[]) => void;
  openSmsBulkPanel: () => void;
};
...
export type TelegramBusinessInfo = BusinessInformationSettings & Partial<Record<string, TelegramBusinessInfoValue>> & {
  ...
  telegram_transport_mode?: 'disabled' | 'direct' | 'system' | 'proxy' | 'relay' | 'cloud_relay' | null;
  ...
  kourosh_cloud_assigned_store_id?: string | null;
  kourosh_cloud_credential_version?: string | number | null;
  kourosh_cloud_telegram_relay_healthy?: TelegramToggleValue | null;
  kourosh_cloud_miniapp_relay_healthy?: TelegramToggleValue | null;
...
export type SettingsTelegramPanelProps = {
  ...
  telegramFormErrors: Record<string, string | undefined>;
  inputClass: string;
  labelClass: string;
  ...
  telegramDiagnosticsViewModel: SettingsTelegramDiagnosticsViewModel;
  ...
  telegramRecentChatsViewModel: SettingsTelegramRecentChatsViewModel;
  ...
  handleBusinessInfoSubmit: (event?: React.FormEvent) => void | Promise<boolean>;
  handleTelegramSettingsSubmit: (event?: React.FormEvent) => void | Promise<void>;
  handleBusinessInfoChange: React.ChangeEventHandler<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>;
  setBusinessInfo: React.Dispatch<React.SetStateAction<BusinessInformationSettings>>;
  setNotification: React.Dispatch<React.SetStateAction<NotificationMessage | null>>;
...
export type SettingsUsersPanelProps = {
  ...
  refreshUsersData: () => void | Promise<void>;
...
export type SettingsUsersModalsProps = {
  ...
  editUserFormErrors: Partial<EditUserFormData>;
  ...
  setResetPasswordErrors: Dispatch<SetStateAction<Partial<ResetPasswordData>>>;
