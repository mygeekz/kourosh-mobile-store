  ...
  return (
    <section id="dashboard-visual-quality" className="style-button-report" data-ui-dashboard-visual-report="true">
      <header className="style-button-report__header">
        <div>
          <strong><i className="fa-solid fa-gauge-high" aria-hidden="true" /> نتیجه آخرین بررسی کیفیت داشبورد</strong>
      ...
      {!loading && !error && !report ? (
        <div className="style-button-report__empty">
          ...
          <div>
            ...
            <span>هنوز نتیجه‌ای برای کنترل کیفیت داشبورد ثبت نشده است. پس از اجرای بررسی خودکار سیستم، نتیجه از همین صفحه قابل بازخوانی خواهد بود.</span>
                      ...
                      <article key={key} className="style-button-report__result" data-status={result.passed ? 'passed' : 'failed'}>
                        ...
                        {imageUrl ? (
                          <a className="style-button-report__screenshot" href={imageUrl} target="_blank" rel="noreferrer">
                            <img src={imageUrl} alt={`تصویر بررسی داشبورد ${paletteLabel(result.palette)} ${THEME_LABELS[result.theme] ?? result.theme} ${VIEWPORT_LABELS[result.viewport] ?? result.viewport}`} loading="lazy" />
