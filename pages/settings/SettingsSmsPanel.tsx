...
import SmsTestRecipientAllowlistCard from '../../components/SmsTestRecipientAllowlistCard';
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <TextField controlOnly
              ...
              value={value}
              onChange={handleBusinessInfoChange}
              className={`${inputClass} settings-sms-control mt-2 !rounded-[13px] !text-[12px]`}
              placeholder="BodyId"
              dir="ltr"
              inputMode="numeric"
            />
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={() => openSmsPatternPreview(`پیش‌نمایش و تست: ${pattern.label}`, pattern.previewTemplate || '', pattern.tokens, value)}
              ...
              <Button
                ...
                onClick={() => openSmsPatternCheck(`پیش‌نمایش و تست: ${pattern.label}`, value, pattern.tokens, pattern.previewTemplate || '')}
    ...
    <div className="settings-panel-root settings-sms-panel settings-sms-redesign-v2 space-y-4" data-ui-settings-panel="sms" data-ui-sms-redesign="v2">
      <Surface surface="glass" variant="panel" scheme="adaptive" className="rounded-[24px]" contentClassName="p-4 sm:p-5">
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-3xl text-[11px] leading-6 text-slate-500 dark:text-slate-400 sm:text-[12px]">
                اتصال سرویس‌دهنده، قالب‌های پیام، ارسال خودکار، سلامت پیکربندی و سوابق ارسال را از یک بخش مدیریت کنید.
              </p>
            </div>
          </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <Button type="button" variant="secondary" size="xs" className="settings-sms-action rounded-[11px]" onClick={() => scrollToSection('sms-test-allowlist-section')} leftIcon={<i className="fa-solid fa-shield-check" />}>شماره‌های تست</Button>
      ...
      <Surface id="sms-provider-section" surface="glass" variant="panel" scheme="adaptive" className="rounded-[22px]" contentClassName="p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(220px,0.7fr)_minmax(0,1.3fr)]" data-ui-settings-grid="form">
          <div className="min-w-0">
            ...
            <SelectField controlOnly size="md" icon={false} name="sms_provider" value={smsProviderKey} onChange={handleBusinessInfoChange} className={`${inputClass} settings-sms-control`}>
      ...
      <SmsTestRecipientAllowlistCard
        value={String(businessInfo.sms_test_recipient_allowlist_json || '')}
        dirty={infoChanged}
        onChange={(serialized) => setBusinessInfo((previous) => ({ ...previous, sms_test_recipient_allowlist_json: serialized }))}
      />
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3" data-ui-settings-grid="cards">
          {automationFields.map((field) => (
            <Surface key={field.key} surface="glass" variant="subtle" scheme="adaptive" className="settings-sms-automation rounded-[17px]" contentClassName="p-3">
              ...
              <SelectField controlOnly size="sm" icon={false} name={field.key} value={String(businessInfo[field.key] || 'off')} onChange={handleBusinessInfoChange} className="settings-sms-control mt-2 !rounded-[12px] !text-[11px]">
        ...
        {isMeliPayamak ? (
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="rounded-[16px] border border-sky-200 bg-sky-50/70 px-4 py-3 text-[10px] leading-5 text-sky-800 dark:border-sky-900/50 dark:bg-sky-950/20 dark:text-sky-300">
              <i className="fa-solid fa-circle-info ml-1" />بررسی ارسال مستقیم و گروهی برای ملی‌پیامک در دسترس است. وضعیت سایر سرویس‌ها بر پایه تنظیمات و قالب‌های ثبت‌شده بررسی می‌شود.
            </div>
          </div>
        )}
      </Surface>
