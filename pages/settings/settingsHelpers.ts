...
import type { SettingsTabKey } from '../../config/ui/settings-navigation';

const LOCAL_DOMAIN_SUFFIX_WHITELIST = new Set(['home.arpa', 'internal', 'lan', 'localhost', 'local']);
...
export const normalizeLocalSuffix = (value: unknown) => {
  ...
  return raw && LOCAL_DOMAIN_SUFFIX_WHITELIST.has(raw) ? raw : '';
};
...
const parsePricingJalaliDate = (value: string) => {
  const raw = toLatinDigits(value).trim().replace(/[.\-]/g, '/');
  if (!raw) return null;
  const parts = raw.split('/');
  if (parts.length !== 3) return null;
  const normalized = [parts[0].padStart(4, '0'), parts[1].padStart(2, '0'), parts[2].padStart(2, '0')].join('/');
  const parsed = moment(normalized, 'jYYYY/jMM/jDD', true).locale('fa');
  return parsed.isValid() ? parsed : null;
};

export const parsePricingDecisionDateFilter = (value: string, endOfDay = false) => {
  if (!String(value || '').trim()) return endOfDay ? Number.POSITIVE_INFINITY : 0;
  const parsed = parsePricingJalaliDate(value);
  if (!parsed) return endOfDay ? Number.POSITIVE_INFINITY : 0;
...
export const pricingFilterValueToDate = (value: string): Date | null => {
  const parsed = parsePricingJalaliDate(value);
  return parsed ? parsed.toDate() : null;
};

export const dateToPricingFilterValue = (value: Date | null): string => {
...
export const formatPricingDatePreview = (value: string, fallback: string) => {
  const parsed = parsePricingJalaliDate(value);
  return parsed ? parsed.locale('fa').format('jYYYY/jMM/jDD') : fallback;
};
