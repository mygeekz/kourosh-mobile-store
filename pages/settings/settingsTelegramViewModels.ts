import { buildTelegramTemplatePreset, TELEGRAM_TEMPLATE_DEFINITIONS } from '../../shared/messages';
import type {
  TelegramAudience,
...
export const tgCategoryMeta: Record<string, { icon: string; tone: string; description: string; quickHint: string; heroChip: string; heroBar: string }> = {
  ...
  'فاکتورها': {
    ...
    description: 'ثبت فاکتور و پرداخت فاکتور را با پیام‌های استاندارد و قابل اتکا نگه دار.',
...
export const buildTelegramAudiencePresetValue = (telegramInfo: TelegramBusinessInfo, key: string, audience: TelegramAudience) => {
  ...
  return buildTelegramTemplatePreset(key, audience, policy);
};

export const buildTelegramTemplateDefs = (): TelegramTemplateDef[] => TELEGRAM_TEMPLATE_DEFINITIONS.map((item) => ({
  ...item,
