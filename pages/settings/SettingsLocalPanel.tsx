...
const PWA_RESET_STEP_DEFINITIONS: PwaResetStepDefinition[] = [
  { id: 'registrations', label: 'اتصال نسخه نصب‌شده', icon: 'fa-solid fa-link-slash' },
  { id: 'caches', label: 'فایل‌های ذخیره‌شده برنامه', icon: 'fa-solid fa-box-archive' },
  { id: 'manifest', label: 'اطلاعات نصب برنامه', icon: 'fa-solid fa-file-code' },
  { id: 'worker-asset', label: 'سرویس اجرای برنامه', icon: 'fa-solid fa-gears' },
  { id: 'registration', label: 'فعال‌سازی نسخه تازه', icon: 'fa-solid fa-rotate' },
  { id: 'controller', label: 'آمادگی اجرای صفحه', icon: 'fa-solid fa-shield-halved' },
];
...
const SettingsLocalPanel = ({
      ...
      try {
        ...
        setPwaResetMessage(
          completedAfterReload && result.success
            ? 'بازنشانی برنامه نصب‌شده انجام شد و بررسی پس از بارگذاری مجدد نیز سالم است.'
            : result.message,
        );
      } catch (error) {
        if (!cancelled) setPwaResetError(error instanceof Error ? error.message : 'بررسی وضعیت نصب کامل نشد.');
      } finally {
  ...
  const handleInspectPwaRuntime = async () => {
    ...
    try {
      ...
      setPwaResetError(error instanceof Error ? error.message : 'بررسی وضعیت نصب کامل نشد.');
    } finally {
  ...
  const handleResetPwaRuntime = async () => {
    ...
    const confirmed = await confirmAction({
      title: 'بازنشانی برنامه نصب‌شده',
      description: 'اطلاعات نصب برنامه بازنشانی و دوباره آماده می‌شوند. اطلاعات فروشگاه، پایگاه داده و نشست ورود حذف نمی‌شوند.',
      confirmText: 'بازنشانی برنامه',
      cancelText: 'انصراف',
      tone: 'warning',
    ...
    try {
      ...
      setPwaResetError(error instanceof Error ? error.message : 'بازنشانی برنامه کامل نشد.');
    } finally {
              ...
              <div className="settings-hero-card rounded-[28px] border border-sky-200/70 bg-[radial-gradient(circle_at_top_right,_rgba(14,165,233,0.14),_transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(240,249,255,0.96))] p-5 shadow-[0_24px_70px_-36px_rgba(14,165,233,0.22)] dark:border-sky-900/30 dark:bg-[linear-gradient(135deg,rgba(2,6,23,0.92),rgba(15,23,42,0.9))]">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="max-w-3xl">
                    <div className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-white/80 px-3 py-1 text-xs font-black text-sky-700 shadow-sm dark:border-sky-900/40 dark:bg-slate-900/70 dark:text-sky-200">
                      ...
                      دسترسی محلی · اتصال امن · نصب برنامه
                    </div>
                    <div className="mt-3 text-2xl font-black text-slate-900 dark:text-white">دسترسی محلی و نصب برنامه در شبکه فروشگاه</div>
                    <div className="mt-2 text-sm leading-7 text-slate-600 dark:text-slate-300">
                      این بخش دسترسی محلی برنامه در شبکه فروشگاه، اتصال امن و تنظیمات لازم برای دستگاه‌های داخل شبکه را مدیریت می‌کند.
                    </div>
                  </div>
                  <div className="grid min-w-0 grid-cols-2 gap-3" data-ui-settings-grid="form">
                    ...
                    <div className="rounded-2xl border border-white/80 bg-white/90 px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
                      <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400">وضعیت اتصال امن</div>
                      <div className={`mt-1 text-sm font-black ${localDomainValue ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}`}>
                        {localDomainValue ? 'آماده' : 'نیاز به نام محلی'}
                      </div>
                    </div>
                  </div>
                </div>
                <div className="telegram-monitor-actions">
                  <Button type="button" onClick={handleGenerateLocalCertificate} disabled={isGeneratingLocalCert || !localHostnameValue} loading={isGeneratingLocalCert} loadingText="در حال ساخت گواهی…" variant="secondary" size="sm" leftIcon={<i className="fa-solid fa-certificate" />}>
                    ساخت/بازسازی گواهی موبایل
                  </Button>
                  <Button type="button" onClick={handleDownloadHostsScript} disabled={!localHostnameValue} variant="secondary" size="sm" leftIcon={<i className="fa-solid fa-file-arrow-down" />}>
                    دانلود تنظیم اتصال شبکه
                  </Button>
              ...
              <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 md:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div className="text-right">
                    ...
                    <p className="mt-0.5 line-clamp-1 text-[11px] leading-5 text-slate-500 dark:text-slate-400">نام و پسوند محلی را مشخص کنید تا نشانی امن برنامه برای شبکه فروشگاه آماده شود.</p>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2" data-ui-settings-grid="form">
                  <div className="rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/40">
                    <div className={labelClass}><i className="fa-solid fa-network-wired ml-2 text-sky-500" />نام محلی دستگاه</div>
                  ...
                  <div className="rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/40">
                    <div className={labelClass}><i className="fa-solid fa-globe ml-2 text-sky-500" />پسوند دامنه محلی</div>
                    <div className="relative">
                    ...
                    <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">برای تنظیم جدید، home.arpa پیشنهاد می‌شود. گزینه‌های قدیمی فقط برای سازگاری با تنظیمات قبلی نمایش داده می‌شوند.</p>
                    {(localSuffixValue === 'localhost' || localSuffixValue === 'local') && (
                      <div className="mt-2 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-black text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
                        ...
                        تنظیم قدیمی — نیازمند بازبینی دستی
                      </div>
                    )}
                  </div>
                  ...
                  <div className="rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/40">
                    <div className={labelClass}><i className="fa-solid fa-file-lines ml-2 text-sky-500" />تنظیم پیشنهادی نام محلی</div>
                    <div className="relative">
                  ...
                  <div className="settings-grid-span-full rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/40">
                    ...
                    <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">این مقدار نشانی محلی برنامه در شبکه فروشگاه است. اگر خالی باشد، نشانی مناسب به‌صورت خودکار ساخته می‌شود.</p>
                    <p className="mt-1 text-xs leading-6 text-amber-600 dark:text-amber-300">برای استفاده روی دستگاه‌های دیگر شبکه فروشگاه، نام محلی باید در شبکه قابل شناسایی باشد. گواهی ریشه فقط برای برقراری اتصال امن محلی استفاده می‌شود.</p>
                  </div>
              ...
              <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 md:p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-6 text-slate-500 dark:text-slate-400">ابتدا «دانلود گواهی ریشه موبایل» را بزنید. این فایل، مرجع اعتماد گواهی‌های محلی کوروش است و فقط باید روی کامپیوتر و گوشی‌های متعلق به فروشگاه نصب شود.</p>
                  </div>
                  <Button type="button" onClick={handleDownloadLocalCertificate} disabled={!localHostnameValue} variant="secondary" size="sm" leftIcon={<i className="fa-solid fa-download" />}>
                    دانلود گواهی ریشه
                  </Button>
                ...
                <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 md:p-6" aria-labelledby="pwa-reset-heading">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <h3 id="pwa-reset-heading" className="mt-2 text-lg font-black text-slate-950 dark:text-white">سلامت و بازنشانی برنامه</h3>
                      <p className="mt-1 max-w-3xl text-xs leading-6 text-slate-500 dark:text-slate-400">
                        وضعیت نسخه نصب‌شده بدون حذف اطلاعات بررسی می‌شود. بازنشانی فقط فایل‌ها و اطلاعات فنی نصب را بازسازی می‌کند؛ پایگاه داده، تنظیمات و نشست ورود دست‌نخورده باقی می‌مانند.
                      </p>
                    </div>
                    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                      ...
                      <Button
                        ...
                        loadingText="در حال بازنشانی برنامه…"
                        loadingHint="بازسازی نسخه نصب‌شده"
                        variant="warning"
                        size="sm"
                        className="w-full sm:w-auto"
                        leftIcon={<i className="fa-solid fa-arrows-rotate" />}
                      >
                        بازنشانی برنامه نصب‌شده
                      </Button>
                    </div>
                  </div>
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4 text-sm leading-7 text-slate-700 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-200">
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-3" data-ui-settings-grid="cards">
                  <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950/60">
                    <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400"><i className="fa-solid fa-server ml-2 text-sky-500" />نام میزبان</div>
                    <div className="mt-1 font-black" dir="ltr">kourosh</div>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950/60">
                    <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400"><i className="fa-solid fa-globe ml-2 text-sky-500" />پسوند دامنه</div>
