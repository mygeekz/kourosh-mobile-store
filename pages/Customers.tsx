...
import { BidiText, FormGrid, MANAGEMENT_DIRECTORY_ROW_CLASS, ManagementDirectoryTable, TextareaField, TextField, exactDigitsError, phoneNumberError, requiredTextError } from '@/components/ui';
...
const getCustomerDueRowAccent = (badge?: CustomerDueBadge | null): 'danger' | 'warning' | 'info' | 'neutral' => {
  if (!badge) return 'neutral';
  if (badge.label.includes('عقب')) return 'danger';
  if (badge.label.includes('امروز')) return 'warning';
  if (badge.label.includes('فردا') || badge.label.includes('روز')) return 'info';
  return 'neutral';
};
...
const CustomersPage: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const fullNameError = requiredTextError(newCustomer.fullName, 'نام کامل');
    const phoneError = phoneNumberError(normalizePhoneForValidation(newCustomer.phoneNumber), { optional: true, label: 'شماره تماس' });
    const nationalCodeError = exactDigitsError(normalizeNationalCodeForValidation(newCustomer.nationalCode), 'کد ملی', 10, { optional: true });
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-7xl min-w-0 gap-4 px-3 text-right sm:px-4" dir="rtl" data-ui-people-page="customers" data-ui-people-scope="list">
        ...
        {isLoading ? (
          ...
          <ManagementDirectoryTable
            id="customers-print-area"
            title="فهرست مشتریان"
            rangeLabel={<>نمایش {pageStart.toLocaleString('fa-IR')} تا {pageEnd.toLocaleString('fa-IR')} از {directoryTotal.toLocaleString('fa-IR')} مشتری</>}
            info="مانده حساب و تعهدات از پرونده‌های ثبت‌شده محاسبه می‌شوند."
            ariaLabel="جدول فهرست مشتریان"
            caption="فهرست مشتریان، وضعیت حساب، تعهدات، اعتبار و عملیات پرونده"
            dataUi="customers"
            columns={[
              { key: 'identity', label: 'مشتری و ارتباط', widthClassName: 'w-[36%]' },
              { key: 'account', label: 'حساب و تعهدات', widthClassName: 'w-[29%]' },
              { key: 'activity', label: 'اعتبار و فعالیت', widthClassName: 'w-[25%]' },
          ...
          >
                  {pagedCustomers.map((customer) => {
                    ...
                    return (
                      <tr key={customer.id} className={MANAGEMENT_DIRECTORY_ROW_CLASS} data-ui-row-accent={getCustomerDueRowAccent(due)}>
                        <td className="px-2.5 py-2 align-middle">
                          <div className="min-w-0 space-y-1.5">
                            <div className="flex min-w-0 items-start gap-2.5">
                              <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-sm font-black text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">{(customer.fullName || '?').trim().charAt(0)}</span>
                        ...
                        <td className="px-2.5 py-2 align-middle">
                          <div className="space-y-1.5">
                            ...
                            {due ? (
                              due.saleId ? (
                                <Link to={`/installment-sales/${due.saleId}?pay=next`} className={`inline-flex max-w-full flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] font-black leading-5 underline-offset-4 hover:underline ${due.className}`} title={due.hint}>
                              ...
                              ) : <span className={`inline-flex max-w-full flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] font-black leading-5 ${due.className}`}><i className={`fa-solid shrink-0 ${due.icon}`} aria-hidden="true" /> {due.label}</span>
      ...
      {isAddModalOpen && (
        ...
        >
          <form noValidate onSubmit={handleAddCustomerSubmit} className="modal-template-form modal-template-form--split modal-template-form--customer" data-ui-customer-modal="canonical-split">
            ...
            <aside className="modal-template-side">
              <PeopleModalSummaryCard
                ...
                metrics={[
                  { icon: 'fa-phone', label: 'شماره تماس', value: <BidiText kind="phone">{newCustomer.phoneNumber || 'ثبت نشده'}</BidiText>, hint: newCustomer.phoneNumber ? 'برای تماس و پیام‌رسانی' : 'اختیاری؛ بعداً قابل تکمیل' },
            ...
            <div className="modal-template-main">
              <FormGrid columns={2} className="modal-template-section modal-template-section--grid">
                <ModalField label="نام کامل" iconClass="fa-solid fa-user" required error={formErrors.fullName}>
                  <TextField type="text" id="fullName" name="fullName" value={newCustomer.fullName}
                         onChange={handleInputChange} required maxLength={120} placeholder="نام و نام خانوادگی مشتری" />
                </ModalField>
                <ModalField label="شماره تماس" iconClass="fa-solid fa-phone" error={formErrors.phoneNumber}>
                  <TextField type="tel" id="phoneNumber" name="phoneNumber" value={newCustomer.phoneNumber}
                         onChange={handleInputChange} valueKind="phone" inputMode="tel" maxLength={18} placeholder="مثال: 09123456789" />
                </ModalField>
                <ModalField label="کد ملی" iconClass="fa-solid fa-id-card" error={formErrors.nationalCode}>
                  <TextField type="text" id="nationalCode" name="nationalCode" value={newCustomer.nationalCode}
                         ...
                         }} valueKind="code" inputMode="numeric" maxLength={10} placeholder="0012345678" autoComplete="off" />
                </ModalField>
              </FormGrid>
              <FormGrid columns={1} className="modal-template-section modal-template-section--stack">
                <ModalField label="آدرس محل سکونت / قرارداد" iconClass="fa-solid fa-location-dot">
                  <TextareaField controlOnly id="address" name="address" rows={2} value={newCustomer.address}
                              onChange={handleInputChange} maxLength={700} placeholder="آدرس ثبت‌شده مشتری" />
                </ModalField>
                <ModalField label="یادداشت داخلی" iconClass="fa-solid fa-note-sticky">
                  <TextareaField controlOnly id="notes" name="notes" rows={3} value={newCustomer.notes}
                              onChange={handleInputChange} maxLength={2000} placeholder="توضیحات پیگیری، رفتار پرداخت یا نکات ارتباطی" />
                </ModalField>
              </FormGrid>
