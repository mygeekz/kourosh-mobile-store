...
import { Dialog as Modal, Drawer, TextField } from '@/components/ui';
...
import {
  REPORT_CENTERS,
  findReportCenter,
  getVisibleCenterTabs,
  isReportTabActive,
  type ReportCenter,
} from "../app/reports/reportCenters";
...
function PremiumModal({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={title}
      widthClass={maxWidthClass}
      variant="expansive"
      layout="split"
      tone="info"
      kicker="مرکز گزارش‌ها"
      wrapperClassName="reports-premium-modal-overlay"
      panelClassName="reports-premium-modal-panel"
      bodyClassName="reports-premium-modal-body"
      ariaDescription={subtitle}
    >
      <div className="reports-premium-modal-card">
        {subtitle || icon ? (
          <div className="reports-premium-modal-summary">
            <span
              className="reports-premium-modal-summary__icon"
              aria-hidden="true"
            >
              {icon ?? <i className="fa-solid fa-chart-line" />}
            </span>
            <div className="min-w-0">{subtitle ? <p>{subtitle}</p> : null}</div>
          </div>
        ) : null}
        <div className="reports-premium-modal-content">{children}</div>
      </div>
      <ModalActions
        onCancel={onClose}
        submitText={primaryLabel}
        onSubmitClick={() => (onPrimary ? onPrimary() : onClose())}
        submitType="button"
        align="end"
        hideHelper={false}
        helperIconClass="fa-solid fa-shield-halved"
        helperTitle="استاندارد گزارش"
        helperText="این پنجره با لایه مودال مشترک، فوکوس امن و اکشن‌بار یکپارچه اجرا می‌شود."
      />
    </Modal>
  );
}
...
const LEGACY_REPORT_META: ReportMeta[] = [
  {
    path: "/reports",
    title: "مرکز گزارش‌ها",
    description: "هشت مرکز کاربردی گزارش‌های فروشگاه",
  },
  {
    path: "/reports/analytics",
    title: "داشبورد تحلیلی قدیمی",
    description: "مسیر سازگاری داشبورد تحلیلی پیشین",
  },
  {
    path: "/reports/analysis",
    title: "هاب تحلیل قدیمی",
    description: "مسیر سازگاری هاب تحلیل پیشین",
  },
];

const CENTER_REPORT_META: ReportMeta[] = REPORT_CENTERS.flatMap((center) =>
  center.tabs.flatMap((tab) => [
    { path: tab.path, title: tab.title, description: tab.description },
    ...(tab.aliases || []).map((path) => ({
      path,
      title: tab.title,
      description: `مسیر سازگاری ${tab.shortTitle}`,
    })),
  ]),
);

const REPORT_META: ReportMeta[] = Array.from(
  new Map([...LEGACY_REPORT_META, ...CENTER_REPORT_META].map((item) => [item.path, item])).values(),
);

function pickMeta(pathname: string): ReportMeta {
  const exact = REPORT_META.find((m) => m.path === pathname);
  if (exact) return exact;
  const pref = REPORT_META.filter((m) => pathname.startsWith(m.path)).sort(
...
function getReportHealthState(
  item: Pick<ReportMeta, "path" | "title">,
): ReportHealthState {
  const path = item.path.toLowerCase();
  if (
    path.includes("audit") ||
    path.includes("aging") ||
    path.includes("dead-stock") ||
    path.includes("collection")
  ) {
    return {
      tone: "warn",
      label: "هشدار",
      title: "این گزارش برای کنترل ریسک یا اختلاف نیازمند بررسی دقیق‌تر است.",
    };
  }
  if (
    ...
    return {
      tone: "review",
      label: "بررسی",
      title: "گزارش کلیدی است و بهتر است قبل از تصمیم مدیریتی مرور شود.",
    };
  }
  return {
    tone: "good",
    label: "سالم",
    title: "گزارش در وضعیت عادی و آماده استفاده است.",
  };
}

function ReportHealthMiniBadge({
  item,
}: {
  item: Pick<ReportMeta, "path" | "title">;
}) {
  ...
  return (
    <span
      className={`reports-health-badge reports-health-badge--${health.tone}`}
      title={health.title}
    >
...
function ReportCenterNavigation({
  center,
  pathname,
  roleName,
}: {
  ...
  const tabs = getVisibleCenterTabs(center, roleName);

  return (
    <section
      className="rounded-[22px] border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/80 print:hidden"
      aria-label={`تب‌های مرکز ${center.title}`}
      data-report-center-navigation={center.id}
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <i className={`fa-solid ${center.icon} mt-1 shrink-0 text-base text-slate-500 dark:text-slate-300`} aria-hidden="true" />
          <div className="min-w-0">
            <div className="text-sm font-black text-slate-950 dark:text-white">{center.title}</div>
            <div className="mt-0.5 text-xs leading-6 text-slate-500 dark:text-slate-400">{center.managerQuestion}</div>
          </div>
        </div>
        <Link to="/reports" className="inline-flex min-h-[40px] shrink-0 items-center justify-center gap-2 rounded-[16px] border border-slate-200 px-3 text-xs font-black text-slate-600 transition hover:border-primary/30 hover:text-primary dark:border-slate-700 dark:text-slate-300">
          <i className="fa-solid fa-table-cells-large" aria-hidden="true" />
          مرکز گزارشات
        </Link>
      </div>

      <nav className="mt-3 flex flex-wrap gap-2" aria-label={`نماهای ${center.title}`}>
        {tabs.map((tab) => {
          const active = isReportTabActive(tab, pathname);
          return (
            <Link
              key={tab.path}
              to={tab.path}
              ...
              className={`inline-flex min-h-[44px] min-w-0 flex-1 basis-[138px] items-center justify-center rounded-[18px] border px-3 text-center text-xs font-black leading-5 transition ${active
                ? 'border-primary bg-primary text-white shadow-sm'
                : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-primary/30 hover:bg-white hover:text-primary dark:border-slate-800 dark:bg-slate-950/35 dark:text-slate-300 dark:hover:border-primary/35 dark:hover:bg-slate-900'
              }`}
            >
              {tab.shortTitle}
            </Link>
...
type ReportCommandItem = {
  ...
  subtitle?: string;
  icon: string;
  group: "مرکزها" | "گزارش‌ها" | "اقدام‌ها" | "نمایش";
  ...
  const renderReportLink = (item: ReportMeta, kind: "favorite" | "recent") => (
    <div key={`${kind}:${item.path}`} className="reports-workspace-row">
      ...
      >
        ...
        <span className="reports-workspace-row__subtitle">
          {item.description}
        </span>
      </Link>
      {kind === "favorite" ? (
  ...
  return (
    <div className="reports-workspace-panel" dir="rtl">
      <div className="reports-workspace-panel__head">
        <div>
          <div className="reports-workspace-panel__title">
            فضای کاری گزارش‌ها
          </div>
          <div className="reports-workspace-panel__subtitle">
            گزارش‌های منتخب و بازدیدهای اخیر در یک نمای مدیریتی
          </div>
        </div>
      ...
      <div className="reports-workspace-panel__grid">
        <section>
          ...
          <div className="reports-workspace-stack">
            {favorites.length ? (
              favorites.map((item) => renderReportLink(item, "favorite"))
            ) : (
              <div className="reports-workspace-empty">
                هنوز گزارشی pin نشده است.
              </div>
            )}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="reports-workspace-stack">
            {recent.length ? (
              recent.map((item) => renderReportLink(item, "recent"))
            ) : (
              <div className="reports-workspace-empty">
                هنوز گزارشی باز نشده است.
              </div>
            )}
          </div>
        </section>
...
function ReportsCommandPalette({
  ...
  return (
    <DialogShell
      ...
      ariaLabel="جستجوی سریع گزارش‌ها"
      overlayClassName="reports-command reports-command--dialog-shell app-modal-backdrop !items-start !justify-center !px-3 !pb-4 !pt-[10vh]"
      panelClassName="reports-command__panel reports-command__panel--dialog-shell"
      closeOnBackdrop
    >
      <div className="reports-command__search">
        <i className="fa-solid fa-magnifying-glass" />
        <TextField controlOnly controlSize="sm"
          autoFocus
          value={query}
      ...
      <div className="reports-command__list" role="listbox">
        {filtered.length ? (
          filtered.map((item, idx) => (
            ...
            >
              <span className="reports-command__icon">
                <i className={`fa-solid ${item.icon}`} />
              </span>
              <span className="min-w-0 flex-1 text-right">
                <span className="reports-command__title">{item.title}</span>
                {item.subtitle ? (
                  <span className="reports-command__subtitle">
                    {item.subtitle}
                  </span>
                ) : null}
              </span>
...
function ReportsCentralActionBar({
  ...
  return (
    <div
      className="reports-central-action-bar reports-central-action-bar--stage3"
      data-report-actions="centralized"
    >
      <div className="reports-central-action-bar__meta">
        <span className="reports-central-action-bar__eyebrow">
          عملیات گزارش
        </span>
        <span className="reports-central-action-bar__title">
          ارسال، زمان‌بندی و خروجی
        </span>
      </div>
      <div
        className="reports-central-action-bar__group"
        role="toolbar"
        aria-label="اکشن‌های مرکزی گزارش"
      >
        ...
        <button
          type="button"
          onClick={onSendNow}
          className={`${BTN_PRIMARY} reports-central-action-bar__primary`}
          data-report-action="send"
        >
...
const ReportsLayout: React.FC = () => {
  ...
  const registerReportExports = useCallback(
    (h: ExportHandlers) => setExportHandlers(h || {}),
    [],
  );
  ...
  const currentCenter = useMemo(() => findReportCenter(pathname), [pathname]);

  const [reportViewMode, setReportViewMode] = useState<"executive" | "analyst">(
    () => {
      try {
        return localStorage.getItem("reports:viewMode") === "analyst"
          ? "analyst"
          : "executive";
      } catch {
        return "executive";
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "reports:favorites",
        JSON.stringify(favoriteReports.slice(0, 12)),
      );
    } catch {
      // ignore storage failures
    }
  ...
  useEffect(() => {
    ...
    setRecentReports((current) =>
      [pathname, ...current.filter((item) => item !== pathname)].slice(0, 12),
    );
  }, [isHub, pathname]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "reports:recent",
        JSON.stringify(recentReports.slice(0, 12)),
      );
    } catch {
      // ignore storage failures
    }
  ...
  const currentReportCanBePinned =
    !isHub && REPORT_META.some((item) => item.path === pathname);
  ...
  const doExportXlsx = async () => {
    ...
    if (!el) return;
    // Keep ExcelJS/report extraction out of the reports shell chunk until the
    // user explicitly clicks the export action.
    const { exportReportToXlsx } = await import("../utils/reportXlsx");
  ...
  const commandItems = useMemo<ReportCommandItem[]>(() => {
    const reportItems: ReportCommandItem[] = REPORT_CENTERS.map((center) => ({
      id: `center:${center.id}`,
      title: center.title,
      subtitle: center.managerQuestion,
      icon: currentCenter?.id === center.id ? "fa-circle-check" : center.icon,
      group: "مرکزها",
      action: () => navigate(center.path),
    }));
        ...
        ? [
            {
              ...
              title: currentReportPinned
                ? "حذف گزارش فعلی از منتخب‌ها"
                : "ثبت کردن گزارش فعلی",
              subtitle: currentReportPinned
                ? "این گزارش از نوار منتخب‌ها حذف می‌شود"
                : "این گزارش به نوار دسترسی سریع اضافه می‌شود",
  ...
  const loadViews = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/reports/saved-filters?reportKey=${encodeURIComponent(reportKey)}`,
        {
      ...
      if (!json?.success)
        throw new Error(json?.message || "خطا در دریافت نماهای ذخیره‌شده");
      const rows = Array.isArray(json.data) ? json.data : [];
      const mapped = rows.map((r: any) => {
        let parsed: any = {};
        try {
          parsed = r?.filtersJson
            ? JSON.parse(String(r.filtersJson))
            : r?.filters || {};
        } catch {
          parsed = {};
        }
  ...
  const saveView = async () => {
    ...
    try {
      const res = await apiFetch("/api/reports/saved-filters", {
        method: "POST",
        headers: {
      ...
      const json = await res.json();
      if (!json?.success)
        throw new Error(json?.message || "خطا در ذخیره نمای گزارش");
      setViewName("");
      await loadViews();
    } catch {
      // silent
  ...
  const applyView = (row: any) => {
    const search = String(
      row?.filtersObj?.search || row?.filters?.search || "",
    );
  ...
  const deleteView = async (id: number) => {
    if (!id) return;
    try {
      const res = await apiFetch(`/api/reports/saved-filters/${id}`, {
        method: "DELETE",
        headers: { ...(getAuthHeaders(token) as any) },
      });
      const json = await res.json();
      if (!json?.success)
        throw new Error(json?.message || "خطا در حذف نمای گزارش");
      await loadViews();
    } catch {
      // silent
    }
  ...
  const sendToTelegramNow = async () => {
    ...
    try {
      ...
      const payloadJson = {
        range: { fromJ: fromJ || undefined, toJ: toJ || undefined },
      };
      const res = await apiFetch("/api/reports/send-telegram", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(getAuthHeaders(token) as any),
      ...
      const json = await res.json();
      if (!json?.success)
        throw new Error(json?.message || "خطا در ارسال گزارش");
      setSendResult(
        `ارسال انجام شد: ${json.data?.sent ?? 0} از ${json.data?.total ?? 0}`,
      );
    } catch (e: any) {
  ...
  return (
    <div
      className="reports-layout-executive reports-redesign-v1 space-y-4"
      data-report-view-mode={reportViewMode}
      data-ui-report-layout="true"
      data-ui-report-page="true"
    >
      {!hideOuterHeader ? (
        <motion.div
        ...
        >
          <div
            ...
            data-ui-report-header-main="true"
          >
            <div
              className="min-w-0 flex-1 text-right"
              data-ui-report-title-cluster="true"
            >
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                <span className="reports-header-kicker">
                  {isHub ? "مرکز گزارش‌ها" : "نمای مدیریتی"}
                </span>
                <h1 className="truncate text-base font-black text-slate-950 dark:text-slate-100 sm:text-lg">
                  {meta.title}
                </h1>
                {!isHub ? (
                  <Link to="/reports" className="reports-header-back-link">
                    <i className="fa-solid fa-arrow-right" />
                    بازگشت
                  </Link>
                ) : null}
              </div>
              <p className="mt-1 max-w-3xl truncate text-xs leading-6 text-slate-500 dark:text-slate-400">
                {meta.description}
              </p>
            </div>
            ...
            <div
              ...
              data-ui-report-header-actions="true"
            >
              <button
                type="button"
                className="reports-workspace-trigger"
              ...
              <button
                type="button"
                className="reports-command-trigger"
                onClick={() => setModal("command")}
                title="جستجوی سریع گزارش‌ها (Ctrl/Cmd + K)"
              >
              ...
              <div
                className="reports-view-mode-toggle"
                role="group"
                aria-label="حالت نمایش گزارش"
              >
                <button
                  type="button"
              ...
              {currentReportCanBePinned ? (
                <button
                  ...
                  className={
                    currentReportPinned
                      ? "reports-favorite-pin is-active"
                      : "reports-favorite-pin"
                  }
          ...
          {favoriteItems.length ? (
            <div
              className="reports-favorites-bar"
              aria-label="گزارش‌های پین‌شده"
              data-ui-report-shortcuts="favorites"
            >
              ...
              {favoriteItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={item.path === pathname ? "is-active" : ""}
                >
                  {item.title}
                </Link>
              ))}
          ...
          {recentItems.length ? (
            <div
              className="reports-recent-bar"
              aria-label="آخرین گزارش‌های بازشده"
              data-ui-report-shortcuts="recent"
            >
      ...
      <Drawer
        isOpen={modal === "workspace"}
        onClose={() => setModal(null)}
        title="فضای کاری گزارش‌ها"
        ariaDescription="گزارش‌های منتخب و بازدیدهای اخیر در یک نمای مدیریتی"
        size="md"
        side="inline-end"
        hideHeader
        bodyPadding="none"
        bodyScroll="hidden"
        overlayClassName="reports-workspace reports-workspace--drawer"
        panelAttributes={{ 'data-report-drawer-kind': 'reports-workspace' }}
      >
            ...
            <div className="space-y-3">
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-950/40 p-3">
                ...
                <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                  اگر Chat IDها یا توکن تنظیم نشده باشد، ارسال انجام نمی‌شود.
                  تنظیمات را از دکمه «تنظیمات تلگرام» تکمیل کن.
                </div>
              </div>
              {sendResult ? (
