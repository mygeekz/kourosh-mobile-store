  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        console.error(e);
        setErr(e?.message || 'خطا در عملیاتی ناشناخته');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="p-4" dir="rtl">
      <div className="print:hidden bg-white dark:bg-gray-800 rounded-xl shadow p-4 mb-4 flex items-center gap-4">
        ...
        <label className="inline-flex items-center gap-2">
          <CheckboxField controlOnly size="sm" type="checkbox" checked={showName} onChange={e => setShowName(e.target.checked)} />
          <span>نمایش نام</span>
        </label>

        <label className="inline-flex items-center gap-2">
          <CheckboxField controlOnly size="sm" type="checkbox" checked={showPrice} onChange={e => setShowPrice(e.target.checked)} />
      ...
      {loading ? (
        ...
        <div className="text-center text-red-600 py-10">خطا در عملیات: {err}</div>
      ) : items.length === 0 ? (
