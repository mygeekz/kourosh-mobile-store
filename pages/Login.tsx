...
import { Button, PasswordVisibilityButton, TextField, requiredTextError } from '../components/ui';
import {
...
const LoginPage = () => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const usernameError = requiredTextError(username, 'نام کاربری');
    const passwordError = requiredTextError(password, 'کلمه عبور');
    if (usernameError) errors.username = usernameError;
    if (passwordError) errors.password = passwordError;
    setLoginErrors(errors);
    if (Object.keys(errors).length) {
      focusErrorsSoon(errors, { username: 'username', password: 'password' });
      return;
    }
    try {
    ...
    <>
      ...
      >
        ...
        >
          <FormErrorSummary
            errors={loginErrors}
            labels={{ username: 'نام کاربری', password: 'کلمه عبور' }}
            fieldIdMap={{ username: 'username', password: 'password' }}
          />
          <TextField
            surface="glass"
            id="username"
            ...
            required
            value={username}
            onChange={(event) => { setUsername(event.target.value); if (loginErrors.username) setLoginErrors((current) => ({ ...current, username: undefined })); }}
          ...
          <TextField
            ...
            required
            value={password}
            onChange={(event) => { setPassword(event.target.value); if (loginErrors.password) setLoginErrors((current) => ({ ...current, password: undefined })); }}
