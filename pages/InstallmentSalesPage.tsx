...
const getCollectionSourceLabel = (source?: string | null) => {
  ...
  if (source === 'manual_receipt') return 'دریافت نقدی پرونده';
  return 'پرداخت قسط';
};
    ...
    >
      <div className="mx-auto grid max-w-7xl min-w-0 gap-4 px-3 text-right sm:px-4" dir="rtl" data-ui-management-page="installments">
        <ManagementDirectoryOverview
          ...
          navigation={(
            <div className="grid w-full shrink-0 grid-cols-2 items-center gap-1 rounded-2xl border border-slate-200 bg-slate-50 p-1 sm:w-72 dark:border-slate-700 dark:bg-slate-900" aria-label="نمای فروش اقساطی" role="tablist">
              <button
                type="button"
                role="tab"
                aria-selected={tab === 'main'}
                data-skip-global-button="true"
                className={`inline-flex min-h-10 min-w-0 items-center justify-center gap-2 rounded-xl px-2.5 text-xs font-black transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 dark:focus-visible:ring-slate-500 dark:focus-visible:ring-offset-slate-950 ${tab === 'main' ? 'bg-slate-900 text-white shadow-sm dark:bg-white dark:text-slate-950' : 'text-slate-700 hover:bg-white dark:text-slate-200 dark:hover:bg-slate-800'}`}
                onClick={() => setTab('main')}
              >
                <i className="fa-solid fa-file-invoice-dollar shrink-0 text-[11px]" aria-hidden="true" />
                <span className="whitespace-nowrap">لیست اقساط</span>
              </button>
              <button
                type="button"
                role="tab"
                ...
                data-skip-global-button="true"
                className={`inline-flex min-h-10 min-w-0 items-center justify-center gap-2 rounded-xl px-2.5 text-xs font-black transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 dark:focus-visible:ring-slate-500 dark:focus-visible:ring-offset-slate-950 ${tab === 'telegram' ? 'bg-slate-900 text-white shadow-sm dark:bg-white dark:text-slate-950' : 'text-slate-700 hover:bg-white dark:text-slate-200 dark:hover:bg-slate-800'}`}
                onClick={() => setTab('telegram')}
              >
                <i className="fa-brands fa-telegram shrink-0 text-[11px]" aria-hidden="true" />
          ...
          >
            ...
            {isLoading ? (
              <div className="w-full overflow-x-auto overscroll-x-contain" role="region" aria-label="در حال بارگذاری فهرست فروش اقساطی" tabIndex={0}>
                <Table className="w-full min-w-[62rem] table-fixed border-collapse text-xs" dir="rtl" data-ui-bidi-scope="rtl-table" layout="managed" density="compact" data-ui-table-layout="managed" data-ui-table-density="compact">
                  <colgroup>
                    <col className="w-[28%]" />
                    <col className="w-[22%]" />
                    <col className="w-[16%]" />
