...
type AccessoryFormErrorKey = 'productId' | 'quantity';
type ServiceFormErrorKey = 'serviceId' | 'quantity' | 'price';
...
const AddInstallmentSalePage: React.FC = () => {
  ...
  const [accessoryFormErrors, setAccessoryFormErrors] = useState<Partial<Record<AccessoryFormErrorKey, string>>>({});
  ...
  const [serviceFormErrors, setServiceFormErrors] = useState<Partial<Record<ServiceFormErrorKey, string>>>({});
  ...
  const validateCurrentCheck = () => {
    ...
    const ownershipError = requiredSelectionError(currentCheck.ownershipType, 'مالکیت چک');
    const issuerNameError = requiredTextError(issuerName, 'نام و نام خانوادگی صادرکننده');
    const issuerNationalCodeError = exactDigitsError(issuerNationalCode, 'کد ملی صادرکننده', 10);
    if (ownershipError) errors.ownershipType = ownershipError;
    if (issuerNameError) errors.issuerName = issuerNameError;
    ...
    const sayadiError = exactDigitsError(sayadiId, 'شناسه صیادی', 16);
    const checkNumberError = requiredTextError(checkNumber, 'شماره چک');
    const bankNameError = requiredTextError(bankName, 'نام بانک صادرکننده');
    const amountError = positiveNumberError(currentCheck.amount, 'مبلغ چک');
    const dueDateError = requiredSelectionError(currentCheckDueDate, 'تاریخ سررسید چک');
    if (sayadiError) errors.sayadiId = sayadiError;
  ...
  const selectAccessoryProduct = (value: number | null) => {
    ...
    setAccessoryFormErrors((current) => ({ ...current, productId: undefined }));
  ...
  const addAccessoryLine = () => {
    const errors = {
      productId: requiredSelectionError(accessoryProductId, 'محصول'),
      quantity: positiveNumberError(accessoryQty, 'تعداد', { integer: true }),
    } satisfies Partial<Record<AccessoryFormErrorKey, string | undefined>>;
    const normalizedErrors = Object.fromEntries(Object.entries(errors).filter(([, value]) => Boolean(value))) as Partial<Record<AccessoryFormErrorKey, string>>;
    setAccessoryFormErrors(normalizedErrors);
    if (Object.keys(normalizedErrors).length) {
      scheduleTimeout(() => focusFirstError(normalizedErrors as FormErrors, { productId: 'accessoryProductId', quantity: 'accessoryQty' }), 40);
      return;
    }
  ...
  const selectServiceToAdd = (value: number | null) => {
    ...
    setServiceFormErrors((current) => ({ ...current, serviceId: undefined }));
    if (id) {
  ...
  const addServiceLine = () => {
    const errors = {
      serviceId: requiredSelectionError(serviceToAddId, 'خدمت'),
      quantity: positiveNumberError(serviceQty, 'تعداد', { integer: true }),
      price: positiveNumberError(serviceModalPrice, 'قیمت خدمت'),
    } satisfies Partial<Record<ServiceFormErrorKey, string | undefined>>;
    const normalizedErrors = Object.fromEntries(Object.entries(errors).filter(([, value]) => Boolean(value))) as Partial<Record<ServiceFormErrorKey, string>>;
    setServiceFormErrors(normalizedErrors);
    if (Object.keys(normalizedErrors).length) {
      scheduleTimeout(() => focusFirstError(normalizedErrors as FormErrors, { serviceId: 'serviceToAddId', quantity: 'serviceQty', price: 'serviceModalPrice' }), 40);
      return;
    }
  ...
  const validateForm = (): boolean => {
    ...
    const customerError = requiredSelectionError(formData.customerId, 'مشتری');
    ...
    const buyerNationalCode = normalizeNumericIdentity(formData.buyerNationalCode);
    const buyerNationalCodeError = exactDigitsError(buyerNationalCode, 'کد ملی خریدار', 10);
    if (buyerNationalCodeError) errors.buyerNationalCode = buyerNationalCodeError;
    ...
    const discountError = nonNegativeNumberError(discountToman, 'تخفیف');
    ...
    const profitPercentError = nonNegativeNumberError(profitPercent, 'درصد سود ماهانه');
    if (profitPercentError) errors.profitPercent = profitPercentError;
    ...
    if (saleType === 'installment') {
      ...
      const installmentCountError = positiveNumberError(months, 'تعداد اقساط', { integer: true });
      const installmentAmountError = positiveNumberError(nominalInstallmentAmount, 'مبلغ هر قسط');
      if (installmentCountError) errors.numberOfInstallments = installmentCountError;
      if (installmentAmountError) errors.installmentAmount = installmentAmountError;
...
const submitStageIcon = submitStageProgress === 1
  ...
  return (
    <div className="installment-form-page space-y-6 text-right max-w-7xl mx-auto px-4" dir="rtl">
      ...
      <form noValidate ref={formRef} onKeyDownCapture={onKeyDownCapture} onSubmit={handleSubmit} className="max-w-6xl mx-auto" aria-busy={isLoading} data-ergonomic-form="true">
        <FormErrorSummary
          ...
          >
            ...
            <div>
              ...
              <TextField
                ...
                error={formErrors.buyerNationalCode}
                valueKind="code"
                inputMode="numeric"
                autoComplete="off"
                maxLength={10}
              ...
              />
              <p className="mt-1.5 text-[11px] text-slate-500 dark:text-slate-400">از پروفایل مشتری تکمیل می‌شود و همراه قرارداد ثبت خواهد شد.</p>
        ...
        {isAccessoryModalOpen && (
          <Modal
            ...
            onClose={() => { setAccessoryFormErrors({}); setIsAccessoryModalOpen(false); }}
          ...
          >
            <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]" dir="rtl">
              <PanelCard
                ...
                density="compact"
              >
                <ModalField label="محصول" iconClass="fa-solid fa-box" required error={accessoryFormErrors.productId}>
                  <SearchableSelectField<number>
                    inputId="accessoryProductId"
              ...
              >
                <ModalField
                  ...
                  error={accessoryFormErrors.quantity}
                >
                  <TextField
                    id="accessoryQty"
                    type="number"
                    min={1}
                    value={accessoryQty}
                    onChange={(e) => { setAccessoryQty(Math.max(1, Number(e.target.value || 1))); if (accessoryFormErrors.quantity) setAccessoryFormErrors((current) => ({ ...current, quantity: undefined })); }}
                    inputMode="numeric"
                    dir="ltr"
                  />
                </ModalField>
            ...
            <DialogActions
              onCancel={() => { setAccessoryFormErrors({}); setIsAccessoryModalOpen(false); }}
              cancelText="انصراف"
        ...
        {isServiceModalOpen && (
          <Modal
            title="افزودن خدمت"
            onClose={() => { setServiceFormErrors({}); setIsServiceModalOpen(false); }}
            widthClass="max-w-4xl"
          ...
          >
            <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(18rem,0.9fr)]" dir="rtl">
              <PanelCard
                ...
                density="compact"
              >
                <ModalField label="انتخاب خدمت" iconClass="fa-solid fa-wrench" required error={serviceFormErrors.serviceId}>
                  <SearchableSelectField<number>
                    inputId="serviceToAddId"
              ...
              >
                <FormGrid columns={2}>
                  <ModalField label={`قیمت خدمت (${getCurrencyUnitLabel(readStoredCurrencyUnit())})`} iconClass="fa-solid fa-coins" required error={serviceFormErrors.price}>
                    <PriceInput
                      id="serviceModalPrice"
                      name="serviceModalPrice"
                      value={String(serviceModalPrice || '')}
                      onChange={(e: any) => { setServiceModalPrice(toNumber(e.target.value)); if (serviceFormErrors.price) setServiceFormErrors((current) => ({ ...current, price: undefined })); }}
                  ...
                  <ModalField label="تعداد" iconClass="fa-solid fa-hashtag" required error={serviceFormErrors.quantity}>
                    <TextField
                      id="serviceQty"
                      type="number"
                      min={1}
                      value={serviceQty}
                      onChange={(e) => { setServiceQty(Math.max(1, Number(e.target.value || 1))); if (serviceFormErrors.quantity) setServiceFormErrors((current) => ({ ...current, quantity: undefined })); }}
                      inputMode="numeric"
                      dir="ltr"
                    />
                  </ModalField>
            ...
            <DialogActions
              onCancel={() => { setServiceFormErrors({}); setIsServiceModalOpen(false); }}
              cancelText="انصراف"
              submitText="افزودن به قرارداد"
              submitType="button"
        ...
        <div id="installment-step-finance" className={["mb-8", wizardStep === 2 ? '' : 'hidden lg:block'].join(' ')}>
          ...
          >
          {/* قیمت‌ها */}
          <FormGrid columns={4} className="mb-4">
            <div className="space-y-1">
              ...
              <TextField controlOnly type="text" valueKind="currency" readOnly value={formatCurrencyText(phonesSellTotal, readStoredCurrencyUnit())} />
            </div>
            ...
            <div className="space-y-1">
              ...
              <TextField controlOnly type="text" valueKind="currency" readOnly value={formatCurrencyText(accessoriesSellTotal, readStoredCurrencyUnit())} />
            </div>
            ...
            <div className="space-y-1">
              ...
              <TextField controlOnly type="text" valueKind="currency" readOnly value={formatCurrencyText(servicesSellTotal, readStoredCurrencyUnit())} />
            </div>
            ...
            <div className="space-y-1">
              ...
              <TextField controlOnly id="actualSalePrice" type="text" valueKind="currency" readOnly value={formatCurrencyText(finalPrice, readStoredCurrencyUnit())} />
            </div>
          </FormGrid>

          {/* پیش‌پرداخت، سود ماهانه، تخفیف */}
          <FormGrid columns={3} className="mb-2">
            <div className="space-y-1">
            ...
            <div>
              {renderNumberField({
                ...
                min: 0,
                id: 'profitPercent',
                error: formErrors.profitPercent,
              })}
            ...
            <div className="space-y-1">
              ...
              <PriceInput
                ...
                error={(formErrors as any).discount}
          ...
          {saleType === 'installment' ? (
            <FormGrid columns={3}>
              ...
              <div className="space-y-1">
                ...
                <TextField controlOnly type="text" valueKind="currency" readOnly value={formatCurrencyText(remainingAfterDownPayment, readStoredCurrencyUnit())} />
              </div>
            </FormGrid>
          ) : (
            <FormGrid columns={3}>
              ...
              <div className="space-y-1">
                ...
                <TextField controlOnly type="text" valueKind="currency" readOnly value={formatCurrencyText(totalInterest, readStoredCurrencyUnit())} />
              </div>
