import { FormGrid, FormGridItem, SearchableSelectField, TableActionGroup, TextField, compactValidationErrors, positiveNumberError, requiredSelectionError } from '@/components/ui';
...
const Purchases: React.FC = () => {
  ...
  const purchaseFieldIds = useMemo(() => {
    const ids: Record<string, string> = { supplierId: 'purchase-supplier' };
    items.forEach((_, idx) => {
      ids[`item${idx}Product`] = `purchase-item-${idx}-product`;
      ids[`item${idx}Quantity`] = `purchase-item-${idx}-quantity`;
    });
    return ids;
  }, [items]);

  const purchaseFieldLabels = useMemo(() => {
    const labels: Record<string, string> = { supplierId: 'تأمین‌کننده' };
    items.forEach((_, idx) => {
      labels[`item${idx}Product`] = `کالای ردیف ${(idx + 1).toLocaleString('fa-IR')}`;
      labels[`item${idx}Quantity`] = `تعداد ردیف ${(idx + 1).toLocaleString('fa-IR')}`;
    });
    return labels;
  }, [items]);
  ...
  const validatePurchase = () => {
    const next: Record<string, string | undefined> = {
      supplierId: requiredSelectionError(supplierId, 'تأمین‌کننده'),
    };
    items.forEach((item, idx) => {
      if (!item.productId) return;
      next[`item${idx}Product`] = requiredSelectionError(item.productId, `کالای ردیف ${(idx + 1).toLocaleString('fa-IR')}`);
      next[`item${idx}Quantity`] = positiveNumberError(item.quantity, `تعداد ردیف ${(idx + 1).toLocaleString('fa-IR')}`, { integer: true });
    });
    const hasValidItem = items.some((item) => item.productId && Number(item.quantity) > 0);
    if (!hasValidItem) {
      const first = 0;
      next[`item${first}Product`] = next[`item${first}Product`] || 'حداقل یک کالا را انتخاب کنید.';
    }
    const errors = compactValidationErrors(next);
    setFormErrors(errors);
    if (Object.keys(errors).length) focusErrorsSoon(errors, purchaseFieldIds);
    return errors;
  };
  ...
  return (
    <div className="p-4 md:p-6 inventory-shell-modern" dir="rtl">
      ...
      <div className="app-card inventory-card-premium p-4 md:p-6 space-y-4">
        ...
        <FormErrorSummary errors={formErrors} labels={purchaseFieldLabels} fieldIdMap={purchaseFieldIds} />
        <FormGrid columns={3} gap="md">
          <SearchableSelectField<number>
              inputId={purchaseFieldIds.supplierId}
              label="تأمین‌کننده"
              required
              error={formErrors.supplierId}
              value={supplierId || null}
              onValueChange={(value) => { setSupplierId(value ?? ""); if (formErrors.supplierId) setFormErrors((prev) => ({ ...prev, supplierId: undefined })); }}
            ...
            <TextField controlSize="sm"
              label="شماره فاکتور تأمین‌کننده (اختیاری)"
              valueKind="code"
              value={invoiceNumber}
          ...
          {items.map((it, idx) => {
            ...
            return (
              <div key={idx} className="app-card-muted p-3">
                <FormGrid columns={4} gap="md" align="start">
                  <FormGridItem span={2}>
                    <SearchableSelectField<number>
                      inputId={purchaseFieldIds[`item${idx}Product`]}
                      label="کالا"
                      required
                      error={formErrors[`item${idx}Product`]}
                      ...
                      onValueChange={(value) => {
                        ...
                        setFormErrors((prev) => ({ ...prev, [`item${idx}Product`]: undefined }));
                      }}
                  ...
                  <FormGridItem>
                    <PriceInput
                      id={purchaseFieldIds[`item${idx}Quantity`]}
                      name={`quantity-${idx}`}
                      value={String(it.quantity ?? '')}
                      error={formErrors[`item${idx}Quantity`]}
                      onChange={(e) => { updateItem(idx, { quantity: Number(e.target.value || 0) }); setFormErrors((prev) => ({ ...prev, [`item${idx}Quantity`]: undefined })); }}
                  ...
                  <FormGridItem span="full" className="flex items-center justify-between gap-2">
