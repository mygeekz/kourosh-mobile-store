import { SelectField, Table, TextField } from '@/components/ui';
...
const money = (n: number) => formatReportMoneyText(n ?? 0);
const faNum = (n: number) => formatExactNumberText(n ?? 0);
...
const TopCustomersReportPage: React.FC = () => {
  ...
  const fetchTopCustomers = async () => {
    ...
    try {
      const res = await apiFetch(`/api/reports/top-customers?fromDate=${encodeURIComponent(fromDate)}&toDate=${encodeURIComponent(toDate)}`, {
  ...
  useEffect(() => {
    if (pendingRestorePageRef.current != null) {
      setPageIndex(pendingRestorePageRef.current);
      if (rows.length > 0) pendingRestorePageRef.current = null;
      return;
    }
    setPageIndex(0);
  ...
  const restoreReportUiState = React.useCallback((state: Record<string, unknown>) => {
    const restoredPage = Math.max(0, Number(state.pageIndex || 0));
  ...
  const summary = useMemo(() => {
    ...
    const avgBasket = totalTx > 0 ? totalSpent / totalTx : 0;
    const top = rows[0];
    const topShare = top && totalSpent > 0 ? ((top.totalSpent ?? 0) / totalSpent) * 100 : 0;
        ...
        <div className="top-customers-filter-row">
          ...
          <div className="top-customers-filter-fields">
            <label className="top-customers-date-box">
              <span>از تاریخ</span>
              <ShamsiDatePicker selectedDate={startDate} onDateChange={setStartDate} />
            </label>
            <label className="top-customers-date-box">
              <span>تا تاریخ</span>
              <ShamsiDatePicker selectedDate={endDate} onDateChange={(date) => setEndDate(date && startDate && date < startDate ? startDate : date)} />
            </label>
            ...
            <Table className="top-customers-table">
              ...
              <tbody>
                {pageRows.map((row, index) => {
                  ...
                  return (
                    <tr key={row.customerId} data-navigation-anchor={reportNavigationAnchor('top-customers', row.customerId)}>
                      ...
                      <td>
                        <Link className="top-customers-row-action" to={`/customers/${row.customerId}`} onClick={(event) => onDrilldownClick(event, `/customers/${row.customerId}`, { contextLabel: `${row.fullName || 'مشتری'} • رتبه ${faNum(rank)}` })}>
