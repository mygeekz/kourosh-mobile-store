...
const RfmReport: React.FC = () => {
  ...
  const reportUiState = useMemo(() => ({
    ...
    activeSegment,
  }), [activeSegment, pageIndex, pageSize, searchQuery]);
  ...
  const restoreReportUiState = React.useCallback((state: Record<string, unknown>) => {
    ...
    setActiveSegment(String(state.activeSegment || 'all'));
  ...
  const avgRecency = useMemo(() => (items.length ? items.reduce((acc, it) => acc + (Number(it.recencyDays) || 0), 0) / items.length : 0), [items]);
              ...
              <Table className="rfm-table-final">
                ...
                <tbody>
                  {pageRows.map((row) => {
                    ...
                    return (
                      <tr key={row.customerId} data-navigation-anchor={reportNavigationAnchor('rfm', row.customerId)}>
                        <td>
                        ...
                        <td>
                          <div className="rfm-row-actions">
                            <Link to={customerPath(row.customerId)} onClick={(event) => onDrilldownClick(event, customerPath(row.customerId), { contextLabel: `${row.customerName || 'مشتری'} • سگمنت ${row.rfm || '—'}` })} className="rfm-row-action">
