...
import { Button, DataTableShell, TextField } from '@/components/ui';
...
const fmtQty = (n: any) => formatExactNumberText(Number(n || 0));
...
export default function InventoryFifo() {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await apiFetch('/api/reports/inventory-fifo', { headers: { Authorization: `Bearer ${token}` } });
      const js = await res.json();
      if (!res.ok || js?.success === false) throw new Error(js?.message || 'خطا در دریافت گزارش انبار');
      setData(js.data || []);
      try {
        const bRes = await apiFetch('/api/reports/inventory-aging-buckets', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const exportExcel = async () => {
    if (!token) return;
    const res = await apiFetch('/api/exports/inventory-fifo.xlsx', { headers: { Authorization: `Bearer ${token}` } });
...
{notification ? <Notification message={notification.message} type={notification.type} /> : null}
      ...
      <DataTableShell data-ui-inventory-fifo-table="true">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-gray-500 dark:text-gray-400">موردی یافت نشد.</div>
            ...
            <Table layout="managed" density="comfortable" className="min-w-[860px] w-full text-sm">
                            ...
                            <div key={i} className="rounded-xl border bg-white/60 p-2 dark:bg-slate-900/40 dark:border-slate-800">
                              <div className="flex items-center justify-between gap-2">
                                <div className="text-xs text-gray-600 dark:text-gray-300">
                                  {moment(l.entryDate).locale('fa').format('jYYYY/jMM/jDD')} • {formatExactNumberText(Number(l.ageDays))} روز
                                </div>
