...
const fmtMoney = (n: number | undefined | null) => formatReportMoneyText(n ?? 0);
...
const InstallmentsCalendarPage: React.FC = () => {
  ...
  const reportUiState = useMemo(() => ({
    ...
    typeFilter,
    statusFilter,
  }), [fromDate, searchQuery, statusFilter, toDate, typeFilter]);
  ...
  const restoreReportUiState = React.useCallback((state: Record<string, unknown>) => {
    ...
    setTypeFilter((['all', 'payment', 'check'].includes(String(state.typeFilter)) ? String(state.typeFilter) : 'all') as 'all' | 'payment' | 'check');
    setStatusFilter((['all', 'pending', 'passed', 'paid', 'cashed', 'bounced'].includes(String(state.statusFilter)) ? String(state.statusFilter) : 'all') as 'all' | 'pending' | 'passed' | 'paid' | 'cashed' | 'bounced');
  }, []);

  const { onDrilldownClick } = useReportDrilldownNavigation({
    reportKey: 'installments-calendar',
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/reports/installments-calendar?${qs.toString()}`, { headers: getAuthHeaders(token) });
  ...
  const metricCards = [
    { label: 'کل سررسیدها', value: formatExactNumberText(stats.counts.all), hint: 'آیتم‌های فیلترشده', icon: 'fa-calendar-check', tone: 'blue' },
    { label: 'جمع مبلغ', value: fmtMoney(stats.total), hint: 'بر اساس بازه انتخابی', icon: 'fa-sack-dollar', tone: 'green' },
    { label: 'در انتظار', value: formatExactNumberText(stats.counts.pending), hint: 'اقساط و چک‌های باز', icon: 'fa-hourglass-half', tone: 'amber' },
    { label: 'سررسید گذشته', value: formatExactNumberText(stats.counts.passed), hint: 'نیازمند پیگیری فوری', icon: 'fa-triangle-exclamation', tone: 'red' },
  ];
  ...
  return (
    <div className="report-page installments-calendar-executive-page" dir="rtl">
      ...
      <section className="installments-calendar-filter-panel" aria-label="فیلترهای تقویم اقساط و چک‌ها">
        <div className="installments-calendar-filter-row">
          ...
          <div className="installments-calendar-date-group">
            <label className="installments-calendar-date-box"><span>از تاریخ</span><ShamsiDatePicker selectedDate={fromDate} onDateChange={setFromDate} /></label>
            <label className="installments-calendar-date-box"><span>تا تاریخ</span><ShamsiDatePicker selectedDate={toDate} onDateChange={setToDate} /></label>
      ...
      <section className="installments-calendar-toolbar" aria-label="جستجو و فیلتر تقویم">
        ...
        <div className="installments-calendar-statusbar" aria-label="وضعیت آیتم‌ها"><button type="button" className={statusFilter === 'all' ? 'is-active' : ''} onClick={() => setStatusFilter('all')}>همه {formatExactNumberText(stats.counts.all)}</button><button type="button" className={statusFilter === 'pending' ? 'is-active' : ''} onClick={() => setStatusFilter('pending')}>در انتظار {formatExactNumberText(stats.counts.pending)}</button><button type="button" className={statusFilter === 'passed' ? 'is-active' : ''} onClick={() => setStatusFilter('passed')}>سررسید گذشته {formatExactNumberText(stats.counts.passed)}</button><button type="button" className={statusFilter === 'paid' ? 'is-active' : ''} onClick={() => setStatusFilter('paid')}>پرداخت شده {formatExactNumberText(stats.counts.paid)}</button><button type="button" className={statusFilter === 'cashed' ? 'is-active' : ''} onClick={() => setStatusFilter('cashed')}>وصول شده {formatExactNumberText(stats.counts.cashed)}</button><button type="button" className={statusFilter === 'bounced' ? 'is-active' : ''} onClick={() => setStatusFilter('bounced')}>برگشتی {formatExactNumberText(stats.counts.bounced)}</button></div>
      </section>

      {isLoading ? <div className="installments-calendar-empty-state">در حال دریافت…</div> : grouped.length === 0 ? <div className="installments-calendar-empty-state">آیتمی برای نمایش وجود ندارد.</div> : <div className="installments-calendar-groups">{grouped.map((g) => { const dateTotal = g.items.reduce((a, it) => a + (it.amount || 0), 0); return <section key={g.date} className="installments-calendar-day-group"><header className="installments-calendar-day-group__head"><div className="installments-calendar-day-title"><span className="installments-calendar-day-title__icon"><i className="fa-regular fa-clock" /></span><strong>{g.date}</strong><small>{formatExactNumberText(g.items.length)} آیتم</small></div><div className="installments-calendar-day-total">{fmtMoney(dateTotal)}</div></header><div className="installments-calendar-item-grid">{g.items.map((it) => <article key={`${it.type}-${it.id}`} data-navigation-anchor={reportNavigationAnchor('installments-calendar', `${it.type}-${it.id}`)} className="installments-calendar-item-card"><div className="installments-calendar-item-card__main"><span className="installments-calendar-item-card__icon"><i className={it.type === 'payment' ? 'fa-solid fa-hand-holding-dollar' : 'fa-solid fa-money-check'} /></span><div className="installments-calendar-item-card__content"><div className="installments-calendar-item-card__title"><strong>{it.type === 'payment' ? 'قسط' : 'چک'} • {fmtMoney(it.amount)}</strong>{statusPill(it.status)}</div>{it.type === 'check' && (it.bankName || it.checkNumber) ? <small>{it.bankName ? `بانک: ${it.bankName}` : ''}{it.bankName && it.checkNumber ? ' • ' : ''}{it.checkNumber ? `چک: ${it.checkNumber}` : ''}</small> : null}<Link className="installments-calendar-customer-link" to={`/customers/${it.customerId}`} onClick={(event) => onDrilldownClick(event, `/customers/${it.customerId}`, { contextLabel: `${it.customerFullName} • ${it.type === 'payment' ? 'قسط' : 'چک'} ${fmtMoney(it.amount)}` })}>{it.customerFullName}</Link>{it.customerPhoneNumber ? <small dir="ltr">{it.customerPhoneNumber}</small> : null}</div></div><Link className="installments-calendar-item-action" to={`/installment-sales/${it.saleId}`} onClick={(event) => onDrilldownClick(event, `/installment-sales/${it.saleId}`, { contextLabel: `${it.customerFullName} • ${it.type === 'payment' ? 'قسط' : 'چک'} ${fmtMoney(it.amount)}` })}><i className="fa-solid fa-arrow-up-from-bracket" />جزئیات</Link></article>)}</div></section>; })}</div>}
    </div>
  );
};
