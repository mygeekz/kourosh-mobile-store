...
const faNum = (value: number) => formatExactNumberText(value);
...
const CohortReport: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const avgM2 = m2.length ? (m2.reduce((a, b) => a + b, 0) / m2.length) * 100 : 0;
    ...
    return {
      ...
      bestScore: bestScore > 0 ? bestScore * 100 : 0,
    };
  }, [rows, maxMonths]);
    ...
    <div className="report-page cohort-executive-page" dir="rtl">
        ...
        <div className="cohort-metric-grid">
          <article className="cohort-metric-card">
            ...
            <div>
              ...
              <strong>{formatExactNumberText(stats.cohorts)}</strong>
          ...
          <article className="cohort-metric-card">
            ...
            <div>
              ...
              <strong>{formatExactNumberText(stats.totalCustomers)}</strong>
          ...
          <article className="cohort-metric-card">
            ...
            <div>
              ...
              <strong>{formatReadablePercentText(stats.avgM2, 1)}</strong>
          ...
          <article className="cohort-metric-card">
            ...
            <div>
              ...
              <strong>ماه {formatExactNumberText(stats.bestMonth)}</strong>
              <small>بالاترین نگهداشت: {formatReadablePercentText(stats.bestScore, 1)}</small>
            </div>
          </article>
        </div>
      </section>
      ...
      <section className="cohort-section" aria-label="ماتریس بازگشت مشتری">
              ...
              <tbody>
                {pageRows.map((row) => (
                  <tr key={row.cohortMonth}>
                    ...
                    <td className="cohort-sticky-col cohort-sticky-col--count">{formatExactNumberText((row.customersCount || 0))}</td>
                    {Array.from({ length: maxMonths }, (_, i) => {
                      const val = row.counts && row.counts[i] != null ? row.counts[i] : 0;
                      const percentNumber = row.customersCount ? (val / row.customersCount) * 100 : 0;
                      return (
                        <td key={i} className="cohort-heat-cell" style={row.customersCount ? cellStyle(percentNumber) : undefined} title={`${formatExactNumberText(val)} نفر - ${formatReadablePercentText(percentNumber, 1)}`}>
                          <strong>{formatExactNumberText(val)}</strong>
                          <span>{formatReadablePercentText(percentNumber, 1)}</span>
                        </td>
                      );
                    })}
                  </tr>
        ...
        <div className="cohort-pagination">
          <span>صفحه {formatExactNumberText(safePage)} از {formatExactNumberText(pageCount)}</span>
          <div>
