...
const money = (value: number) => formatReportMoneyText(value);
...
const CreditorsReportPage: React.FC = () => {
  ...
  const fetchCreditors = async () => {
    ...
    try {
      const response = await apiFetch('/api/reports/creditors', { headers: getAuthHeaders(token) });
  ...
  const avgCredit = useMemo(() => (creditors.length ? totalCredit / creditors.length : 0), [creditors, totalCredit]);
              ...
              <Table className="creditors-table-final">
                ...
                <tbody>
                  {pageRows.map((row) => {
                    ...
                    const pct = maxCredit ? Math.min(100, (credit / maxCredit) * 100) : 0;
                    return (
                      <tr key={row.id} data-navigation-anchor={reportNavigationAnchor('creditors', row.id)}>
                        <td>
                        ...
                        <td>
                          <div className="creditors-row-actions">
                            <Link to={partnerLedgerPath(row.id)} onClick={(event) => onDrilldownClick(event, partnerLedgerPath(row.id), { contextLabel: `${row.partnerName || 'همکار'} • بستانکاری ${money(credit)}` })} className="creditors-row-action creditors-row-action--primary">
                              <i className="fa-solid fa-user-tie" />
                              <span>حساب همکار</span>
                            </Link>
                            {row.sourceUrl ? (
                              <Link to={reportSourcePath(row.sourceUrl)} onClick={(event) => onDrilldownClick(event, reportSourcePath(row.sourceUrl), { contextLabel: `${row.partnerName || 'همکار'} • ${row.sourceLabel || 'سند ریشه'}`, targetEntity: { sourceLabel: row.sourceLabel || undefined, preview: { eyebrow: 'سند ریشه بستانکاری', status: 'بستانکار', statusTone: 'success', items: [{ label: 'همکار', value: row.partnerName || 'بدون نام', iconClass: 'fa-solid fa-handshake' }, { label: 'مانده بستانکاری', value: money(credit), iconClass: 'fa-solid fa-coins', tone: 'success' }], note: row.sourceLabel || undefined } } })} className="creditors-row-action" title={row.sourceLabel || 'مشاهده سند ریشه'}>
