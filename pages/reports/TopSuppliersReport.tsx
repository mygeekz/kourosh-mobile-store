...
const formatPrice = (n: number) => formatReportMoneyText(n ?? 0);
const formatNum = (n: number) => formatExactNumberText(n ?? 0);
...
const TopSuppliersReportPage: React.FC = () => {
  ...
  const fetchTopSuppliers = async () => {
    ...
    try {
      const res = await apiFetch(`/api/reports/top-suppliers?fromDate=${fromDate}&toDate=${toDate}`, {
  ...
  const insights = useMemo(() => {
    ...
    const avgPurchase = totalTransactions > 0 ? totalPurchaseValue / totalTransactions : 0;
    const topSupplier = rows[0];
    const topShare = totalPurchaseValue > 0 && topSupplier
      ? ((topSupplier.totalPurchaseValue ?? 0) / totalPurchaseValue) * 100
      : 0;
    return {
      count: rows.length,
      totalPurchaseValue,
          ...
          <div className="top-suppliers-bento-grid" aria-label="کارت‌های تأمین‌کنندگان برتر">
            {topCards.map((supplier, index) => {
              const share = insights.totalPurchaseValue > 0
                ? ((supplier.totalPurchaseValue ?? 0) / insights.totalPurchaseValue) * 100
                : 0;
              return (
          ...
          <div className="top-suppliers-table-header">
            ...
            <div className="top-suppliers-table-tools">
              ...
              <label className="top-suppliers-page-size">
                <SelectField controlOnly unstyled showChevron={false} value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))} aria-label="تعداد ردیف در صفحه">
              ...
              <Table className="top-suppliers-table">
                ...
                <tbody>
                  {pageRows.map((row, index) => (
                    <tr key={row.partnerId} data-navigation-anchor={reportNavigationAnchor('top-suppliers', row.partnerId)}>
                      ...
                      <td>
                        <Link to={`/partners/${row.partnerId}`} onClick={(event) => onDrilldownClick(event, `/partners/${row.partnerId}`, { contextLabel: `${row.partnerName || 'تأمین‌کننده'} • رتبه ${formatNum(safePageIndex * pageSize + index + 1)}` })} className="top-suppliers-action-button">
