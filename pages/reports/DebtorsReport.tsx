import { Table, TextField, SelectField } from '@/components/ui';
...
const money = (n: number) => formatReportMoneyText(n);
const faNum = (value: number | string) => formatExactNumberText(value);
...
const DebtorsReportPage: React.FC = () => {
  ...
  const fetchDebtors = async () => {
    ...
    try {
      const response = await apiFetch('/api/reports/debtors', { headers: getAuthHeaders(token) });
  ...
  useEffect(() => {
    if (!token) return;
    void fetchDebtors();
    const refreshAfterInstallmentPayment = () => { void fetchDebtors(); };
    window.addEventListener('kourosh:installment-payment-updated', refreshAfterInstallmentPayment);
    return () => {
      window.removeEventListener('kourosh:installment-payment-updated', refreshAfterInstallmentPayment);
    };
  ...
  const restoreReportUiState = React.useCallback((state: Record<string, unknown>) => {
    ...
    setPageSize(PAGE_SIZE_OPTIONS.includes(Number(state.pageSize)) ? Number(state.pageSize) : 10);
  ...
  const avgDebt = useMemo(() => (debtors.length ? totalDebt / debtors.length : 0), [debtors, totalDebt]);
    ...
    <div className="report-page debtors-executive-page" dir="rtl">
      ...
      <section className="debtors-executive-panel" aria-label="گزارش مشتریان بدهکار">
        ...
        <div className="debtors-executive-toolbar">
          ...
          <Link to="/reports/collection-center" onClick={(event) => onDrilldownClick(event, '/reports/collection-center', { contextLabel: 'مرکز وصول • فیلتر بدهکاران فعلی' })} className="debtors-toolbar-link">
              ...
              <Table className="debtors-table-final">
                ...
                <tbody>
                  {pageRows.map((row) => {
                    ...
                    const pct = maxDebt ? Math.min(100, (debt / maxDebt) * 100) : 0;
                    const initial = String(row.fullName || '؟').trim().charAt(0) || '؟';
                    return (
                      <tr key={row.id} data-navigation-anchor={reportNavigationAnchor('debtors', row.id)}>
                        <td>
                        ...
                        <td>
                          <div className="debtors-row-actions">
                            <Link to={customerLedgerPath(row.id)} onClick={(event) => onDrilldownClick(event, customerLedgerPath(row.id), { contextLabel: `${row.fullName || 'مشتری'} • بدهی ${money(debt)}` })} className="debtors-row-action debtors-row-action--primary">
                              <i className="fa-solid fa-user" />
                              پرونده مشتری
                            </Link>
                            {row.sourceUrl ? (
                              <Link to={reportSourcePath(row.sourceUrl)} onClick={(event) => onDrilldownClick(event, reportSourcePath(row.sourceUrl), { contextLabel: `${row.fullName || 'مشتری'} • ${row.sourceLabel || 'سند ریشه'}`, targetEntity: { sourceLabel: row.sourceLabel || undefined, preview: { eyebrow: 'سند ریشه بدهی', status: 'بدهکار', statusTone: 'warning', items: [{ label: 'مشتری', value: row.fullName || 'بدون نام', iconClass: 'fa-regular fa-user' }, { label: 'مانده بدهی', value: money(debt), iconClass: 'fa-solid fa-coins', tone: 'warning' }], note: row.sourceLabel || undefined } } })} className="debtors-row-action" title={row.sourceLabel || 'مشاهده سند ریشه'}>
                            ...
                            <Link to={collectionCenterPath(row.fullName)} onClick={(event) => onDrilldownClick(event, collectionCenterPath(row.fullName), { contextLabel: `${row.fullName || 'مشتری'} • پیگیری وصول` })} className="debtors-row-action">
                              <i className="fa-solid fa-headset" />
                              پیگیری وصول
                            </Link>
                          </div>
