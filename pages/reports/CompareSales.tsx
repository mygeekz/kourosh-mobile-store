...
export default function CompareSalesPage() {
  ...
  const exportPDF = () => {
    ...
    doc.setFont(VAZIR_FAMILY, hasRegular ? "normal" : "bold");
    doc.text(
      `تعداد فاکتور: ${kpi.count.toLocaleString("fa-IR")}`,
      pageWidth - marginX,
      y + 18,
      {
        align: "right",
    ...
    doc.text(
      `مجموع فروش: ${kpi.total.toLocaleString("fa-IR")} تومان`,
      pageWidth - marginX,
      y + 36,
      {
        align: "right",
    ...
    doc.text(
      `مجموع سود: ${kpi.profit.toLocaleString("fa-IR")} تومان`,
      pageWidth - marginX,
      y + 54,
      {
        align: "right",
    ...
    doc.text(
      `میانگین فروش: ${kpi.avg.toLocaleString("fa-IR")} تومان`,
      pageWidth - marginX,
      y + 72,
      {
        align: "right",
  ...
  return (
    <div className="report-page" dir="rtl">
      ...
      <div className="compare-sales-executive-page" dir="rtl">
        <section
          className="compare-sales-filter-panel"
          aria-label="فیلترهای مقایسه دوره‌ای فروش"
        >
          ...
          <div className="compare-sales-filter-row compare-sales-filter-row--secondary">
            <span className="compare-sales-filter-label">
              میانبرهای مقایسه‌ای
            </span>
            <button onClick={() => preset("this_week")} type="button">
              هفته جاری
            </button>
            <button onClick={() => preset("this_year")} type="button">
              سال جاری
            </button>
            <button onClick={() => preset("this_month", "prev")} type="button">
              ماه جاری + دوره قبلی
            </button>
            <button
              onClick={() => preset("this_month", "last_year")}
              type="button"
            >
              ماه جاری + سال قبل
            </button>
                ...
                {[
                  {
                    icon: "fa-receipt",
                    k: "تعداد فاکتور",
                    v: kpi.count.toLocaleString("fa-IR"),
                  },
                  {
                    icon: "fa-sack-dollar",
                    k: "مجموع فروش",
                    ...
                    {detailsRows.map((r) => (
                      <tr key={r.id}>
                        <td>
                          ...
                          <small>
                            شناسه سند: {r.id.toLocaleString("fa-IR")}
                          </small>
                        </td>
