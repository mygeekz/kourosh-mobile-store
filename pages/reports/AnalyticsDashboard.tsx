import { Table, DataTableShell, IconGlyph, CheckboxField } from '@/components/ui';
...
import { formatReportMoneyText, formatReportPercentText, formatReportMoneyPreview } from '../../utils/reportPresentation';
...
const fmtChartMoney = (n: any) => formatReportMoneyPreview(Number(n || 0));
const tooltipMoney = (value: any, label = 'درآمد') => [fmtMoney(value), label];
const startOfCurrentJalaliMonth = () => moment().startOf('jMonth').startOf('day').toDate();

const fmtPercent = (n: any) => {
  const v = Number(n || 0);
  const sign = v > 0 ? '+' : '';
  return `${sign}${formatReportPercentText(v, 2, false)}`;
};
...
export default function AnalyticsDashboard() {
  ...
  const fetchMainAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/reports/analytics-dashboard?${qs.toString()}`,
  ...
  const kpis = useMemo(() => {
    ...
    const avgDaily = data?.salesTrend?.length ? totalRevenue / data.salesTrend.length : 0;
    ...
    const prevAvg = compareData?.salesTrend?.length ? prevRevenue / compareData.salesTrend.length : 0;
      ...
      {!isLoading && data && !hasAnyData ? (
        <div className="analytics-executive-card p-6">
          <div className="flex items-start gap-3">
            <IconGlyph tone="warning" className="h-10 w-10" aria-hidden="true"><i className="fa-solid fa-circle-info text-amber-700 dark:text-amber-200" /></IconGlyph>
            <div>
              ...
              {isLoading ? (
                ...
                <div className="analytics-executive-debt-snapshot-card">
                  ...
                  <div className="analytics-executive-debt-snapshot-card__content">
                    <div className="analytics-executive-debt-snapshot-card__label">آخرین وضعیت ثبت‌شده بدهی</div>
                    <div className="analytics-executive-debt-snapshot-card__value">{fmtMoney(debtSnapshotSummary.debt || kpis.lastDebt)}</div>
                ...
                <EmptyAnalyticsState icon="fa-solid fa-scale-balanced" title="روند بدهی هنوز داده کافی ندارد" text="پس از ثبت سابقه بیشتر، نمودار روند بدهی نمایش داده می‌شود." />
              )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div className="analytics-executive-card">
              ...
              <DataTableShell className="mt-3" data-ui-analytics-product-table="true">
                <Table layout="managed" density="comfortable" className="min-w-[620px] w-full text-sm">
                  ...
                  <tbody>
                    {(data?.bestProducts || []).map((p) => (
                      <tr key={p.id} className="border-t border-gray-100 dark:border-slate-800">
                        ...
                        <td className="p-2 whitespace-nowrap">{formatExactNumberText(Number(p.qty))}</td>
          ...
          <div className="analytics-executive-card">
            ...
            <div className="mt-3 h-80">
              {isLoading ? (
                ...
                <EmptyAnalyticsState icon="fa-solid fa-scale-balanced" title="داده کافی برای روند بدهی وجود ندارد" text="پس از ثبت سابقه بدهی، این نمودار فعال می‌شود." />
              )}
            </div>
