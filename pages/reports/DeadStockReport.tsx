...
import { DataTableShell, PageKit, SelectField, TextField } from '@/components/ui';
...
export default function DeadStockReport() {
  ...
  }), [days, pageIndex, pageSize, query]);

  const restoreReportUiState = useCallback((state: Record<string, unknown>) => {
    suppressPageResetRef.current = true;
    const restoredDays = Number(state.days || 60);
    setDays([30, 60, 90, 120, 180].includes(restoredDays) ? restoredDays : 60);
  ...
  const avgDormancyDays = useMemo(() => {
    ...
    return withDays.length ? withDays.reduce((a, b) => a + b, 0) / withDays.length : 0;
  }, [rows]);
      ...
      toolbarRight={
        <div className="dead-stock-threshold-panel" dir="rtl" aria-label="کنترل آستانه رکود">
          ...
          <label className="dead-stock-threshold-select" aria-label="انتخاب آستانه رکود">
            <SelectField controlOnly unstyled showChevron={false} value={days} onChange={(e) => setDays(Number(e.target.value))}>
    ...
    >
      <div className="dead-stock-layout" dir="rtl">
        ...
        <section className="dead-stock-table-section" aria-label="جدول کالاهای راکد">
          <div className="dead-stock-table-header">
            ...
            <div className="dead-stock-table-tools">
              ...
              <label className="dead-stock-page-size" aria-label="تعداد در صفحه">
                <SelectField controlOnly unstyled showChevron={false} value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))}>
          ...
          <DataTableShell className="dead-stock-table-scroll" data-ui-dead-stock-table="true">
            <Table layout="managed" density="comfortable" className="dead-stock-table">
              <thead>
                <tr>
                  <th>کالا</th>
                  <th>دسته</th>
              ...
              <tbody>
                {pageRows.map((r) => {
                  ...
                  return (
                    <tr key={r.productId} data-navigation-anchor={reportNavigationAnchor('dead-stock', r.productId)}>
                      <td>
                      ...
                      <td>
                        <button
                          type="button"
                          onClick={() => openDrilldown(`/products?search=${encodeURIComponent(r.name)}`, { contextLabel: `${r.name} • ${risk.label}`, anchorId: reportNavigationAnchor('dead-stock', r.productId) })}
