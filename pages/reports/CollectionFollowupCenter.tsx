...
import { ActionLink, AppSearchField, Button, CheckboxField, Drawer, PanelCard, Surface, SurfaceHeader, type SurfaceTone } from '@/components/ui';
  ...
  return (
    <div className="space-y-5" dir="rtl" data-ui-collection-followup-center="true">
        ...
        <section className="grid gap-3 xl:grid-cols-2" aria-label="فهرست پرونده‌های وصول">
          {loading ? (
            ...
            <PanelCard className="xl:col-span-2" tone="warning" title="با فیلتر فعلی موردی نمایش داده نمی‌شود" subtitle="برای دیدن همه موارد، جستجو را پاک کن و سطح ریسک را روی «همه» بگذار." icon={<i className="fa-solid fa-filter-circle-xmark" />}>
              <Button type="button" variant="secondary" size="md" onClick={() => { setQuery(''); setLevel('all'); setOnlyUntouched(false); setFromDate(moment().subtract(24, 'months').toDate()); setToDate(new Date()); }} leftIcon={<i className="fa-solid fa-rotate-left" />}>نمایش همه موارد قابل پیگیری</Button>
            </PanelCard>
          ) : items.map((item) => {
            const meta = LEVEL_META[item.level];
      ...
      <Drawer
        ...
        title="جزئیات پرونده وصول"
        size="lg"
        hideHeader
        panelAttributes={{
          id: 'collection-followup-panel',
            'data-report-drawer-kind': 'collection-followup',
        }}
      >
        {selected ? (
