...
import ReportControlDock, {
  ReportControlDateSection, ReportControlFooter, ReportControlSearch, ReportControlStatus, } from '../../components/reports/ReportControlDock';
...
import { AppSearchField, Button, DataTableShell } from '@/components/ui';
type Row = {
  productId: number;
  name: string;
  qty: number;
...
const fmtMoney = (n: any) => formatReportMoneyText(Number(n || 0));
const fmtNum = (n: any) => formatExactNumberText(Number(n || 0));
...
const RevenueShareSector = (props: PieSectorShapeProps) => (
  <Sector {...props} fill={chartPalette[(props.index ?? 0) % chartPalette.length]} />
);
...
export default function ProductProfitReal() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/reports/product-profit-real?${qs.toString()}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <section className="product-profit-real-hero">
        ...
        <ReportControlDock
          ariaLabel="کنترل‌های سود واقعی هر محصول"
          presentation="approved"
          title="کنترل گزارش"
          subtitle="بازه زمانی، جستجوی محصول و محاسبه سود واقعی"
          icon={<i className="fa-solid fa-sliders" aria-hidden="true" />}
          footer={(
            <ReportControlFooter
              ariaLabel="عملیات و وضعیت سود واقعی هر محصول"
              statuses={(
                <ReportControlStatus tone="info" icon={<i className="fa-regular fa-calendar" aria-hidden="true" />}>
                  ...
                  <bdi dir="ltr" className="font-black">{fromDate ? moment(fromDate).locale('en').format('jYYYY/jMM/jDD') : '—'}</bdi>
                  <span aria-hidden="true" className="font-black text-[var(--ds-text-muted)]">|</span>
                  <bdi dir="ltr" className="font-black">{toDate ? moment(toDate).locale('en').format('jYYYY/jMM/jDD') : '—'}</bdi>
                </ReportControlStatus>
              )}
              actions={(
                <>
          ...
          <ReportControlSearch>
            <ReportFilterField label="جستجو" icon={<i className="fa-solid fa-magnifying-glass" />} className="report-control-approved__field" minWidthClassName="min-w-0">
              <AppSearchField
                ...
                ariaLabel="جستجوی محصول در گزارش سود واقعی"
                size="md"
                clearable
              />
            </ReportFilterField>
      ...
      <section className="product-profit-real-kpi-grid">
        ...
        <article className="product-profit-real-kpi product-profit-real-kpi--margin">
          ...
          <div><div className="product-profit-real-kpi__label">حاشیه سود کل</div><div className="product-profit-real-kpi__value">{formatReportPercentText(summary.marginPct, 2, false)}</div><p>{formatExactNumberText(riskSummary.negativeItems)} کالا با سود منفی</p></div>
        </article>
      </section>
        ...
        <div className="product-profit-real-chart-card product-profit-real-chart-card--share">
          <div className="product-profit-real-chart-card__head">
            <div>
              ...
              <p className="product-profit-real-chart-card__subtext">{formatReportPercentText(revenueConcentration)} از کل درآمد در ۶ محصول اول متمرکز است.</p>
            </div>
            <div className="product-profit-real-chart-card__summary">
              ...
              <small>{topShareLegend[0] ? `${formatReportPercentText(topShareLegend[0].percent)} از درآمد` : '—'}</small>
            </div>
          </div>
                ...
                {topShareLegend.map((item, i) => (
                  <div key={item.name + i} className="product-profit-real-donut-legend__item">
                    ...
                    <div className="product-profit-real-donut-legend__text">
                      ...
                      <small>{fmtMoney(item.value)}</small>
                    </div>
                    <span className="product-profit-real-donut-legend__percent">{formatReportPercentText(item.percent)}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="product-profit-real-chart-card product-profit-real-chart-card--profit">
          <div className="product-profit-real-chart-card__head">
            ...
            <div className="product-profit-real-chart-card__summary">
              ...
              <small>{riskSummary.bestItem ? fmtMoney(riskSummary.bestItem.profit) : '—'}</small>
            </div>
          </div>
              ...
              {topProfit.map((item, index) => {
                ...
                return (
                  <div key={item.name + index} className="product-profit-real-profit-row">
                    <div className="product-profit-real-profit-row__rank">{formatExactNumberText((index + 1))}</div>
      ...
      <DataTableShell
        ...
        title="سود واقعی به تفکیک محصول"
        titleIcon={<i className="fa-solid fa-chart-line" aria-hidden="true" />}
        meta={(
          <div className="ux-table-shell__meta-pill">
            <i className="fa-solid fa-list-ol" aria-hidden="true" />
            <span>{formatExactNumberText(filtered.length)} محصول نتیجه</span>
          </div>
        )}
        aria-label="جدول سود واقعی هر محصول"
      >
        {isLoading ? (
          ...
          <div className="product-profit-real-table-scroll">
            <Table className="product-profit-real-table report-table ux-data-table" layout="managed" data-ui-bidi-scope="rtl-table">
              <thead >
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.productId} className={r.profit < 0 ? 'is-negative' : ''}>
                    ...
                    <td className="p-3 whitespace-nowrap">{formatReportPercentText(r.shareOfRevenue)}</td>
                    <td className="p-3 whitespace-nowrap">{formatReportPercentText(r.marginPct, 2, false)}</td>
                  </tr>
                ))}
              </tbody>
            </Table>
