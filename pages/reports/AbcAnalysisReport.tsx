...
import { PageKit, SelectField } from '@/components/ui';
...
export default function AbcAnalysisReport() {
  ...
  const restoreReportUiState = useCallback((state: Record<string, unknown>) => {
    ...
    setMetric(String(state.metric) === 'profit' ? 'profit' : 'sales');
  }, []);

  const { openDrilldown } = useReportDrilldownNavigation({
    reportKey: 'abc-analysis',
      ...
      toolbarRight={
        <div className="abc-filter-bar" dir="rtl">
          ...
          <ReportFilterField label="از تاریخ" icon={<i className="fa-regular fa-calendar" />} minWidthClassName="abc-filter-field abc-filter-field--date">
            <ShamsiDatePicker value={fromDate} onChange={(d: any) => d && setFromDate(d)} />
          </ReportFilterField>
          <ReportFilterField label="تا تاریخ" icon={<i className="fa-regular fa-calendar-check" />} minWidthClassName="abc-filter-field abc-filter-field--date">
            <ShamsiDatePicker value={toDate} onChange={(d: any) => d && setToDate(d)} />
          </ReportFilterField>
          <ReportFilterField label="مبنای تحلیل" icon={<i className="fa-solid fa-filter-circle-dollar" />} minWidthClassName="abc-filter-field abc-filter-field--metric">
            <SelectField controlOnly unstyled showChevron={false} value={metric} onChange={(e) => setMetric(e.target.value as any)} className="abc-filter-select">
    ...
    >
      ...
      <div className="abc-table-shell">
        <table className="abc-table">
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.productId} data-navigation-anchor={reportNavigationAnchor('abc-analysis', r.productId)}>
                ...
                <td className="px-4 py-3">
                  <button
                    type="button"
                    onClick={() => openDrilldown(`/products?search=${encodeURIComponent(r.name)}`, { contextLabel: `${r.name} • گروه ${r.bucket}`, anchorId: reportNavigationAnchor('abc-analysis', r.productId) })}
