...
const fmt = (num: number, _digits = 0) => formatExactNumberText(Number.isFinite(Number(num)) ? Number(num) : 0);
  ...
  return (
    <div className="report-page purchase210-page" dir="rtl">
      ...
      <section className="purchase210-table-card">
        ...
        {filteredData.length > 0 ? (
          <div className="purchase210-pagination">
            <span>
              صفحه{" "}
              {formatExactNumberText((table.getState().pagination.pageIndex + 1))}{" "}
              از {formatExactNumberText(Math.max(1, table.getPageCount()))}
            </span>
            <div>
              <button
                type="button"
      ...
      <Drawer
        ...
        title="تحلیل پیشنهاد خرید"
        size="md"
        hideHeader
        panelClassName="purchase210-drawer purchase210-drawer--dialog-shell"
        overlayClassName="purchase210-drawer-overlay purchase210-drawer-overlay--dialog-shell"
      >
        {selectedSuggestion ? (
          <>
            <header>
