import { Table, CheckboxField, Dialog as Modal, DialogActions as ModalActions, SelectField, TextareaField, TextField } from '@/components/ui';
...
export type FollowupCreateDialogProps = {
  open: boolean;
  onClose: () => void;
  customerId: string;
  ...
  nextDate: Date | null;
  onNextDateChange: (value: Date | null) => void;
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <TextField controlOnly unstyled className="followups-modal-input" value={customerId} onChange={(event) => onCustomerIdChange(event.target.value)} placeholder="شناسه مشتری" />
        <TextareaField controlOnly className="followups-modal-input min-h-[96px]" value={note} onChange={(event) => onNoteChange(event.target.value)} placeholder="متن پیگیری" />
        <ShamsiDatePicker selectedDate={nextDate} onDateChange={onNextDateChange} />
      </div>
      <ModalActions
        onCancel={onClose}
        cancelText="انصراف"
...
export default function FollowupsReport() {
  ...
  const quickSetDue = async (row: Row, daysFromNow: number) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/customers/${row.customerId}/followups/${row.id}`, {
        method: 'PATCH',
...
const openEdit = (row: Row) => {
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await apiFetch(`/api/customers/${editingRow.customerId}/followups/${editingRow.id}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          note,
...
const closeFollowup = async (customerId: number, followupId: number) => {
    if (!token) return;
    try {
      const res = await apiFetch(`/api/customers/${customerId}/followups/${followupId}/close`, {
        method: 'POST',
...
const fetchData = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/reports/followups?${qs.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const js = await res.json();
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchQuery, pageSize, status, dateField, fromDate, toDate, owner, onlyMine, noDueOnly]);
  ...
  const reportUiState = useMemo(() => ({
    ...
    pageSize,
    currentPage,
  }), [currentPage, dateField, fromDate, noDueOnly, onlyMine, owner, pageSize, searchQuery, status, toDate]);

  const restoreReportUiState = useCallback((state: Record<string, unknown>) => {
    suppressPageResetRef.current = true;
    setStatus((['open', 'closed', 'all'].includes(String(state.status)) ? String(state.status) : 'open') as 'open' | 'closed' | 'all');
    setDateField(String(state.dateField) === 'created' ? 'created' : 'next');
    ...
    setOnlyMine(Boolean(state.onlyMine));
    setNoDueOnly(Boolean(state.noDueOnly));
    setSearchQuery(String(state.searchQuery || ''));
    setPageSize([10, 20, 50].includes(Number(state.pageSize)) ? Number(state.pageSize) : 10);
    setCurrentPage(Math.max(1, Number(state.currentPage || 1)));
  }, []);
  ...
  return (
    <div className="followups-executive-page" dir="rtl">
      ...
      <section className="followups-panel followups-panel--header" aria-label="گزارش پیگیری‌ها">
        ...
        <div className="followups-filter-row followups-filter-row--secondary">
          ...
          <div className="followups-page-size-final">
            <SelectField controlOnly unstyled showChevron={false} aria-label="تعداد ردیف در صفحه" value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))}>
          ...
          <label className="followups-owner-field">
            <TextField controlOnly unstyled value={owner} onChange={(e) => setOwner(e.target.value)} placeholder="ثبت‌کننده اختیاری…" disabled={onlyMine} />
          </label>

          <label className="followups-toggle-chip">
            <CheckboxField controlOnly checked={onlyMine} onChange={(e) => setOnlyMine(e.target.checked)} />
            <span>فقط پیگیری‌های من</span>
          </label>

          <label className="followups-toggle-chip">
            <CheckboxField controlOnly checked={noDueOnly} onChange={(e) => setNoDueOnly(e.target.checked)} />
        ...
        {isLoading ? (
          ...
          <div className="followups-table-shell">
            <Table className="followups-table">
              ...
              <tbody>
                {pageRows.map((r) => (
                  <tr key={r.id} data-navigation-anchor={reportNavigationAnchor('followups', r.id)}>
                    <td>
                    ...
                    <td>
                      <a className="followups-customer-link" href={`#/customers/${r.customerId}`} onClick={(event) => onDrilldownClick(event, `/customers/${r.customerId}`, { contextLabel: `${r.customerName || `مشتری #${r.customerId}`} • پیگیری #${r.id}` })}>
                    ...
                    <td>
                      <div className="followups-row-actions">
                        ...
                        <a className="followups-row-action followups-row-action--customer" href={`#/customers/${r.customerId}`} onClick={(event) => onDrilldownClick(event, `/customers/${r.customerId}`, { contextLabel: `${r.customerName || `مشتری #${r.customerId}`} • پیگیری #${r.id}` })}><i className="fa-solid fa-user-clock" /><span>پرونده مشتری</span></a>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <FollowupCreateDialog
        ...
        onCustomerIdChange={setNewCustomerId}
        note={newNote}
        onNoteChange={setNewNote}
        nextDate={newNextDate}
        onNextDateChange={setNewNextDate}
      ...
      >
        <div className="space-y-3">
          <TextareaField controlOnly className="followups-modal-input min-h-[120px]" value={editNote} onChange={(e) => setEditNote(e.target.value)} />
          <ShamsiDatePicker selectedDate={editNextDate} onDateChange={setEditNextDate} />
