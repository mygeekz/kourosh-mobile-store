import { AppSearchField, Button, DataTableShell, EmptyState, IconGlyph, SelectField } from '@/components/ui';
...
type ReportSideDrawerSize = "sm" | "md" | "wide";

function ReportSideDrawer({
  open,
  onClose,
  ariaLabel,
  size = "md",
  ...
  return (
    <DialogShell
      ...
      layer="drawer"
      ariaLabel={ariaLabel}
      overlayClassName={`report-side-drawer-backdrop report-side-drawer-backdrop--${tone} app-modal-backdrop !items-stretch !justify-end !p-0`}
      panelClassName={`report-side-drawer-panel report-side-drawer-panel--${size}`}
      closeOnBackdrop
    >
      {children}
    </DialogShell>
...
const money = (n: number) =>
  formatReportMoneyText(n ?? 0);
const numberFa = (n: number, _options?: Intl.NumberFormatOptions) =>
  formatExactNumberText(Number(n) || 0);
const percentFa = (n: number) => formatExactPercentText(n);
...
const getRowDiscountAudit = (r: Row) => {
  ...
  const totalDiscount = Math.max(
    0,
    Number(r.totalDiscountAmount ?? itemDiscount + invoiceShare),
  );
...
const getRowDiscountReason = (r: Row) => {
  ...
  if (audit.hasItemDiscount)
    reasons.push(`تخفیف آیتم: ${money(audit.itemDiscount)}`);
  if (audit.hasInvoiceDiscount)
    reasons.push(`سهم تخفیف فاکتور: ${money(audit.invoiceShare)}`);
...
const getRowDiscountTypeMeta = (r: Row) => {
  ...
  return {
    ...
    caption: audit.hasAnyDiscount
      ? "کل تخفیف ثبت شده، اما نوع آن از داده‌ها قابل تفکیک نیست"
      : "برای این ردیف تخفیف آیتم یا فاکتور ثبت نشده است",
...
const DISCOUNT_AUDIT_MODES: Array<{
...
}> = [
  {
    value: "all",
    label: "همه",
    caption: "نمایش همه ردیف‌های فروش غیرگوشی",
    icon: "fa-layer-group",
  },
  {
    value: "item",
    label: "تخفیف آیتم",
    caption: "فقط ردیف‌هایی که روی خود قلم تخفیف مستقیم دارند",
    icon: "fa-tag",
  },
  {
    value: "invoice",
    label: "تخفیف فاکتور",
    caption: "فقط ردیف‌هایی که از تخفیف کلی فاکتور سهم گرفته‌اند",
    icon: "fa-file-invoice-dollar",
  },
];
...
const PROFIT_VIEW_MODES: Array<{
...
}> = [
  {
    value: "total",
    label: "سود کل",
    caption: "سود کامل ردیف بر اساس مبلغ نهایی فروش، بدون وابستگی به وصول",
    icon: "fa-chart-line",
  },
  {
    value: "received",
    label: "سود وصول‌شده",
    caption: "سود قابل شناسایی متناسب با پول دریافت‌شده",
...
const getAuditModeDescription = (mode: DiscountAuditMode) => {
  if (mode === "item")
    return "فقط ردیف‌هایی دیده می‌شوند که روی خود آیتم تخفیف مستقیم ثبت شده است.";
  if (mode === "invoice")
    return "فقط ردیف‌هایی دیده می‌شوند که سهمی از تخفیف کلی فاکتور دریافت کرده‌اند.";
...
const getAuditModeEmptyDescription = (mode: DiscountAuditMode) => {
  if (mode === "item")
    return "در این بازه یا جستجوی فعلی، ردیفی با تخفیف مستقیم آیتم پیدا نشد.";
  if (mode === "invoice")
    return "در این بازه یا جستجوی فعلی، ردیفی با سهم تخفیف کلی فاکتور پیدا نشد.";
...
const getCalculationHealthMeta = (status: CalculationHealth["status"]) => {
  if (status === "error") {
    return {
      ...
      className:
        "border-rose-200 bg-rose-50/90 text-rose-900 dark:border-rose-900/60 dark:bg-rose-950/25 dark:text-rose-100",
      badgeClassName:
        "border-rose-200 bg-white text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/50 dark:text-rose-200",
    };
  }
  if (status === "warning") {
    return {
      ...
      className:
        "border-amber-200 bg-amber-50/90 text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-100",
      badgeClassName:
        "border-amber-200 bg-white text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/50 dark:text-amber-200",
    };
  }
  return {
    ...
    className:
      "border-emerald-200 bg-emerald-50/90 text-emerald-900 dark:border-emerald-900/60 dark:bg-emerald-950/25 dark:text-emerald-100",
    badgeClassName:
      "border-emerald-200 bg-white text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/50 dark:text-emerald-200",
  };
};

const getCalculationIssueMeta = (
  severity?: CalculationHealthIssue["severity"],
) => {
  if (severity === "error") {
    return {
      label: "خطای جدی",
...
const getRowProfitBreakdown = (r: Row | null) => {
  ...
  const collectionRate =
    lineTotal > 0
      ? Math.min(100, Math.max(0, (receivedAmount / lineTotal) * 100))
      : Number(r.collectionRate || 0);
  const realizedProfit = Number(
    r.realizedProfit ?? fullProfit * (collectionRate / 100),
  );
  const unrecognizedProfit = Number(
    r.unrecognizedProfit ?? fullProfit - realizedProfit,
  );
  return {
    lineTotal,
    lineCost,
...
const getProfitViewTitle = (mode: ProfitViewMode) =>
  mode === "received" ? "سود وصول‌شده" : "سود کل";
const getProfitViewDescription = (mode: ProfitViewMode) =>
  mode === "received"
...
const getCollectionRiskMeta = (level: CollectionRiskLevel) => {
  ...
  if (level === "urgent") {
    return {
      ...
      icon: "fa-bell",
      className:
        "border-orange-200 bg-orange-50/90 text-orange-900 dark:border-orange-900/60 dark:bg-orange-950/25 dark:text-orange-100",
      badgeClassName:
        "border-orange-200 bg-white text-orange-700 dark:border-orange-900/60 dark:bg-orange-950/50 dark:text-orange-200",
  ...
  return {
    ...
    icon: "fa-shield-check",
    className:
      "border-sky-200 bg-sky-50/90 text-sky-900 dark:border-sky-900/60 dark:bg-sky-950/25 dark:text-sky-100",
    badgeClassName:
      "border-sky-200 bg-white text-sky-700 dark:border-sky-900/60 dark:bg-sky-950/50 dark:text-sky-200",
...
const getCollectionRiskSummaryText = (risk: CollectionRiskSummary) => {
  const parts = [
    risk.counts.critical
      ? formatExactNumberText(risk.counts.critical) + " بحرانی"
      : "",
    risk.counts.urgent
      ? formatExactNumberText(risk.counts.urgent) + " فوری"
      : "",
    risk.counts.followup
      ? formatExactNumberText(risk.counts.followup) + " نیازمند پیگیری"
      : "",
    risk.counts.low ? formatExactNumberText(risk.counts.low) + " کم‌ریسک" : "",
  ].filter(Boolean);
  return parts.length
    ? parts.join("، ")
    : "فاکتور باز برای پیگیری وصول در فیلتر فعلی دیده نشد";
};
...
export default function ProductSalesReport() {
  ...
  const monthStart = useMemo(
    () => moment().locale("fa").startOf("jMonth").toDate(),
    [],
  );
  const monthEnd = useMemo(
    () => moment().locale("fa").endOf("jMonth").toDate(),
    [],
  );
  ...
  const suppressDetailsPageResetRef = useRef(false);
  ...
  const [collectionRisk, setCollectionRisk] = useState<CollectionRiskSummary>(
    () => emptyCollectionRisk(),
  );
  ...
  const selectedAuditRowsCount =
    auditMode === "item"
      ? itemDiscountedRowsCount
      : auditMode === "invoice"
        ? invoiceDiscountedRowsCount
        : auditCounts.all;
  ...
  const safeDetailsPage = Math.min(
    Math.max(1, Number(pagination.page || detailsPage || 1)),
    detailsTotalPages,
  );
  const paginatedRows = rows;
  const detailStartRow = Number(
    pagination.startRow ||
      (rows.length ? (safeDetailsPage - 1) * detailsPageSize + 1 : 0),
  );
  const detailEndRow = Number(
    pagination.endRow ||
      (rows.length ? (safeDetailsPage - 1) * detailsPageSize + rows.length : 0),
  );
  const totalFilteredRows = Number(
    pagination.totalRows || selectedAuditRowsCount || 0,
  );

  useEffect(() => {
    if (suppressDetailsPageResetRef.current) {
      suppressDetailsPageResetRef.current = false;
      return;
    }
    setDetailsPage(1);
  ...
  const reportUiState = useMemo(() => ({
    ...
    healthDrawerOpen,
    riskDrawerOpen,
  }), [auditMode, detailsPage, detailsPageSize, fromDate, healthDrawerOpen, profitView, query, riskDrawerOpen, toDate]);

  const restoreReportUiState = useCallback((state: Record<string, unknown>) => {
    suppressDetailsPageResetRef.current = true;
    ...
    const restoredAudit = String(state.auditMode || 'all');
    setAuditMode((['all', 'item', 'invoice'].includes(restoredAudit) ? restoredAudit : 'all') as DiscountAuditMode);
    setProfitView(String(state.profitView) === 'full' ? 'full' : 'received');
    const restoredPageSize = Number(state.detailsPageSize || 50);
    setDetailsPageSize(DETAILS_PAGE_SIZE_OPTIONS.includes(restoredPageSize as any) ? restoredPageSize : 50);
    setDetailsPage(Math.max(1, Number(state.detailsPage || 1)));
    setHealthDrawerOpen(Boolean(state.healthDrawerOpen));
    setRiskDrawerOpen(Boolean(state.riskDrawerOpen));
  }, []);
  ...
  const filteredUnrecognizedProfit = Number(
    filteredSummary.unrecognizedProfit || 0,
  );
  const activeFilteredProfit =
    profitView === "received" ? filteredProfit : filteredTotalProfit;
  const activeSummaryProfit =
    profitView === "received"
      ? Number(summary.realizedProfit || 0)
      : Number(summary.totalProfit || 0);
  const filteredDiscountTotal = Number(
    filteredSummary.totalDiscountAmount || 0,
  );
  const filteredInvoiceDiscountTotal = Number(
    filteredSummary.invoiceDiscountShare || 0,
  );
  const filteredItemDiscountTotal = Number(
    filteredSummary.itemDiscountAmount || 0,
  );
  const filteredOutstanding = Math.max(0, filteredTotal - filteredReceived);
  const filteredCollectionRate =
    filteredTotal > 0 ? (filteredReceived / filteredTotal) * 100 : 0;
  const cashCollectionRate = useMemo(
    () =>
      summary.cashSales > 0
        ? (summary.cashReceived / summary.cashSales) * 100
        : 0,
    [summary.cashSales, summary.cashReceived],
  );
  const creditCollectionRate = useMemo(
    () =>
      summary.creditSales > 0
        ? (summary.creditReceived / summary.creditSales) * 100
        : 0,
    [summary.creditSales, summary.creditReceived],
  );
  const installmentCollectionRate = useMemo(
    () =>
      summary.installmentSales > 0
        ? (summary.installmentReceived / summary.installmentSales) * 100
        : 0,
    [summary.installmentSales, summary.installmentReceived],
  );
  const collectionRiskMeta = getCollectionRiskMeta(collectionRisk.status);
  const highRiskCount =
    Number(collectionRisk.counts.critical || 0) +
    Number(collectionRisk.counts.urgent || 0);
  ...
  const calcRowBreakdown = (r: Row | null) => {
    ...
    const itemDiscount = Math.max(
      0,
      Math.min(Number(r.discountPerItem || 0), gross),
    );
    const beforeGlobal = Math.max(
      0,
      Number(r.originalLineTotal ?? gross - itemDiscount),
    );
    const invoiceShare = Math.max(0, Number(r.globalDiscountShare || 0));
    ...
    const shareRate =
      invoiceDiscountBase > 0 ? (beforeGlobal / invoiceDiscountBase) * 100 : 0;
    ...
    const collectionRate =
      afterDiscount > 0
        ? Math.min(100, (received / afterDiscount) * 100)
        : Number(r.collectionRate || 0);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/api/reports/product-sales/details?${params.toString()}`,
      );
      ...
      const nextFilteredSummary = (data.filteredSummary ||
        {}) as Partial<FilteredSummary>;
      setFilteredSummary({
        ...
        totalDiscountAmount: Number(
          nextFilteredSummary.totalDiscountAmount || 0,
        ),
        itemDiscountAmount: Number(nextFilteredSummary.itemDiscountAmount || 0),
        invoiceDiscountShare: Number(
          nextFilteredSummary.invoiceDiscountShare || 0,
        ),
      });
      ...
      const nextHealth = (data.calculationHealth ||
        {}) as Partial<CalculationHealth>;
      const nextHealthStatus =
        nextHealth.status === "error" || nextHealth.status === "warning"
          ? nextHealth.status
          : "healthy";
      setCalculationHealth({
        ...
        totalAbsoluteDifference: Number(
          nextHealth.totalAbsoluteDifference || 0,
        ),
        issues: Array.isArray(nextHealth.issues)
          ? (nextHealth.issues as CalculationHealthIssue[])
          : [],
      });

      const nextRisk = (data.collectionRisk ||
        {}) as Partial<CollectionRiskSummary>;
      const riskCounts = (nextRisk.counts || {}) as Partial<
        Record<CollectionRiskLevel, number>
      >;
      const riskStatus: CollectionRiskLevel =
        nextRisk.status === "critical" ||
        nextRisk.status === "urgent" ||
        nextRisk.status === "followup"
          ? nextRisk.status
          : "low";
      setCollectionRisk({
        status: riskStatus,
        ...
        items: Array.isArray(nextRisk.items)
          ? (nextRisk.items as CollectionRiskItem[])
          : [],
      });
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const riskItems = (js?.data?.collectionRisk?.items ||
        []) as CollectionRiskItem[];
      (exportExcel as any).__riskMap = new Map(
        riskItems.map((item) => [`${item.sourceType}:${item.orderId}`, item]),
      );
    } catch (e: any) {
      setNotification({
        message: e?.message || "خطا در آماده‌سازی خروجی Excel",
        type: "error",
      });
      return;
    }

    const riskMap: Map<string, CollectionRiskItem> =
      (exportExcel as any).__riskMap ||
      new Map(
        collectionRisk.items.map((item) => [
          `${item.sourceType}:${item.orderId}`,
          item,
        ]),
      );
    const out = exportRows.map((r) => {
      ...
      return {
        ...
        activeProfit:
          profitView === "received" ? profit.realizedProfit : profit.fullProfit,
  ...
  return (
    <div
      dir="rtl"
      data-report-collection-risk-ready={
        !isLoading && collectionRisk.items.length > 0 ? "true" : "false"
      }
    >
      <Notification
      ...
      >
        <ReportControlDock
          ariaLabel="کنترل‌های فروش کالا و خدمات"
          presentation="approved"
          title="کنترل گزارش"
          subtitle="بازه زمانی، حالت حسابرسی و نمای سود فروش کالا و خدمات"
          icon={<i className="fa-solid fa-sliders" aria-hidden="true" />}
          footer={(
            <ReportControlFooter
              ariaLabel="عملیات و وضعیت فروش کالا و خدمات"
              statuses={(
                <>
                  <ReportControlStatus tone="info" icon={<i className="fa-regular fa-calendar" aria-hidden="true" />}>
                    ...
                    <bdi dir="ltr" className="font-black">{fromDate ? toShamsiStr(fromDate) : '—'}</bdi>
                    <span aria-hidden="true" className="font-black text-[var(--ds-text-muted)]">|</span>
                    <bdi dir="ltr" className="font-black">{toDate ? toShamsiStr(toDate) : '—'}</bdi>
                  </ReportControlStatus>
                  <ReportControlStatus tone="neutral" icon={<i className="fa-solid fa-filter" aria-hidden="true" />}>
                    <span>{getAuditModeTitle(auditMode)}</span>
                  </ReportControlStatus>
                </>
              )}
              actions={(
                <>
                  <Button
                    ...
                    disabled={isLoading || totalFilteredRows === 0}
          ...
          <ReportControlDateSection
            ...
            fromField={(
              <ReportFilterField label="از تاریخ" icon={<i className="fa-regular fa-calendar" />} className="report-control-approved__field" minWidthClassName="min-w-0">
                <ShamsiDatePicker selectedDate={fromDate} onChange={setFromDate} size="standard" />
              </ReportFilterField>
            )}
            toField={(
              <ReportFilterField label="تا تاریخ" icon={<i className="fa-regular fa-calendar-check" />} className="report-control-approved__field" minWidthClassName="min-w-0">
                <ShamsiDatePicker selectedDate={toDate} onChange={setToDate} size="standard" />
              </ReportFilterField>
            )}
          />

          <ReportControlFilters>
            <ReportFilterField label="حسابرسی تخفیف" icon={<i className="fa-solid fa-tags" />} className="report-control-approved__field" minWidthClassName="min-w-0">
              <SelectField
                controlOnly
                size="md"
                icon={false}
                value={auditMode}
                onValueChange={(value) => setAuditMode(value as DiscountAuditMode)}
                options={DISCOUNT_AUDIT_MODES.map((mode) => ({
                  value: mode.value,
                  label: `${mode.label} (${formatExactNumberText(mode.value === 'item' ? itemDiscountedRowsCount : mode.value === 'invoice' ? invoiceDiscountedRowsCount : auditCounts.all)})`,
                }))}
              />
            </ReportFilterField>

            <ReportFilterField label="نمای سود" icon={<i className="fa-solid fa-chart-line" />} className="report-control-approved__field" minWidthClassName="min-w-0">
              <SelectField
                controlOnly
                size="md"
                icon={false}
                value={profitView}
                onValueChange={(value) => setProfitView(value as ProfitViewMode)}
                options={PROFIT_VIEW_MODES.map((mode) => ({ value: mode.value, label: mode.label }))}
              />
            </ReportFilterField>
          </ReportControlFilters>
        </ReportControlDock>

        {!isLoading && totalFilteredRows === 0 ? (
          <EmptyState
            title="داده‌ای برای نمایش نیست"
            description={getAuditModeEmptyDescription(auditMode)}
            actionLabel="بازخوانی"
            onAction={fetchData}
          />
        ) : null}
        ...
        {/* Executive KPI Row */}
        <section
          className="product-sales-metrics-section"
          aria-label="خلاصه فروش کالا و خدمات"
        >
          <div className="product-sales-section-head">
            <div>
              <h3>خلاصه درآمد عملیاتی</h3>
              <p>
                شاخص‌های اصلی فروش، وصول و سود کالاهای انبار و خدمات در بازه
                انتخابی.
              </p>
            </div>
          </div>
          ...
          <div className="product-sales-metrics-grid product-sales-metrics-grid--primary">
            <article className="product-sales-metric-card">
              <span className="product-sales-metric-card__icon is-green">
                <i className="fa-solid fa-money-bill-trend-up" />
              </span>
              <div className="product-sales-metric-card__body">
                <span className="product-sales-metric-card__label">
                  فروش نقدی
                </span>
                ...
                <em>
                  درصد وصول:{" "}
                  {formatExactPercentText(cashCollectionRate)}
                </em>
              </div>
            </article>

            <article className="product-sales-metric-card">
              <span className="product-sales-metric-card__icon is-orange">
              ...
              <div className="product-sales-metric-card__body">
                ...
                <em>
                  درصد وصول:{" "}
                  {formatExactPercentText(creditCollectionRate)}
                </em>
              </div>
            </article>

            <article className="product-sales-metric-card">
              <span className="product-sales-metric-card__icon is-blue">
              ...
              <div className="product-sales-metric-card__body">
                ...
                <small>
                  وصول‌شده در بازه: {money(summary.installmentReceived)}
                </small>
                <em>
                  درصد وصول:{" "}
                  {formatExactPercentText(installmentCollectionRate)}
                </em>
              </div>
            </article>

            <article className="product-sales-metric-card">
              <span className="product-sales-metric-card__icon is-violet">
                <i className="fa-solid fa-chart-line" />
              </span>
              <div className="product-sales-metric-card__body">
                <span className="product-sales-metric-card__label">
                  {getProfitViewTitle(profitView)}
                </span>
                <strong>{money(activeSummaryProfit)}</strong>
                <small>
                  {profitView === "received"
                    ? "بر پایه مبالغ وصول‌شده"
                    : "سود کامل فروش ثبت‌شده"}
                </small>
                {profitView === "received" && (
                  <em>سود وصول‌نشده: {money(summary.unrecognizedProfit)}</em>
                )}
              </div>
            </article>
          </div>
        ...
        <section
          className="product-sales-metrics-section"
          aria-label="کنترل فروش فیلترشده"
        >
          <div className="product-sales-section-head">
            <div>
              <h3>کنترل فروش فیلترشده</h3>
              <p>
                نتیجه فیلتر، جستجو و حالت حسابرسی فعلی؛ بدون تکرار جزئیات داخل
                جدول.
              </p>
            </div>
          </div>
          ...
          <div className="product-sales-metrics-grid product-sales-metrics-grid--secondary">
            <article className="product-sales-metric-card is-compact">
              ...
              <div className="product-sales-metric-card__body">
                ...
                <small>{formatExactNumberText(totalFilteredRows)} ردیف</small>
              </div>
            </article>
            ...
            <article className="product-sales-metric-card is-compact">
              ...
              <div className="product-sales-metric-card__body">
                ...
                <small>
                  درصد وصول:{" "}
                  {formatExactPercentText(filteredCollectionRate)}
                </small>
              </div>
            </article>
            ...
            <article className="product-sales-metric-card is-compact">
              ...
              <div className="product-sales-metric-card__body">
                <span className="product-sales-metric-card__label">
                  {getProfitViewTitle(profitView)} فیلتر
                </span>
            ...
            <article className="product-sales-metric-card is-compact">
              <span className="product-sales-metric-card__icon is-slate">
              ...
              <div className="product-sales-metric-card__body">
                ...
                <strong className="product-sales-date-value">
                  {fromDate ? toShamsiStr(fromDate) : "—"} تا{" "}
                  {toDate ? toShamsiStr(toDate) : "—"}
                </strong>
                <small>{formatExactNumberText(totalFilteredRows)} ردیف</small>
              </div>
            </article>
        ...
        <div className="mt-4 rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {topProducts.map((p, idx) => (
              <div
                key={`${p.itemType}-${p.productId}`}
                className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 p-3"
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      {p.itemType === "service" ? "خدمت" : "لوازم"} • کد:{" "}
                      {formatExactNumberText(Number(p.productId || 0))} •
                      تعداد: {formatExactNumberText(Number(p.qty || 0))}
                    </div>
                  </div>
                </div>
                <div className="mt-2 font-extrabold text-gray-900 dark:text-gray-100">
                  {money(p.amount)}
                </div>
              </div>
            ))}
          ...
          <DataTableShell
            ...
            meta={(
              <div className="ux-table-shell__meta-pill">
                ...
                <span>
                  نمایش {formatExactNumberText(detailStartRow)} تا {formatExactNumberText(detailEndRow)} از {formatExactNumberText(totalFilteredRows)} ردیف
                </span>
              </div>
            )}
            actions={(
              <div className="w-full min-w-[9rem] sm:w-[9rem] lg:w-[9.5rem]">
                <SelectField
                  controlOnly
                  size="sm"
                  value={String(detailsPageSize)}
                  onValueChange={(value) => setDetailsPageSize(Number(value))}
                  ariaLabel="تعداد ردیف در صفحه"
                  options={DETAILS_PAGE_SIZE_OPTIONS.map((size) => ({ value: String(size), label: `${formatExactNumberText(size)} ردیف` }))}
                />
              </div>
            )}
            toolbar={(
              <ReportControlDock
                ariaLabel="جستجو در جزئیات فروش کالا و خدمات"
                embedded
                search={(
                  <AppSearchField
                    value={query}
                    onChange={setQuery}
                    placeholder="جستجوی جدول بر اساس قلم، سند یا پرداخت…"
                    ariaLabel="جستجوی جدول فروش کالا و خدمات"
                    clearable
                  />
                )}
                filters={auditOnly ? (
            ...
            footer={detailsTotalPages > 1 ? (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-xs font-bold text-[var(--ds-text-muted)]">
                  صفحه {formatExactNumberText(safeDetailsPage)} از {formatExactNumberText(detailsTotalPages)}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Button type="button" variant="secondary" size="sm" onClick={() => setDetailsPage(1)} disabled={safeDetailsPage <= 1}>اول</Button>
                  <Button type="button" variant="secondary" size="sm" onClick={() => setDetailsPage((page) => Math.max(1, page - 1))} disabled={safeDetailsPage <= 1}>قبلی</Button>
                  <Button type="button" variant="secondary" size="sm" onClick={() => setDetailsPage((page) => Math.min(detailsTotalPages, page + 1))} disabled={safeDetailsPage >= detailsTotalPages}>بعدی</Button>
                  <Button type="button" variant="secondary" size="sm" onClick={() => setDetailsPage(detailsTotalPages)} disabled={safeDetailsPage >= detailsTotalPages}>آخر</Button>
                </div>
              </div>
            ) : undefined}
            aria-label="جدول جزئیات فروش کالا و خدمات"
          >
            <div className="overflow-auto">
              <table className="product-sales-compact-table report-table ux-data-table" data-ui-table="true" data-ui-table-layout="managed" data-ui-bidi-scope="rtl-table">
                <colgroup>
                    ...
                    const paymentLabel =
                      r.paymentType === "installment"
                        ...
                        : r.paymentType === "credit"
                          ...
                          : "نقدی";
                    const sourceLabel =
                      r.sourceType === "installment" ? "اقساطی" : "فاکتور";
                    return (
                      <tr
                        key={`${r.sourceType || "invoice"}-${r.itemType || "inventory"}-${r.orderId}-${r.productId}-${detailStartRow + i}`}
                        data-navigation-anchor={reportNavigationAnchor('product-sales', `${r.sourceType || 'invoice'}-${r.orderId}-${r.productId}-${detailStartRow + i}`)}
                        className={
                          auditOnly && discountAudit.hasAnyDiscount
                            ? "is-audit-row"
                            : ""
                        }
                      >
                        <td>
                          <div className="product-sales-table-main">
                            ...
                            <span>
                              شناسه سند:{" "}
                              {formatExactNumberText(Number(r.orderId || 0))}
                            </span>
                          </div>
                        </td>
                        <td>
                          <button
                            ...
                            onClick={() =>
                              r.itemType !== "service"
                                ? openDrilldown(
                                    `/products?search=${encodeURIComponent(r.productName)}`,
                                    { contextLabel: `${r.productName} • سند #${formatExactNumberText(Number(r.orderId || 0))}`, anchorId: reportNavigationAnchor('product-sales', `${r.sourceType || 'invoice'}-${r.orderId}-${r.productId}-${detailStartRow + i}`) },
                                  )
                                : undefined
                            }
                            ...
                            title={
                              r.itemType === "service"
                                ? "خدمت ثبت‌شده"
                                : "نمایش این کالا در صفحه کالاها"
                            }
                          >
                            <span className="product-sales-item-link__icon">
                              <i
                                className={`fa-solid ${r.itemType === "service" ? "fa-screwdriver-wrench" : "fa-box"}`}
                              />
                            </span>
                            <span>
                              <strong>{r.productName}</strong>
                              <small>
                                {r.itemType === "service" ? "خدمت" : "لوازم"} •
                                کد{" "}
                                {formatExactNumberText(Number(r.productId || 0))}{" "}
                                • تعداد{" "}
                                {formatExactNumberText(Number(r.quantity || 0))}
                              </small>
                            </span>
                          </button>
                        </td>
                        ...
                        <td>
                          <div className="product-sales-table-money">
                            ...
                            <span>
                              وصول‌شده: {money(Number(r.receivedAmount || 0))}
                            </span>
                            <small>
                              قیمت واحد: {money(Number(r.unitPrice || 0))}
                            </small>
                          </div>
                        </td>
                        <td>
                          <div className="product-sales-table-money">
                            ...
                            <span>
                              تخفیف: {money(discountAudit.totalDiscount)}
                            </span>
                        ...
                        <td>
                          <div className="product-sales-row-actions">
                            <button
                              ...
                              onClick={() =>
                                openDrilldown(
                                  r.sourceType === "installment"
                                    ? `/installment-sales/${r.orderId}`
                                    : `/invoices/${r.orderId}`,
                                  { contextLabel: `${r.productName} • ${r.sourceType === 'installment' ? 'قرارداد اقساطی' : 'فاکتور'} #${formatExactNumberText(Number(r.orderId || 0))}`, anchorId: reportNavigationAnchor('product-sales', `${r.sourceType || 'invoice'}-${r.orderId}-${r.productId}-${detailStartRow + i}`) },
                                )
                              }
      ...
      <ReportSideDrawer
        ...
        ariaLabel="جزئیات اولویت پیگیری وصول"
        size="wide"
        tone="warning"
      >
        <div className="border-b border-slate-200 bg-gradient-to-l from-slate-50 via-white to-white px-5 py-5 dark:border-slate-800 dark:from-slate-900/60 dark:via-slate-950 dark:to-slate-950">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-3 text-lg font-black text-slate-950 dark:text-slate-50">
                جزئیات اولویت پیگیری وصول
              </h3>
              <p className="mt-1 text-sm leading-7 text-slate-500 dark:text-slate-400">
                این Drawer نشان می‌دهد چرا هر سند در سطح کم‌ریسک، نیازمند
                پیگیری، فوری یا بحرانی قرار گرفته است.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setRiskDrawerOpen(false)}
              data-skip-global-button="true" className="grid h-10 w-10 shrink-0 place-items-center !border-0 !bg-transparent text-slate-500 !shadow-none transition hover:text-slate-950 dark:text-slate-300 dark:hover:text-white"
            >
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-2xl border border-rose-200 bg-rose-50 px-3 py-3 dark:border-rose-900/60 dark:bg-rose-950/25">
              <div className="text-[10px] font-black text-rose-600 dark:text-rose-200">
                بحرانی
              </div>
              <div className="mt-1 text-base font-black text-rose-700 dark:text-rose-100">
                {formatExactNumberText(collectionRisk.counts.critical)}
              </div>
            </div>
            <div className="rounded-2xl border border-orange-200 bg-orange-50 px-3 py-3 dark:border-orange-900/60 dark:bg-orange-950/25">
              <div className="text-[10px] font-black text-orange-600 dark:text-orange-200">
                فوری
              </div>
              <div className="mt-1 text-base font-black text-orange-700 dark:text-orange-100">
                {formatExactNumberText(collectionRisk.counts.urgent)}
              </div>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 dark:border-amber-900/60 dark:bg-amber-950/25">
              <div className="text-[10px] font-black text-amber-600 dark:text-amber-200">
                نیازمند پیگیری
              </div>
              <div className="mt-1 text-base font-black text-amber-700 dark:text-amber-100">
                {formatExactNumberText(collectionRisk.counts.followup)}
              </div>
            </div>
            <div className="rounded-2xl border border-sky-200 bg-sky-50 px-3 py-3 dark:border-sky-900/60 dark:bg-sky-950/25">
              <div className="text-[10px] font-black text-sky-600 dark:text-sky-200">
                کم‌ریسک
              </div>
              <div className="mt-1 text-base font-black text-sky-700 dark:text-sky-100">
                {formatExactNumberText(collectionRisk.counts.low)}
              </div>
            </div>
          </div>
          ...
          {collectionRisk.items.length === 0 ? (
            <div className="mt-4 rounded-[26px] border border-emerald-200 bg-emerald-50 p-5 text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/25 dark:text-emerald-100">
              <div className="flex items-center gap-3">
                <IconGlyph tone="neutral" className="h-11 w-11" aria-hidden="true"><i className="fa-solid fa-check-double" /></IconGlyph>
                <div>
                  <div className="text-sm font-black">
                    سند باز برای پیگیری پیدا نشد
                  </div>
                  <div className="mt-1 text-xs leading-6 opacity-80">
                    در فیلتر فعلی، فاکتور اعتباری/اقساطی با مانده قابل پیگیری
                    دیده نمی‌شود.
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4 space-y-3">
              {collectionRisk.items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/55"
                  >
                    <div className="flex flex-col gap-3 sm:flex-row-reverse sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-black ${meta.badgeClassName}`}
                          >
                            <i
                              className={`fa-solid ${meta.icon} text-[10px]`}
                            />
                            {meta.label}
                          </span>
                          <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-black text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                            {item.sourceType === "installment"
                              ? "اقساطی"
                              : "فاکتور"}{" "}
                            #{formatExactNumberText(item.orderId)}
                          </span>
                          <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-black text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                            امتیاز {formatExactNumberText(item.score)} / ۱۰۰
                          </span>
                        </div>
                        <div className="mt-3 text-sm font-black text-slate-950 dark:text-slate-50">
                          {item.customerName}
                        </div>
                        <p className="mt-1 text-xs leading-6 text-slate-500 dark:text-slate-400">
                          ...
                          {item.dueDate
                            ? `• نزدیک‌ترین سررسید: ${item.dueDate}`
                            : ""}
                        </p>
                      </div>
      ...
      <ReportSideDrawer
        ...
        ariaLabel="جزئیات سلامت محاسبات"
        size="md"
        tone="info"
      >
        <div className="border-b border-slate-200 bg-gradient-to-l from-slate-50 via-white to-white px-5 py-5 dark:border-slate-800 dark:from-slate-900/60 dark:via-slate-950 dark:to-slate-950">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-3 text-lg font-black text-slate-950 dark:text-slate-50">
                جزئیات سلامت محاسبات
              </h3>
              <p className="mt-1 text-sm leading-7 text-slate-500 dark:text-slate-400">
                این لیست اختلاف جمع فاکتور، پخش تخفیف کلی، فرمول ردیف‌ها و
                اختلاف‌های رُندی را جدا می‌کند.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 dark:border-slate-800 dark:bg-slate-900/50">
              ...
              <div className="mt-1 text-base font-black text-slate-950 dark:text-slate-50">
                {formatExactNumberText(calculationHealth.checkedDocs)}
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 dark:border-slate-800 dark:bg-slate-900/50">
              <div className="text-[10px] font-black text-slate-500 dark:text-slate-400">
              ...
              <div className="mt-1 text-base font-black text-slate-950 dark:text-slate-50">
                {formatExactNumberText(calculationHealth.checkedRows)}
              </div>
            </div>
            <div className="rounded-2xl border border-rose-200 bg-rose-50 px-3 py-3 dark:border-rose-900/60 dark:bg-rose-950/25">
              <div className="text-[10px] font-black text-rose-600 dark:text-rose-200">
              ...
              <div className="mt-1 text-base font-black text-rose-700 dark:text-rose-100">
                {formatExactNumberText(calculationHealth.errorCount)}
              </div>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 dark:border-amber-900/60 dark:bg-amber-950/25">
              <div className="text-[10px] font-black text-amber-600 dark:text-amber-200">
              ...
              <div className="mt-1 text-base font-black text-amber-700 dark:text-amber-100">
                {formatExactNumberText(calculationHealth.warningCount)}
              </div>
            </div>
          </div>
            ...
            <div className="mt-4 rounded-[26px] border border-emerald-200 bg-emerald-50 p-5 text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/25 dark:text-emerald-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs leading-6 opacity-80">
                    جمع ردیف‌ها، تخفیف‌ها و مبالغ نهایی در بازه فعلی سالم هستند.
                  </div>
                </div>
              </div>
            </div>
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row-reverse sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-black ${issueMeta.className}`}
                          >
                            <i
                              className={`fa-solid ${issueMeta.icon} text-[10px]`}
                            />
                            {issueMeta.label}
                          </span>
                          <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-black text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
                            {issue.sourceType === "installment"
                              ? "اقساطی"
                              : "فاکتور"}{" "}
                            #{formatExactNumberText(issue.orderId)}
                          </span>
                        </div>
                        <div className="mt-3 text-sm font-black text-slate-950 dark:text-slate-50">
                          {issue.title}
                      ...
                      <div className="shrink-0 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-left dark:border-slate-800 dark:bg-slate-950/70">
                        ...
                        <div className="mt-1 text-sm font-black text-slate-950 dark:text-slate-50">
                          {money(Math.abs(Number(issue.difference || 0)))}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-600 dark:text-slate-300 sm:grid-cols-4">
                      <div className="rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-950/60">
                        ...
                        <div className="mt-1 font-black">
                          {money(Number(issue.expectedAmount || 0))}
                        </div>
                      </div>
                      <div className="rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-950/60">
                        <div className="text-[10px] text-slate-400">
                        ...
                        <div className="mt-1 font-black">
                          {money(Number(issue.actualAmount || 0))}
                        </div>
                      </div>
                      <div className="rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-950/60">
                        <div className="text-[10px] text-slate-400">
                        ...
                        <div className="mt-1 font-black">
                          {money(Number(issue.invoiceDiscountShareTotal || 0))}
                        </div>
                      </div>
                      <div className="rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-950/60">
                        <div className="text-[10px] text-slate-400">
                        ...
                        <div className="mt-1 font-black">
                          {formatExactNumberText(Number(issue.rowsCount || 0))}
                        </div>
                      </div>
                    </div>

                    {Array.isArray(issue.rowIssues) &&
                      issue.rowIssues.length > 0 && (
                        <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/55">
                          ...
                          <div className="mt-2 space-y-2">
                            {issue.rowIssues.map((rowIssue, idx) => (
                              <div
                                key={`${issue.id}-${idx}`}
                                className="rounded-xl bg-white px-3 py-2 text-xs dark:bg-slate-900/70"
                              >
                                <div className="font-black text-slate-800 dark:text-slate-100">
                                  {rowIssue.productName}
                                </div>
                                <div className="mt-1 leading-5 text-slate-500 dark:text-slate-400">
                                  {rowIssue.reason} — اختلاف:{" "}
                                  {money(
                                    Math.abs(Number(rowIssue.difference || 0)),
                                  )}
                                </div>
                              </div>
                            ))}
      ...
      <ReportSideDrawer
        open={Boolean(calculationRow)}
        onClose={() => setCalculationRow(null)}
        ariaLabel="جزئیات محاسبه تخفیف و سود"
        size="sm"
        tone="info"
      >
        <div className="border-b border-slate-200 bg-gradient-to-l from-indigo-50 via-white to-white px-5 py-5 dark:border-slate-800 dark:from-indigo-950/25 dark:via-slate-950 dark:to-slate-950">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-3 text-lg font-black text-slate-950 dark:text-slate-50">
                جزئیات محاسبه تخفیف و سود
              </h3>
              <p className="mt-1 text-sm leading-7 text-slate-500 dark:text-slate-400">
                سهم تخفیف کلی فاکتور بر اساس وزن مبلغی همین ردیف بین اقلام تقسیم
                شده است.
              </p>
            </div>
            <button
        ...
        {(() => {
          ...
          const calcCards = [
            {
              label: "مبلغ ناخالص ردیف",
              value: money(b.gross),
              icon: "fa-layer-group",
            },
            {
              label: "تخفیف خود آیتم",
              value: money(b.itemDiscount),
              icon: "fa-tag",
            },
            {
              label: "مبلغ بعد از تخفیف آیتم",
              value: money(b.beforeGlobal),
              icon: "fa-equals",
            },
            {
              label: "سهم از تخفیف کلی فاکتور",
              value: money(b.invoiceShare),
              icon: "fa-scissors",
            },
            {
              label: "کل تخفیف این ردیف",
              value: money(b.totalDiscount),
              icon: "fa-percent",
            },
            {
              label: "جمع نهایی ردیف",
              value: money(b.afterDiscount),
            ...
            <div className="flex-1 overflow-auto p-5">
              <div className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/40">
                ...
                <div className="mt-2 text-base font-black text-slate-900 dark:text-slate-100">
                  {calculationRow!.productName}
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-black shadow-sm ${drawerDiscountType.className}`}
                  >
                    <i
                      className={`fa-solid ${drawerDiscountType.icon} text-[10px]`}
                    />
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                  <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-700 dark:bg-slate-950">
                    سند #
                    {formatExactNumberText(Number(calculationRow!.orderId || 0))}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-700 dark:bg-slate-950">
                    {calculationRow!.itemType === "service" ? "خدمت" : "لوازم"}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-700 dark:bg-slate-950">
                    تعداد {formatExactNumberText(Number(b.quantity || 0))}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-1 gap-3">
                {calcCards.map((card) => (
                  <div
                    key={card.label}
                    className="flex items-center justify-between gap-3 rounded-[22px] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/60"
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <IconGlyph tone="neutral" className="h-10 w-10 shrink-0" aria-hidden="true"><i className={"fa-solid " + card.icon} /></IconGlyph>
                      <span className="text-sm font-bold text-slate-600 dark:text-slate-300">
                        {card.label}
                      </span>
                    </div>
                    <strong className="whitespace-nowrap text-sm font-black text-slate-950 dark:text-slate-50">
                      {card.value}
                    </strong>
                  </div>
                ))}
              ...
              <div className="mt-4 rounded-[26px] border border-indigo-200 bg-indigo-50/80 p-4 text-indigo-950 shadow-sm dark:border-indigo-900/60 dark:bg-indigo-950/20 dark:text-indigo-100">
                <div className="flex min-w-0 flex-1 flex-row-reverse items-start gap-3 text-right">
                  <IconGlyph tone="neutral" className="h-10 w-10 shrink-0" aria-hidden="true"><i className="fa-solid fa-square-root-variable" /></IconGlyph>
                  <div className="min-w-0 flex-1 text-right">
                    <div className="text-sm font-black">
                      فرمول عددی همین ردیف
                    </div>
                    <p className="mt-1 text-xs leading-6 opacity-80">
                      این بخش دقیقاً نشان می‌دهد تخفیف آیتم و سهم تخفیف کلی
                      فاکتور چطور روی همین قلم اعمال شده است.
                    </p>
                    ...
                    <div className="mt-3 space-y-2">
                      <div className="rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs leading-6 dark:border-white/10 dark:bg-slate-950/45">
                        ...
                        <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                          {numberFa(b.quantity)} × {money(b.unitPrice)} ={" "}
                          {money(b.gross)}
                        </div>
                      </div>
                      ...
                      <div className="rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs leading-6 dark:border-white/10 dark:bg-slate-950/45">
                        <div className="font-black text-indigo-700 dark:text-indigo-200">
                          ۲) بعد از تخفیف خود آیتم
                        </div>
                        <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                          {money(b.gross)} − {money(b.itemDiscount)} ={" "}
                          {money(b.beforeGlobal)}
                        </div>
                      </div>

                      <div className="rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs leading-6 dark:border-white/10 dark:bg-slate-950/45">
                        <div className="font-black text-indigo-700 dark:text-indigo-200">
                          ۳) سهم این ردیف از تخفیف کلی فاکتور
                        </div>
                        {b.orderDiscount > 0 && b.invoiceDiscountBase > 0 ? (
                          <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                            {money(b.beforeGlobal)} ÷{" "}
                            {money(b.invoiceDiscountBase)} ={" "}
                            {percentFa(b.shareRate)} از فاکتور
                            <br />
                            {money(b.orderDiscount)} × {percentFa(b.shareRate)}{" "}
                            = {money(b.invoiceShare)}
                          </div>
                        ) : (
                          <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                            برای این سند تخفیف کلی فاکتور ثبت نشده؛ سهم فاکتور ={" "}
                            {money(0)}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs leading-6 dark:border-white/10 dark:bg-slate-950/45">
                        ...
                        <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                          {money(b.beforeGlobal)} − {money(b.invoiceShare)} ={" "}
                          {money(b.afterDiscount)}
                        </div>
                        <div className="mt-1 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                          کل تخفیف این ردیف: {money(b.itemDiscount)} +{" "}
                          {money(b.invoiceShare)} = {money(b.totalDiscount)} /
                          نرخ موثر تخفیف: {percentFa(b.effectiveDiscountRate)}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-4 rounded-[24px] border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-sm font-black text-slate-700 dark:text-slate-200">
                    وصول‌شده برای این ردیف
                  </span>
                  <strong className="text-sm font-black text-slate-950 dark:text-slate-50">
                    {money(b.received)}
                  </strong>
                </div>
                ...
                <div className="mt-2 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                  درصد وصول:{" "}
                  {formatExactPercentText(b.collectionRate)}
                </div>
              </div>
              ...
              <div className="mt-4 rounded-[26px] border border-emerald-200 bg-emerald-50/80 p-4 text-emerald-950 shadow-sm dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-100">
                <div className="flex min-w-0 flex-1 flex-row-reverse items-start gap-3 text-right">
                  <IconGlyph tone="neutral" className="h-10 w-10 shrink-0" aria-hidden="true"><i className="fa-solid fa-hand-holding-dollar" /></IconGlyph>
                  <div className="min-w-0 flex-1 text-right">
                    <div className="text-sm font-black">
                      تطبیق سود همین ردیف
                    </div>
                    <p className="mt-1 text-xs leading-6 opacity-80">
                      سود کل ردیف با نسبت وصول ضرب می‌شود تا سود قابل شناسایی
                      حسابداری به‌دست بیاید.
                    </p>
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <div className="rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs dark:border-white/10 dark:bg-slate-950/45">
                        <div className="font-black opacity-70">
                          بهای تمام‌شده
                        </div>
                        <div className="mt-1 font-black">
                          {money(b.lineCost)}
                    ...
                    <div className="mt-3 rounded-2xl border border-white/70 bg-white/75 px-3 py-2 text-xs leading-6 dark:border-white/10 dark:bg-slate-950/45">
                      <div className="font-black text-emerald-700 dark:text-emerald-200">
                        فرمول سود وصول‌شده
                      </div>
                      <div className="mt-1 font-mono text-[12px] text-slate-700 dark:text-slate-200">
                        {money(b.fullProfit)} × {percentFa(b.collectionRate)} ={" "}
                        {money(b.realizedProfit)}
                      </div>
                    </div>
                  </div>
