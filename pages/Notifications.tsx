...
const EMPTY_OUTBOX_STATS: OutboxStats = { pending: 0, processing: 0, sent: 0, failed: 0, total: 0 };
...
const formatFaDateTime = (value?: string | null) => {
  if (!value) return '—';
  const rendered = formatIsoToShamsiDateTime(value);
  return rendered === 'نامشخص' || rendered === 'تاریخ نامعتبر' ? String(value) : rendered;
};

const formatDueDate = (value?: string | null) => {
  if (!value) return '—';
  const rendered = formatIsoToShamsi(value);
  return rendered === 'نامشخص' || rendered === 'تاریخ نامعتبر' ? String(value) : rendered;
};
...
const getNotificationRoute = (item: UnifiedNotificationItem): string | null => {
  ...
  if (type.includes('Check') || type.includes('Installment') || type === 'OverdueInstallment') return '/installment-sales';
...
const NotificationsPage: React.FC = () => {
  ...
  const fetchOutbox = async () => {
    if (!isAdmin) {
      setOutbox([]);
      setOutboxStats(EMPTY_OUTBOX_STATS);
      return;
    }
    try {
      const response = await apiFetch('/api/notifications/outbox?status=ALL&limit=8&offset=0');
      const result = await response.json().catch(() => ({}));
      if (!response.ok || result?.success === false) throw new Error(result?.message || 'خطا در دریافت صف ارسال');
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const [response, collectionItems] = await Promise.all([
        apiFetch('/api/notifications?includeDismissed=1'),
        buildCollectionNotifications(),
      ]);
      const result = await response.json().catch(() => ({}));
      ...
      const merged = Array.from(
        new Map([...collectionItems, ...baseItems].map((item) => [String(item.id), item])).values(),
      );
      const dismissedIds = new Set<string>(
        Array.isArray(result?.meta?.dismissedIds)
          ? result.meta.dismissedIds.map((id: unknown) => String(id))
          : baseItems.filter((item: UnifiedNotificationItem) => item.isDismissed).map((item: UnifiedNotificationItem) => String(item.id)),
      );
      setItems(merged);
      setHiddenIds(dismissedIds);
  ...
  const categoryCounts = useMemo(() => {
    ...
    const base: Record<NotificationCategory, number> = { all: source.length, finance: 0, sales: 0, customers: 0, inventory: 0, services: 0, system: 0 };
    source.forEach((item) => { base[getCategoryKey(item)] += 1; });
    return base;
  }, [activeItems, archivedItems, queueFilter]);
  ...
  const exportRows = visibleItems.map((item) => ({
    ...
    due: (item as any)?.meta?.dueDate ? formatDueDate((item as any).meta.dueDate) : '',
  ...
  const sendSms = async (item: UnifiedNotificationItem) => {
    ...
    try {
      ...
      setNotification({ type: 'success', text: 'درخواست پیامک در صف ارسال ثبت شد.' });
      await fetchOutbox();
    } catch (err: any) {
  ...
  const sendTelegram = async (item: UnifiedNotificationItem) => {
    ...
    try {
      ...
      setNotification({ type: 'success', text: 'درخواست تلگرام در صف ارسال ثبت شد.' });
      await fetchOutbox();
    } catch (err: any) {
  ...
  const archiveNotification = async (item: UnifiedNotificationItem) => {
    ...
    setNotification(null);
    try {
      const response = await apiFetch(`/api/notifications/${encodeURIComponent(id)}/dismiss`, { method: 'POST' });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || result?.success === false) throw new Error(result?.message || 'ثبت وضعیت اعلان انجام نشد.');
      setHiddenIds((prev) => new Set([...prev, id]));
      setNotification({ type: 'success', text: 'اعلان به بخش انجام‌شده منتقل شد.' });
      ...
      setNotification({ type: 'error', text: err?.message || 'ثبت وضعیت اعلان انجام نشد.' });
    } finally {
      setArchiveBusy(false);
    }
  };

  const restoreNotification = async (item: UnifiedNotificationItem) => {
    const id = String(item.id);
    setArchiveBusy(true);
    setNotification(null);
    try {
      const response = await apiFetch(`/api/notifications/${encodeURIComponent(id)}/dismiss`, { method: 'DELETE' });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || result?.success === false) throw new Error(result?.message || 'بازگردانی اعلان انجام نشد.');
      setHiddenIds((prev) => {
        const next = new Set(prev);
        next.delete(id);
        return next;
      });
      setNotification({ type: 'success', text: 'اعلان به صندوق فعال بازگردانده شد.' });
      window.dispatchEvent(new CustomEvent('kourosh:notifications-updated', { detail: { id, action: 'restore' } }));
      window.dispatchEvent(new CustomEvent('kourosh:header-quick-refresh'));
    } catch (err: any) {
      setNotification({ type: 'error', text: err?.message || 'بازگردانی اعلان انجام نشد.' });
    } finally {
      setArchiveBusy(false);
    }
  };
  ...
  const archiveVisible = async () => {
    if (!visibleItems.length || archiveBusy) return;
    const ids = visibleItems.map((item) => String(item.id));
    setArchiveBusy(true);
    setNotification(null);
    try {
      const response = await apiFetch('/api/notifications/dismiss-batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || result?.success === false) throw new Error(result?.message || 'ثبت گروهی اعلان‌ها انجام نشد.');
      setHiddenIds((prev) => new Set([...prev, ...ids]));
      setNotification({ type: 'success', text: `${asFa(ids.length)} اعلان به بخش انجام‌شده منتقل شد.` });
      window.dispatchEvent(new CustomEvent('kourosh:notifications-updated', { detail: { ids, action: 'dismiss-batch' } }));
      window.dispatchEvent(new CustomEvent('kourosh:header-quick-refresh'));
    } catch (err: any) {
      setNotification({ type: 'error', text: err?.message || 'ثبت گروهی اعلان‌ها انجام نشد.' });
    } finally {
      setArchiveBusy(false);
    }
  };

  const restoreVisible = async () => {
    const ids = visibleItems.map((item) => String(item.id));
    if (!ids.length || archiveBusy) return;
    setArchiveBusy(true);
    setNotification(null);
    try {
      const response = await apiFetch('/api/notifications/restore-batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || result?.success === false) throw new Error(result?.message || 'بازگردانی گروهی اعلان‌ها انجام نشد.');
      setHiddenIds((prev) => {
        const next = new Set(prev);
        ids.forEach((id) => next.delete(id));
        return next;
      });
      setNotification({ type: 'success', text: `${asFa(ids.length)} اعلان به صندوق فعال بازگردانده شد.` });
      window.dispatchEvent(new CustomEvent('kourosh:notifications-updated', { detail: { ids, action: 'restore-batch' } }));
      window.dispatchEvent(new CustomEvent('kourosh:header-quick-refresh'));
    } catch (err: any) {
      setNotification({ type: 'error', text: err?.message || 'بازگردانی گروهی اعلان‌ها انجام نشد.' });
    } finally {
      setArchiveBusy(false);
    }
  };
  ...
  const openSmsModal = (item: UnifiedNotificationItem) => {
    const phone = String((item as any)?.meta?.customerPhone || (item as any)?.meta?.phone || '');
  ...
  const renderItemMeta = (item: UnifiedNotificationItem) => {
    ...
    const pairs = [
      ...
      (meta.customerPhone || meta.phone) ? { icon: 'fa-phone', text: meta.customerPhone || meta.phone } : null,
      meta.dueDate ? { icon: 'fa-calendar', text: formatDueDate(meta.dueDate) } : null,
  ...
  const selectNotification = (item: UnifiedNotificationItem) => {
  ...
  const renderDetails = () => {
    ...
    const dueText = meta.dueDate ? formatDueDate(meta.dueDate) : (isTodayItem(selectedItem) ? 'امروز' : '—');
    return (
      <aside className="notifications-saas-detail" data-severity={getSeverity(selectedItem)} data-category={cat} data-type={String(selectedItem.type || '')}>
        ...
        <div className="notifications-saas-detail__facts">
          ...
          <div><span>تاریخ/سررسید</span><strong>{meta.dueDate ? formatDueDate(meta.dueDate) : (isTodayItem(selectedItem) ? 'امروز' : '—')}</strong></div>
          <div><span>مبلغ</span><strong>{meta.amount || meta.outstandingAmount ? `${asFa(meta.amount || meta.outstandingAmount)} تومان` : '—'}</strong></div>
          <div><span>شماره تماس</span><strong>{meta.customerPhone || meta.phone || '—'}</strong></div>
        </div>
        ...
        <div className="notifications-saas-detail__actions">
          ...
          {queueFilter === 'archived' ? (
            <button type="button" onClick={() => restoreNotification(selectedItem)} disabled={archiveBusy}>
              <i className="fa-solid fa-rotate-left" />بازگردانی
            </button>
          ) : (
            <button type="button" onClick={() => archiveNotification(selectedItem)} disabled={archiveBusy}>
  ...
  const renderOutbox = () => {
    ...
    const getStatusLabel = (status?: string) => {
      ...
      if (normalized === 'sent' || normalized === 'done') return 'ارسال‌شده';
  ...
  return (
    <div className="notifications-saas-page-shell notifications-compact-v2" dir="rtl" data-ui-page-shell="true" data-ui-notifications-saas="true" data-ui-notifications-compact="true">
      <div className="notifications-saas-frame">
        <header className="notifications-saas-frame__header">
          <div className="notifications-saas-brand">
            ...
            <div>
              <span className="notifications-saas-kicker"><i className="fa-solid fa-shield-halved" /> مرکز عملیات روزانه</span>
              <h1>مرکز اعلان‌ها</h1>
              <p>هشدارهای واقعی فروشگاه، پیگیری مشتری، سررسیدها و وضعیت صف پیام‌رسانی در یک نمای فشرده.</p>
            </div>
          </div>
          <div className="notifications-compact-header-meta">
            <span><i className="fa-solid fa-database" /> داده زنده فروشگاه</span>
            <span><i className="fa-regular fa-clock" /> آخرین بروزرسانی: {formatFaDateTime(lastSyncedAt)}</span>
          </div>
        ...
        <section className="notifications-compact-stats" aria-label="خلاصه اعلان‌ها">
          {stats.map((stat) => (
            <button
              key={stat.key}
              type="button"
              data-stat={stat.key}
              onClick={() => {
                if (stat.key === 'urgent') setQueueFilter('urgent');
                else if (stat.key === 'today') setQueueFilter('today');
                else if (stat.key === 'actionable') setQueueFilter('actionable');
                else setQueueFilter('active');
              }}
            >
              <span><i className={`fa-solid ${stat.icon}`} /></span>
              <strong>{asFa(stat.value)}</strong>
              <em>{stat.label}</em>
        ...
        <section className="notifications-saas-commandbar" aria-label="ابزارهای اعلان‌ها">
          <div className="notifications-saas-commandbar__actions">
            ...
            />
            {queueFilter === 'archived' ? (
              <button type="button" className="notifications-saas-command notifications-saas-command--soft" onClick={restoreVisible} disabled={!visibleItems.length || archiveBusy}>
                <i className="fa-solid fa-rotate-left" /> بازگردانی موارد نمایش‌داده‌شده
              </button>
            ) : (
              <button type="button" className="notifications-saas-command notifications-saas-command--primary" onClick={archiveVisible} disabled={!visibleItems.length || archiveBusy}>
                <i className="fa-solid fa-check" /> انجام همه موارد نمایش‌داده‌شده
              </button>
            )}
          </div>
          <div className="notifications-saas-commandbar__tools">
            <button type="button" className="notifications-saas-command notifications-saas-command--soft" onClick={refresh} disabled={isRefreshing}>
              <i className={`fa-solid fa-rotate-right ${isRefreshing ? 'animate-spin' : ''}`} /> {isRefreshing ? 'در حال بروزرسانی' : 'بروزرسانی'}
            </button>
        ...
        <section className="notifications-saas-filterbar">
          <div className="notifications-saas-search notifications-saas-search--wide">
            ...
            <TextField controlOnly unstyled value={query} onChange={(e) => setQuery(e.target.value)} placeholder="جستجو در عنوان، مشتری، تلفن یا متن اعلان" />
          </div>
          <SelectField controlOnly unstyled showChevron={false} value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value as NotificationCategory)} aria-label="نوع اعلان">
            {(Object.keys(CATEGORY_META) as NotificationCategory[]).map((key) => (
              <option key={key} value={key}>{CATEGORY_META[key].label}</option>
            ))}
          </SelectField>
          <SelectField controlOnly unstyled showChevron={false} value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value as any)} aria-label="وضعیت اهمیت">
          ...
          <div className="notifications-saas-mini-queues">
            {(['today', 'actionable'] as QueueFilter[]).map((key) => (
        ...
        <section className="notifications-saas-workbench notifications-saas-workbench--mockup">
          <aside className="notifications-saas-categories notifications-saas-categories--mockup">
            ...
            <div className="notifications-saas-category-summary notifications-saas-category-summary--mockup">
              ...
              <p><span><i className="fa-solid fa-bell" />فعال</span><strong>{asFa(activeItems.length)}</strong></p>
              <p><span><i className="fa-solid fa-triangle-exclamation" />نیازمند توجه</span><strong>{asFa(activeItems.filter(isUrgentItem).length)}</strong></p>
              <p><span><i className="fa-solid fa-box-archive" />انجام‌شده</span><strong>{asFa(archivedItems.length)}</strong></p>
            </div>
          </aside>
          ...
          <main className="notifications-saas-main notifications-saas-main--mockup">
            <div className="notifications-saas-main__head">
              <div>
                <h2>{queueFilter === 'archived' ? 'اعلان‌های انجام‌شده' : 'اعلان‌های فعال'}</h2>
                <p>نمایش {asFa(pageStartIndex)} تا {asFa(pageEndIndex)} از {asFa(visibleItems.length)} مورد مطابق فیلتر فعلی</p>
              </div>
      ...
      {smsModal.open ? (
        <div className="notifications-saas-sms-backdrop" dir="rtl" data-ui-notifications-modal="sms-compose">
          <div className="notifications-saas-sms-modal">
            ...
            <TextareaField controlOnly value={smsModal.text} onChange={(e) => setSmsModal((prev) => ({ ...prev, text: e.target.value }))} />
