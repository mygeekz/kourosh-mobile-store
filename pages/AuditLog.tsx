...
const AuditLogPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser && !['Admin', 'Manager'].includes(currentUser.roleName)) {
      setNotification({ type: 'error', text: 'شما اجازه دسترسی به تاریخچه عملیات را ندارید.' });
      navigate('/');
    }
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      setLogs([]);
      setNotification({ type: 'error', text: error?.message || 'خطا در دریافت تاریخچه عملیات.' });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <main className="audit-operations-v2" dir="rtl" data-ui-audit-operations="true">
      ...
      <section className="audit-operations-v2__hero">
        ...
        <div className="audit-operations-v2__hero-actions">
          <div className="audit-operations-v2__source">
            <span><i className="fa-solid fa-database" /> منبع: سوابق ثبت‌شده</span>
      ...
      <section className="audit-operations-v2__stats" aria-label="خلاصه تاریخچه عملیات">
        {statCards.map((item) => (
      ...
      <section className="audit-operations-v2__table-card">
        ...
        {isLoading ? (
          ...
          <DataTableShell className="audit-operations-v2__table-shell" data-ui-audit-log-table="true">
            <Table layout="managed" density="comfortable" className="audit-operations-v2__table">
      ...
      >
        {selectedLog ? (
          <>
            <header>
              <div><span><i className="fa-solid fa-clock-rotate-left" /></span><div><h2>جزئیات رویداد #{selectedLog.id.toLocaleString('fa-IR')}</h2><p>اطلاعات ثبت‌شده این رویداد</p></div></div>
