...
const accentToClasses = (accent: WidgetDef["accent"]) => {
  switch (accent) {
    case "emerald":
      return {
        bg: "from-emerald-500/12 to-teal-500/10",
        icon: "text-emerald-700 dark:text-emerald-300",
        ring: "ring-emerald-500/20",
      };
    case "violet":
      return {
        bg: "from-violet-500/12 to-fuchsia-500/10",
        icon: "text-violet-700 dark:text-violet-300",
        ring: "ring-violet-500/20",
      };
    case "rose":
      return {
        bg: "from-rose-500/12 to-pink-500/10",
        icon: "text-rose-700 dark:text-rose-300",
        ring: "ring-rose-500/20",
      };
    case "sky":
      return {
        bg: "from-sky-500/12 to-indigo-500/10",
        icon: "text-sky-700 dark:text-sky-300",
        ring: "ring-sky-500/20",
      };
    case "amber":
      return {
        bg: "from-amber-500/12 to-orange-500/10",
        icon: "text-amber-700 dark:text-amber-300",
        ring: "ring-amber-500/20",
      };
    case "teal":
      return {
        bg: "from-teal-500/12 to-cyan-500/10",
    ...
    default:
      return {
        bg: "from-indigo-500/12 to-fuchsia-500/10",
...
export default function AddWidgetModal({
  ...
  return (
    <DialogShell
      isOpen={open}
      onClose={onClose}
      overlayClassName="dashboard-add-widget-overlay ux-overlay-backdrop px-3"
      panelClassName="ux-stable-panel ux-stable-modal-panel dashboard-add-widget-modal dashboard-add-widget-modal--executive w-[min(1040px,96vw)] overflow-hidden rounded-3xl bg-white shadow-[0_20px_90px_rgba(0,0,0,0.18)] ring-1 ring-slate-200 dark:bg-slate-950 dark:ring-slate-800"
      ariaLabel="افزودن کارت جدید"
      panelAttributes={{
    ...
    >
      <div
        data-ui-dashboard-modal="add-widget"
        dir="rtl"
        className="dashboard-add-widget-modal__content"
      >
        <div className="dashboard-add-widget-modal__header dashboard-add-widget-modal__header--executive px-6 py-5 border-b border-black/5 dark:border-white/10">
          <div className="dashboard-add-widget-modal__heading flex items-start justify-between gap-3">
            <div className="dashboard-add-widget-titlebar min-w-0">
              ...
              <div className="dashboard-add-widget-titlebar__copy min-w-0">
                <div className="dashboard-add-widget-kicker">
                  مدیریت کارت‌های داشبورد
                </div>
                <h3 className="dashboard-add-widget-title text-lg font-extrabold text-gray-900 dark:text-white">
                  افزودن کارت جدید
                </h3>
                <p className="dashboard-add-widget-subtitle text-sm text-gray-600 dark:text-gray-300 mt-0.5">
                  کارت مورد نیاز را انتخاب کنید؛ پس از افزودن، ترتیب و اندازه آن
                  از حالت ویرایش داشبورد قابل تنظیم است.
                </p>
              </div>
            </div>
          ...
          <div
            className="dashboard-add-widget-summary"
            aria-label="خلاصه کارت‌های قابل افزودن"
          >
          ...
          <div className="dashboard-add-widget-controls mt-4 flex flex-col sm:flex-row gap-3">
            <label className="dashboard-add-widget-search relative flex-1">
              ...
              <TextField controlOnly controlSize="sm"
                ...
                data-ui-control-kind="dashboard-widget-search"
            ...
            <div
              className="dashboard-add-widget-categories flex gap-2 overflow-auto no-scrollbar"
              role="tablist"
              aria-label="دسته‌بندی کارت‌ها"
            >
              {categories.map((c) => (
                <button
                  key={c}
                  ...
                  >
                    <div className="dashboard-add-widget-card__main flex items-start justify-between gap-3">
                      <div className="dashboard-add-widget-card__identity flex items-start gap-3 min-w-0">
                        ...
                        >
                          <i
                            className={`${w.icon || "fa-solid fa-square-poll-vertical"} ${a.icon}`}
                          />
                        </span>

                        <div className="dashboard-add-widget-card__copy min-w-0">
                          <div className="dashboard-add-widget-card__title text-sm font-extrabold text-gray-900 dark:text-white truncate">
                            {w.title}
                          </div>
                          <div className="dashboard-add-widget-card__meta mt-0.5 flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                            ...
                            <span
                              className="dashboard-add-widget-card__id"
                              dir="ltr"
                            >
                              {w.id}
                            </span>
        ...
        <div className="dashboard-add-widget-modal__footer dashboard-add-widget-modal__footer--executive px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex items-center justify-between gap-3">
          <div className="dashboard-add-widget-footnote text-xs text-gray-600 dark:text-gray-300">
            کارت‌های افزوده‌شده از حالت ویرایش داشبورد قابل جابه‌جایی، تغییر
            اندازه و مخفی‌سازی هستند.
          </div>
          <button
