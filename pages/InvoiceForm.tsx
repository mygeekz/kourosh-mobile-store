...
import { FormGrid, FormGridItem, SearchableSelectField, TextField, positiveNumberError, requiredSelectionError } from '@/components/ui';
...
export default function InvoiceForm() {
  ...
  const validateForm = (): FormErrors => {
    ...
    const customerError = requiredSelectionError(customerId, 'مشتری');
    ...
    lineItems.forEach((li, idx) => {
      ...
      const itemError = requiredSelectionError(li.itemId, 'شرح کالا/خدمت');
      const quantityError = positiveNumberError(li.quantity, 'تعداد', { integer: true });
      if (itemError) {
        errors[idKey] = itemError;
        errorLabels[idKey] = `قلم ${idx + 1} — شرح`;
      }
      if (quantityError) {
        errors[qKey] = quantityError;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 text-right" dir="rtl">
      ...
      <form noValidate ref={formRef} onKeyDownCapture={onKeyDownCapture} onSubmit={handleSubmit} className="space-y-4" data-ergonomic-form="true">
        <FormErrorSummary
          errors={formErrors}
          labels={errorLabels}
          fieldIdMap={{
            ...
            {lineItems.map((item, idx) => {
              ...
              return (
                <div key={idx} className="app-card-muted p-3">
                  <FormGrid columns={4} align="start">
                    <FormGridItem span={2}>
                    ...
                    <FormGridItem className="flex justify-end">
                      <Button
                        type="button"
