import { Table, ManagementDirectoryPagination, AppSearchField, DataTableShell, FinancialTimeline, FinancialTimelineEvent, FilterChipGroup, IconGlyph, SelectField, TableActionGroup, CheckboxField } from '@/components/ui';
...
const PartnerLedgerWorkspaceSection: React.FC<Props> = ({ ctx }) => {
  ...
  return (
    <>
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950 sm:p-5" aria-labelledby="partner-ledger-title" data-ui-people-detail-ledger="partner-standard-v282">
        <PeopleDetailSectionHeading
          id="partner-ledger-title"
          iconClass="fa-solid fa-book-open"
          title="دفتر حساب همکار"
          subtitle="مرور مانده، پرداخت‌ها و سوابق مالی همکار."
          className="mb-4 scroll-mt-24 border-b border-slate-200 pb-4 dark:border-slate-800"
          actions={<Button type="button" onClick={openLedgerModal} variant="primary" size="sm" leftIcon={<i className="fa-solid fa-money-bill-transfer" />}>ثبت دریافت / پرداخت</Button>}
        />
        ...
        <div className="mb-4 rounded-2xl border border-slate-200 p-3 dark:border-slate-800" data-ui-partner-ledger-toolbar="true">
          <div className="flex flex-wrap items-center justify-between gap-2" dir="rtl">
            <div className="flex min-w-0 items-center gap-2">
              <IconGlyph tone="neutral" size="sm" aria-hidden="true"><i className="fa-solid fa-file-invoice-dollar" /></IconGlyph>
              <div className="min-w-0 text-right">
                <div className="text-[11px] font-black text-slate-500 dark:text-slate-400">مدیریت تراکنش‌های مالی</div>
                <div className="mt-0.5 text-[13px] font-black text-slate-900 dark:text-slate-50">جستجو، فیلترها و نوع نمایش</div>
              </div>
            </div>
            <span className="inline-flex min-h-8 shrink-0 items-center gap-1.5 rounded-xl border border-slate-200 px-2.5 text-[11px] font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300">
              <i className="fa-solid fa-filter-circle-dollar" aria-hidden="true" />
              {filteredLedgerEntries.length.toLocaleString('fa-IR')} رکورد
            </span>
          </div>

          <div className="mt-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-[minmax(0,1fr)_minmax(15rem,0.7fr)_auto] md:items-center" dir="rtl">
            <AppSearchField
              value={ledgerSearch}
              onChange={setLedgerSearch}
              placeholder="جستجو در شرح، مبلغ، تاریخ، شناسه یا مرجع…"
              ariaLabel="جستجو در دفتر حساب همکار"
              size="sm"
              clearable
            />
            ...
            <div className="relative justify-self-start md:justify-self-end">
              <Button
                ...
                variant={isLedgerColumnPickerOpen ? 'primary' : 'secondary'}
                size="xs"
                leftIcon={<i className="fa-solid fa-table-columns" />}
              >
                ستون‌ها
              </Button>
              {isLedgerColumnPickerOpen ? (
                <div ref={ledgerColumnPickerPanelRef} className="absolute start-0 top-full z-20 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-lg dark:border-slate-700 dark:bg-slate-950">
                    ...
                    {[
                      ...
                      return (
                        <label key={item.key} className="flex cursor-pointer items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 transition hover:bg-white dark:border-slate-800 dark:bg-slate-900/70 dark:hover:bg-slate-900">
                          <CheckboxField controlOnly size="sm"
                            type="checkbox"
                            checked={checked}
                  ...
                  <Button type="button" onClick={() => setIsLedgerColumnPickerOpen(false)} variant="secondary" size="xs" className="mt-3">بستن</Button>
                </div>
              ) : null}
            </div>
          </div>

          <div className="mt-2 flex min-w-0 flex-wrap items-center gap-2 border-t border-slate-200 pt-2 dark:border-slate-800" dir="rtl">
            <FilterChipGroup
              label="بازه"
              ariaLabel="بازه زمانی دفتر حساب"
              value={ledgerRange}
              onValueChange={(value) => setLedgerRange(value as any)}
              options={[
            ...
            <FilterChipGroup
              label="نوع"
              ariaLabel="نوع تراکنش‌های دفتر حساب"
              value={ledgerViewFilter}
              onValueChange={(value) => setLedgerViewFilter(value as any)}
              options={[
            ...
            <FilterChipGroup
              label="نما"
              ariaLabel="نوع نمایش دفتر حساب"
              value={ledgerDisplayMode}
              onValueChange={(value) => setLedgerDisplayMode(value as 'table' | 'timeline')}
              showIcons
              options={[
                { value: 'table', label: 'جدول', icon: <i className="fa-solid fa-table-cells-large" aria-hidden="true" /> },
                { value: 'timeline', label: 'تایم‌لاین', icon: <i className="fa-solid fa-timeline" aria-hidden="true" /> },
              ] as const}
            />
          </div>
          ...
          {activeLedgerBatchId ? (
            <div className="mt-2 flex flex-wrap items-center gap-1.5" dir="rtl">
              <Button type="button" onClick={handlePrintActiveBatch} variant="secondary" size="xs" leftIcon={<i className="fa-solid fa-print" />}>چاپ</Button>
              <Button type="button" onClick={handleExportActiveBatchCsv} variant="secondary" size="xs" leftIcon={<i className="fa-solid fa-file-excel" />}>Excel</Button>
              <Button type="button" onClick={() => setActiveLedgerBatchId('')} variant="ghost" size="xs" leftIcon={<i className="fa-solid fa-xmark" />}>حذف فیلتر</Button>
            </div>
          ) : null}
        </div>
        ...
        {filteredLedgerEntries.length === 0 ? (
          ...
          <div>
            ...
            <DataTableShell className="overflow-hidden rounded-2xl" data-ui-partner-ledger-table="true" aria-label="جدول دفتر حساب همکار">
              <Table layout="managed" density="comfortable" className="w-full min-w-max table-auto divide-y divide-slate-200 text-xs dark:divide-slate-800">
  ...
  <bdi className="mt-1 block w-full text-right font-mono text-sm font-semibold text-violet-700 dark:text-violet-300" dir="ltr">{systemId}</bdi>
        ...
        {Number(ledgerDirectory?.summary?.total || 0) > 0 ? (
          <ManagementDirectoryPagination
            page={Math.max(1, Number(ledgerDirectory?.page || ledgerPage || 1))}
            totalPages={Math.max(1, Number(ledgerDirectory?.totalPages || 1))}
            pageSize={Math.max(1, Number(ledgerDirectory?.pageSize || ledgerPageSize || 25))}
            pageSizeOptions={[25, 50, 100] as const}
            total={Math.max(0, Number(ledgerDirectory?.total || 0))}
            pageStart={Number(ledgerDirectory?.total || 0) ? (((Number(ledgerDirectory?.page || 1) - 1) * Number(ledgerDirectory?.pageSize || ledgerPageSize || 25)) + 1) : 0}
            pageEnd={Math.min(Number(ledgerDirectory?.total || 0), Number(ledgerDirectory?.page || 1) * Number(ledgerDirectory?.pageSize || ledgerPageSize || 25))}
            ariaLabel="صفحه‌بندی تراکنش‌های همکار"
            pageSizeAriaLabel="تعداد تراکنش دفتر همکار در هر صفحه"
            onPageChange={(page) => setLedgerPage(page)}
            onPageSizeChange={(pageSize) => setLedgerPageSize(String(pageSize) as '25' | '50' | '100')}
          />
        ) : null}
