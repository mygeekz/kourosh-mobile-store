...
export const buildSoldPhoneDailyPriceRows = (purchaseHistory: any[]) => {
  const rows = purchaseHistory
    ...
    .map((item: any) => {
      ...
      const installmentManualReceiptPaid = num(item?.installmentSaleManualReceiptAmount || 0);
      const reconstructedInstallmentCollected = Math.max(0, installmentDownPayment + installmentTransactionPaid + installmentCheckPaid + installmentManualReceiptPaid);
      const installmentCollectedAmount = Math.max(reconstructedInstallmentCollected, num(item?.installmentCollectedAmount || 0));
      ...
      return {
        ...
        installmentManualReceiptAmount: installmentManualReceiptPaid,
...
export const buildPartnerBusinessReadModel = ({
  ...
  const shareValue = purchaseItemCount > 0
    ...
    : buildMissingValue('کل سهم شریک', 'هیچ کالای مرتبط با این همکار برای محاسبه سهم موجود نیست.');
  ...
  if (purchaseItemCount === 0) warnings.push('برای این همکار کالای مرتبط ثبت نشده یا اطلاعات قابل نمایش وجود ندارد.');
  ...
  if (purchaseItemCount === 0 && ledgerItemCount === 0) {
    ...
    reasons.push(missingSourceItems > 0
      ...
      : 'مانده قابل بررسی است و ثبت نهایی تسویه با تایید مدیر انجام می‌شود.');
  ...
  const auditTrailChecklist: PartnerSettlementReviewStep[] = [
    ...
    },
    {
      ...
      status: 'ready',
      detail: 'این پیش‌نمایش هیچ فایل یا داده حساس را ذخیره نمی‌کند.',
    },
    {
  ...
  const atomicSubmitContract: PartnerSettlementAtomicSubmitContract = {
    ...
    mutationLocks: [
      ...
      'ثبت نهایی فقط پس از تأیید کامل شرایط تسویه مجاز است.',
    ],
    blockingReasons: atomicSubmitBlockingReasons,
    nextSafeStepLabel: 'ثبت ایمن تسویه و جلوگیری از ثبت تکراری',
  ...
  const atomicSubmitDryRunHarness: PartnerSettlementAtomicSubmitDryRunHarness = {
    ...
    noPartialMutationChecks: [
      ...
      'ثبت تسویه باید کامل انجام شود و از ثبت ناقص یا نیمه‌کاره جلوگیری شود.',
    ],
  ...
  return {
    kpis: [
      ...
      buildKnownValue('سرمایه فروش قابل بررسی', soldPhonesCurrentPurchaseAmount, 'مبنای سرمایه فروش از اطلاعات موجود قابل محاسبه نبود.'),
    ],
    relatedProducts: {
