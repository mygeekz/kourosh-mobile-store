...
import { BidiText, FormGrid, ModalField, SelectField, TextareaField, TextField } from '@/components/ui';
...
const PartnerEditProfileModal: React.FC<Props> = ({ ctx }) => {
  ...
  return (
    <Modal
      ...
      variant="operational"
    >
      <form noValidate onSubmit={handleEditSubmit} className="space-y-4" dir="rtl">
        <FormErrorSummary
        ...
        <div className="grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
                ...
                <div key={item.label} className="flex min-w-0 items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 break-words text-sm font-semibold text-slate-900 dark:text-white">
                      {item.ltr ? <BidiText kind="phone">{item.value}</BidiText> : item.value}
                    </p>
                  </div>
                  <span
          ...
          <FormGrid columns={2} aria-label="اطلاعات همکار">
            <ModalField
              label="نام همکار"
            ...
            <ModalField label="ایمیل" iconClass="fa-regular fa-envelope" error={editFormErrors.email as string | undefined} className="md:col-span-2">
              <TextField
                id="editEmail"
                name="email"
                type="email"
            ...
            <ModalField label="آدرس" iconClass="fa-solid fa-location-dot" error={editFormErrors.address as string | undefined} className="md:col-span-2">
              <TextareaField
                id="editAddress"
                name="address"
                rows={3}
            ...
            <ModalField label="یادداشت داخلی" iconClass="fa-regular fa-note-sticky" error={editFormErrors.notes as string | undefined} className="md:col-span-2">
              <TextareaField
                id="editNotes"
                name="notes"
                rows={3}
