...
const PartnerSettlementReconciliationExportSignoffPackSection: React.FC<Props> = ({ ctx }) => {
  ...
  const signoffPack = {
    ...
    managerSignoffChecklist: [
      { key: 'ledger-trace-matched', ok: expectedLedgerMatched, label: 'ردیف‌های دفتر با نتیجه ثبت‌شده تطبیق شده‌اند.' },
    ...
    <section className="partner-detail-section-shell partner-settlement-reconciliation-export-signoff mx-6 mt-5 rounded-[30px] border border-indigo-200/80 bg-indigo-50/55 px-5 py-6 shadow-[0_16px_45px_rgba(79,70,229,0.08)] dark:border-indigo-900/50 dark:bg-indigo-950/20 sm:px-6" data-partner-settlement-reconciliation-export-signoff="true">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-white px-3 py-1.5 text-[11px] font-bold text-indigo-700 shadow-sm dark:border-indigo-900/50 dark:bg-slate-950 dark:text-indigo-200">
            ...
            گزارش تطبیق و تأیید مدیر
          </div>
          <h3 className="mt-3 text-xl font-black tracking-tight text-slate-950 dark:text-slate-50 sm:text-2xl">گزارش نهایی تسویه همکار</h3>
          <p className="mt-2 text-sm font-medium leading-7 text-slate-600 dark:text-slate-300">
            این بخش نتیجه ثبت تسویه، سوابق دفتر و وضعیت تطبیق را برای بررسی نهایی مدیر در یک گزارش یکپارچه نمایش می‌دهد.
          </p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-4">
        <article className="rounded-[22px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="text-[13px] font-black text-slate-800 dark:text-slate-100">مبلغ تسویه</div>
          ...
          <p className="mt-2 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">از ردیف‌های مرتبط دفتر همکار محاسبه شده است.</p>
        </article>
        <article className="rounded-[22px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="text-[13px] font-black text-slate-800 dark:text-slate-100">ردیف‌های مرتبط</div>
          ...
          <p className="mt-2 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">شناسه ثبت، بسته پردازش و کد صحت بررسی می‌شوند.</p>
        </article>
        <article className="rounded-[22px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="text-[13px] font-black text-slate-800 dark:text-slate-100">مانده قابل بررسی</div>
          ...
          <p className="mt-2 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">برای تأیید نهایی باید مانده و ردیف باز وجود نداشته باشد.</p>
        </article>
        <article className="rounded-[22px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="text-[13px] font-black text-slate-800 dark:text-slate-100">ردیف باز تکراری</div>
          ...
          <p className="mt-2 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">ردیف تسویه‌شده نباید دوباره در فهرست باز قرار گیرد.</p>
        </article>
      </div>
        ...
        <div className="rounded-[24px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">این چک‌لیست وضعیت فعلی تطبیق تسویه را برای تأیید مدیر نمایش می‌دهد.</p>
            </div>
            <Button type="button" onClick={focusSignoffBatch} disabled={!idempotencyKey} variant="secondary" size="sm" data-partner-settlement-signoff-ledger-focus="true" leftIcon={<i className="fa-solid fa-arrow-down-long" />}>نمایش گروه ثبت در دفتر</Button>
          </div>
        ...
        <div className="rounded-[24px] border border-indigo-200 bg-white p-4 dark:border-indigo-900/40 dark:bg-slate-950/70">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold leading-6 text-slate-500 dark:text-slate-400">فایل خروجی شامل نتیجه ثبت، گردش دفتر و محدوده تغییرات بررسی‌شده است.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button type="button" onClick={copySignoffPack} variant="secondary" size="sm" data-partner-settlement-signoff-copy="true" leftIcon={<i className="fa-regular fa-copy" />}>کپی گزارش</Button>
              <Button type="button" onClick={downloadSignoffPack} variant="secondary" size="sm" data-partner-settlement-signoff-download="true" leftIcon={<i className="fa-solid fa-download" />}>دانلود فایل</Button>
            </div>
          </div>
          <div className="mt-3 text-xs font-bold leading-6 text-slate-500 dark:text-slate-400" data-partner-settlement-signoff-export-state={exportState}>
            {exportState === 'copied' && 'گزارش برای مرور مدیر کپی شد.'}
            {exportState === 'downloaded' && 'فایل گزارش آماده شد.'}
            {exportState === 'failed' && 'ساخت فایل خروجی انجام نشد؛ دوباره تلاش کنید یا گزارش را کپی کنید.'}
            {exportState === 'idle' && 'فایل خروجی آماده دریافت است و تغییری در اطلاعات ثبت‌شده فروشگاه ایجاد نمی‌کند.'}
          </div>
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs font-bold leading-7 text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300" data-partner-settlement-signoff-pack-preview="true">فایل بایگانی شامل شناسه تسویه، نتیجه تطبیق، سوابق مرتبط دفتر و چک‌لیست تأیید مدیر است.</div>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 text-xs font-semibold leading-6 text-slate-600 dark:text-slate-300 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/70"><b className="text-slate-900 dark:text-slate-50">شناسه تسویه: </b><span className="break-all">{settlementId || '—'}</span></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/70"><b className="text-slate-900 dark:text-slate-50">کد ثبت یکتا: </b><span className="break-all">{idempotencyKey || '—'}</span></div>
