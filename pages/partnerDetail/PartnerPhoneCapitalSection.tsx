import { Table, AppSearchField, DataTableShell, IconGlyph, ManagementDirectoryPagination, SelectField } from '@/components/ui';
...
const PartnerPhoneCapitalSection: React.FC<Props> = ({ ctx }) => {
  ...
  const renderResponsiveCapitalCard = (row: any) => {
    const delta = Number(row.dailyPriceDelta || 0);
    const rowSourceLabel = String(row.settlementPriceSourceLabel || row.saleReferenceLabel || 'ثبت مستقیم گوشی');
    const settlementStatus = getPartnerCapitalMeta(row);
    const saleClosureStatus = getSaleClosureMeta(row);
    const rowBalance = Number(row.phoneSettlementBalance || 0);
    const paymentCount = Number(row.phoneSettlementPaymentCount || 0);
    const isTimelineOpen = expandedPhoneSettlementTimelineId === Number(row.id);
    ...
    return (
      <article
        key={`partner-capital-card-${row.id}`}
        className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/70"
        data-ui-partner-capital-card="true"
      >
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 p-3 dark:border-slate-800">
          <div className="flex min-w-0 items-start gap-2.5 text-right">
            <IconGlyph tone="neutral" size="sm" className="mt-0.5 shrink-0" aria-hidden="true"><i className="fa-solid fa-mobile-screen-button" /></IconGlyph>
            <div className="min-w-0">
              <div className="break-words text-sm font-black leading-6 text-slate-950 dark:text-slate-50">{row.name || 'گوشی فروخته‌شده'}</div>
              <bdi className="mt-1 block max-w-full overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[11px] text-slate-500 dark:text-slate-400" dir="ltr">{row.identifier || 'IMEI ثبت نشده'}</bdi>
            </div>
          </div>
          <span className="shrink-0 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-black text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">{row.status || 'فروخته‌شده'}</span>
        </div>

        <div className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-2">
          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-right dark:border-slate-800 dark:bg-slate-900/55">
            <div className="text-[11px] font-black text-slate-500 dark:text-slate-400">قیمت خرید روز / اولیه</div>
            <div className="mt-1 whitespace-nowrap text-sm font-black tabular-nums text-slate-950 dark:text-slate-50">{formatPrice(row.settlementPurchasePrice)}</div>
            <div className="mt-1 whitespace-nowrap text-[11px] font-semibold tabular-nums text-slate-500 dark:text-slate-400">اولیه: {formatPrice(row.initialPurchasePrice)}</div>
            <div className="mt-1 text-[11px] font-semibold leading-5 text-slate-500 dark:text-slate-400">آخرین تغییر: {row.currentPurchasePriceUpdatedAt ? formatIsoToShamsi(row.currentPurchasePriceUpdatedAt) : 'ثبت نشده'}</div>
            <div className={`mt-1 whitespace-nowrap text-[11px] font-black tabular-nums ${delta >= 0 ? 'text-amber-700 dark:text-amber-300' : 'text-emerald-700 dark:text-emerald-300'}`}>
              {delta >= 0 ? '+' : '-'}{formatCurrencyText(Math.abs(delta), readStoredCurrencyUnit())}
            </div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-right dark:border-slate-800 dark:bg-slate-900/55">
            <div className={`flex items-start gap-2 text-sm font-black leading-6 ${capitalToneClass}`}>
            ...
            <div className="mt-2 whitespace-nowrap text-xs font-black tabular-nums text-emerald-700 dark:text-emerald-300">برگشتی: {formatCurrencyText(Number(row.phoneSettlementPaidAmount || 0), readStoredCurrencyUnit())}</div>
            <div className={`mt-1 whitespace-nowrap text-xs font-black tabular-nums ${rowBalance > 0 ? 'text-rose-700 dark:text-rose-300' : 'text-slate-500 dark:text-slate-400'}`}>مانده: {formatCurrencyText(rowBalance, readStoredCurrencyUnit())}</div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-right dark:border-slate-800 dark:bg-slate-900/55">
            <div className={`flex items-start gap-2 text-sm font-black leading-6 ${saleToneClass}`}>
            ...
            {saleClosureStatus.isInstallment && !saleClosureStatus.isClosed ? (
              <div className="mt-2 text-xs font-black leading-5 text-amber-700 dark:text-amber-300">مانده اقساط مشتری: <span className="whitespace-nowrap tabular-nums">{formatCurrencyText(saleClosureStatus.remainingAmount, readStoredCurrencyUnit())}</span></div>
            ) : (
              <div className="mt-2 text-xs font-semibold leading-5 text-slate-500 dark:text-slate-400">پرونده فروش مشتری در وضعیت نهایی قرار دارد.</div>
            )}
          </section>

          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-right dark:border-slate-800 dark:bg-slate-900/55">
            <div className="text-[11px] font-black text-slate-500 dark:text-slate-400">تاریخ و منبع فروش</div>
            <bdi className="mt-1 block whitespace-nowrap text-right text-xs font-black tabular-nums text-slate-800 dark:text-slate-100" dir="ltr">{row.soldAt ? formatIsoToShamsi(row.soldAt) : '—'}</bdi>
            <div className="mt-2 min-w-0">{renderPhoneSaleSourceLink(row, rowSourceLabel, true)}</div>
            {row.phoneSettlementLastPaymentDate ? (
              <div className="mt-2 border-t border-slate-200 pt-2 text-[11px] font-semibold text-slate-500 dark:border-slate-700 dark:text-slate-400">
                آخرین پرداخت: <bdi className="whitespace-nowrap tabular-nums" dir="ltr">{formatIsoToShamsi(row.phoneSettlementLastPaymentDate)}</bdi>
              </div>
            ) : null}
          </section>
        </div>

        <div className="flex items-center justify-end border-t border-slate-100 px-3 py-2.5 dark:border-slate-800">
          <Button type="button" onClick={() => togglePhoneSettlementTimeline(row)} variant="secondary" size="sm">
            {isTimelineOpen ? 'بستن جزئیات' : paymentCount > 0 ? `تاریخچه تسویه (${paymentCount.toLocaleString('fa-IR')})` : 'جزئیات تسویه'}
          </Button>
        </div>

        {isTimelineOpen ? (
          <div className="border-t border-slate-100 bg-slate-50/35 p-2.5 text-right dark:border-slate-800 dark:bg-slate-950/20" dir="rtl">{renderPhoneSettlementTimeline(row, true)}</div>
        ) : null}
      </article>
    );
  };
            ...
            <div className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
              ...
              {soldPhoneSettlementFilterCounts.all === 0 ? (
                ...
                <>
                  ...
                  <div className="grid gap-3 px-3 pb-3 xl:hidden" data-ui-partner-capital-responsive-cards="true">
                    {filteredSoldPhoneDailyPriceRows.map(renderResponsiveCapitalCard)}
                  </div>

                  <div className="hidden xl:block" data-ui-partner-capital-desktop-table="true">
                  <DataTableShell className="rounded-xl" data-ui-capital-table="true" aria-label="جزئیات سرمایه و وضعیت فروش گوشی‌های همکار">
                    <Table layout="auto" density="comfortable" className="w-full min-w-[64rem] table-auto text-right text-xs" dir="rtl">
                      ...
                      <thead className="sticky top-0 z-10 bg-slate-50 text-xs font-black text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                        <tr>
                          ...
                          <th scope="col" className="w-40 px-2.5 py-2.5">تاریخ و منبع</th>
                        ...
                        {filteredSoldPhoneDailyPriceRows.map((item: any) => {
                          ...
                          return (
                            ...
                            <tr className="bg-white align-middle transition hover:bg-slate-50 dark:bg-slate-950/45 dark:hover:bg-slate-900/60">
                              ...
                              <td className="w-40 px-2.5 py-2.5 align-top" data-ui-partner-date-source="true">
                                <div className="flex min-w-0 flex-col items-stretch gap-1.5">
                                  <bdi className="block whitespace-nowrap text-right font-bold leading-5 text-slate-700 dark:text-slate-200" dir="ltr">
                                  ...
                                  {item.phoneSettlementLastPaymentDate && (
                                    <div className="border-t border-slate-100 pt-1 text-[11px] font-semibold leading-5 text-slate-500 dark:border-slate-800 dark:text-slate-400">
                                      <span>آخرین پرداخت: </span>
                                      <bdi className="whitespace-nowrap" dir="ltr">{formatIsoToShamsi(item.phoneSettlementLastPaymentDate)}</bdi>
                                    </div>
                                  )}
                                </div>
                              </td>
                              <td className="px-2 py-2.5 align-top text-end">
                                <div className="flex items-center justify-end">
                                  <Button type="button" onClick={() => togglePhoneSettlementTimeline(item)} variant="secondary" size="xs" className="whitespace-nowrap">
                            ...
                            {isTimelineOpen && (
                              <tr className="bg-slate-50 dark:bg-slate-950/30">
                                <td colSpan={6} className="px-3 pb-3 pt-1 text-right" dir="rtl">
                                  {renderPhoneSettlementTimeline(item, true)}
                                </td>
                              </tr>
                            )}
                            </React.Fragment>
                  ...
                  <ManagementDirectoryPagination
                    page={Math.max(1, Number(phoneSettlementPage || 1))}
                    totalPages={Math.max(1, Number(phoneSettlementTotalPages || 1))}
                    pageSize={Math.max(1, Number(phoneSettlementPageSize || 25))}
                    pageSizeOptions={[25, 50, 100] as const}
                    total={Math.max(0, Number(phoneSettlementTotal || 0))}
                    pageStart={Number(phoneSettlementTotal || 0) ? (((Math.max(1, Number(phoneSettlementPage || 1)) - 1) * Math.max(1, Number(phoneSettlementPageSize || 25))) + 1) : 0}
                    pageEnd={Math.min(Number(phoneSettlementTotal || 0), Math.max(1, Number(phoneSettlementPage || 1)) * Math.max(1, Number(phoneSettlementPageSize || 25)))}
                    ariaLabel="صفحه‌بندی جزئیات سرمایه و وضعیت فروش گوشی‌های همکار"
                    pageSizeAriaLabel="تعداد گوشی در هر صفحه"
                    onPageChange={(page) => setPhoneSettlementPage(page)}
                    onPageSizeChange={(pageSize) => setPhoneSettlementPageSize(String(pageSize) as '25' | '50' | '100')}
                  />
                </>
              )}
            </div>
