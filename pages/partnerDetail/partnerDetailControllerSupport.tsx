...
export const getSaleClosureMeta = (item: any): SaleClosureMeta => {
  ...
  const explicitRemainingRaw = item?.installmentCustomerRemainingAmount;
  const hasExplicitRemaining = explicitRemainingRaw !== undefined && explicitRemainingRaw !== null && Number.isFinite(Number(explicitRemainingRaw));
  const remainingAmount = hasExplicitRemaining
    ? Math.max(0, Number(explicitRemainingRaw))
    : Math.max(0, actualTotal - collected);
