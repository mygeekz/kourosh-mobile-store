...
import { FormGrid, SelectField, TextareaField, TextField, emailAddressError, phoneNumberError, requiredSelectionError, requiredTextError } from '@/components/ui';
...
const PartnersPage: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const partnerNameError = requiredTextError(newPartner.partnerName, 'نام همکار');
    const partnerTypeError = requiredSelectionError(newPartner.partnerType, 'نوع همکار');
    const phoneError = phoneNumberError(newPartner.phoneNumber, { optional: true, label: 'شماره تماس' });
    const emailError = emailAddressError(newPartner.email, { optional: true, label: 'ایمیل' });
  ...
  return (
    ...
    >
      ...
      {isAddModalOpen && (
        ...
        >
          <form noValidate onSubmit={handleAddPartnerSubmit} className="modal-template-form modal-template-form--split modal-template-form--partner" data-ui-partner-modal="canonical-split">
            ...
            <div className="modal-template-main">
              <FormGrid columns={2} className="modal-template-section modal-template-section--grid">
                <ModalField label="نام همکار" iconClass="fa-solid fa-building-user" required error={formErrors.partnerName}>
                  <TextField type="text" id="partnerName" name="partnerName" value={newPartner.partnerName} onChange={handleInputChange} required placeholder="مثلاً: تأمین‌کننده کوروش" />
                </ModalField>
                <ModalField label="نوع همکار" iconClass="fa-solid fa-diagram-project" required error={formErrors.partnerType}>
                  <SelectField id="partnerType" name="partnerType" value={newPartner.partnerType} onChange={handleInputChange} required>
                ...
                <ModalField label="فرد رابط" iconClass="fa-solid fa-user-tie">
                  <TextField type="text" id="contactPerson" name="contactPerson" value={newPartner.contactPerson} onChange={handleInputChange} placeholder="نام مسئول هماهنگی یا فروش" />
                </ModalField>
                <ModalField label="شماره تماس" iconClass="fa-solid fa-phone" error={formErrors.phoneNumber}>
                  <TextField type="tel" id="phoneNumber" name="phoneNumber" value={newPartner.phoneNumber} onChange={handleInputChange} valueKind="phone" placeholder="مثال: 09123456789" />
                </ModalField>
              </FormGrid>
              <FormGrid columns={1} className="modal-template-section modal-template-section--stack">
                <ModalField label="ایمیل" iconClass="fa-solid fa-envelope" error={formErrors.email}>
                  <TextField type="email" id="email" name="email" value={newPartner.email} onChange={handleInputChange} valueKind="email" placeholder="example@domain.com" />
                </ModalField>
                <ModalField label="آدرس" iconClass="fa-solid fa-location-dot">
                  <TextareaField controlOnly id="address" name="address" value={newPartner.address} onChange={handleInputChange} rows={2} placeholder="آدرس یا موقعیت همکاری" />
                </ModalField>
                <ModalField label="یادداشت داخلی" iconClass="fa-solid fa-note-sticky">
                  <TextareaField controlOnly id="notes" name="notes" value={newPartner.notes} onChange={handleInputChange} rows={3} placeholder="شرایط همکاری، توضیحات تسویه یا نکات مدیریتی" />
                </ModalField>
              </FormGrid>
