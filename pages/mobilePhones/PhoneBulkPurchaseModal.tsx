...
import { focusErrorsSoon, normalizeNumericInput, toSafeNumber } from '../../utils/formBehavior';
...
const PhoneBulkPurchaseModal: React.FC<Props> = ({
  ...
  const validationFieldIds = React.useMemo<Record<string, string>>(() => {
    const ids: Record<string, string> = {
      supplierId: 'bulk-phone-purchase-supplier',
      purchaseDate: 'bulk-phone-purchase-date',
    };
    rows.forEach((row, index) => {
      ids[`row.${row.id}.model`] = `bulk-phone-${row.id}-model`;
      ids[`row.${row.id}.color`] = `bulk-phone-${row.id}-color`;
      ids[`row.${row.id}.storage`] = `bulk-phone-${row.id}-storage`;
      ids[`row.${row.id}.ram`] = `bulk-phone-${row.id}-ram`;
      ids[`row.${row.id}.imei`] = `bulk-phone-${row.id}-imei`;
      ids[`row.${row.id}.purchasePrice`] = `bulk-phone-${row.id}-purchase-price`;
    });
    return ids;
  }, [rows]);

  const validationFieldLabels = React.useMemo<Record<string, string>>(() => {
    const labels: Record<string, string> = {
      supplierId: 'تأمین‌کننده فاکتور خرید',
      purchaseDate: 'تاریخ خرید مشترک',
    };
    rows.forEach((row, index) => {
      const rowLabel = `ردیف ${(index + 1).toLocaleString('fa-IR')}`;
      labels[`row.${row.id}.model`] = `${rowLabel} — مدل`;
      labels[`row.${row.id}.color`] = `${rowLabel} — رنگ`;
      labels[`row.${row.id}.storage`] = `${rowLabel} — حافظه`;
      labels[`row.${row.id}.ram`] = `${rowLabel} — رم`;
      labels[`row.${row.id}.imei`] = `${rowLabel} — IMEI`;
      labels[`row.${row.id}.purchasePrice`] = `${rowLabel} — قیمت خرید`;
    });
    return labels;
  }, [rows]);

  const summaryErrors = React.useMemo<FormErrors>(() => {
    const errors: Record<string, string | undefined> = {
      supplierId: headerErrors.supplierId,
      purchaseDate: headerErrors.purchaseDate,
    };
    rows.forEach((row) => {
      const rowError = rowErrors[row.id] || {};
      (['model', 'color', 'storage', 'ram', 'imei', 'purchasePrice'] as const).forEach((field) => {
        errors[`row.${row.id}.${field}`] = rowError[field];
      });
    });
    return compactValidationErrors(errors);
  }, [headerErrors, rowErrors, rows]);
  ...
  const validate = () => {
    const nextHeaderErrors: typeof headerErrors = compactValidationErrors({
      supplierId: requiredSelectionError(supplierId, 'تأمین‌کننده فاکتور خرید'),
      purchaseDate: purchaseDate ? undefined : requiredTextError('', 'تاریخ خرید مشترک'),
    });
    ...
    rows.forEach((row) => {
      ...
      const errors = compactValidationErrors({
        model: requiredTextError(row.model, 'مدل'),
        color: requiredTextError(row.color, 'رنگ'),
        storage: requiredSelectionError(row.storage, 'حافظه'),
        ram: requiredSelectionError(row.ram, 'رم'),
        imei: /^\d{15,16}$/.test(imei) ? undefined : 'IMEI باید ۱۵ یا ۱۶ رقم باشد.',
        purchasePrice: nonNegativeNumberError(row.purchasePrice, 'قیمت خرید'),
      }) as BulkPurchaseRowErrors;
    ...
    const focusErrors: FormErrors = compactValidationErrors({
      ...nextHeaderErrors,
      ...Object.fromEntries(
        rows.flatMap((row) => Object.entries(nextRowErrors[row.id] || {}).map(([field, message]) => [
          `row.${row.id}.${field}`,
          message,
        ])),
      ),
    });
    focusErrorsSoon(focusErrors, validationFieldIds);
    return Object.keys(focusErrors).length === 0;
  };
  ...
  const submit = async () => {
    ...
    if (!purchaseDateIso) {
      const nextErrors = { ...headerErrors, purchaseDate: 'تاریخ خرید معتبر نیست.' };
      setHeaderErrors(nextErrors);
      focusErrorsSoon(compactValidationErrors(nextErrors), validationFieldIds);
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        if (Array.isArray(result.errors)) {
          ...
          const serverFieldErrors = compactValidationErrors(Object.fromEntries(
            rows.flatMap((row) => Object.entries(mappedErrors[row.id] || {}).map(([field, message]) => [
              `row.${row.id}.${field}`,
              message,
            ])),
          ));
          focusErrorsSoon(serverFieldErrors, validationFieldIds);
        }
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50/50 p-3 dark:bg-slate-950/60 sm:p-4 lg:p-5">
          <div className="space-y-4">
            <section className="rounded-[20px] border border-slate-200 bg-white p-3.5 shadow-[0_14px_34px_-30px_rgba(15,23,42,0.28)] dark:border-slate-800 dark:bg-slate-950 sm:p-4">
              ...
              <FormErrorSummary errors={summaryErrors} labels={validationFieldLabels} fieldIdMap={validationFieldIds} className="mb-4" />
              ...
              <FormGrid columns={3}>
                <div>
                  <SearchableSelectField<string>
                    ...
                    inputId="bulk-phone-purchase-supplier"
                    label="تأمین‌کننده"
                    required
                ...
                <div>
                  <div className={labelClass}>تاریخ خرید <span className="text-danger">*</span></div>
                  <ShamsiDatePicker
                    ...
                    error={headerErrors.purchaseDate}
                    preview="انتخاب تاریخ خرید"
                    size="compact"
                  />
                </div>

                <FormGridItem span="full" className="xl:col-span-1">
                <div className="grid grid-cols-3 overflow-hidden rounded-[16px] border border-slate-200 bg-slate-50/70 dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <section data-ui-bulk-purchase-invoice-grid="true" className="overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_14px_34px_-30px_rgba(15,23,42,0.28)] dark:border-slate-800 dark:bg-slate-950">
              <div className="flex flex-col gap-2.5 border-b border-slate-200 px-3.5 py-3 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between sm:px-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    ...
                    className="min-h-[40px] rounded-[12px] px-3 text-[11px] font-black"
                  >
                    افزودن ردیف
                  </Button>
                </div>
              ...
              <div className="hidden border-b border-slate-200 bg-slate-50/90 px-3 py-2 text-[10px] font-black text-slate-600 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-300 xl:block">
                <div className={`grid items-center gap-2 ${desktopRowGridClass}`}>
                  ...
                  <div>مدل <span className="text-danger">*</span></div>
                  <div>رنگ <span className="text-danger">*</span></div>
                  <div>حافظه <span className="text-danger">*</span></div>
                  <div>رم <span className="text-danger">*</span></div>
                  <div>IMEI <span className="text-danger">*</span></div>
                  <div>قیمت خرید (تومان) <span className="text-danger">*</span></div>
              ...
              <div className="max-h-[48vh] space-y-2.5 overflow-y-auto p-2.5 sm:p-3 xl:space-y-0 xl:p-0">
                {rows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>مدل <span className="text-danger">*</span></label>
                        <AddableAutocomplete
                          value={row.model}
                          onChange={(value) => updateRow(row.id, 'model', value)}
                          options={phoneModels}
                          onAdd={addPhoneModel}
                          inputId={`bulk-phone-${row.id}-model`}
                          required
                          preview="انتخاب مدل"
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>رنگ <span className="text-danger">*</span></label>
                        <AddableAutocomplete
                          value={row.color}
                          onChange={(value) => updateRow(row.id, 'color', value)}
                          options={phoneColors}
                          onAdd={addPhoneColor}
                          inputId={`bulk-phone-${row.id}-color`}
                          required
                          preview="انتخاب رنگ"
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>حافظه <span className="text-danger">*</span></label>
                        <SelectField
                          id={`bulk-phone-${row.id}-storage`}
                          required
                          value={row.storage}
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>رم <span className="text-danger">*</span></label>
                        <SelectField
                          id={`bulk-phone-${row.id}-ram`}
                          required
                          value={row.ram}
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>IMEI <span className="text-danger">*</span></label>
                        <TextField
                          id={`bulk-phone-${row.id}-imei`}
                          required
                          value={row.imei}
                      ...
                      <div>
                        <label className={`${labelClass} xl:hidden`}>قیمت خرید <span className="text-danger">*</span></label>
                        <PriceInput
                          id={`bulk-phone-${row.id}-purchase-price`}
                          required
                ...
                <Button
                  ...
                  disabled={rows.length >= MAX_BULK_ROWS}
                  autoIcon={false}
                  className="min-h-[38px] w-full rounded-[10px] border-dashed px-3 text-[10px] font-black xl:rounded-none xl:border-x-0 xl:border-b-0"
                >
