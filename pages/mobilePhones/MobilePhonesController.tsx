...
import { Dialog as Modal, compactValidationErrors, nonNegativeNumberError, requiredSelectionError, requiredTextError } from '@/components/ui';
...
const renderBar = (value: number, max: number, tone: 'violet' | 'sky' | 'amber' | 'emerald' | 'rose' = 'sky') => {
 ...
 const errors: Record<string, string> = compactValidationErrors({
 model: isEdit
 ? (data.model !== undefined ? requiredTextError(data.model, 'مدل') : undefined)
 : requiredTextError(data.model, 'مدل'),
 imei: !imei
 ? (isEdit ? undefined : requiredTextError('', 'IMEI'))
 : (/^\d{15,16}$/.test(imei) ? undefined : 'IMEI باید ۱۵ یا ۱۶ رقم باشد.'),
 purchasePrice: isEdit && !purchasePriceStr.trim()
 ? undefined
 : nonNegativeNumberError(purchasePriceStr, 'قیمت خرید'),
 currentPurchasePrice: nonNegativeNumberError(currentPurchasePriceStr, 'قیمت خرید روز', { optional: true }),
 salePrice: nonNegativeNumberError(salePriceStr, 'قیمت فروش', { optional: true }),
 batteryHealth: batteryHealthStr.trim() && (
 !Number.isFinite(toSafeNumber(batteryHealthStr, NaN)) ||
 toSafeNumber(batteryHealthStr, NaN) < 0 ||
 toSafeNumber(batteryHealthStr, NaN) > 100
 ) ? 'سلامت باتری باید عددی بین ۰ تا ۱۰۰ باشد.' : undefined,
 status: isEdit ? undefined : requiredSelectionError((data as any).status, 'وضعیت'),
 });

 if (purchasePriceStr.trim() && toSafeNumber(purchasePriceStr) > 0 && !(data as any).supplierId) {
 errors.supplierId = 'برای ثبت قیمت خرید، انتخاب تأمین‌کننده الزامی است.';
 }
 ...
 const prepareResponse = await apiFetch(`/api/phones/${editingPhone.id}/sensitive-update/prepare`, { method: 'POST', headers: getAuthHeaders(token), body: JSON.stringify(payload) });
 ...
 if (prepared?.data?.required) {
   const approved = await confirmAccountingAction({ title: 'تأیید تغییر مالی گوشی', description: String(prepared.data?.impact?.summaryText || 'این تغییر روی اطلاعات مالی گوشی اثر دارد.'), confirmText: 'تأیید و ذخیره تغییر', tone: 'danger', iconClass: 'fa-solid fa-scale-balanced' });
   if (!approved) return;
   confirmationToken = String(prepared.data.token || '');
 }
 const response = await apiFetch(`/api/phones/${editingPhone.id}`, { method: 'PUT', headers: getAuthHeaders(token), body: JSON.stringify(confirmationToken ? { ...payload, confirmationToken } : payload) });
 const result = await response.json();
 ...
 */
 ...
 if (!phoneAiStrategyAdvisorEnabled) return { recommended: pricingIntelligenceSettings.strategy, title: 'پیشنهادگر هوشمند غیرفعال است', reason: 'این فیچر از تنظیمات ماژول‌های تجاری خاموش شده است.', maturity: 'خاموش', icon: 'fa-power-off', tone: 'border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300', canApply: false, cards: [] as Array<{ label: string; value: string; icon: string }> };
