        ...
        <>
          <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[9px] font-black">
            ...
            <span className="rounded-full border border-slate-200 bg-white/75 px-2 py-1 text-slate-600 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-300">
              {estimate.mlAdvisory.executionMode === 'approved-artifact' ? 'منبع: مدل تأییدشده' : estimate.mlAdvisory.executionMode === 'portable-trained-regression' ? 'منبع: مدل تحلیلی' : estimate.mlAdvisory.executionMode === 'advisory-disabled' ? 'پیشنهاد هوشمند غیرفعال' : 'برآورد جایگزین'}
            </span>
            <span className="rounded-full border border-slate-200 bg-white/75 px-2 py-1 text-slate-600 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-300">
