import { FinancialTimeline, FinancialTimelineEvent, IconGlyph, ManagementDirectoryPagination, SelectField, TableActionGroup, TextField } from '@/components/ui';
      ...
      >
        <div className="p-3 sm:p-4">
          ...
          <PeopleDetailSectionHeading
            ...
            subtitle="وضعیت مالی، گردش‌ها و عملیات این مشتری را از یک نمای استاندارد مدیریت کنید."
            ...
            actions={<>
              <details className="group relative" >
                ...
                <div className="absolute end-0 top-full z-20 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
                  <Button unstyled type="button" onClick={printProfile} className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:text-slate-100 dark:hover:bg-slate-800">
                    <span>چاپ / PDF دفتر حساب</span><i className="fa-solid fa-print text-slate-400" />
                  </Button>
                  <Button unstyled type="button" onClick={openTelegramReport} className="mt-1 flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:text-slate-100 dark:hover:bg-slate-800">
                    <span>گزارش برای ارسال</span><i className="fa-solid fa-paper-plane text-slate-400" />
                  </Button>
                </div>
              </details>
          ...
          <div className="flex flex-col gap-2.5 rounded-2xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-800 dark:bg-slate-950 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="order-2 xl:order-3 flex w-full items-center xl:w-auto">
              <PeopleDetailSegmentedActions
                ...
                activeKey={ledgerViewFilter}
                onChange={(key) => setLedgerViewFilter(key as 'all' | 'debit' | 'credit')}
                ariaLabel="فیلتر جهت دفتر مشتری"
              />
            </div>
            ...
            <Button type="button" onClick={printProfile} disabled={ledgerDirectoryLoading} variant="secondary" size="icon" autoIcon={false} className="order-4 xl:order-1" title="چاپ / PDF" aria-label="چاپ دفتر حساب">
            ...
            <div className="grid grid-cols-1 gap-3 xl:grid-cols-[280px_minmax(0,1fr)]">
              <aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <Button
                  ...
                  rightIcon={<i className="fa-regular fa-folder-open" />}
                >
                  {firstInstallmentSaleId ? 'مشاهده پرونده اقساط' : 'پرونده اقساطی فعالی وجود ندارد'}
                </Button>
              </aside>
              ...
              >
                ...
                {safeLedgerTotal > 0 ? (
                  <ManagementDirectoryPagination
                    page={Math.max(1, Number(ledgerPage || 1))}
                    totalPages={safeLedgerTotalPages}
                    pageSize={Math.max(1, Number(ledgerPageSize || 25))}
                    pageSizeOptions={[25, 50, 100] as const}
                    total={safeLedgerTotal}
                    pageStart={ledgerPageStart}
                    pageEnd={ledgerPageEnd}
                    ariaLabel="صفحه‌بندی گردش حساب"
                    pageSizeAriaLabel="تعداد گردش حساب در هر صفحه"
