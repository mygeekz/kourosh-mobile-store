        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]">
              <div className="space-y-4 rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
                ...
                <div className="space-y-2.5 rounded-[20px] border border-slate-200 bg-slate-50/70 p-3.5 dark:border-slate-700 dark:bg-slate-800/40">
                  ...
                  {editingEntrySourceTarget ? (
                    <Button type="button" onClick={() => navigate(editingEntrySourceTarget.path)} variant="secondary" size="sm" className="w-full" title={editingEntrySourceTarget.label} rightIcon={<i className="fa-solid fa-chevron-left" />}>
                      {editingEntrySourceTarget.label}
                    </Button>
                  ) : (
            ...
            <div className="flex flex-col-reverse gap-2.5 sm:flex-row sm:items-center sm:justify-between">
              <Button type="button" onClick={() => setEditingEntry(null)} variant="secondary" size="sm">انصراف</Button>
              <Button type="submit" disabled={!token} variant="primary" size="sm" leftIcon={<i className="fa-regular fa-floppy-disk" />}>ذخیره رکورد دفتر</Button>
            </div>
          </form>
        </Modal>
      )}
