...
import { Dialog as Modal, exactDigitsError, phoneNumberError, positiveNumberError, requiredSelectionError, requiredTextError } from '@/components/ui';
...
import { scrollToAppSection } from '../../utils/scrollToAppSection';
...
const fetchCustomerTrustHistory = async (customerId: number) => {
  ...
  const validateEditForm = (): boolean => {
    ...
    const fullNameError = requiredTextError(editingCustomer.fullName, 'نام کامل');
    const phoneError = phoneNumberError(editingCustomer.phoneNumber, { optional: true, label: 'شماره تماس' });
    const nationalCodeError = exactDigitsError(editingCustomer.nationalCode, 'کد ملی', 10, { optional: !contractEditOpenedRef.current });
  ...
  const validateLedgerForm = (): boolean => {
    ...
    const descriptionError = requiredTextError(newLedgerEntry.description, 'شرح تراکنش');
    const amount = transactionType === 'credit' ? newLedgerEntry.credit : newLedgerEntry.debit;
    const amountError = positiveNumberError(amount, 'مبلغ تراکنش');
    const dateError = requiredSelectionError(ledgerDateSelected, 'تاریخ تراکنش');
    if (descriptionError) errors.description = descriptionError;
    if (amountError) errors.amountType = amountError;
    if (dateError) errors.transactionDate = dateError;
    setLedgerFormErrors(errors);
    focusErrorsSoon(errors as any, { referenceMode: 'ledgerReferenceMode', amountType: 'ledgerAmount', transactionDate: 'ledgerDatePicker', description: 'ledgerDescription' });
    return Object.keys(errors).length === 0;
  };
  ...
  const handleLedgerDelete = async (entryId: number) => {
    if (!id || !token) return;
    const reasonInput = window.prompt('علت برگشت این سند دفتر مشتری را وارد کنید:');
    if (reasonInput == null) return;
    const reason = reasonInput.trim();
    if (reason.length < 3) { setNotification({ type: 'warning', text: 'علت برگشت سند حداقل ۳ کاراکتر الزامی است.' }); return; }
    setIsDeletingEntry(true);
    try {
      const prepareRes = await apiFetch(`/api/customers/${id}/ledger/${entryId}/reversal/prepare`, { method: 'POST', headers: getAuthHeaders(token), body: JSON.stringify({ reason }) });
      ...
      const response = await apiFetch(`/api/customers/${id}/ledger/${entryId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(token),
        body: JSON.stringify({ reason, confirmationToken: prepared.data.token }),
      });
      const result = await response.json();
  ...
  const handleLedgerEdit = async () => {
    ...
    try {
      ...
      const prepareRes = await apiFetch(`/api/customers/${id}/ledger/${editingEntry.id}/correction/prepare`, { method: 'POST', headers: { ...getAuthHeaders(token), 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const prepared = await prepareRes.json().catch(() => null);
...
const parseSaleItemMeta = (sale: any) => {
  ...
  const scrollToLedger = () => scrollToAppSection(
    document.querySelector<HTMLElement>('[data-ui-customer-ledger-root="standalone"]')
      || document.getElementById('customer-ledger-section'),
  );
  const scrollToHistory = () => scrollToAppSection(document.getElementById('customer-history-section'));
