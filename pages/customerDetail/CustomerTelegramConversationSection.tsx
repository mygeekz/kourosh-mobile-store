...
import { PeopleDetailSectionHeading, PeopleDetailSegmentedActions } from '@/components/people/PeopleDetailFoundation';
...
<MessageComposerModal
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 text-slate-900 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 sm:p-4" aria-label="مرکز گفتگوی تلگرام مشتری" data-ui-people-detail-telegram="customer-standard-v282">
        {(() => {
          ...
          return (
            <div className="space-y-3">
              <PeopleDetailSectionHeading
                id="customer-telegram-title"
                iconClass="fa-brands fa-telegram"
                title="گفتگوی تلگرام مشتری"
                subtitle="گفتگو، ارسال سریع، مدیریت اتصال و خطاهای ارسال در یک پنل واحد."
                className="border-b border-slate-200 pb-3 dark:border-slate-800"
                actions={<>
                  <Button type="button" onClick={() => fetchTelegramConversation(profile.id)} disabled={tgConvLoading} variant="secondary" size="xs" leftIcon={<i className={["fa-solid fa-rotate", tgConvLoading ? "fa-spin" : ""].join(' ')} />}>تازه‌سازی</Button>
                  <Button type="button" onClick={() => sendTelegramAction('status')} disabled={!canSendTelegram || tgIsSending} variant="secondary" size="xs" leftIcon={<i className="fa-solid fa-wallet" />}>وضعیت حساب</Button>
                  <Button type="button" onClick={() => sendTelegramAction('menu')} disabled={!canSendTelegram || !secureLinked || tgIsSending} variant="primary" size="xs" leftIcon={<i className="fa-solid fa-paper-plane" />}>ارسال منو</Button>
                </>}
              />
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1.65fr)_minmax(17rem,0.7fr)]">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-900/35">
                    <div className="mb-3 flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
                      <PeopleDetailSegmentedActions
                        ...
                        activeKey={tgDirectionFilter}
                        onChange={(key) => setTgDirectionFilter(key as typeof tgDirectionFilter)}
                        ariaLabel="فیلتر جهت پیام‌های مشتری"
                      />
                      ...
                      <Button type="button" onClick={jumpToFirstTgResult} disabled={!tgFilteredConvItems.length} variant="secondary" size="xs" leftIcon={<i className="fa-solid fa-location-crosshairs" />}>پرش</Button>
                    </div>
                    ...
                    <div className="relative rounded-2xl border border-slate-200 bg-white p-3 shadow-inner dark:border-slate-800 dark:bg-slate-950">
                      <div ref={tgTimelineRef} onScroll={(e) => { const el = e.currentTarget; const nearBottom = (el.scrollHeight - el.scrollTop - el.clientHeight) < 140; if (nearBottom) setTgNewSinceScroll(false); }} className="max-h-[520px] space-y-3 overflow-y-auto px-1 py-1">
                            ...
                            <div id={`tg-customer-msg-${m.id}`} key={m.id} className={["flex items-end gap-3", outgoing ?"justify-start" :"justify-end"].join(' ')}>
                              ...
                              <div className={["max-w-[84%] rounded-xl border px-3 py-2 text-xs leading-6 shadow-sm transition", outgoing ? (isFailed ?"border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-900/40 dark:bg-rose-950/20 dark:text-rose-100" :"border-blue-100 bg-blue-50/90 text-slate-800 dark:border-blue-900/40 dark:bg-blue-950/20 dark:text-slate-100") :"border-slate-200 bg-white text-slate-800 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100"].join(' ')}>
                                ...
                                <div className="mt-3 flex flex-wrap items-center gap-2">
                                  ...
                                  {isFailed ? (
                                    <>
                                      <Button type="button" onClick={() => retryTelegramOutbox(String(m.id))} disabled={tgIsSending} variant="danger" size="xs" leftIcon={<i className="fa-solid fa-rotate-left" />}>تلاش مجدد</Button>
                                      <Button type="button" onClick={() => setNotification({ type: 'error', text: m.lastError || 'خطای ثبت‌شده برای این پیام موجود نیست.' })} variant="secondary" size="xs">مشاهده خطا</Button>
                                    </>
                                  ) : null}
                                </div>
                              </div>
                        ...
                        {tgNewSinceScroll ? (
                          <Button type="button" onClick={() => { const el = tgTimelineRef.current; if (el) { el.scrollTop = el.scrollHeight; setTgNewSinceScroll(false); } }} variant="primary" size="xs" className="mt-3">مشاهده جدیدترین پیام‌ها</Button>
                        ) : null}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      <PeopleDetailSegmentedActions
                        ...
                        activeKey={tgQuickPreset}
                        onChange={(key) => applyTgQuickPreset(key as typeof tgQuickPreset)}
                        ariaLabel="متن‌های آماده تلگرام مشتری"
                      />
                    ...
                    {tgAttachment ? (
                      <div className="mb-3 rounded-2xl border border-emerald-200 bg-emerald-50/70 px-3 py-2 text-xs text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-200">
                        <div className="flex items-center justify-between gap-2"><div className="truncate"><span className="font-black ms-2">پیوست:</span>{tgAttachment.originalName || tgAttachment.relPath}</div><Button type="button" unstyled onClick={() => setTgAttachment(null)} className="grid h-8 w-8 place-items-center border-0 bg-transparent text-emerald-700 shadow-none transition hover:text-emerald-900 dark:text-emerald-200 dark:hover:text-emerald-100" aria-label="حذف پیوست"><i className="fa-solid fa-xmark" /></Button></div>
                      </div>
                    ) : null}
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          {["{name}","{phone}","{amount}","{dueDate}","{link}","{installmentNo}","{remainingAmount}"].map((ch) => (
                            <Button key={ch} type="button" onClick={() => setTgQuickReply(v => (v || '') + ch)} variant="ghost" size="xs" autoIcon={false}>{ch}</Button>
                          ))}
                        </div>
                      </div>
                ...
                <aside className="space-y-3">
                  <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <Button type="button" onClick={() => setTgShowChatId(v => !v)} disabled={!tgChatIdInput.trim()} variant="ghost" size="xs" autoIcon={false}>{tgShowChatId ? 'مخفی' : 'نمایش'}</Button>
                      </div>
                      <div className="flex gap-2">
                        <TextField controlOnly controlSize="sm" type={tgShowChatId ? 'text' : 'password'} inputMode="numeric" dir="ltr" value={tgChatIdInput} onChange={(e) => setTgChatIdInput(e.target.value)} placeholder="مثلاً -1001234567890" className="h-11 min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-3 text-sm font-bold text-slate-900 outline-none    dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100" />
                        <Button type="button" unstyled onClick={async () => { if (!tgChatIdInput.trim()) return; try { await navigator.clipboard.writeText(tgChatIdInput.trim()); setNotification({ type: 'success', text: 'شناسه مقصد تلگرام کپی شد.' }); } catch { setNotification({ type: 'error', text: 'کپی شناسه مقصد تلگرام انجام نشد.' }); } }} disabled={!tgChatIdInput.trim()} className="inline-flex h-7 w-7 items-center justify-center border-0 bg-transparent text-slate-500 shadow-none transition hover:text-slate-900 disabled:opacity-50 dark:text-slate-300 dark:hover:text-white" aria-label="کپی شناسه"><i className="fa-regular fa-copy" /></Button>
                      </div>
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <Button type="button" onClick={saveManualChatId} disabled={tgIsSending || !tgChatIdInput.trim()} variant="primary" size="sm" leftIcon={<i className="fa-regular fa-floppy-disk" />}>ذخیره مقصد ارسال</Button>
                        <Button type="button" onClick={unlinkChatId} disabled={tgIsSending || !linked} variant="danger" size="sm" leftIcon={<i className="fa-regular fa-trash-can" />}>حذف اتصال</Button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <Button type="button" onClick={toggleTelegramOptout} disabled={tgIsSending} variant="secondary" size="sm" className="mt-3 w-full" leftIcon={<i className="fa-solid fa-toggle-on" />}>
