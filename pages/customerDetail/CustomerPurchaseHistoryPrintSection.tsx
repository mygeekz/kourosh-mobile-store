...
import { Table, TableViewport } from '@/components/ui';
...
{/* تاریخچه خرید */}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100" aria-labelledby="customer-purchase-history-title">
        <PeopleDetailSectionHeading
          id="customer-purchase-history-title"
          iconClass="fa-solid fa-clock-rotate-left"
          title="تاریخچه خرید مشتری"
          subtitle="فروش‌های ثبت‌شده در پرونده مشتری"
          className="border-b border-slate-200 px-4 py-3 dark:border-slate-800 sm:px-5"
        />
        {purchaseHistory.length === 0 ? (
          ...
          <TableViewport
            ariaLabel="تاریخچه خرید مشتری"
            className="w-full overflow-x-auto overscroll-x-contain"
            data-ui-customer-purchase-history="standard-v297"
            data-ui-purchase-history-parity="installments-v321"
            data-ui-purchase-history-kind="customer"
          >
            <Table layout="auto" density="compact" minWidthClassName="min-w-[48rem]" className="w-full divide-y divide-slate-200 text-xs dark:divide-slate-800">
              <thead className="bg-slate-50 dark:bg-slate-900/70">
                <tr className="text-right [&>th]:px-2.5 [&>th]:py-2 [&>th]:font-black [&>th]:text-slate-600 dark:[&>th]:text-slate-200">
                  <th className="min-w-[7rem] whitespace-nowrap"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-calendar-day text-sky-500" /> تاریخ فروش</span></th>
                  <th className="min-w-[10rem]"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-box text-indigo-500" /> شرح کالا</span></th>
                  <th className="min-w-[8rem] whitespace-nowrap"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-credit-card text-violet-500" /> نوع خرید</span></th>
                  <th className="min-w-[10rem] whitespace-nowrap"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-fingerprint text-slate-500" /> IMEI</span></th>
                  <th className="min-w-[5rem] whitespace-nowrap"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-bullseye text-amber-500" /> تعداد</span></th>
                  <th className="min-w-[9rem] whitespace-nowrap"><span className="inline-flex items-center gap-2"><i className="fa-solid fa-sack-dollar text-emerald-500" /> قیمت نهایی</span></th>
                </tr>
              </thead>
                ...
                {purchaseHistory.map(sale => {
                  ...
                  return (
                    <tr key={sale.id} className="transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                      <td className="whitespace-nowrap px-2.5 py-2 align-middle text-slate-700 dark:text-slate-200">{formatIsoToShamsi(sale.transactionDate)}</td>
                      <td className="px-2.5 py-2 align-middle font-bold text-slate-900 dark:text-slate-100"><div className="min-w-0 break-words [overflow-wrap:anywhere]">{meta.cleanName}</div></td>
                      <td className="whitespace-nowrap px-2.5 py-2 align-middle"><span className={`inline-flex items-center gap-1.5 font-black ${typeClass}`}><i className={`fa-solid ${typeIcon}`} /> {meta.purchaseTypeLabel}</span></td>
                      <td className="whitespace-nowrap px-2.5 py-2 align-middle">{meta.imei ? <bdi dir="ltr" className="font-semibold text-slate-600 dark:text-slate-300">{meta.imei}</bdi> : <span className="text-slate-400">—</span>}</td>
                      <td className="whitespace-nowrap px-2.5 py-2 align-middle">{sale.quantity.toLocaleString('fa-IR')}</td>
                      <td className="whitespace-nowrap px-2.5 py-2 align-middle font-black text-indigo-700 dark:text-indigo-300">{formatPrice(sale.totalPrice)}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div id="customer-ledger-print-area" className="hidden" aria-hidden="true">
        <div className="customer-print-report">
          ...
          <div className="customer-print-table-wrap">
            <table data-ui-table="print" className="customer-print-table">
              <thead>
                <tr>
