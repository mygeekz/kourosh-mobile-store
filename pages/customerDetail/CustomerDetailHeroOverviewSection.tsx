import { IconGlyph, inferIconGlyphTone, TextField } from '@/components/ui';
import { PeopleDetailHeroHeader, PeopleDetailQuickActionGrid } from '@/components/people/PeopleDetailFoundation';
...
const CustomerDetailHeroOverviewSection: React.FC<Props> = ({ ctx }) => {
  ...
  const trustBadgeTone = !customerTrustProfile || customerTrustLoading ? 'neutral' : trustScore >= 68 ? 'success' : trustScore >= 50 ? 'warning' : 'danger';
  const legacyCashedCheckAdjustment = Math.max(0, Number((profile as any).legacyCashedCheckAdjustment || 0));
  ...
  return (
    <>
      <PeopleDetailHeroHeader
        ...
        subtitle="نمای کامل مشتری برای پیگیری ارتباطات، گردش حساب، سوابق خرید و تعاملات."
        badges={<>
          <FinancialStatusBadge label={<bdi dir="ltr">{profile.phoneNumber || 'بدون شماره'}</bdi>} tone="neutral" icon="fa-solid fa-phone" size="sm" />
          <FinancialStatusBadge label={`${purchaseHistory.length.toLocaleString('fa-IR')} خرید ثبت‌شده`} tone="customer" icon="fa-solid fa-bag-shopping" size="sm" />
          <FinancialStatusBadge label={`امتیاز اعتماد: ${customerTrustLoading ? '...' : customerTrustProfile ? `${trustScore.toLocaleString('fa-IR')} از ۱۰۰` : 'نامشخص'}`} tone={trustBadgeTone} icon={trustTone.icon} size="sm" />
          <FinancialStatusBadge label={profile.currentBalance > 0 ? 'بدهکار' : profile.currentBalance < 0 ? 'بستانکار' : 'تسویه'} tone={profile.currentBalance > 0 ? 'danger' : 'success'} icon="fa-solid fa-wallet" size="sm" />
          <FinancialStatusBadge label={customerTelegramSecureLinked ? 'تلگرام متصل امن' : customerTelegramLegacyDelivery ? 'تلگرام قدیمی؛ فقط ارسال' : 'تلگرام لینک امن نشده'} tone={customerTelegramSecureLinked ? 'info' : customerTelegramLegacyDelivery ? 'warning' : 'danger'} icon="fa-brands fa-telegram" size="sm" />
          {customerTelegramLinkedAt ? <FinancialStatusBadge label={`آخرین اتصال: ${customerTelegramLinkedAt}`} tone="neutral" icon="fa-regular fa-clock" size="sm" /> : null}
        </>}
        actions={<>
          <Button
            onClick={() => { setPrefillMessageText(''); setPrefillChannels(undefined); setIsMessageModalOpen(true); }}
            variant="primary"
            size="sm"
            title="ارسال پیامک/تلگرام"
            leftIcon={<i className="fa-solid fa-paper-plane" />}
          >ارسال پیام</Button>
          <Button onClick={openTelegramReport} variant="secondary" size="sm" title="ارسال گزارش کامل مشتری در تلگرام" leftIcon={<i className="fa-brands fa-telegram" />}>ارسال گزارش</Button>
          <Button onClick={openEditModal} variant="secondary" size="sm" leftIcon={<i className="fas fa-edit" />}>ویرایش پروفایل</Button>
          <Button type="button" onClick={openQrLinkModal} disabled={tgIsSending || tgQrLoading} variant="secondary" size="sm" title="مدیریت اتصال تلگرام" leftIcon={<i className="fa-solid fa-link" />}>اتصال تلگرام</Button>
        </>}
      />

      <div className="px-4 pb-4 sm:px-5 sm:pb-5">
          <section className="space-y-3" aria-label="داشبورد اعتبار و حساب مشتری">
            <div className="grid gap-2.5 lg:grid-cols-[1fr_1.25fr_1fr]">
              <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                ...
                <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/25 dark:text-amber-200">
                  {legacyCashedCheckAdjustment > 0
                    ? `مانده فعلی با لحاظ ${formatCurrencyText(legacyCashedCheckAdjustment, readStoredCurrencyUnit())} وصول قطعی چک قدیمی محاسبه شده است؛ تاریخ واقعی وصول آن هنوز در مرکز تطبیق حسابداری قابل تکمیل است.`
                    : profile.currentBalance > 0
                      ? 'پیشنهاد: قبل از فروش اعتباری جدید، دفتر حساب و تعهدات فعال بررسی شود.'
                      : 'وضعیت حساب فعلی مانع مستقیم برای فروش جدید ایجاد نمی‌کند.'}
                </div>
                <Button
                  type="button"
                  aria-controls="customer-ledger-section"
                ...
                <Link to={`/accounting-drilldown/customer/${profile.id}`} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-black text-sky-700 hover:bg-sky-50 hover:underline dark:text-sky-300 dark:hover:bg-sky-950/20"><i className="fa-solid fa-route" /> مسیر رسیدن به این مانده</Link>
              </div>
                    ...
                    {managerActionSummary.map((item) => (
                      <div key={item.label} className={`rounded-xl border p-3 ${item.tone}`}>
                        <div className="flex h-full flex-col gap-3 text-right">
                          ...
                          <Button type="button" onClick={item.onAction} variant="secondary" size="xs" className="mt-auto w-full" rightIcon={<i className={`${item.ctaIcon} text-xs`} />}>
                            {item.ctaLabel}
                          </Button>
                        </div>
                      </div>
                    ...
                    {managerActionCards.map((action) => (
                      <div key={action.title} className={`rounded-xl border p-3 ${action.tone}`}>
                        <div className="flex h-full flex-col gap-3 text-right">
                          ...
                          <Button type="button" onClick={action.onAction} variant="secondary" size="xs" className="mt-auto w-full" rightIcon={<i className={`${action.ctaIcon} text-xs`} />}>
                            {action.ctaLabel}
                          </Button>
                        </div>
                      </div>
        ...
        <section className="border-t border-slate-200/80 bg-slate-50/55 px-3 py-3 dark:border-slate-800 dark:bg-slate-950/25" aria-label="اکشن‌ها و اطلاعات تکمیلی مشتری">
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_18px_42px_-42px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950">
            ...
            <PeopleDetailQuickActionGrid
              actions={quickActions.map((action) => ({ key: action.key, label: action.label, sub: action.sub, icon: action.icon, onClick: action.onClick }))}
              primaryKey="telegram"
              ariaLabel="اکشن‌های سریع پرونده مشتری"
            />
          </div>
