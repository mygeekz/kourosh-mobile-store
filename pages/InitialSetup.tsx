...
import { Button, FormGrid, FormGridItem, PasswordVisibilityButton, TextField, requiredTextError } from '@/components/ui';
import {
...
const InitialSetupPage: React.FC = () => {
  ...
  const submitSetup = async (event: FormEvent) => {
    ...
    const usernameRequired = requiredTextError(username, 'نام کاربری مدیر');
    const passwordRequired = requiredTextError(password, 'کلمه عبور جدید');
    const confirmRequired = requiredTextError(confirmPassword, 'تکرار کلمه عبور');
    if (usernameRequired) errors.username = usernameRequired;
    else if (!usernameValid) errors.username = 'نام کاربری باید ۳ تا ۶۴ کاراکتر و فقط شامل حروف، عدد، نقطه، خط تیره یا زیرخط باشد.';
    if (passwordRequired) errors.password = passwordRequired;
    else if (!passwordChecks.length) errors.password = `کلمه عبور باید حداقل ${policy.minLength.toLocaleString('fa-IR')} کاراکتر باشد.`;
    else if (!passwordChecks.bytes) errors.password = `کلمه عبور نباید بیشتر از ${policy.maxBytes.toLocaleString('fa-IR')} بایت باشد.`;
    else if (!passwordChecks.letter) errors.password = 'کلمه عبور باید حداقل یک حرف داشته باشد.';
    else if (!passwordChecks.number) errors.password = 'کلمه عبور باید حداقل یک عدد داشته باشد.';
    else if (!passwordChecks.differs) errors.password = 'کلمه عبور نباید نام کاربری را در خود داشته باشد.';
    if (confirmRequired) errors.confirmPassword = confirmRequired;
    else if (!passwordChecks.matches) errors.confirmPassword = 'تکرار کلمه عبور با کلمه عبور جدید یکسان نیست.';
    setSetupErrors(errors);
    if (Object.keys(errors).length || !formValid) {
      focusErrorsSoon(errors, { username: 'setup-username', password: 'setup-password', confirmPassword: 'setup-confirm-password' });
      return;
    }
    setSubmitting(true);
    ...
    <>
      ...
      >
        ...
        >
          ...
          {step === 1 && (
            <form noValidate onSubmit={submitSetup} className="mt-4">
              <FormErrorSummary errors={setupErrors} labels={{ username: 'نام کاربری مدیر', password: 'کلمه عبور جدید', confirmPassword: 'تکرار کلمه عبور' }} fieldIdMap={{ username: 'setup-username', password: 'setup-password', confirmPassword: 'setup-confirm-password' }} />
              <FormGrid columns={2}>
              <TextField
                surface="glass"
                id="setup-username"
                ...
                autoFocus
                value={username}
                onChange={(event) => { setUsername(event.target.value); if (setupErrors.username) setSetupErrors((current) => ({ ...current, username: undefined })); }}
                dir="ltr"
                ...
                error={setupErrors.username || (username && !usernameValid ? '۳ تا ۶۴ کاراکتر؛ حروف، عدد، نقطه، خط تیره یا زیرخط.' : undefined)}
              />
              <TextField
                surface="glass"
                id="setup-password"
                ...
                value={password}
                onChange={(event) => { setPassword(event.target.value); if (setupErrors.password) setSetupErrors((current) => ({ ...current, password: undefined })); }}
                dir="ltr"
              ...
              <TextField
                ...
                onChange={(event) => { setConfirmPassword(event.target.value); if (setupErrors.confirmPassword) setSetupErrors((current) => ({ ...current, confirmPassword: undefined })); }}
                dir="ltr"
                icon={<i className="fa-solid fa-key" aria-hidden="true" />}
                trailingAction={(
                  <PasswordVisibilityButton
              ...
              <FormGridItem span="full" className="grid grid-cols-1 gap-3 sm:grid-cols-[auto_1fr]">
                ...
                <InitialSetupSubmitAction loading={submitting} disabled={submitting} className={cn('w-full', authGoldPrimaryActionClasses)} />
              </FormGridItem>
              </FormGrid>
            </form>
          )}
