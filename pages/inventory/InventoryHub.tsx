...
import { DataTableShell, PageShell, SearchableSelectField, TextField } from '@/components/ui';
...
function InventoryAlertsPanel({ onNavigateProducts }: { onNavigateProducts: () => void }) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [aRes, pRes] = await Promise.all([apiFetch('/api/inventory/alerts'), apiFetch('/api/products')]);
    ...
    <div className="inventory-shell-modern" dir="rtl">
      ...
      <div className="app-card inventory-card-premium p-4 md:p-6">
        ...
        {loading ? (
          ...
          <DataTableShell className="mt-4" data-ui-inventory-alerts-table="true">
...
function StockAdjustPanel() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiFetch(`/api/products/${Number(productId)}/adjust-stock`, {
        method: 'POST',
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            <div className="md:col-span-5">
              ...
              <SearchableSelectField<number>
                ...
                ariaLabel="جستجو و انتخاب کالا برای اصلاح موجودی"
              />
              {selected ? (
            ...
            <div className="md:col-span-2">
              ...
              <TextField controlOnly className="app-input" value={reason} onChange={(e) => setReason(e.target.value)} preview="مثلاً مرجوعی" />
            </div>

            <div className="md:col-span-3">
              <label className="app-label">یادداشت (اختیاری)</label>
              <TextField controlOnly className="app-input" value={notes} onChange={(e) => setNotes(e.target.value)} preview="جزئیات" />
            </div>
