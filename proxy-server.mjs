...
const routeFor = (pathname) => {
  const isLegacyInventoryAlertsRoute =
    pathname === '/inventory/alerts' || pathname.startsWith('/inventory/alerts/');
  if (
    pathname === '/health' ||
    pathname.startsWith('/api') ||
    pathname.startsWith('/uploads') ||
    isLegacyInventoryAlertsRoute
  ) {
...
const buildForwardHeaders = (req, targetHost, targetPort) => {
  ...
  // Never forward a caller-supplied chain. The backend trusts this proxy's
  // loopback connection, so the proxy must provide the observed peer itself.
