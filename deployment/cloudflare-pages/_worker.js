...
const managerSnapshotSchemaMissing = (error) => /(?:no such table|table .* does not exist).*manager_snapshots|manager_snapshots.*(?:no such table|does not exist)/i.test(
  String(error?.message || error?.cause?.message || "")
);

const managerSchemaMigrationRequired = (reqId) => failure(
  503,
  "MINIAPP_EDGE_SCHEMA_MIGRATION_REQUIRED",
  "ساختار ذخیره‌سازی پنل مدیر هنوز روی Cloudflare D1 به‌روزرسانی نشده است.",
  reqId,
  { "Retry-After": "5" },
);
...
// Snapshot sync is the signed recovery channel for an installation. Unlike
// normal Mini App reads, it must remain usable even when the D1 live_origin is
// stale/invalid so a restored local installation can repair that value.
const getInstallationForSync = async (db, installationId) => {
  const row = await db.prepare(
    "SELECT tenant_id, installation_id, credential_version, installation_public_key_pem, bot_id, public_host, live_origin, status FROM tenant_installations WHERE installation_id = ? AND status = 'active' LIMIT 1"
  ).bind(installationId).first();
  if (!row) return null;
  const publicHost = safePublicHost(row.public_host);
  if (!TENANT_ID_PATTERN.test(String(row.tenant_id || "")) || !INSTALLATION_ID_PATTERN.test(String(row.installation_id || "")) || !BOT_ID_PATTERN.test(String(row.bot_id || "")) || !publicHost) return null;
  return {
    tenantId: String(row.tenant_id),
    installationId: String(row.installation_id),
    credentialVersion: Number(row.credential_version),
    ...
    botId: String(row.bot_id),
    publicHost,
    liveOrigin: safeOrigin(row.live_origin),
    status: String(row.status || ""),
  };
};
...
const getSnapshot = async (db, tenantId, subjectKind, subjectKey) => {
  if (subjectKind === "manager") {
    const row = await db.prepare(
      "SELECT tenant_id, 'manager' AS subject_kind, subject_key, installation_id, snapshot_version, schema_version, state, generated_at, received_at, authorization_valid_until, payload_json, content_hash FROM manager_snapshots WHERE tenant_id = ? AND subject_key = ? LIMIT 1"
    ).bind(tenantId, subjectKey).first();
    return snapshotRowToStored(row);
  }
  return snapshotRowToStored(await db.prepare(
...
const workspaceFromSnapshot = (kind, snapshot) => {
  const displayName = String(snapshot?.data?.profile?.displayName || (kind === "customer" ? "مشتری کوروش" : kind === "partner" ? "همکار کوروش" : "مدیر کوروش"));
  if (kind === "manager") return {
    kind: "manager", subjectId: 0, displayName, roleName: snapshot?.data?.profile?.roleName || null,
    capabilities: ["staff:executive:read"], permissions: Array.isArray(snapshot?.data?.permissions) ? snapshot.data.permissions.map(String) : [],
  };
  return {
    kind, subjectId: 0, displayName,
    capabilities: kind === "customer" ? [...CUSTOMER_CAPABILITIES] : [...PARTNER_CAPABILITIES], permissions: [],
  };
};

const identityFromSnapshots = (selectedKind, snapshots, telegramUserId) => {
  const workspaces = ["manager", "customer", "partner"]
    .filter((kind) => snapshots[kind])
    .map((kind) => workspaceFromSnapshot(kind, snapshots[kind]));
  const selected = workspaces.find((workspace) => workspace.kind === selectedKind);
  if (!selected) return null;
  if (selected.kind === "manager") return {
    kind: "staff", subjectId: 0, displayName: selected.displayName, telegramUserId,
    roleName: selected.roleName || undefined, capabilities: [...selected.capabilities], permissions: [...selected.permissions], workspaces,
  };
  return {
    kind: selected.kind, subjectId: 0, displayName: selected.displayName, telegramUserId,
    capabilities: [...selected.capabilities], permissions: [], workspaces,
  };
};
...
const validateLiveIdentity = (identity, telegramUserId, expectedKind = null) => {
  ...
  if (identity.kind === "staff") {
    ...
    if (permissions.length > 64 || permissions.some((item) => typeof item !== "string" || item.length > 96)) return null;
    const hasManagementPermission = permissions.some((item) => MANAGEMENT_PERMISSION_KEYS.has(item));
    if (!hasManagementPermission && !["Admin", "Manager"].includes(identity.roleName)) return null;
  }
  if (identity.workspaces !== undefined) {
    if (!Array.isArray(identity.workspaces) || identity.workspaces.length > 3) return null;
    for (const workspace of identity.workspaces) {
      if (!workspace || !["customer", "partner", "manager"].includes(workspace.kind)) return null;
      if (!Number.isSafeInteger(Number(workspace.subjectId)) || Number(workspace.subjectId) <= 0) return null;
      if (!Array.isArray(workspace.capabilities) || workspace.capabilities.length > 32 || workspace.capabilities.some((item) => typeof item !== "string" || item.length > 96)) return null;
      if (!Array.isArray(workspace.permissions) || workspace.permissions.length > 64 || workspace.permissions.some((item) => typeof item !== "string" || item.length > 96)) return null;
    }
  }
...
const authenticate = async (request, env, reqId) => {
  ...
  const requestedWorkspace = parsed.value?.workspaceKind == null ? null : String(parsed.value.workspaceKind);
  if (requestedWorkspace && !["manager", "customer", "partner"].includes(requestedWorkspace)) {
    return failure(400, "MINIAPP_WORKSPACE_INVALID", "فضای کاری درخواستی معتبر نیست.", reqId);
  }
  let validated;
  try {
  ...
  const liveTimeoutMs = numericEnv(env, "KOUROSH_EDGE_LIVE_TIMEOUT_MS", 5000, 1000, 10000);
  const authSession = { liveOrigin: tenant.liveOrigin, localSessionToken: null };
  const live = await proxyLive({ session: authSession, request, path: "/api/miniapp/auth", method: "POST", body: JSON.stringify({ initData: String(parsed.value.initData), ...(requestedWorkspace ? { workspaceKind: requestedWorkspace } : {}) }), contentType: "application/json", timeoutMs: liveTimeoutMs });
  ...
  if (live.kind === "response" && live.status >= 200 && live.status < 300 && live.json?.success === true && live.json?.data?.identity && live.json?.data?.sessionToken) {
    identity = validateLiveIdentity(
  live.json.data.identity,
  String(validated.user.id),
  requestedWorkspace === "manager" ? "staff" : requestedWorkspace
);
  ...
  if (!identity) {
    const snapshotLookup = await tryStorage(() => Promise.all([
      getSnapshot(db, tenant.tenantId, "manager", subjectKey),
      getSnapshot(db, tenant.tenantId, "customer", subjectKey),
      getSnapshot(db, tenant.tenantId, "partner", subjectKey),
    ]), "auth_snapshot_lookup");
    if (!snapshotLookup.ok) {
      if (managerSnapshotSchemaMissing(snapshotLookup.error)) return managerSchemaMigrationRequired(reqId);
      return storageUnavailable(reqId, "اطلاعات ذخیره‌شده موقتاً در دسترس نیست.");
    }
    const [manager, customer, partner] = snapshotLookup.value;
    const snapshots = {
      manager: activeSnapshot(manager) ? manager : null,
      customer: activeSnapshot(customer) ? customer : null,
      partner: activeSnapshot(partner) ? partner : null,
    };
    const selectedKind = requestedWorkspace || (snapshots.manager ? "manager" : snapshots.customer ? "customer" : snapshots.partner ? "partner" : null);
    if (!selectedKind || !snapshots[selectedKind]) {
      const requestedRevoked = requestedWorkspace && [manager, customer, partner].find((row) => row?.subjectKind === requestedWorkspace && row?.state === "revoked");
      if (requestedRevoked) return failure(403, "MINIAPP_ACCOUNT_UNLINKED", "دسترسی این فضای کاری غیرفعال شده است.", reqId);
      return failure(503, "MINIAPP_OFFLINE_SNAPSHOT_UNAVAILABLE", requestedWorkspace ? "برای فضای کاری انتخاب‌شده اطلاعات ذخیره‌شده معتبری وجود ندارد." : "فروشگاه آفلاین است و اطلاعات ذخیره‌شده معتبری برای این حساب موجود نیست.", reqId);
    }
    offlineSnapshot = snapshots[selectedKind];
    identity = identityFromSnapshots(selectedKind, snapshots, String(validated.user.id));
    if (!identity) return failure(503, "MINIAPP_OFFLINE_SNAPSHOT_UNAVAILABLE", "اطلاعات ذخیره‌شده این فضای کاری قابل استفاده نیست.", reqId);
  }
...
const managerHasPermissions = (snapshot, required, any = false) => {
  const permissions = new Set(Array.isArray(snapshot?.data?.permissions) ? snapshot.data.permissions.map(String) : []);
  return any ? required.some((permission) => permissions.has(permission)) : required.every((permission) => permissions.has(permission));
};

const managerPermissionDenied = () => ({
  error: { status: 403, code: "MINIAPP_MANAGER_PERMISSION_REQUIRED", message: "مجوز لازم برای مشاهده این بخش فعال نیست." },
});

const filterManagerDirectory = (items, q, fields) => {
  const needle = String(q || "").trim().toLocaleLowerCase("fa-IR");
  if (!needle) return items;
  return items.filter((item) => fields.some((field) => String(item?.[field] || "").toLocaleLowerCase("fa-IR").includes(needle)));
};

const managerSnapshotResponse = (snapshot, pathname, url, identity) => {
  const data = snapshot.data || {};
  const requireAll = (...permissions) => managerHasPermissions(snapshot, permissions);
  const requireAny = (...permissions) => managerHasPermissions(snapshot, permissions, true);
  if (pathname === "/api/miniapp/manager/me") return { data: {
    id: 0,
    displayName: data.profile?.displayName || identity?.displayName || "مدیر کوروش",
    roleName: data.profile?.roleName || identity?.roleName || null,
    permissions: Array.isArray(data.permissions) ? data.permissions : [],
    workspaces: Array.isArray(identity?.workspaces) ? identity.workspaces : [],
    verified: true,
  } };
  if (pathname === "/api/miniapp/manager/dashboard") return requireAll("dashboard.read") ? { data: data.dashboard || { generatedAt: snapshot.generatedAt, widgets: {} } } : managerPermissionDenied();
  if (pathname === "/api/miniapp/manager/sales-summary") {
    if (!requireAny("sales.read", "profits.read")) return managerPermissionDenied();
    const period = ["today", "week", "month"].includes(String(url.searchParams.get("period") || "today")) ? String(url.searchParams.get("period") || "today") : "today";
    return { data: data.sales?.[period] || null };
  }
  if (pathname === "/api/miniapp/manager/customers") {
    if (!requireAll("customers.read")) return managerPermissionDenied();
    const directory = data.customers?.directory || { items: [] };
    const filtered = filterManagerDirectory(Array.isArray(directory.items) ? directory.items : [], url.searchParams.get("q") || url.searchParams.get("search"), ["fullName", "name"]);
    return { data: { ...pageSlice(filtered, url), ...(directory.summary ? { summary: directory.summary } : {}) } };
  }
  let match = pathname.match(/^\/api\/miniapp\/manager\/customers\/([1-9]\d*)$/);
  if (match) {
    if (!requireAll("customers.read")) return managerPermissionDenied();
    return { data: data.customers?.details?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/customers\/([1-9]\d*)\/ledger$/);
  if (match) {
    if (!requireAll("customers.read", "customers.ledger.read")) return managerPermissionDenied();
    return { data: data.customers?.ledger?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/customers\/([1-9]\d*)\/purchases$/);
  if (match) {
    if (!requireAll("customers.read", "sales.read")) return managerPermissionDenied();
    return { data: data.customers?.purchases?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/customers\/([1-9]\d*)\/installments$/);
  if (match) {
    if (!requireAll("customers.read", "installments.read")) return managerPermissionDenied();
    return { data: data.customers?.installments?.[match[1]] };
  }
  if (pathname === "/api/miniapp/manager/partners") {
    if (!requireAll("partners.read")) return managerPermissionDenied();
    const directory = data.partners?.directory || { items: [] };
    const filtered = filterManagerDirectory(Array.isArray(directory.items) ? directory.items : [], url.searchParams.get("q") || url.searchParams.get("search"), ["name", "partnerName"]);
    return { data: { ...pageSlice(filtered, url), ...(directory.summary ? { summary: directory.summary } : {}) } };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/partners\/([1-9]\d*)$/);
  if (match) {
    if (!requireAll("partners.read")) return managerPermissionDenied();
    return { data: data.partners?.details?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/partners\/([1-9]\d*)\/ledger$/);
  if (match) {
    if (!requireAll("partners.read", "partners.ledger.read")) return managerPermissionDenied();
    return { data: data.partners?.ledger?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/partners\/([1-9]\d*)\/purchases$/);
  if (match) {
    if (!requireAll("partners.read")) return managerPermissionDenied();
    return { data: data.partners?.purchases?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/partners\/([1-9]\d*)\/settlements$/);
  if (match) {
    if (!requireAll("partners.read", "partners.ledger.read")) return managerPermissionDenied();
    return { data: data.partners?.settlements?.[match[1]] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/partners\/([1-9]\d*)\/accounting-breakdown$/);
  if (match) {
    if (!requireAll("partners.read", "partners.ledger.read", "profits.read")) return managerPermissionDenied();
    return { data: data.partners?.accounting?.[match[1]] };
  }
  if (pathname === "/api/miniapp/manager/installments/due") {
    if (!requireAll("installments.read")) return managerPermissionDenied();
    const scope = ["overdue", "today", "next7"].includes(String(url.searchParams.get("scope") || "today")) ? String(url.searchParams.get("scope") || "today") : "today";
    return { data: data.dues?.[scope] };
  }
  match = pathname.match(/^\/api\/miniapp\/manager\/installments\/([1-9]\d*)$/);
  if (match) {
    if (!requireAll("installments.read")) return managerPermissionDenied();
    return { data: data.installmentDetails?.[match[1]] };
  }
  if (pathname === "/api/miniapp/manager/repairs") {
    if (!requireAll("repairs.read")) return managerPermissionDenied();
    const stored = data.repairs || { items: [] };
    const filtered = filterManagerDirectory(Array.isArray(stored.items) ? stored.items : [], url.searchParams.get("q") || url.searchParams.get("search"), ["customerFullName", "deviceModel", "problemDescription", "status", "technicianName"]);
    return { data: { ...pageSlice(filtered, url), ...(stored.summary ? { summary: stored.summary } : {}) } };
  }
  if (pathname === "/api/miniapp/manager/inventory/phones") {
    if (!requireAll("inventory.read")) return managerPermissionDenied();
    const stored = data.inventoryPhones || { items: [] };
    const filtered = filterManagerDirectory(Array.isArray(stored.items) ? stored.items : [], url.searchParams.get("q") || url.searchParams.get("search"), ["brand", "model", "color", "status"]);
    return { data: { ...stored, ...pageSlice(filtered, url) } };
  }
  if (pathname === "/api/miniapp/manager/notifications") {
    const stored = data.notifications || { items: [], unreadCount: 0 };
    const limit = Math.max(1, Math.min(100, Number(url.searchParams.get("limit") || 40)));
    const unreadOnly = ["1", "true"].includes(String(url.searchParams.get("unreadOnly") || "").toLowerCase());
    const items = (Array.isArray(stored.items) ? stored.items : Array.isArray(stored) ? stored : []).filter((item) => !unreadOnly || !item?.readAt).slice(0, limit);
    return { data: { ...(Array.isArray(stored) ? {} : stored), items } };
  }
  return { data: undefined };
};

const isManagerNotificationMutationPath = (pathname) => /^\/api\/miniapp\/manager\/notifications\/(?:[1-9]\d*\/read|read-all)$/.test(String(pathname || ""));

const isManagerAssociatedIdentity = (identity) => identity?.kind === "staff"
  || (Array.isArray(identity?.workspaces) && identity.workspaces.some((workspace) => workspace?.kind === "manager"));
...
const serveSnapshotRead = async (env, session, request, reqId) => {
  ...
  const snapshotKind = session.identity.kind === "staff" ? "manager" : session.identity.kind;
  const snapshotLookup = await tryStorage(() => getSnapshot(db, session.tenantId, snapshotKind, session.subjectKey));
  ...
  const url = new URL(request.url);
  let data;
  if (snapshotKind === "customer") data = customerSnapshotResponse(snapshot, url.pathname);
  else if (snapshotKind === "partner") data = partnerSnapshotResponse(snapshot, url.pathname, url);
  else {
    const result = managerSnapshotResponse(snapshot, url.pathname, url, session.identity);
    if (result.error) return failure(result.error.status, result.error.code, result.error.message, reqId, snapshotHeaders(snapshot));
    data = result.data;
  }
  if (data === undefined || data === null) return failure(404, "MINIAPP_ROUTE_NOT_FOUND", "اطلاعات این مسیر در Snapshot موجود نیست.", reqId, snapshotHeaders(snapshot));
...
const authenticatedRead = async (request, env, reqId) => {
  ...
  const routeKind = url.pathname.startsWith("/api/miniapp/customer/") ? "customer"
    : url.pathname.startsWith("/api/miniapp/partner/") ? "partner"
      : (url.pathname.startsWith("/api/miniapp/staff/") || url.pathname.startsWith("/api/miniapp/manager/")) ? "staff"
        : null;
  ...
  const timeoutMs = numericEnv(env, "KOUROSH_EDGE_LIVE_TIMEOUT_MS", 5000, 1000, 10000);
  let liveSession = session;
  if (!liveSession.localSessionToken && liveSession.liveOrigin && liveSession.initData) {
    const activeWorkspaceKind = liveSession.identity.kind === "staff" ? "manager" : liveSession.identity.kind;
    const reauth = await proxyLive({ session: liveSession, request, path: "/api/miniapp/auth", method: "POST", body: JSON.stringify({ initData: liveSession.initData, workspaceKind: activeWorkspaceKind }), contentType: "application/json", timeoutMs });
  ...
  if (liveSession.localSessionToken && liveSession.liveOrigin) {
    ...
    if (live.kind === "response") {
      ...
      // 5xx may safely fall through to a permission-filtered read snapshot for customer/partner/manager.
    }
  }
  // Legacy staff endpoints stay live-only. Offline Manager reads are supported only
  // by the dedicated /api/miniapp/manager/* surface.
  if (url.pathname.startsWith("/api/miniapp/staff/")) {
    return failure(503, "MINIAPP_STAFF_OFFLINE_UNAVAILABLE", "اتصال زنده به فروشگاه برقرار نیست. این بخش فقط هنگام اتصال زنده فعال است.", reqId);
  }
  return serveSnapshotRead(env, session, request, reqId);
};


const authenticatedManagerMutation = async (request, env, reqId) => {
  const url = new URL(request.url);
  if (request.method !== "POST" || !isManagerNotificationMutationPath(url.pathname)) {
    return failure(405, "MINIAPP_MUTATION_NOT_ALLOWED", "این عملیات مجاز نیست.", reqId, { Allow: "POST" });
  }
  if (Number(request.headers.get("content-length") || 0) > 0) {
    return failure(400, "MINIAPP_MUTATION_BODY_NOT_ALLOWED", "این عملیات نیازی به بدنه درخواست ندارد.", reqId);
  }
  if (!sameOriginHeaderIsSafe(request)) return failure(403, "MINIAPP_EDGE_ORIGIN_MISMATCH", "مبدأ درخواست Mini App معتبر نیست.", reqId);
  const auth = await requireSession(request, env, reqId);
  if (auth.error) return auth.error;
  const session = auth.session;
  if (session.identity.kind !== "staff" || !isManagerAssociatedIdentity(session.identity)) {
    return failure(403, "MINIAPP_STAFF_ACCESS_REQUIRED", "دسترسی لازم برای این بخش فعال نیست.", reqId);
  }

  const timeoutMs = numericEnv(env, "KOUROSH_EDGE_LIVE_TIMEOUT_MS", 5000, 1000, 10000);
  let liveSession = session;
  if (!liveSession.localSessionToken && liveSession.liveOrigin && liveSession.initData) {
    const reauth = await proxyLive({
      session: liveSession,
      request,
      path: "/api/miniapp/auth",
      method: "POST",
      body: JSON.stringify({ initData: liveSession.initData, workspaceKind: "manager" }),
      contentType: "application/json",
      timeoutMs,
    });
    observeLiveResult("auth", reauth, { requestId: reqId, route: "/api/miniapp/auth" });
    if (reauth.kind === "response" && reauth.status >= 200 && reauth.status < 300 && reauth.json?.success === true && reauth.json?.data?.sessionToken) {
      const refreshedIdentity = validateLiveIdentity(reauth.json?.data?.identity, liveSession.telegramUserId, "staff");
      if (!refreshedIdentity) return failure(502, "MINIAPP_LIVE_IDENTITY_INVALID", "هویت دریافتی از فروشگاه معتبر نیست.", reqId, liveHeaders());
      liveSession = { ...liveSession, identity: refreshedIdentity, localSessionToken: String(reauth.json.data.sessionToken) };
    } else if (reauth.kind === "response" && reauth.status < 500) {
      return jsonResponse(reauth.status, reauth.json || { success: false, code: "MINIAPP_AUTH_FAILED", message: "دسترسی این حساب فعال نیست." }, reqId, liveHeaders());
  ...
  if (liveSession.localSessionToken && liveSession.liveOrigin) {
    const live = await proxyLive({ session: liveSession, request, path: url.pathname, method: "POST", timeoutMs });
    observeLiveResult("manager_mutation", live, { requestId: reqId, route: url.pathname });
    if (live.kind === "response") {
      return jsonResponse(live.status, live.json || { success: false, code: "MINIAPP_LIVE_RESPONSE_INVALID", message: "پاسخ فروشگاه معتبر نبود." }, reqId, liveHeaders());
    }
  }
  return failure(503, "MINIAPP_STAFF_OFFLINE_UNAVAILABLE", "اتصال زنده به فروشگاه برقرار نیست. دسترسی مدیریتی فقط هنگام اتصال زنده فعال است.", reqId);
};
...
const forbiddenSnapshotKeys = new Set([
  ...
  "currentPurchasePrice", "current_purchase_price", "internalProfit", "internal_profit", "phoneNumber", "phone_number",
...
const managerProfitKeys = new Set([
...
const containsManagerProfitData = (value) => {
  if (Array.isArray(value)) return value.some(containsManagerProfitData);
  if (!value || typeof value !== "object") return false;
  for (const [key, child] of Object.entries(value)) if (managerProfitKeys.has(key) || containsManagerProfitData(child)) return true;
  return false;
};
...
const validateCandidate = (candidate) => {
  ...
  if (!["customer", "partner", "manager"].includes(candidate.subjectKind) || !Number.isSafeInteger(candidate.localSubjectId) || candidate.localSubjectId < 1 || !/^[1-9][0-9]{0,19}$/.test(String(candidate.telegramUserId || ""))) return false;
  ...
  if (candidate.subjectKind === "manager" && validUntil - generated > 60 * 60 * 1000) return false;
  ...
  if (candidate.data) {
    const permissions = Array.isArray(candidate.data.permissions) ? candidate.data.permissions.map(String) : [];
    if (candidate.subjectKind === "manager") {
      if (!permissions.length || permissions.some((permission) => !permission.endsWith(".read"))) return false;
      if (containsManagerProfitData(candidate.data) && !permissions.includes("profits.read")) return false;
    } else if (containsManagerProfitData(candidate.data)) return false;
  }
...
const verifyConnectorSignedRequest = async (request, tenant, rawBody, options) => {
  ...
  const codePrefix = String(options?.codePrefix || "MINIAPP_CONNECTOR");
  if (installationId !== tenant.installationId || credentialVersion !== tenant.credentialVersion || !REQUEST_ID_PATTERN.test(reqId) || !SHA256_PATTERN.test(bodySha) || !SIGNATURE_PATTERN.test(signature)) return { ok: false, code: `${codePrefix}_AUTH_INVALID` };
  const timestampMs = Date.parse(timestamp);
  if (!Number.isFinite(timestampMs) || Math.abs(Date.now() - timestampMs) > SNAPSHOT_SYNC_CLOCK_SKEW_MS) return { ok: false, code: `${codePrefix}_TIMESTAMP_INVALID` };
  if (sha256Hex(rawBody) !== bodySha) return { ok: false, code: `${codePrefix}_BODY_HASH_INVALID` };
  const canonical = [String(options?.canonicalPrefix || "KOUROSH-MINIAPP-CONNECTOR-V1"), "POST", String(options?.path || ""), installationId, String(credentialVersion), reqId, timestamp, bodySha].join("\n");
  try {
    const key = createPublicKey(tenant.publicKeyPem);
    if (key.asymmetricKeyType !== "ed25519" || !verifySignature(null, Buffer.from(canonical, "utf8"), key, base64UrlDecode(signature))) return { ok: false, code: `${codePrefix}_SIGNATURE_INVALID` };
  } catch {
    return { ok: false, code: `${codePrefix}_SIGNATURE_INVALID` };
  }
  return { ok: true, requestId: reqId };
};

const verifySyncRequest = (request, tenant, rawBody) => verifyConnectorSignedRequest(request, tenant, rawBody, {
  canonicalPrefix: "KOUROSH-MINIAPP-SNAPSHOT-SYNC-V1",
  path: SNAPSHOT_SYNC_PATH,
  codePrefix: "MINIAPP_SNAPSHOT_SYNC",
});

const verifyDiagnosticsRequest = (request, tenant, rawBody) => verifyConnectorSignedRequest(request, tenant, rawBody, {
  canonicalPrefix: "KOUROSH-MINIAPP-DIAGNOSTICS-V1",
  path: DIAGNOSTICS_PATH,
  codePrefix: "MINIAPP_DIAGNOSTIC",
});
...
const upsertSnapshotD1 = async (db, snapshot) => {
  ...
  if (snapshot.subjectKind === "manager") {
    const result = await db.prepare(`INSERT INTO manager_snapshots (
      tenant_id, subject_key, installation_id, snapshot_version, schema_version, state, generated_at, received_at, authorization_valid_until, payload_json, content_hash
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(tenant_id, subject_key) DO UPDATE SET
      installation_id = excluded.installation_id, snapshot_version = excluded.snapshot_version, schema_version = excluded.schema_version,
      state = excluded.state, generated_at = excluded.generated_at, received_at = excluded.received_at,
      authorization_valid_until = excluded.authorization_valid_until, payload_json = excluded.payload_json, content_hash = excluded.content_hash
    WHERE manager_snapshots.installation_id = excluded.installation_id
      AND excluded.snapshot_version > manager_snapshots.snapshot_version`).bind(
        snapshot.tenantId, snapshot.subjectKey, snapshot.installationId, snapshot.snapshotVersion, snapshot.schemaVersion, snapshot.state,
        snapshot.generatedAt, snapshot.receivedAt, snapshot.authorizationValidUntil, payloadJson, snapshot.contentHash,
      ).run();
    if (Number(result?.meta?.changes || 0) === 1) return { status: "accepted" };
    const current = await getSnapshot(db, snapshot.tenantId, "manager", snapshot.subjectKey);
  ...
  WHERE subject_snapshots.installation_id = excluded.installation_id
    AND excluded.snapshot_version > subject_snapshots.snapshot_version`).bind(
      ...
      snapshot.schemaVersion, snapshot.state, snapshot.generatedAt, snapshot.receivedAt, snapshot.authorizationValidUntil, payloadJson, snapshot.contentHash,
    ).run();
  if (Number(result?.meta?.changes || 0) === 1) return { status: "accepted" };
...
const ingestSnapshot = async (request, env, reqId) => {
  ...
  const tenantLookup = await tryStorage(() => getInstallationForSync(db, installationId), "snapshot_tenant_lookup");
  ...
  let liveOriginUpdated = false;
  if (Object.prototype.hasOwnProperty.call(body, "liveOrigin")) {
    const signedLiveOrigin = safeOrigin(body.liveOrigin);
    if (!signedLiveOrigin || new URL(signedLiveOrigin).hostname.toLowerCase() === tenant.publicHost.toLowerCase()) {
      return failure(403, "MINIAPP_SNAPSHOT_SYNC_LIVE_ORIGIN_INVALID", "مبدأ زنده امضاشده معتبر نیست.", reqId);
    }
    if (signedLiveOrigin !== tenant.liveOrigin) {
      const liveOriginWrite = await tryStorage(() => db.prepare(
        "UPDATE tenant_installations SET live_origin = ?, updated_at = ? WHERE installation_id = ? AND tenant_id = ? AND status = 'active'"
      ).bind(signedLiveOrigin, nowIso(), tenant.installationId, tenant.tenantId).run(), "snapshot_live_origin_reconcile");
      if (!liveOriginWrite.ok) return storageUnavailable(reqId, "همگام‌سازی مبدأ زنده موقتاً در دسترس نیست.");
      if (Number(liveOriginWrite.value?.meta?.changes || 0) !== 1) return failure(409, "MINIAPP_SNAPSHOT_SYNC_LIVE_ORIGIN_CONFLICT", "مبدأ زنده قابل همگام‌سازی نیست.", reqId);
      tenant.liveOrigin = signedLiveOrigin;
      liveOriginUpdated = true;
    }
  }
  ...
  if (!writeAttempt.ok) {
    if (stored.subjectKind === "manager" && managerSnapshotSchemaMissing(writeAttempt.error)) return managerSchemaMigrationRequired(reqId);
    return storageUnavailable(reqId, "ذخیره‌سازی Snapshot موقتاً در دسترس نیست.");
  }
  const write = writeAttempt.value;
  if (write.status === "accepted" || write.status === "idempotent") return success({ result: write.status, snapshotVersion: stored.snapshotVersion, state: stored.state, liveOriginUpdated }, reqId);
...
const inspectDiagnosticSnapshot = async (request, env, reqId) => {
  if (request.method !== "POST") return failure(405, "MINIAPP_DIAGNOSTIC_METHOD_NOT_ALLOWED", "روش درخواست مجاز نیست.", reqId, { Allow: "POST" });
  const url = new URL(request.url);
  if (url.search) return failure(400, "MINIAPP_DIAGNOSTIC_QUERY_NOT_ALLOWED", "پارامتر اضافی در درخواست تشخیصی مجاز نیست.", reqId);
  if (!isJsonContentType(request)) return failure(415, "MINIAPP_DIAGNOSTIC_CONTENT_TYPE_REQUIRED", "درخواست تشخیصی باید JSON باشد.", reqId);
  const db = env.KOUROSH_EDGE_DB;
  if (!db) return failure(503, "MINIAPP_EDGE_STORAGE_UNAVAILABLE", "ذخیره‌سازی Edge آماده نیست.", reqId);
  let parsed;
  try { parsed = await readJsonBody(request, DIAGNOSTICS_MAX_BODY_BYTES); } catch (error) {
    const status = Number(error?.status || 400);
    return failure(status, status === 413 ? "MINIAPP_DIAGNOSTIC_BODY_TOO_LARGE" : "MINIAPP_DIAGNOSTIC_BODY_INVALID", status === 413 ? "حجم درخواست تشخیصی بیش از حد مجاز است." : "بدنه درخواست تشخیصی معتبر نیست.", reqId);
  }
  const installationId = String(request.headers.get("x-kourosh-installation-id") || "");
  if (!INSTALLATION_ID_PATTERN.test(installationId)) return failure(401, "MINIAPP_DIAGNOSTIC_AUTH_INVALID", "اعتبارنامه تشخیصی معتبر نیست.", reqId);
  const tenantLookup = await tryStorage(() => getInstallationForSync(db, installationId), "diagnostic_tenant_lookup");
  if (!tenantLookup.ok) return storageUnavailable(reqId, "ذخیره‌سازی تشخیصی Edge موقتاً در دسترس نیست.");
  const tenant = tenantLookup.value;
  if (!tenant) return failure(401, "MINIAPP_DIAGNOSTIC_AUTH_INVALID", "اعتبارنامه تشخیصی معتبر نیست.", reqId);
  const verified = await verifyDiagnosticsRequest(request, tenant, parsed.raw);
  if (!verified.ok) return failure(401, verified.code, "امضای درخواست تشخیصی معتبر نیست.", reqId);
  const replay = await tryStorage(() => consumePersistentReplay(db, tenant.installationId, verified.requestId), "diagnostic_replay_guard");
  if (!replay.ok) return storageUnavailable(reqId, "ذخیره‌سازی تشخیصی Edge موقتاً در دسترس نیست.");
  if (!replay.value) return failure(409, "MINIAPP_DIAGNOSTIC_REPLAY_REJECTED", "درخواست تشخیصی تکراری رد شد.", reqId);

  const body = parsed.value;
  const subjectKind = String(body?.subjectKind || "");
  const telegramUserId = String(body?.telegramUserId || "");
  if (body?.protocolVersion !== 1 || String(body?.botId || "") !== tenant.botId || !["customer", "partner", "manager"].includes(subjectKind) || !/^[1-9][0-9]{0,19}$/.test(telegramUserId)) {
    return failure(400, "MINIAPP_DIAGNOSTIC_SCOPE_INVALID", "محدوده درخواست تشخیصی معتبر نیست.", reqId);
  }
  const subjectKey = deriveTelegramSubjectKey(env, tenant.tenantId, tenant.botId, telegramUserId);
  const snapshotLookup = await tryStorage(() => getSnapshot(db, tenant.tenantId, subjectKind, subjectKey), "diagnostic_snapshot_lookup");
  if (!snapshotLookup.ok) return storageUnavailable(reqId, "بررسی Snapshot روی Edge موقتاً در دسترس نیست.");
  const snapshot = snapshotLookup.value;
  return success({
    edgeVersion: EDGE_VERSION,
    ...
    liveOriginConfigured: Boolean(tenant.liveOrigin),
  }, reqId);
};
...
const handleApi = async (request, env, reqId) => {
  ...
  if (isManagerNotificationMutationPath(url.pathname)) return authenticatedManagerMutation(request, env, reqId);
  if (url.pathname === "/api/miniapp/me" || url.pathname.startsWith("/api/miniapp/customer/") || url.pathname.startsWith("/api/miniapp/partner/") || url.pathname.startsWith("/api/miniapp/staff/") || url.pathname.startsWith("/api/miniapp/manager/")) return authenticatedRead(request, env, reqId);
...
export const createKouroshMiniAppEdgeHandler = () => ({
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === DIAGNOSTICS_PATH) return await inspectDiagnosticSnapshot(request, env, reqId);
