...
const loadingButtonQaPath = '/__qa/loading-buttons';
const dashboardVisualQaPath = '/__qa/dashboard-visual';
const styleVisualQaPath = '/__qa/style-visual';

export const publicRoutes: AppRouteDefinition[] = [
  route('/login', <Page.LoginPage />),
  route('/setup', <Page.InitialSetupPage />),
  route('/install', <Page.InstallApp />),
  ...(import.meta.env.VITE_LOADING_BUTTON_QA === '1'
    ? [route(loadingButtonQaPath, <Page.LoadingButtonRuntimeMatrix />)]
    : []),
  ...(import.meta.env.VITE_DASHBOARD_VISUAL_QA === '1'
    ? [route(dashboardVisualQaPath, <Page.DashboardRuntimeVisualMatrix />)]
    : []),
  ...(import.meta.env.VITE_STYLE_VISUAL_QA === '1'
    ? [route(styleVisualQaPath, <Page.StyleSettingsRuntimeVisualMatrix />)]
    : []),
];
...
export const reportRoutes: AppRouteDefinition[] = [
  ...
  route('all-sales-ledger', <Page.AllSalesLedgerReport />),
  ...
  route('ml-operator-overview', gated('smart_insights', <Page.MlOperatorOverviewPage />)),
...
export const roleRouteGroups: RoleRouteGroup[] = [
  ...
  },
  {
    ...
    routes: [
      ...
      route('/accounting-reconciliation', <Page.AccountingReconciliationCenter />),
    ],
  },
  {
    id: 'accounting-drilldown',
    label: 'Accounting drill-down',
    allowedRoles: ['Admin', 'Manager', 'Salesperson'],
    routes: [
      route('/accounting-drilldown/:kind/:id', <Page.AccountingDrilldownPage />),
    ],
  },
  {
    id: 'admin-settings',
    ...
    routes: [
      ...
      route('/settings/:settingsTab', <Page.Settings />),
    ],
  },
];
