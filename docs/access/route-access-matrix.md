...
| `audit-log:accounting-reconciliation` | `/accounting-reconciliation` | `role-protected` | Admin, Manager | — | Read-only live accounting reconciliation center; no financial mutation endpoint. |
...
| `admin-settings:settings-tab` | `/settings/:settingsTab` | `role-protected` | Admin | — | Canonical route-backed settings tabs rendered inside the single application sidebar. |
