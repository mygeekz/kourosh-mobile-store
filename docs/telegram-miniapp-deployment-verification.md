...
## Final validation continuation — 2026-09-08, before full restart

- Re-ran all 17 release checks successfully. Read-only production Bot `getMe` through the actual SystemTelegramTransport returned HTTP 200, success true, and the expected Bot ID. This confirms current transport/credentials, not receipt of `/start` from a user. Logs also contain intermittent polling timeouts to `10.10.34.36:443`; reliability must be checked after restart and real commands.
- No Telegram session is accessible through the available UI controls (only an empty Codex browser). Real Manager-only, Manager+Partner, Partner-only and Customer-only results are still pending user-assisted verification. No production identities were fabricated or reassigned.
- No existing Kourosh scheduled task or Startup item was found for API/gateway. Added `scripts/start-miniapp-production.ps1` and registered the current-user Startup link `Kourosh Mini App.lnk` to run it hidden. It starts the existing API and gateway only when absent, refuses an unrecognized port owner, avoids duplicate initializing runtimes, and records sanitized boot/startup evidence under `%LOCALAPPDATA%/Kourosh/miniapp-startup/startup-result.json`. Both verify-only and normal invocation passed while correctly reusing existing processes.
- This is **logon startup**, not an unattended pre-login Windows service. The existing tunnel is an Automatic Windows service. A Windows login is needed for the application startup and its user-profile configuration. No automatic login or saved credentials were added.
- Baseline boot time before the requested restart: `2026-09-07T08:47:33.5000000Z`. Restart survival remains pending until the recorded boot time advances and public/API/Bot/snapshot checks pass after boot. A process restart must not be substituted for this evidence.
- Exact accounting warning: `Legacy accounting reconciliation failed: SQLITE_CONSTRAINT: customer_ledger financial fields are immutable; use reversal + replacement`. Logged in `runPostSeedDataRepairs`, `server/db/migrations/dataRepair.ts:20`, following `runLegacyAccountingReconciliation`. The guard in `server/db/schema/audit.schema.ts` prohibits changes to financial and reference fields. Reconciliation wraps its repair operations in a transaction and rolls back on failure; the error is caught so API startup continues. These files are unchanged from pre-repair commit `6407bc3`. No accounting data or guards were changed. Authentication/sync is operational despite this warning; correctness of unreconciled financial dashboard figures is not certified by connectivity tests.

After reboot: log into the same Windows account, reopen this Codex task, read the startup-result boot time, verify ports 3001/4180 and Cloudflared HTTP/2, inspect fresh polling/snapshot diagnostics, then check production D1 receipt times. Do not restart again just because this checklist is present. All real Telegram identity cases and dashboard results must be recorded before declaring production ready.
