...
for (const file of targetFiles) {
  ...
  for (const opening of selectOpenings) {
    ...
    else if (!hasUnstyled && !hasHiddenChevron) styledCanonicalMode += 1;
...
assert.equal(
  ...
  0,
  `SelectField legacy compatibility mode must be all-or-nothing (controlOnly + unstyled + showChevron={false}); canonical styled controlOnly remains valid; found ${partialCompatibilityMode} partial controls.`,
);
