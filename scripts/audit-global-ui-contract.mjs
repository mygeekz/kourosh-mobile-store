...
const selectFieldSource = read('components/ui/SelectField.tsx');
...
expect(selectFieldSource.includes('!ps-10'), 'SelectField leading-icon state must reserve logical leading space for selected text.');
expect(selectFieldSource.includes('truncate rounded-2xl'), 'SelectField must keep selected text stable inside its utility-owned control.');
