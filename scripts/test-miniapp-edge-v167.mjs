...
class FakeD1 {
  ...
  async run(sql, args) {
    ...
    if (sql.startsWith("UPDATE tenant_installations SET live_origin")) {
      const [nextLiveOrigin, updatedAt, installation, tenant] = args;
      const row = this.tenants.get(String(installation));
      if (!row || row.status !== "active" || row.tenant_id !== tenant) return { meta: { changes: 0 } };
      row.live_origin = String(nextLiveOrigin);
...
const signSyncRequest = (candidate, options = {}) => {
  const requestId = options.requestId || b64u(randomBytes(18));
  const body = JSON.stringify({ protocolVersion: 1, botId, candidate, ...(options.liveOrigin ? { liveOrigin: options.liveOrigin } : {}) });
...
// A signed local snapshot sync repairs stale D1 live_origin after settings reset/restore.
tenantRow.live_origin = "https://stale-live.example.com";
response = await edge.fetch(signSyncRequest(makeCandidate(3, customerData("Snapshot v3")), { liveOrigin }), env);
assert.equal(response.status, 200);
const liveOriginRepair = await response.json();
assert.equal(liveOriginRepair.data.liveOriginUpdated, true);
assert.equal(tenantRow.live_origin, liveOrigin);

// The public Mini App host can never be installed as its own live origin.
response = await edge.fetch(signSyncRequest(makeCandidate(4, customerData("Snapshot v4")), { liveOrigin: `https://${publicHost}` }), env);
assert.equal(response.status, 403);
assert.equal((await response.json()).code, "MINIAPP_SNAPSHOT_SYNC_LIVE_ORIGIN_INVALID");
...
assert.equal(response.headers.get("x-kourosh-snapshot-version"), "3");
...
assert.equal((await response.json()).data.customer.fullName, "Snapshot v3");
