...
const gradientMatches = [...css.matchAll(/(?:linear|radial)-gradient\(/g)];
assert(contract.surfacePolicy?.progressIndicatorGradientAllowed === true, 'Clock manifest must explicitly document the live progress-indicator gradient exception.');
assert(gradientMatches.length <= 1, 'Canonical clock contract may use a gradient only for the live progress indicator.');
assert(
  gradientMatches.length === 0 || /\.app-dashboard-clock__progress > span\s*\{[\s\S]*?(?:linear|radial)-gradient\(/.test(css),
  'Clock gradient, when present, must stay scoped to the live progress indicator rather than a card/surface.',
);
...
assert(styleEntry?.bootstrapOrder === contract.runtimeOrder, 'Clock style runtime order must match the reviewed clock manifest.');
