...
export const stopExpectedWindowsStableTunnelProcess = (pid, options = {}) => {
  if (!Number.isSafeInteger(Number(pid)) || Number(pid) <= 0) {
    throw Object.assign(new Error("CLOUDFLARE_TUNNEL_PROCESS_ID_INVALID"), { code: "CLOUDFLARE_TUNNEL_PROCESS_ID_INVALID" });
  }
  ...
  if (result.error || result.status !== 0) {
    throw Object.assign(new Error("CLOUDFLARE_TUNNEL_PROCESS_STOP_FAILED"), { code: "CLOUDFLARE_TUNNEL_PROCESS_STOP_FAILED" });
  }
};

const waitForStableTunnelProcessExit = async (pid, tunnelId, options = {}) => {
  const inspect = options.inspectExisting || inspectExistingCloudflared;
  const deadline = Date.now() + boundedMs(options.processExitTimeoutMs, 4_000, 500, 10_000);
  while (Date.now() < deadline) {
    const processes = await inspect(options);
    const stillRunning = processes.some((item) => Number(item?.pid) === Number(pid) && isExpectedCloudflaredNamedTunnelProcess(item, tunnelId));
    if (!stillRunning) return true;
    await (options.sleepImpl || sleep)(150);
  }
  return false;
};
...
export const startOrReuseWindowsStableTunnel = async (preflight, options = {}) => {
  ...
  // Always materialize the current hostname mapping before deciding to reuse a
  // running cloudflared process. Settings can be restored/changed while an old
  // named-tunnel process is still alive; returning before this write used to
  // leave the on-disk runtime config stale and made the UI look configured
  // while Cloudflare kept routing the previous Live Origin.
  ...
  if (existing) {
    const healthy = await (options.checkExistingHealth || waitForStableLiveOrigin)(runtime.liveOriginUrl, {
      ...options,
      timeoutMs: boundedMs(options.existingHealthTimeoutMs, 3_500, 500, 10_000),
      intervalMs: 500,
    });
    if (healthy) return { action: "reuse", pid: Number(existing.pid) || null, liveOriginUrl: runtime.liveOriginUrl, tunnelId: runtime.tunnelId, configFile: config.configFile };
    await (options.stopExpectedProcess || stopExpectedWindowsStableTunnelProcess)(Number(existing.pid), options);
    const exited = await waitForStableTunnelProcessExit(Number(existing.pid), runtime.tunnelId, options);
    if (!exited) throw Object.assign(new Error("CLOUDFLARE_TUNNEL_PROCESS_DID_NOT_STOP"), { code: "CLOUDFLARE_TUNNEL_PROCESS_DID_NOT_STOP" });
    restartReason = "live_origin_unhealthy";
  }
  ...
  child.unref?.();
  return { action: restartReason ? "restarted_unhealthy" : "started", pid: child.pid || null, liveOriginUrl: runtime.liveOriginUrl, tunnelId: runtime.tunnelId, configFile: config.configFile, source: cloudflared.source, restartReason };
};
...
const main = async () => {
  ...
  process.stdout.write(`[STABLE TUNNEL] ${result.action === "reuse" ? "Reusing" : result.action === "restarted_unhealthy" ? "Restarted unhealthy" : "Started"} Cloudflare Named Tunnel${result.pid ? ` PID ${result.pid}` : ""}.\n`);
