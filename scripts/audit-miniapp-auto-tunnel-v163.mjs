...
const telegramBotHelpers = read("server/utils/telegramBotHelpers.ts");
const telegramUpdateHandler = read("server/bootstrap/telegram/telegramUpdateHandlerCore.ts");
...
assert.match(menuService, /chat_id: scopedChatId/, "Menu sync must support explicit private-chat Menu Button reconciliation");
assert.match(telegramBotHelpers, /syncTelegramMenuButton\(effectiveSettings, \{ chatId \}\)/, "Persistent /start menu reconciliation must use the verified sync service instead of an in-memory fingerprint cache");
assert.doesNotMatch(telegramBotHelpers, /createTelegramMenuButtonEnsurer/, "Persistent menu path must not use the stale in-memory fingerprint helper");
assert.match(telegramUpdateHandler, /ensureTelegramPersistentMenu\(chatId\)/, "Telegram /start and menu flows must reconcile the current private chat scope");
...
assert.match(panel, /منوی تلگرام:/, "Settings UI must show Telegram Menu sync status without exposing internals");
