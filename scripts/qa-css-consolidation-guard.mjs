...
const bootstrapStylesPath = path.join(root, 'app/bootstrap/styles.ts');
const bootstrapStyles = fs.existsSync(bootstrapStylesPath) ? fs.readFileSync(bootstrapStylesPath, 'utf8') : '';
...
for (const importPath of requiredImports) {
  assert.ok(styleEntries.includes(importPath), `Missing required consolidated CSS import: ${importPath}`);
...
for (const oldImport of removedImports) {
  assert.ok(!styleEntries.includes(oldImport), `Old stage CSS import is still present: ${oldImport}`);
}
