...
const files = {
  ...
  navigationContext: 'utils/navigationContext.ts',
...
if (exists(files.sidebarHook)) {
  ...
  if (sidebarHook.includes('sidebarVariant')) failures.push('useMainLayoutSidebar.ts must not restore the retired sidebarVariant branch.');
}
...
if (exists(files.titleHook)) {
  ...
  for (const token of [
    ...
    'resolveNavigationContext(pathname).pageTitle',
  ]) {
    if (!titleHook.includes(token)) failures.push(`useCurrentPageTitle.ts missing centralized title token: ${token}`);
  }
  for (const forbidden of ['findBestMatch(', 'SIDEBAR_ITEMS', 'فاکتور فروش شماره']) {
    if (titleHook.includes(forbidden)) failures.push(`useCurrentPageTitle.ts must not duplicate centralized navigation-title logic: ${forbidden}`);
  }
}

if (exists(files.navigationContext)) {
  const navigationContext = read(files.navigationContext);
  for (const token of [
    'فاکتور فروش شماره',
    'مرکز پیگیری وصول',
    'پیشنهادهای هوشمند خرید',
    'findBestPathMatch(pathname, SIDEBAR_ITEMS)',
    'پیشخوان مدیریتی کوروش',
    'SIDEBAR_NAV_SECTIONS',
  ]) {
    if (!navigationContext.includes(token)) failures.push(`navigationContext.ts missing centralized navigation/title behavior token: ${token}`);
  }
}
