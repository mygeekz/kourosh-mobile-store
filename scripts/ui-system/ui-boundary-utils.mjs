...
export const findDirectCssImports = () => {
  const runtimeStyleRoots = ['app', 'components', 'pages', 'miniapp'];
  const allowedDirectCssImportFiles = new Set(
    (uiManifest.styleSystem?.directCssImportsAllowedIn ?? []).map((file) => toPosix(file)),
  );
  const codeFiles = runtimeStyleRoots.flatMap((sourceRoot) => {
    const absoluteRoot = path.join(projectRoot, sourceRoot);
    if (!fs.existsSync(absoluteRoot)) return [];
    return walkFiles(absoluteRoot, (file) => /\.(?:ts|tsx|js|jsx)$/.test(file));
  });
  const violations = [];
  for (const absolute of codeFiles) {
    const relative = toPosix(path.relative(projectRoot, absolute));
    if (allowedDirectCssImportFiles.has(relative)) continue;
    const source = fs.readFileSync(absolute, 'utf8');
