...
assert.match(buildEnsurer, /Valid production output matches source identity[\s\S]*reusing dist\//, 'Daily startup must reuse a validated production build when both release and source fingerprint still match.');
...
assert.match(buildEnsurer, /createHash[\s\S]*computeSourceFingerprint[\s\S]*source fingerprint changed/i, 'Staged UI source changes under the same release marker must invalidate stale dist through the reviewed source fingerprint.');
...
assert.match(installPage, /label: 'نسخه قابل نصب'[\s\S]*diagnostics\?\.productionRuntime[\s\S]*نسخه آماده نصب برنامه در حال اجرا است/, 'The install page must expose whether the production PWA runtime is actually being served.');
...
assert.match(installPage, /<details[\s\S]*جزئیات بررسی نصب/, 'Verbose diagnostics must remain collapsible so the primary mobile action stays reachable.');
...
assert.match(settingsLocalPanel, /بازنشانی برنامه نصب‌شده/, 'Local settings must expose the reviewed administrator PWA reset action.');
