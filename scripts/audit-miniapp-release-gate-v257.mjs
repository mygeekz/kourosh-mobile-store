...
const runner = read("scripts/verify-miniapp-release-v257.mjs");

assert.equal(KOUROSH_RELEASE, "v257");
assert.equal(manifest.release, "v257");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v257.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v257.mjs --full");
assert.equal(pkg.scripts["test:v257"], "npm run verify:miniapp:source");
...
for (const required of [
  "audit:miniapp-release-v257",
  "audit:miniapp-rc-v257",
  "test:miniapp-rc-evidence-verifier-v257",
  "test:miniapp-rc-role-matrix-v257",
  "test:miniapp-rc-infrastructure-matrix-v257",
  "test:messaging-v257",
...
for (const required of [
  "verify:miniapp:environment-v257",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v257",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v257"]);
