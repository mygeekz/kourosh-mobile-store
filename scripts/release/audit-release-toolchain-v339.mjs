...
const release = read(".github/workflows/release-gate-v339.yml");

const sourceNumber = Number(source.replace(/^v/, ""));
assert.ok(Number.isInteger(sourceNumber) && sourceNumber >= 339, `release toolchain audit requires v339 or successor; found ${source}`);
...
assert.match(release, /ci:deps:v339:offline/, "fresh runner must replay dependency installation offline");
...
for (const key of ["ci:deps:v339", "ci:deps:v339:offline", "verify:release:toolchain-v339", "audit:release-toolchain-v339", "verify:release:v339"]) {
