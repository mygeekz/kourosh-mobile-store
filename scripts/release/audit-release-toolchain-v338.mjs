...
const release = read(".github/workflows/release-gate-v338.yml");

const sourceNumber = Number(source.replace(/^v/, ""));
assert.ok(Number.isInteger(sourceNumber) && sourceNumber >= 338, `release toolchain audit requires v338 or successor; found ${source}`);
...
assert.match(release, /ci:deps:v338:offline/, "fresh runner must replay dependency installation offline");
...
for (const key of ["ci:deps:v338", "ci:deps:v338:offline", "verify:release:toolchain-v338", "audit:release-toolchain-v338", "verify:release:v338"]) {
