...
const runner = read("scripts/verify-miniapp-release-v265.mjs");

assert.equal(KOUROSH_RELEASE, "v265");
assert.equal(manifest.release, "v265");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v265.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v265.mjs --full");
assert.equal(pkg.scripts["test:v265"], "npm run verify:miniapp:source");
...
for (const required of [
  ...
  "audit:repair-detail-no-custom-css-v265",
  ...
  "test:miniapp-rc-evidence-verifier-v265",
  "test:miniapp-rc-role-matrix-v265",
  "test:miniapp-rc-infrastructure-matrix-v265",
  "test:messaging-v259",
...
for (const required of [
  "verify:miniapp:environment-v265",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v265",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v265"]);
