...
const roleMatrix = read("scripts/test-miniapp-rc-role-matrix-v259.mjs");
const infraMatrix = read("scripts/test-miniapp-rc-infrastructure-matrix-v259.mjs");
const evidenceVerifier = read("scripts/verify-miniapp-rc-evidence-v259.mjs");
const evidenceTemplate = JSON.parse(read("config/quality/miniapp-rc-evidence-v259.example.json"));
...
assert.equal(pkg.scripts["verify:miniapp:rc"], "npm run verify:miniapp && npm run verify:miniapp:rc:evidence-v259");
for (const script of ["test:miniapp-rc-evidence-verifier-v259", "test:miniapp-rc-role-matrix-v259", "test:miniapp-rc-infrastructure-matrix-v259"]) {
  assert.ok(manifest.sourceGate.some((entry) => entry.script === script), `RC source gate missing ${script}`);
}
assert.ok(manifest.externalEvidenceGate.some((entry) => entry.script === "verify:miniapp:rc:evidence-v259"));
...
assert.equal(evidenceTemplate.release, "v259");
assert.ok(evidenceTemplate.checks.every((item) => item.status === "PENDING"), "example evidence must never ship as fake PASS evidence");
assert.equal(fs.existsSync("config/quality/miniapp-rc-evidence-v259.json"), false, "repository must not ship fabricated production RC evidence");
assert.match(read(".gitignore"), /^config\/quality\/miniapp-rc-evidence-v259\.json$/m, "local RC evidence must stay uncommitted");
