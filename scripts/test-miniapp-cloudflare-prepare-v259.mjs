...
assert.ok(Number(KOUROSH_RELEASE.slice(1)) >= 256, `expected v259 or a compatible successor; found ${KOUROSH_RELEASE}`);
const root = fs.mkdtempSync(path.join(os.tmpdir(), "kourosh-miniapp-prepare-v259-"));
try {
  ...
  fs.writeFileSync(path.join(distDir, "miniapp.html"), html("v254"), "utf8");

  assert.throws(
    () => prepareMiniAppCloudflarePages({ root }),
    new RegExp(`dist-miniapp release mismatch: built=v254, source=${KOUROSH_RELEASE}`),
  );
