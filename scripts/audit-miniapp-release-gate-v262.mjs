...
const runner = read("scripts/verify-miniapp-release-v262.mjs");

assert.equal(KOUROSH_RELEASE, "v262");
assert.equal(manifest.release, "v262");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v262.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v262.mjs --full");
assert.equal(pkg.scripts["test:v262"], "npm run verify:miniapp:source");
...
for (const required of [
  "audit:miniapp-release-v262",
  "audit:form-controls-no-custom-css-v262",
  "audit:expenses-no-custom-css-v262",
  "audit:settings-inventory-form-primitives",
  "audit:select-foundation",
  "audit:dialog-form-primitives",
  "audit:style-manifest",
  "audit:miniapp-rc-v262",
  "test:miniapp-rc-evidence-verifier-v262",
  "test:miniapp-rc-role-matrix-v262",
  "test:miniapp-rc-infrastructure-matrix-v262",
  "test:messaging-v259",
...
for (const required of [
  "verify:miniapp:environment-v262",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v262",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v262"]);
