...
const storeOwnershipSource = fs.readFileSync(path.resolve(root, 'pages/settings/StoreOwnershipPage.tsx'), 'utf8');
const storeOwnershipCheckboxes = (storeOwnershipSource.match(/<CheckboxField\b/g) ?? []).length;
assert.equal(storeOwnershipCheckboxes, 4, `StoreOwnershipPage canonical checkbox baseline changed; expected 4, found ${storeOwnershipCheckboxes}.`);
assert.ok(totals.CheckboxField >= 4, `Expected at least 4 canonical CheckboxField consumers; found ${totals.CheckboxField}.`);
