...
const runner = read("scripts/verify-miniapp-release-v260.mjs");

assert.equal(KOUROSH_RELEASE, "v260");
assert.equal(manifest.release, "v260");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v260.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v260.mjs --full");
assert.equal(pkg.scripts["test:v260"], "npm run verify:miniapp:source");
...
for (const required of [
  "audit:miniapp-release-v260",
  "test:form-field-vertical-metrics-v260",
  "audit:settings-inventory-form-primitives",
  "audit:select-foundation",
  "audit:dialog-form-primitives",
  "audit:style-manifest",
  "audit:miniapp-rc-v260",
  "test:miniapp-rc-evidence-verifier-v260",
  "test:miniapp-rc-role-matrix-v260",
  "test:miniapp-rc-infrastructure-matrix-v260",
  "test:messaging-v259",
...
for (const required of [
  "verify:miniapp:environment-v260",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v260",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v260"]);
