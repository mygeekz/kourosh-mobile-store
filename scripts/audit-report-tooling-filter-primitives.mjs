...
assert.match(integratedMobileAnalyticsSource, /<Drawer[\s\S]*?data-report-drawer-kind['\"]?:?[^\n]*mobile-sales-risk/, 'MobileSalesAnalytics risk detail must use the canonical Drawer.');
...
assert.match(reportModalVisualRegressionSource, /data-dialog-layer=\"drawer\"[^\n]*:has\(#mobile-sales-risk-panel\)/, 'Report visual regression must target the canonical Drawer shell for mobile-sales risk details.');
...
assert.match(collectionCenterSource, /<Drawer[\s\S]*?data-report-drawer-kind['\"]?:?[^\n]*collection-followup/, 'CollectionFollowupCenter detail must use the canonical Drawer.');
