...
assert.ok(Number(KOUROSH_RELEASE.slice(1)) >= 252, `expected v252 or a compatible successor; found ${KOUROSH_RELEASE}`);
const root = fs.mkdtempSync(path.join(os.tmpdir(), "kourosh-miniapp-prepare-v252-"));
try {
  ...
  fs.writeFileSync(path.join(distDir, "miniapp.html"), html("v249"), "utf8");

  assert.throws(
    () => prepareMiniAppCloudflarePages({ root }),
    new RegExp(`dist-miniapp release mismatch: built=v249, source=${KOUROSH_RELEASE}`),
  );
