...
const runner = read("scripts/verify-miniapp-release-v263.mjs");

assert.equal(KOUROSH_RELEASE, "v263");
assert.equal(manifest.release, "v263");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v263.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v263.mjs --full");
assert.equal(pkg.scripts["test:v263"], "npm run verify:miniapp:source");
...
for (const required of [
  ...
  "audit:miniapp-rc-v263",
  "test:miniapp-rc-evidence-verifier-v263",
  "test:miniapp-rc-role-matrix-v263",
  "test:miniapp-rc-infrastructure-matrix-v263",
  "test:messaging-v259",
...
for (const required of [
  "verify:miniapp:environment-v263",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v263",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v263"]);
