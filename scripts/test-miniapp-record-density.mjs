// Built application with synthetic records; no production credentials or traffic.
...
const output = await fs.mkdtemp(path.join(os.tmpdir(), `kourosh-records-${before ? 'before' : 'after'}-`));
...
const scenes = [ ['partner','/phones','settlements'], ['partner','/purchases','goods'], ['partner','/ledger','ledger'], ['manager','/directory','directory'], ['customer','/account','customer-ledger'] ];
...
try {
 for (const [role,route,name] of scenes) {
  ...
  const selector=`.${role}-record`;
  const load=async()=>{await page.goto(origin+'/miniapp.html');await page.waitForSelector(selector);await page.evaluate(async()=>{await document.fonts.ready;await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));});};
  ...
  report.scenes.push({name,role,route,heights,money:baselineMoney});
  if(baseline){
   ...
   assert.ok(heights.every((height,i)=>height<previous.heights[i]),`${name}: records should be shorter`);
  }
  if(!before) {
   assert.ok(await page.$(`${selector} .miniapp-record-icon svg`),`${name}: semantic icon`);
   for(const extreme of [false,true]) {
    ...
    for(const width of [320,360,390,430])for(const size of [16,24,32]) {
     ...
     const headingFit=await page.$$eval(`${selector} .miniapp-record-heading`,heads=>heads.every(head=>{
