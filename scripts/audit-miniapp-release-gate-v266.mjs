...
const runner = read("scripts/verify-miniapp-release-v266.mjs");

assert.equal(KOUROSH_RELEASE, "v266");
assert.equal(manifest.release, "v266");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v266.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v266.mjs --full");
assert.equal(pkg.scripts["test:v266"], "npm run verify:miniapp:source");
...
for (const required of [
  "audit:miniapp-release-v266",
  "audit:form-controls-no-custom-css-v263",
  "audit:expenses-no-custom-css-v263",
  "audit:addrepair-no-custom-css-v263",
  "audit:repair-detail-no-custom-css-v266",
  "audit:management-directory-table-standard-v265",
  "audit:management-directory-reference",
  "audit:repairs-workspace-no-custom-css-v266",
  "audit:tailwind-entry-integrity-v266",
  "audit:production-style-prebuild-v266",
  "audit:settings-inventory-form-primitives",
  "audit:select-foundation",
  "audit:dialog-form-primitives",
  "audit:style-manifest",
  "audit:miniapp-rc-v266",
  "test:miniapp-rc-evidence-verifier-v266",
  "test:miniapp-rc-role-matrix-v266",
  "test:miniapp-rc-infrastructure-matrix-v266",
  "test:messaging-v259",
...
for (const required of [
  "verify:miniapp:environment-v266",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v266",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v266"]);
