...
assert.ok(Number(KOUROSH_RELEASE.slice(1)) >= 254, `expected v254 or a compatible successor; found ${KOUROSH_RELEASE}`);
const root = fs.mkdtempSync(path.join(os.tmpdir(), "kourosh-miniapp-prepare-v254-"));
try {
  ...
  fs.writeFileSync(path.join(distDir, "miniapp.html"), html("v253"), "utf8");

  assert.throws(
    () => prepareMiniAppCloudflarePages({ root }),
    new RegExp(`dist-miniapp release mismatch: built=v253, source=${KOUROSH_RELEASE}`),
  );
