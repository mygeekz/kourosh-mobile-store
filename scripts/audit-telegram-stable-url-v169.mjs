...
assert.match(settingsPanel, /فعال‌سازی Mini App اصلی در تنظیمات ربات فقط یک‌بار انجام می‌شود/);
assert.match(settingsPanel, /نشانی ثابت Mini App/);
assert.match(settingsPanel, /با راه‌اندازی دوباره برنامه تغییر نمی‌کند/);
