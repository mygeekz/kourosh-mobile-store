...
const runner = read("scripts/verify-miniapp-release-v258.mjs");

assert.equal(KOUROSH_RELEASE, "v258");
assert.equal(manifest.release, "v258");
assert.equal(manifest.schemaVersion, 2);
assert.equal(manifest.releaseCandidate, true);
assert.equal(pkg.scripts["verify:miniapp:source"], "node scripts/verify-miniapp-release-v258.mjs --source");
assert.equal(pkg.scripts["verify:miniapp"], "node scripts/verify-miniapp-release-v258.mjs --full");
assert.equal(pkg.scripts["test:v258"], "npm run verify:miniapp:source");
...
for (const required of [
  "audit:miniapp-release-v258",
  "audit:miniapp-rc-v258",
  "test:miniapp-rc-evidence-verifier-v258",
  "test:miniapp-rc-role-matrix-v258",
  "test:miniapp-rc-infrastructure-matrix-v258",
  "test:messaging-v258",
  "test:message-preview-studio-v258",
...
for (const required of [
  "verify:miniapp:environment-v258",
  "typecheck:miniapp",
  ...
  "verify:miniapp:built-v258",
]) assert.ok(fullScripts.includes(required), `full gate missing required release check: ${required}`);
assert.deepEqual(evidenceScripts, ["verify:miniapp:rc:evidence-v258"]);
