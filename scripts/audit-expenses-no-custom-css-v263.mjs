...
assert.match(expenses, /className="grid min-w-0 grid-cols-1 overflow-hidden lg:grid-cols-\[minmax\(0,1fr\)_(?:16|18)rem\]"/, 'expense dialogs must use utility-based responsive split composition');
assert.match(expenses, /className="absolute inset-x-0 (?:top-full z-30|top-\[calc\(100%\+0\.5rem\)\] z-40)/, 'expense title combobox popup must be utility styled in the component');
