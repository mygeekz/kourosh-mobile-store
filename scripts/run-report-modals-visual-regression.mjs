...
try {
  ...
  for (const viewport of viewports) {
    ...
    for (const theme of themes) {
      ...
      await capture({ id: 'collection-followup', theme, viewport, frameSelector: '[data-kourosh-layer="drawer-backdrop"]', surfaceSelector: '[data-report-drawer-kind="collection-followup"][data-kourosh-layer="drawer"]' });
      ...
      await capture({ id: 'mobile-sales-risk', theme, viewport, frameSelector: '.kourosh-dialog-shell[data-dialog-layer="drawer"]:has(#mobile-sales-risk-panel)', surfaceSelector: '#mobile-sales-risk-panel' });
      ...
      for (const harnessId of ['ml-operator', 'metadata-import', 'message-composer']) {
        ...
        if (harnessId === 'message-composer') {
          ...
          await capture({ id: harnessId, theme, viewport, frameSelector: '[data-kourosh-layer="drawer-backdrop"]', surfaceSelector: `[data-report-drawer-kind="${harnessId}"][data-kourosh-layer="drawer"]` });
        }
      }
    }
  }
