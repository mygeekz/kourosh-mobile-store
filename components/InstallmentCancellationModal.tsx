...
const InstallmentCancellationModal: React.FC<Props> = ({ isOpen, sale, onClose, onCanceled }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const prepareResponse = await apiFetch(`/api/installment-sales/${sale.id}/cancel/prepare`, {
        method: 'POST',
        headers: { ...getAuthHeaders(token), 'Content-Type': 'application/json' },
        body: JSON.stringify(cancellationPayload),
      });
      const preparePayload = await prepareResponse.json();
      if (!prepareResponse.ok || preparePayload?.success === false) {
        throw new Error(preparePayload?.message || 'محاسبه اثر مالی فسخ انجام نشد.');
      }
      const accepted = await confirmAction({
        title: 'تأیید نهایی اثر مالی فسخ',
        description: String(preparePayload?.data?.impact?.summaryText || 'اثر مالی فسخ را بررسی و تأیید کنید.'),
        confirmText: 'تأیید و اجرای فسخ',
        tone: 'danger',
      ...
      const response = await apiFetch(`/api/installment-sales/${sale.id}/cancel`, {
        ...
        body: JSON.stringify({
          ...cancellationPayload,
          confirmationToken: preparePayload?.data?.token,
        }),
      });
      const payload = await response.json();
      if (!response.ok || payload?.success === false) {
