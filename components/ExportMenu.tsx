...
const ExportMenu: React.FC<Props> = ({ items, label = 'خروجی', className, popoverClassName = '', menuWidth = MENU_WIDTH }) => {
  ...
  const updatePosition = useCallback(() => {
    ...
    const resolvedMenuWidth = Math.max(160, Number(menuWidth || MENU_WIDTH));
    const preferredLeft = rect.right - resolvedMenuWidth;
    const minLeft = boundaryLeft;
    const maxLeft = Math.max(minLeft, boundaryRight - resolvedMenuWidth);
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className={`export-menu-popover export-menu-popover--portal ${popoverClassName} overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-950 shadow-2xl`}
          style={{
            position: 'fixed',
            top: position.top,
            left: position.left,
            right: 'auto',
            bottom: 'auto',
            width: Math.max(160, Number(menuWidth || MENU_WIDTH)),
            pointerEvents: 'auto',
