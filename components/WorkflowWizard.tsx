    ...
    <div className={[className ?? '', sticky ? 'sticky top-2 z-20' : ''].join(' ')}>
      ...
      {showBottomBar ? (
        <Surface
          ...
          variant="bar"
          scheme="adaptive"
          data-ui-workflow-actions="true"
          className="mt-2.5 rounded-[20px]"
          contentClassName="flex items-center justify-between gap-2.5 px-3 py-2.5"
        >
          <Button type="button" variant="secondary" size="sm" onClick={() => canPrev && onStepChange(stepIndex - 1)} disabled={!canPrev} leftIcon={<i className="fa-solid fa-arrow-right" />}>
            قبلی
          </Button>
          ...
          <Button type="button" variant="primary" size="sm" onClick={() => canNext && onStepChange(stepIndex + 1)} disabled={!canNext} rightIcon={<i className="fa-solid fa-arrow-left" />}>
            بعدی
          </Button>
        </Surface>
      ) : null}
