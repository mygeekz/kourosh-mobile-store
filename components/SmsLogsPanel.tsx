...
import { Dialog as Modal, IconGlyph, SelectField, Surface, TextField } from '@/components/ui';
...
const SmsLogsPanel: React.FC = () => {
  ...
  const [meta, setMeta] = useState<SmsLogsMeta>(EMPTY_META);
  ...
  const qs = useMemo(() => {
    ...
    p.set('offset', String((currentPage - 1) * pageSize));
    ...
    return p.toString();
  }, [successFilter, eventType, recipient, currentPage, pageSize]);

  const load = async () => {
    setIsLoading(true);
    try {
      const res = await apiFetch(`/api/sms/logs?${qs}`, { cache: 'no-store' });
  ...
  const retry = async (id: number) => {
    ...
    setToast(null);
    try {
      const res = await apiFetch(`/api/sms/logs/${id}/retry`, {
        method: 'POST',
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data?.success) {
  ...
  const okCount = meta.successCount;
  const failCount = meta.failCount;
  const hasFailedLogs = failCount > 0;
  const isQuickErrorFilterActive = successFilter === 'false';
  const avgDuration = meta.avgDurationMs;
  ...
  return (
    <Surface surface="glass" variant="panel" scheme="adaptive" className="ops-logs-panel sms-logs-panel mt-6 rounded-[30px]" contentClassName="p-5" data-ui-ops-panel="sms-logs" data-ui-ops-surface="logs">
      ...
      <div className="ops-metric-grid mt-4 grid grid-cols-2 gap-3 lg:grid-cols-5" data-ui-ops-metrics="true">
        {[
          { title: 'کل لاگ‌ها', value: meta.total.toLocaleString('fa-IR'), icon: 'fa-database', valueClass: 'text-slate-900 dark:text-slate-50' },
          ...
          { title: 'آخرین ارسال', value: meta.lastCreatedAt ? formatDate(meta.lastCreatedAt) : '—', icon: 'fa-clock', valueClass: 'text-slate-900 dark:text-slate-50' },
        ].map((item) => (
          <Surface key={item.title} surface="glass" variant="subtle" scheme="adaptive" className="ops-metric-card rounded-2xl" contentClassName="px-4 py-3">
            <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 dark:text-slate-400"><IconGlyph size="sm"><i className={`fa-solid ${item.icon}`} /></IconGlyph>{item.title}</div>
            <div className={`mt-1 truncate text-base font-black ${item.valueClass}`}>{item.value}</div>
          </Surface>
        ))}
      </div>

      <Surface surface="glass" variant="subtle" scheme="adaptive" className="ops-filter-panel mt-4 rounded-[24px]" contentClassName="p-4" data-ui-ops-filters="true">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div>
            ...
            <SelectField controlOnly unstyled showChevron={false} className="app-input" value={successFilter} onChange={(e) => setSuccessFilter(e.target.value as any)}>
          ...
          <div>
            ...
            <SelectField controlOnly unstyled showChevron={false} className="app-input" value={eventType} onChange={(e) => setEventType(e.target.value)}>
              {eventOptions.map((x) => (
          ...
          <div>
            ...
            <TextField controlOnly unstyled className="app-input" value={recipient} onChange={(e) => setRecipient(e.target.value)} placeholder="0912..." dir="ltr" />
          </div>
        </div>
      </Surface>
      ...
      {toast ? (
        <div className={`mt-4 app-inline-alert ${toast.ok ? 'app-inline-alert--success' : 'app-inline-alert--danger'}`}>
          <div className="app-inline-alert__row">
            <IconGlyph size="md" tone={toast.ok ? 'success' : 'danger'}><i className={`fa-solid ${toast.ok ? 'fa-circle-check' : 'fa-triangle-exclamation'}`} /></IconGlyph>
      ...
      <div className="mt-4 space-y-3">
        {visibleRows.map((r) => {
          ...
          const canRetry = String(r.provider || '').toLowerCase().includes('meli');
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.72fr)] xl:items-start">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex items-start gap-3">
                      <IconGlyph size="lg" tone={ok ? 'success' : 'danger'}>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-slate-200/70 bg-slate-50/70 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-900/50">
                      <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400">کد پاسخ / مدت زمان</div>
                ...
                <div className="flex flex-col gap-2 xl:items-end">
                  ...
                  {canRetry ? (
                    ...
                    <span className="inline-flex w-full items-center justify-center gap-1 rounded-[11px] border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-bold text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 xl:w-auto">
                      <i className="fa-solid fa-circle-info" />ارسال مجدد برای این سرویس فعال نیست
                    </span>
                  )}
                </div>
              </div>
        ...
        {rows.length === 0 ? (
          <div className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50/80 px-5 py-8 text-center dark:border-slate-700 dark:bg-slate-950/40">
            <IconGlyph size="lg" className="mx-auto">
              <i className="fa-solid fa-magnifying-glass" />
            </IconGlyph>
            <div className="mt-3 text-sm font-black text-slate-900 dark:text-slate-50">لاگی برای فیلتر فعلی پیدا نشد</div>
            <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">فیلترها را تغییر بده یا بعد از بررسی ارسال، بروزرسانی را بزن.</div>
          </div>
        ) : null}
      </div>
      ...
      {meta.total > pageSize ? (
        <div className="mt-4 flex flex-col gap-3 rounded-[24px] border border-slate-200/80 bg-slate-50/70 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-xs font-bold text-slate-500 dark:text-slate-400">
            صفحه {safeCurrentPage.toLocaleString('fa-IR')} از {totalPages.toLocaleString('fa-IR')} · نمایش {visibleRows.length.toLocaleString('fa-IR')} لاگ از {meta.total.toLocaleString('fa-IR')} مورد · اندازه صفحه {pageSize.toLocaleString('fa-IR')}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button
              type="button"
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                { title: 'کد پاسخ / مدت زمان', value: `${getHttpText(selected)} / ${getDurationText(selected)}`, ltr: true },
              ].map((item) => (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <details className="mt-3 text-xs">
                        <summary className="cursor-pointer font-bold">نمایش جزئیات پاسخ</summary>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-950/50">
                <div className="mb-2 text-sm font-black text-slate-900 dark:text-slate-50">جزئیات پاسخ</div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              {String(selected.provider || '').toLowerCase().includes('meli') ? (
