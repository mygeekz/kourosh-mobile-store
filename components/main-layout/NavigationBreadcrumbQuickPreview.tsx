...
import { PortalLayer, useAnchoredOverlayPosition } from '@/components/ui';
...
const NavigationBreadcrumbQuickPreview: React.FC<Props> = ({
  ...
  const { position } = useAnchoredOverlayPosition({
    isOpen: Boolean(preview && anchor),
    anchorElement: anchor,
    panelRef,
    preferredWidth: 360,
    fallbackPanelHeight: 420,
    maxPanelHeight: 520,
    margin: 12,
    gap: 8,
    align: 'center',
  ...
  if (!preview || !anchor || !position) return null;
  ...
  return (
    <PortalLayer layer="popover" className="navigation-breadcrumb-quick-preview-layer">
    <aside
      ref={panelRef}
      className="app-floating-surface overflow-y-auto rounded-2xl border-slate-200 bg-white ring-1 ring-black/5 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-950 dark:ring-white/5"
