...
const learningCopy: Record<
  NonNullable<SmartSalesAdvisorProps["learningStatus"]>,
  { label: string; hint: string }
> = {
  empty: { label: "داده کم", hint: "هنوز برای تحلیل قطعی داده کافی نیست." },
  learning: {
    label: "در حال یادگیری",
    hint: "سیستم از رفتار فروشگاه الگو می‌گیرد.",
  },
  trusted: {
    label: "قابل اعتماد",
    hint: "تحلیل‌ها بر اساس داده‌های کافی ساخته شده‌اند.",
  },
  excellent: {
    label: "بسیار قابل اعتماد",
    hint: "داده‌ها برای تصمیم‌گیری عملیاتی کامل‌تر هستند.",
  },
};
...
const SmartSalesAdvisor: React.FC<SmartSalesAdvisorProps> = ({
  ...
  const [selectedInsight, setSelectedInsight] =
    useState<SmartSalesInsight | null>(null);
  const [expanded, setExpanded] = useState(false);
  const detailDialogId = useId().replace(/:/g, "");
  ...
  const visibleInsights = expanded
    ? sortedInsights
    : sortedInsights.slice(0, compact ? 2 : 3);
  const learning = learningCopy[learningStatus];
  const overallConfidence = sortedInsights.length
    ? Math.round(
        sortedInsights.reduce(
          (sum, item) => sum + clampConfidence(item.confidence),
          0,
  ...
  return (
    <section
      className="rounded-[22px] border border-slate-200 bg-white/95 p-3 shadow-[0_22px_48px_-44px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950/88"
      dir="rtl"
    >
        ...
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              <h3 className="text-[13px] font-black text-slate-900 dark:text-slate-100">
                {title}
              </h3>
              {contextLabel ? (
        ...
        <div className="shrink-0 text-left">
          ...
          <div className="mt-1 text-[10px] font-bold text-slate-400">
            {overallConfidence === null
              ? "اعتماد —"
              : `اعتماد ${overallConfidence.toLocaleString("fa-IR")}٪`}
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2">
        {visibleInsights.length === 0 ? (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-3 text-[12px] font-bold text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-200">
            فعلاً هشدار خاصی دیده نشد؛ با انتخاب مشتری و اقلام، تحلیل لحظه‌ای
            فعال می‌شود.
          </div>
        ) : (
          visibleInsights.map((insight) => {
            ...
            return (
              ...
              >
                <div className="flex min-w-0 items-start gap-2.5">
                  <span
                    className={`mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-xl border text-[11px] ${severity.ring}`}
                  >
                    {insight.severity === "success" ? (
                      ...
                      <i className={insight.icon || severity.icon} />
                    )}
                  </span>
                  <span className="min-w-0">
                    <span className="block truncate text-[12px] font-black text-slate-900 dark:text-slate-100">
                      {insight.title}
                    </span>
                    <span className="mt-0.5 block line-clamp-2 text-[10.5px] leading-5 text-slate-500 dark:text-slate-400">
                      {insight.summary}
                    </span>
                  </span>
                </div>
                <span className="shrink-0 self-start rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-extrabold leading-none text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
                  دلیل
                </span>
              </button>
            );
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
        <div className="text-[10.5px] leading-5 text-slate-500 dark:text-slate-400">
          {primaryInsight
            ? `${severityCopy[primaryInsight.severity].label} • ${learning.hint}`
            : learning.hint}
        </div>
        {sortedInsights.length > (compact ? 2 : 3) ? (
          ...
          >
            {expanded
              ? "نمایش کمتر"
              : `نمایش همه (${sortedInsights.length.toLocaleString("fa-IR")})`}
          </button>
        ) : null}
      </div>
      ...
      <DialogShell
        ...
        overlayClassName="ux-overlay-backdrop app-modal-backdrop smart-sales-advisor-drawer-overlay"
        panelClassName="ux-stable-panel smart-sales-advisor-drawer-panel"
        ariaLabelledBy={`smart-sales-advisor-title-${detailDialogId}`}
        ariaDescribedBy={`smart-sales-advisor-description-${detailDialogId}`}
        panelAttributes={{
          "data-smart-sales-severity": selectedInsight?.severity,
        }}
      >
        {selectedInsight ? (
          <>
            <div className="smart-sales-advisor-drawer__header">
              <div className="flex min-w-0 items-start gap-3">
                <span
                  className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border ${severityCopy[selectedInsight.severity].ring}`}
                >
                  {selectedInsight.severity === "success" ? (
                    ...
                    <i
                      className={
                        ...
                        severityCopy[selectedInsight.severity].icon
                      }
                    />
                  )}
                </span>
                <div className="min-w-0">
                  <div className="text-[12px] font-extrabold text-slate-500 dark:text-slate-400">
                    چرا این پیشنهاد؟
                  </div>
                  <h3
                    id={`smart-sales-advisor-title-${detailDialogId}`}
                    className="mt-1 text-lg font-black text-slate-900 dark:text-slate-100"
                  >
                    {selectedInsight.title}
                  </h3>
                  <p
                    id={`smart-sales-advisor-description-${detailDialogId}`}
                    className="mt-1 text-[12px] leading-6 text-slate-500 dark:text-slate-400"
                  >
            ...
            <div className="smart-sales-advisor-drawer__body">
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/70">
                  ...
                  <div
                    className={`mt-1 text-[12px] font-black ${severityCopy[selectedInsight.severity].text}`}
                  >
                    {severityCopy[selectedInsight.severity].label}
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/70">
                  <div className="text-[10px] font-extrabold text-slate-400">
                  ...
                  <div className="mt-1 text-[12px] font-black text-slate-900 dark:text-slate-100">
                    {clampConfidence(selectedInsight.confidence).toLocaleString(
                      "fa-IR",
                    )}
                    ٪
                  </div>
              ...
              {selectedInsight.metrics && selectedInsight.metrics.length > 0 ? (
                <div className="mt-4 rounded-[22px] border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900/40">
                  <div className="mb-2 text-[12px] font-black text-slate-900 dark:text-slate-100">
                    عددهای اثرگذار
                  </div>
                  <div className="grid gap-2">
                    {selectedInsight.metrics.map((metric) => (
                      <div
                        key={`${selectedInsight.id}-${metric.label}`}
                        className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2 text-[12px] dark:bg-slate-950/60"
                      >
                ...
                <div className="mt-4 rounded-[22px] border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/55">
                  ...
                  <ul className="space-y-2">
                    {selectedInsight.reasons.map((reason) => (
                      <li
                        key={`${selectedInsight.id}-${reason}`}
                        className="flex items-start gap-2 text-[12px] leading-6 text-slate-600 dark:text-slate-300"
                      >
                        <i
                          className="fa-solid fa-check mt-1.5 text-[10px] text-slate-400"
                          aria-hidden="true"
                        />
                        <span>{reason}</span>
                      </li>
            ...
            {selectedInsight.actionTo ? (
              <div className="smart-sales-advisor-drawer__footer">
                <Button
                  type="button"
                  variant="primary"
                  size="sm"
