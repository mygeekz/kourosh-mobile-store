          ...
          visibleInsights.map((insight) => {
            ...
            return (
              <Button
                ...
                autoIcon={false}
                leftIcon={insight.severity === "success" ? <i className="fa-solid fa-check" /> : <i className={insight.icon || severity.icon} />}
                tooltip={insight.summary}
              >
                <span className="grid min-w-0 gap-1 text-right">
                  <strong className="truncate text-xs">{insight.title}</strong>
                  <small className="line-clamp-2 text-[10px] font-semibold opacity-75">{insight.summary}</small>
                </span>
              </Button>
            );
          })
