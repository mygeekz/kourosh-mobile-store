...
const HeaderQuickActions: React.FC<HeaderQuickActionsProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (targetElement?.closest('[data-ui-header-quick-panel]')) return;
  ...
  useLayoutEffect(() => {
    ...
    const updateQuickMenuPosition = () => {
      ...
      const maxHeight = Math.max(180, Math.min(preferredHeight, resolved.availableHeight));
      const originX = Math.round(rect.left + rect.width / 2 - resolved.left);
      const originY = resolved.placement === 'bottom' ? '0px' : '100%';
      ...
      setQuickMenuPosition((current) => ({
        ...
        [activeQuickMenu]: {
          ...
          '--header-quick-panel-origin-x': `${originX}px`,
          '--header-quick-panel-origin-y': originY,
        },
      }));
    };
  ...
  const renderNotificationsPanel = () => (
    <div className="app-header-popover__content app-header-popover__content--notifications" data-ui-header-notifications-preview="true">
      {headerQuickPanels.notifications.length ? (
        <div className="app-header-popover__list app-header-popover__notification-list">
          {headerQuickPanels.notifications.slice(0, 3).map((item) => (
            <Link
              key={item.id}
              to={item.actionLink || '/notifications'}
              onClick={() => setActiveQuickMenu(null)}
              className="app-header-popover__list-item app-header-popover__list-item--link app-header-popover__notification-item"
            >
              <span className="app-header-popover__notification-marker" aria-hidden="true"><i className="fa-solid fa-bell" /></span>
              <div className="app-header-popover__list-main">
