...
const learningGuideByFeature: Record<string, { title: string; body: string }> = {
  ...
  auto_pricing: {
    ...
    body: 'فقط پیشنهاد ارائه می‌کند و اعمال قیمت همیشه نیازمند اقدام صریح کاربر است؛ قیمت‌ها به‌صورت خودکار تغییر نمی‌کنند.',
  },
  sales_agent: {
    title: 'دستیار فروش',
  ...
  profit_engine: {
    ...
    body: 'از بهای خرید و سوابق واقعی سود استفاده می‌کند و محاسبات تجاری ثبت‌شده را نمایش می‌دهد.',
  },
};
...
const sourceLabel = (source?: string) =>
  source === 'sqlite-business-records' ? 'داده ثبت‌شده فروشگاه' : 'منبع داده نامشخص';
        ...
        <div className="border-t border-slate-200/75 px-4 py-3 dark:border-slate-800 sm:px-5">
          <div className="grid gap-2 text-[10px] leading-5 text-slate-600 dark:text-slate-300 md:grid-cols-3" data-ui-settings-grid="cards">
            <div className="flex items-start gap-2 rounded-[14px] border border-emerald-200 bg-emerald-50/75 px-3 py-2 dark:border-emerald-900/60 dark:bg-emerald-950/25">
              ...
              <span>تصمیم‌گیری خودکار و تغییر خودکار داده‌های تجاری غیرفعال هستند.</span>
            </div>
            <div className="flex items-start gap-2 rounded-[14px] border border-slate-200 bg-slate-50/75 px-3 py-2 dark:border-slate-800 dark:bg-slate-900/55">
              ...
              <span>{num(runtime.sourceSummary?.decisionMemory).toLocaleString('fa-IR')} تصمیم و {num(runtime.sourceSummary?.impactEvents).toLocaleString('fa-IR')} رویداد اثر از سوابق ثبت‌شده فروشگاه خوانده شد.</span>
            </div>
            <div className="flex items-start gap-2 rounded-[14px] border border-slate-200 bg-slate-50/75 px-3 py-2 dark:border-slate-800 dark:bg-slate-900/55">
