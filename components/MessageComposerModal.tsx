import { FormGrid, ModalField, SearchableSelectField, TextareaField, TextField, phoneNumberError, requiredSelectionError, requiredTextError } from '@/components/ui';
...
const MessageComposerModal: React.FC<Props> = ({ open, onClose, onQueued, initialRecipient, initialText, initialChannels, initialVariables }) => {
  ...
  const validation = useMemo(() => {
    ...
    const channelError = requiredSelectionError(activeChannels[0] || '', 'مسیر ارسال');
    const recipientError = recipientType === 'manual'
      ? requiredTextError(recipientName, 'نام گیرنده')
      : requiredSelectionError(recipientId, 'گیرنده');
    const textError = requiredTextError(text, 'متن پیام');
    if (channelError) next.channels = channelError;
    if (recipientError) next[recipientType === 'manual' ? 'recipientName' : 'recipientId'] = recipientError;
    if (textError) next.text = textError;
    if (text.length > MESSAGE_LIMIT) next.text = `متن پیام حداکثر ${MESSAGE_LIMIT.toLocaleString('fa-IR')} کاراکتر باشد.`;
    if (channels.sms) {
      const smsPhoneError = phoneNumberError(normalizedPhone, { optional: false, label: 'شماره موبایل' });
      if (smsPhoneError) next.phoneNumber = smsPhoneError;
    }
    if (channels.telegram) {
      const chatIdError = requiredTextError(telegramChatId, 'Chat ID تلگرام');
      if (chatIdError) next.telegramChatId = chatIdError;
    }
    return next;
  }, [activeChannels, recipientType, recipientId, recipientName, text, channels.sms, channels.telegram, normalizedPhone, telegramChatId]);
  ...
  return (
    <InventoryModal
      ...
      tone="info"
    >
      <form noValidate onSubmit={handleSend} className="message-composer-canonical modal-template-form modal-template-form--message" dir="rtl">
          ...
          <section className="modal-template-card message-composer-card message-composer-route">
            ...
            {recipientType === 'manual' ? (
              ...
              <ModalField label={recipientType === 'partner' ? 'همکار انتخاب‌شده' : 'مشتری انتخاب‌شده'} iconClass="fa-solid fa-user" required error={validation.recipientId}>
                <TextField value={recipientName || 'بدون نام'} readOnly />
              </ModalField>
            ) : (
              <ModalField label={recipientType === 'partner' ? 'انتخاب همکار' : 'انتخاب مشتری'} iconClass="fa-solid fa-user" required error={validation.recipientId}>
                <SearchableSelectField<string>
                  ...
                  options={options.map((person) => ({
                    ...
                    label: `${person.name}${person.phoneNumber ? ` — ${person.phoneNumber}` : ''}`,
                    searchText: `${person.name} ${person.phoneNumber || ''} ${person.id}`,
                  }))}
                  placeholder={recipientType === 'partner' ? 'نام همکار را تایپ کنید…' : 'نام، موبایل یا کد مشتری را تایپ کنید…'}
                  noOptionsMessage={recipientType === 'partner' ? 'همکاری مطابق جستجو پیدا نشد' : 'مشتری مطابق جستجو پیدا نشد'}
                  invalid={Boolean(validation.recipientId)}
                  ariaLabel={recipientType === 'partner' ? 'جستجو و انتخاب همکار' : 'جستجو و انتخاب مشتری'}
                />
              </ModalField>
            )}
            ...
            <FormGrid columns={2} className="message-composer-inline-fields">
              {channels.sms ? (
            ...
            {channels.telegram && recipientType !== 'manual' ? (
              <div className="message-composer-route-footer">
                <div className="message-composer-save-chatid">
                  <ToggleSwitch checked={saveTelegramChatId} onCheckedChange={setSaveTelegramChatId} ariaLabel="ذخیره مقصد ارسال تلگرام" size="sm" />
                  <span>ذخیره مقصد ارسال تلگرام (بدون ایجاد دسترسی)</span>
                </div>
                {tgStartLink ? (
