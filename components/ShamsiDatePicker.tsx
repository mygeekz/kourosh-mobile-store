...
import { PortalLayer, TextField, useAnchoredOverlayPosition } from '@/components/ui';
...
const ShamsiDatePicker: React.FC<Props> = ({
  ...
  const fieldId = id || `shamsi-date-${generatedId}`;
  ...
  const { position: panelPos } = useAnchoredOverlayPosition({
    isOpen: open,
    anchorRef: wrapperRef,
    panelRef,
    preferredWidth: CALENDAR_PANEL_WIDTH,
    fallbackPanelHeight: CALENDAR_PANEL_HEIGHT_FALLBACK,
    maxPanelHeight: CALENDAR_PANEL_HEIGHT_FALLBACK,
  ...
  const canonicalControlSize = size === 'standard' ? 'md' : 'sm';
  ...
  return (
    <div
      ...
      data-date-size={size}
      data-ui-control-size={canonicalControlSize}
      data-invalid={invalid || Boolean(error) ? 'true' : 'false'}
      data-field-state={invalid || error ? 'error' : 'default'}
    ...
    >
      ...
      >
        <TextField
          controlOnly
          controlSize={canonicalControlSize}
          id={fieldId}
          ref={inputRef}
          type="text"
          dir="ltr"
      ...
      {error ? (
        <p id={errorId} role="alert" aria-live="polite" className="app-form-field__error mt-1.5 text-xs font-semibold leading-5 text-danger">
          <i className="fa-solid fa-circle-exclamation me-1" aria-hidden="true" />
          {error}
        </p>
      ) : hint ? (
        <p id={hintId} className="app-form-field__hint mt-1.5 text-xs font-semibold leading-5 text-muted-foreground">{hint}</p>
      ) : null}
