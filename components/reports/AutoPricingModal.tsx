...
export default function AutoPricingModal({
  ...
  const pricingKpis = [
    { label: 'پیشنهادهای فعال', value: formatExactNumberText(pricingRows.length), icon: 'fa-tags', tone: 'blue' },
    ...
    { label: 'ریسک بالا', value: formatExactNumberText(highRiskCount), icon: 'fa-triangle-exclamation', tone: highRiskCount ? 'amber' : 'slate' },
  ];
  const pricingMemoryCards = [
    ...
    { label: 'دفعات تکرار', value: `${formatExactNumberText(num(selected.decision?.occurrenceCount))} بار`, icon: 'fa-rotate' },
  ];
  const pricingActionState = getDecisionActionState(selected);
  const modalStyle = useSmartInsightModalCssVars();

  const modal = (
    <div className="apr220-overlay apr220-overlay--sidebar-safe" style={modalStyle} onClick={() => onClose()}>
        ...
        <aside className="apr220-side">
          <article className="apr220-panel apr220-panel--reasons">
            ...
            <div className="apr220-reason-list">
              ...
              {!selected.reasons?.length ? <div className="apr220-empty">دلیل تکمیلی برای این پیشنهاد ثبت نشده است.</div> : null}
            </div>
          </article>
