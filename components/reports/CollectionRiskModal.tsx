...
export default function CollectionRiskModal({
  ...
  const nearestDueLabel = typeof nearest.dueInDays === 'number'
    ? nearest.dueInDays < 0
      ? `${formatExactNumberText(Math.abs(Number(nearest.dueInDays)))} روز معوق`
      : nearest.dueInDays === 0
    ? 'امروز'
    : `${formatExactNumberText(Number(nearest.dueInDays))} روز دیگر`
    : nearest.dueDate
  ...
  const collectionKpis = [
    { label: 'سندهای نیازمند پیگیری', value: metricMap.get('پرونده فوری') || formatExactNumberText(collectionRows.length), icon: 'fa-file-lines', tone: 'blue' },
    ...
    { label: 'مشتریان درگیر', value: formatExactNumberText(involvedCustomers), icon: 'fa-users', tone: 'orange' },
  ];
      ...
      <section className="crm197-content-grid">
            ...
            <div className="crm197-table-body">
              {collectionRows.slice(0, 4).map((row, index) => {
                ...
                const dueLabel = typeof row.dueInDays === 'number'
                  ? row.dueInDays < 0
                    ? `${formatExactNumberText(Math.abs(Number(row.dueInDays)))} روز معوق`
                    : row.dueInDays === 0 ? 'امروز' : `${formatExactNumberText(Number(row.dueInDays))} روز دیگر`
        ...
        <article className="crm197-panel crm197-reasons">
          ...
          <div className="crm197-reason-list">
            ...
            {!collectionReasons.length ? <div className="crm197-empty">دلیل تکمیلی برای این پیشنهاد ثبت نشده است.</div> : null}
          </div>
        </article>
      </section>
