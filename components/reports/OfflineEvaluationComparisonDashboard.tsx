...
import { Table, DataTableShell, TableActionGroup, type TableActionItem } from '@/components/ui';
  ...
  const buildRowActions = (row: DashboardRow): TableActionItem[] => [
    {
      ...
      kind: 'button',
      label: 'جزئیات شاخص‌ها',
      tooltip: 'جزئیات شاخص‌ها',
      variant: 'secondary',
      disabled: !row.id,
    ...
    },
    {
      ...
      kind: 'button',
      label: 'بررسی بخش‌های داده',
      tooltip: 'بررسی بخش‌های داده',
      variant: 'secondary',
      disabled: !row.id,
    ...
    },
    {
      ...
      kind: 'button',
      label: 'کیفیت داده آموزشی',
      tooltip: 'کیفیت داده‌های آموزش',
      variant: 'secondary',
      disabled: !row.id,
    ...
    },
    {
      ...
      kind: 'button',
      label: 'سازگاری ورودی‌ها',
      tooltip: 'تغییرات ساختار داده',
      variant: 'secondary',
      disabled: !row.id,
    ...
    },
    {
      ...
      kind: 'button',
      label: 'سناریوهای آستانه',
      tooltip: 'سناریوهای آستانه',
      variant: 'secondary',
      disabled: !row.id,
  ...
  return (
    <section className="rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/70" aria-label="Offline evaluation comparison dashboard">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-indigo-500">مقایسه تخصصی مدل‌های تحلیلی</span>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">داشبورد ارزیابی و مقایسه مدل‌های هوشمند</h3>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش نتایج ارزیابی ثبت‌شده را مقایسه می‌کند و هیچ تغییری در عملیات فروشگاه ایجاد نمی‌کند.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">مدل‌ها</small>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <em className="not-italic text-xs text-slate-500">نیازمند تکمیل: {nf.format(summary?.blockedCandidateCount || 0)}</em>
        </article>
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">بهترین امتیاز اطلاعات</small>
      ...
      {hasRows ? (
        ...
        >
          <Table layout="managed" density="comfortable" className="w-full min-w-[1120px] divide-y divide-slate-100 text-right text-xs dark:divide-slate-800">
        ...
        <p className="mt-4 rounded-2xl bg-slate-50 px-3 py-3 text-xs font-bold text-slate-500 dark:bg-slate-900 dark:text-slate-400">هنوز نتیجه ارزیابی قابل مقایسه‌ای ثبت نشده است.</p>
      )}
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این داشبورد برای مشاهده و مقایسه نتایج ارزیابی است و هیچ تغییری در اطلاعات تجاری اعمال نمی‌کند.'}
      </p>
    </section>
  );
}
