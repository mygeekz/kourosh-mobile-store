...
import { SmartInsightModalPortal } from './SmartInsightModalShell';
...
export default function StockReorderModal({
  ...
  const averageDaysToEnd = stockRows.length
    ...
    : 0;
  const stockKpis = [
    { label: 'کالاهای در خطر', value: formatExactNumberText(dangerRows.length), suffix: 'کالا', icon: 'fa-triangle-exclamation', tone: 'rose' },
    { label: 'ارزش فروش ۱۴ روز', value: metricMap.get('فروش ۱۴ روز') || money(totalSoldAmount), suffix: '', icon: 'fa-coins', tone: 'violet' },
    { label: 'میانگین روز تا اتمام', value: stockRows.length ? formatExactNumberText(averageDaysToEnd) : '—', suffix: stockRows.length ? 'روز' : '', icon: 'fa-hourglass-half', tone: 'orange' },
    { label: 'پیشنهادهای خرید', value: metricMap.get('تعداد پیشنهاد') || formatExactNumberText(stockRows.length), suffix: 'پیشنهاد', icon: 'fa-cart-shopping', tone: 'emerald' },
  ];
  const stockMemoryCards = [
    ...
    { label: 'دفعات تکرار', value: formatExactNumberText(num(selected.decision?.occurrenceCount)) + ' بار', icon: 'fa-rotate', tone: 'orange' },
        ...
        <div className="stk200-table">
          ...
          <div className="stk200-table-body">
            {stockRows.slice(0, 6).map((row, index) => {
              const daily = num(row.soldQty) / 14;
              const daysToEnd = daily > 0 ? (num(row.stockQuantity) / daily) : null;
              ...
              return (
                <div key={`${row.productId || row.productName || index}`} className="stk200-row">
                  ...
                  <span>{formatExactNumberText(num(row.stockQuantity))}</span>
                  <span>{formatExactNumberText(num(row.soldQty))}</span>
                  <span className={isCritical ? 'stk200-danger-text' : isWarning ? 'stk200-warning-text' : ''}>{daysToEnd == null ? '—' : formatExactNumberText(daysToEnd)}</span>
            ...
            {!stockRows.length ? <div className="stk200-empty">کالای دارای سیگنال خرید در این تحلیل ثبت نشده است.</div> : null}
          </div>
        </div>
      </section>
      ...
      <section className="stk200-lower-grid">
        ...
        <article className="stk200-panel stk200-reasons">
          ...
          <div className="stk200-reason-list">
            ...
            {!stockReasons.length && totalEstimatedProfit <= 0 ? <div className="stk200-empty">دلیل تکمیلی برای این پیشنهاد ثبت نشده است.</div> : null}
          </div>
        </article>
      </section>
    </main>
