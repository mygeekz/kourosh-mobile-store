  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-white/95 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/80" aria-label="Offline candidate robustness metadata">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-indigo-500">بررسی پایداری مدل</span>
          <h4 className="mt-1 text-base font-black text-slate-950 dark:text-white">Robustness Metadata Snapshot</h4>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش اطلاعات پایداری مدل و نتایج ارزیابی شرایط دشوار را برای بررسی کیفیت نمایش می‌دهد.</p>
        </div>
        ...
        <Table layout="managed" density="comfortable" className="min-w-full divide-y divide-slate-100 text-right text-xs dark:divide-slate-800">
          ...
          <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-950">
            {items.length ? items.map((item) => (
              ...
              <tr><td colSpan={7} className="px-3 py-3 text-center font-bold text-slate-500">اطلاعات پایداری هنوز برای این مدل ثبت نشده است.</td></tr>
            )}
          </tbody>
        </Table>
      </DataTableShell>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <article className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-900">
          <strong className="text-xs font-black text-slate-700 dark:text-slate-200">کنترل‌های پایداری</strong>
        ...
        <article className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-900">
          <strong className="text-xs font-black text-slate-700 dark:text-slate-200">نشانه‌های پایداری</strong>
          <ul className="mt-2 space-y-1 text-xs font-bold text-slate-500">
      ...
      <p className="mt-4 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">جزئیات پایداری مدل در سوابق تحلیل هوشمند نگه‌داری می‌شود.</p>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این بخش صرفاً برای بررسی اطلاعات پایداری مدل است و تغییری در اطلاعات فروشگاه ایجاد نمی‌کند.'}
      </p>
    </section>
  );
}
