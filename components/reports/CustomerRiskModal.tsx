...
export default function CustomerRiskModal({
  ...
  const avgRisk = customerRows.length
    ? (customerRows.reduce((sum, row) => sum + num(row.riskScore), 0) / customerRows.length)
    : num(selected.score);
  const avgProfitScore = customerRows.length
    ? (customerRows.reduce((sum, row) => sum + num(row.profitScore), 0) / customerRows.length)
    : 0;
  ...
  const customerKpis = [
    ...
    { label: 'آخرین خرید', value: primaryCustomer.daysSinceLast != null ? formatExactNumberText(num(primaryCustomer.daysSinceLast)) : maxDaysSinceLast ? formatExactNumberText(maxDaysSinceLast) : '—', suffix: primaryCustomer.daysSinceLast != null || maxDaysSinceLast ? 'روز قبل' : '', icon: 'fa-clock', tone: 'blue' },
    ...
    { label: 'مشتریان پرریسک', value: formatExactNumberText(highRiskCount), suffix: 'مورد', icon: 'fa-users', tone: 'emerald' },
  ];
            ...
            {customerRows.slice(0, 6).map((row, index) => {
              ...
              return (
                <div key={`${row.customerId || row.customerName || index}`} className="cust201-row">
                  ...
                  <span>{row.daysSinceLast != null ? `${formatExactNumberText(num(row.daysSinceLast))} روز قبل` : 'ثبت نشده'}</span>
                  <span className="cust201-action-note">{row.action || 'ثبت نشده'}</span>
                </div>
              );
            })}
            {!customerRows.length ? <div className="cust201-empty">مشتری نیازمند توجه در این تحلیل ثبت نشده است.</div> : null}
          </div>
        </div>
      </section>
      ...
      <section className="cust201-lower-grid">
        ...
        <article className="cust201-panel cust201-reasons">
          ...
          <div className="cust201-reason-list">
            ...
            {!customerReasons.length ? <div className="cust201-empty">دلیل تکمیلی برای این پیشنهاد ثبت نشده است.</div> : null}
          </div>
        </article>
      </section>
    </main>
