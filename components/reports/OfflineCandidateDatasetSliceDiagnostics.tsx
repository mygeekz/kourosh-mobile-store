...
const statusLabel = (value?: string | null) => {
  ...
  if (!text) return '—';
  if (text.includes('ready')) return 'آماده بررسی داده';
  if (text.includes('warning')) return 'داده نیازمند بررسی است';
  if (text.includes('no_slice')) return 'اطلاعات بخش‌بندی داده موجود نیست';
...
const sliceTypeLabel = (value?: string | null) => {
  const labels: Record<string, string> = {
    ...
    missingness_band: 'باند داده ناقص',
    target_distribution: 'توزیع هدف',
    class_balance: 'توازن کلاس',
    row_count_band: 'حجم داده',
    missingness: 'داده ناقص',
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت بررسی بخش‌های داده');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت بررسی بخش‌های داده');
      } finally {
  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/70" aria-label="Offline candidate dataset slice diagnostics">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-violet-500">بررسی بخش‌های داده</span>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">بررسی بخش‌های مختلف داده</h3>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش کیفیت و توزیع گروه‌های مختلف داده را نمایش می‌دهد و هیچ اقدام تجاری خودکاری انجام نمی‌دهد.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            )) : <p className="text-xs font-bold text-slate-500">اطلاعات بخش‌بندی داده برای مدل انتخاب‌شده موجود نیست.</p>}
          </div>
        </article>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 rounded-2xl bg-white px-3 py-2 text-xs font-bold text-slate-500 dark:bg-slate-950 dark:text-slate-400">این بخش فقط از اطلاعات ارزیابی ثبت‌شده استفاده می‌کند و فایل‌های اصلی مدل را تغییر نمی‌دهد.</p>
        </article>
      </div>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این صفحه فقط وضعیت بخش‌های داده را نمایش می‌دهد و هیچ تغییر خودکاری در اطلاعات تجاری اعمال نمی‌کند.'}
      </p>
    </section>
  );
}
