  ...
  return (
        ...
        <section className="smart-today-v183">
          <div className="smart-today-v183__header">
            <button
              type="button"
              className="smart-today-v183__count ux-btn ux-btn-secondary ux-btn-sm"
              onClick={() => {
            ...
            >
              ...
              {formatExactNumberText(Math.min(3, num(payload.todayActions?.length)))} اقدام پیشنهادی
            </button>
            <div className="smart-today-v183__heading">
              <div className="smart-today-v183__eyebrow"><i className="fa-solid fa-list-check smart-today-v185__eyebrow-icon" /> فرمان‌های امروز</div>
                ...
                <article key={action.id} className={`smart-today-v183__card ${accentClass}`}>
                  <div className="smart-today-v183__card-top">
                    <div className="smart-today-v183__meta-row">
                      <span className="smart-today-v183__score-pill">امتیاز {formatExactNumberText(num(action.priority))}</span>
                      <span className={`smart-today-v183__priority-pill smart-today-v183__priority-pill--${action.severity === 'critical' ? 'critical' : action.severity === 'high' ? 'high' : 'info'}`}>
                        ...
                        اولویت {formatExactNumberText((index + 1))}
                      </span>
                    </div>
                  ...
                  {action.to ? (
                    <Link to={action.to} className="smart-today-v183__cta ux-btn ux-btn-primary ux-btn-sm">
                      <span>{action.actionLabel || 'ورود به بخش مرتبط'}</span>
                      <i className="fa-solid fa-arrow-left" />
                    </Link>
                  ) : (
                    <button type="button" className="smart-today-v183__cta ux-btn ux-btn-primary ux-btn-sm" onClick={() => linkedInsight ? setSelected(linkedInsight) : undefined}>
                  ...
                  <div className="smart-today-v183__footer-actions">
                    {linkedInsight ? (
                      <button
                        ...
                        className={`smart-today-v183__sub-btn ux-btn ux-btn-sm ${linkedActionState?.isAccepted ? 'ux-btn-secondary smart-today-v183__sub-btn--done' : 'ux-btn-secondary smart-today-v183__sub-btn--pending'}`}
                      >
                    ...
                    {linkedInsight ? (
                      <button type="button" onClick={() => setSelected(linkedInsight)} className="smart-today-v183__sub-btn ux-btn ux-btn-ghost ux-btn-sm smart-today-v183__sub-btn--ghost">
