...
const executiveNarrative = (summary: ExecutiveSummary): string => {
  ...
  if (summary.blockedSources > 0) return 'حداقل یک منبع با خطا یا محدودیت دسترسی همراه است و قبل از اتکا به گزارش باید بررسی شود.';
  if (summary.attentionSources > 0 || summary.emptySources > 0) return 'اطلاعات قابل مشاهده است، اما بخشی از منابع خالی‌اند یا شناسه و کد صحت کامل ندارند و بهتر است توسط مدیر بررسی شوند.';
  return 'همه منابع قابل مشاهده در وضعیت پایدار هستند و شناسه یا کد صحت لازم برای بررسی مدیریتی را دارند.';
};
    ...
    {source ? (
      <div className="space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          <MetricCard label="امتیاز" value={`${toFaNumber(sourceScore(source))}٪`} helper="محاسبه بر اساس اطلاعات فعلی" />
          <MetricCard label="کد صحت" value={source.latestChecksum ? 'دارد' : 'ندارد'} helper="برای تطبیق اطلاعات" />
        </div>
        <div className="flex flex-wrap gap-2">
          <MlOperatorCopyButton value={source.latestId} label="کپی شناسه" compact />
          <MlOperatorCopyButton value={source.latestChecksum} label="کپی کد صحت" compact />
        </div>
      </div>
    ) : (
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <h2 className="text-base font-black text-slate-950 dark:text-white">نمای مدیریتی وضعیت اطلاعات</h2>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500 dark:text-slate-400">
            این خلاصه وضعیت منابع، پوشش شناسه، کد صحت و تازگی اطلاعات را برای تصمیم‌گیری مدیریتی نمایش می‌دهد.
          </p>
        </div>
        ...
        <div className="grid min-w-full gap-2 sm:grid-cols-4 xl:min-w-[640px]">
          ...
          <MetricCard label="رکورد قابل مشاهده" value={toFaNumber(summary.returnedItems)} helper="اطلاعات فعلی" />
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-3">
        ...
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
          ...
          پوشش شناسه: {toFaNumber(summary.idCoverage)} از {toFaNumber(summary.visibleSources)} منبع؛ پوشش کد صحت: {toFaNumber(summary.hashCoverage)} از {toFaNumber(summary.visibleSources)} منبع.
        </div>
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
      ...
      <div className="mt-4 rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <MlOperatorCopyButton value={summary.latestSafeHash} label="کپی کد صحت" compact />
        </div>
      </div>
