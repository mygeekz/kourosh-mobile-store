...
export const ML_OPERATOR_SAVED_VIEW_CONFIGS: MlOperatorSavedViewConfig[] = [
  {
    key: 'allReadable',
    label: 'نمای کامل اطلاعات',
    description: 'بازگشت به همه منابع اطلاعات با تعداد ردیف استاندارد.',
    icon: 'fa-border-all',
    query: '',
    statusFilter: 'all',
    sourceFilter: 'all',
  ...
  {
    key: 'comparisonOnly',
    label: 'بررسی مقایسه‌ها',
    description: 'نمای متمرکز برای خلاصه‌های مقایسه گزینه و مبنا.',
...
const routeLabel = (value: MlOperatorOverviewRouteKey): string => sourceLabels[value] || 'بخش اطلاعات';
...
export function MlOperatorSavedViewsPanel({
  ...
  const sourceText = sourceFilter === 'all' ? 'همه منابع اطلاعات' : routeLabel(sourceFilter as MlOperatorOverviewRouteKey);
  const noteItems = [
    ...
    `${formatExactNumberText(returnedItems)} رکورد در اطلاعات فعلی قابل مشاهده است.`,
