...
const sourceReason = (source: MlOperatorRouteResult): string => {
  ...
  if (source.state === 'unauthorized') return 'دسترسی این منبع نیازمند بررسی مدیریتی است.';
  if (source.state === 'unavailable') return 'این منبع در حال حاضر در دسترس نیست.';
  if (source.state === 'empty') return 'پاسخ معتبر است اما رکوردی برای بازرسی برنگشته است.';
  if (!source.latestChecksum) return 'منبع داده دارد اما کد صحت آخرین رکورد در خلاصه موجود نیست.';
...
const itemReason = (item: unknown): string => {
  ...
  const warnings = warningCount(item);
  const status = itemStatus(item);
  if (errors > 0) return `${toFaNumber(errors)} خطا در اطلاعات این ردیف گزارش شده است.`;
  ...
  if (!safeItemHash(item)) return 'کد صحت برای تطبیق این ردیف در اطلاعات خلاصه پیدا نشد.';
  return 'بدون مورد برجسته';
};
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500 dark:text-slate-400">
            این فهرست موارد نیازمند توجه را بر اساس اطلاعات فعلی برای بررسی مدیریتی اولویت‌بندی می‌کند.
          </p>
        </div>
      ...
      {queueRows.length > 0 ? (
        ...
        <div className="mt-4 rounded-[22px] border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
          ...
          <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">با تغییر فیلترها یا دریافت اطلاعات جدید، این فهرست به‌روزرسانی می‌شود.</p>
        </div>
      )}
    </section>
  );
