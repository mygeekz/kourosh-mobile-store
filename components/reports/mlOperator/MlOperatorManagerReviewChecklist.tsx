...
const sourceReviewDescription = (source: MlOperatorRouteResult): string => {
  ...
  if (source.state === 'unauthorized') return 'دسترسی منبع باید توسط مدیر یا سرپرست بررسی شود.';
  if (source.state === 'unavailable') return 'منبع فعلاً در دسترس نیست و بررسی مدیریتی باید با احتیاط انجام شود.';
  if (source.state === 'empty') return 'منبع پاسخ معتبر دارد اما داده‌ای برای مرور مدیریتی برنگشته است.';
  if (!source.latestChecksum) return 'منبع داده دارد اما کد صحت آخرین رکورد در خلاصه قابل مشاهده نیست.';
  return 'منبع دارای داده، شناسه و کد صحت قابل تطبیق است.';
};
...
const itemReviewDescription = (item: unknown): string => {
  ...
  if (!safeItemHash(item)) return 'کد صحت برای تطبیق این ردیف در اطلاعات موجود نیست.';
  return 'ردیف از نظر کد صحت، وضعیت و شمارش خطا یا هشدار برای بررسی آماده است.';
};
  ...
  for (const source of sources) {
    ...
    source.items.slice(0, 3).forEach((item, index) => {
      ...
      items.push({
        ...
        evidence: 'این ردیف بر اساس اطلاعات ثبت‌شده برای مشاهده جزئیات بررسی شده است.',
        state: rowState,
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <h2 className="text-base font-black text-slate-950 dark:text-white">مرور مدیریتی اطلاعات</h2>
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-3">
        ...
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
          ...
          {toFaNumber(summary.inspectedRows)} ردیف از اطلاعات همین صفحه در مرور مدیریتی بررسی شده است.
        </div>
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
          <i className="fa-solid fa-lock ml-2 text-slate-400" />
          کپی فقط برای شناسه و کد صحت فعال است.
        </div>
      </div>
      ...
      {checklistItems.length > 0 ? (
            ...
            <article key={item.key} className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-[11px] font-black text-slate-600 ring-1 ring-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:ring-slate-800">
                      {item.itemIndex >= 0 ? 'ردیف اطلاعات' : 'کنترل منبع'}
                    </span>
                  </div>
              ...
              <div className="mt-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div className="flex flex-wrap gap-2">
                  ...
                  <MlOperatorCopyButton value={item.safeHash} label="کپی کد صحت" compact />
                </div>
                <button
                  type="button"
        ...
        <div className="mt-4 rounded-[22px] border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
          ...
          <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">با تغییر فیلترها یا دریافت اطلاعات جدید، فهرست بررسی به‌روزرسانی می‌شود.</p>
        </div>
      )}
    </section>
  );
