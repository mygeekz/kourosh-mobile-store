...
const objectSummary = (value: unknown): string | null => {
  ...
  if (isRecord(value)) return `${faNumber(Object.keys(value).length)} مورد اطلاعات`;
  return null;
};
...
const readableValue = (value: unknown): string => {
  ...
  return objectSummary(value) || 'اطلاعات ساختاری';
};
...
const detailTitleByKey: Record<MlOperatorOverviewRouteKey, string> = {
  ...
  packageSnapshots: 'جزئیات سابقه بسته',
};
...
const commonFields = (source: MlOperatorRouteResult, record: Record<string, unknown>): DetailField[] => {
  const fields = [
    ...
    fieldFromKeys(record, 'شناسه گزینه', ['candidateMetadataId', 'candidateId', 'candidatePackageId', 'candidateSummaryId'], true),
    ...
    fieldFromKeys(record, 'کد صحت محتوا', ['contentHash', 'checksum', 'hash', 'receiptHash', 'importPayloadHash', 'exportPayloadHash', 'packageHash', 'snapshotHash'], true),
    ...
    fieldFromKeys(record, 'خلاصه اطلاعات ثبت‌شده', ['appliedMetadataSummary', 'safePayloadMetadataSummary', 'safeAuditMetadata', 'payloadSummary', 'summary']),
  ...
  if (!fields.some((field) => field.label === 'کد صحت محتوا') && source.latestChecksum) {
    fields.push({ label: 'کد صحت محتوا', value: source.latestChecksum, copyValue: safeCopy(source.latestChecksum) });
  }
  ...
  if (!selection) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
        ...
        <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">برای دیدن جزئیات، از کارت یا ردیف موردنظر گزینه مشاهده را بزنید.</p>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-right dark:border-amber-900/70 dark:bg-amber-950/25">
        <div className="flex items-center gap-2 text-sm font-black text-amber-900 dark:text-amber-100">
          ...
          اطلاعات ساختار کامل ندارد
        </div>
        <p className="mt-2 text-xs leading-6 text-amber-800/80 dark:text-amber-100/75">این مورد به شکل خلاصه برای بررسی نمایش داده می‌شود.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {fields.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
          <div className="text-sm font-black text-slate-800 dark:text-slate-100">اطلاعات قابل نمایش محدود است</div>
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 dark:border-emerald-900/70 dark:bg-emerald-950/20">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {['اطلاعات محدود', 'فقط مشاهده', 'بدون تغییر خودکار'].map((label) => (
