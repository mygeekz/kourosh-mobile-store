    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <h2 className="text-base font-black text-slate-950 dark:text-white">مقایسه مدیریتی منابع اطلاعات</h2>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500 dark:text-slate-400">
            این نما منابع قابل مشاهده را کنار هم قرار می‌دهد تا وضعیت، کد صحت، تعداد رکورد و تازگی اطلاعات در یک نگاه بررسی شود.
          </p>
        </div>
        ...
        <div className="grid min-w-full gap-2 sm:grid-cols-2 xl:min-w-[560px] xl:grid-cols-4">
          ...
          <MetricCard label="رکورد قابل مشاهده" value={toFaNumber(summary.totalRecords)} helper="جمع داده‌های بارگذاری‌شده" />
          <MetricCard label="پوشش کد صحت" value={`${toFaNumber(summary.checksumCoverage)}٪`} helper="برای تطبیق اطلاعات" />
          <MetricCard label="نیازمند توجه" value={toFaNumber(summary.attentionCount)} helper="نیازمند بررسی مدیریتی" />
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
          ...
          <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">بر اساس وضعیت، تعداد رکورد، شناسه و کد صحت.</p>
        </div>
        <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
      ...
      <div className="mt-4 rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          ...
          <div className="rounded-2xl bg-white p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:ring-slate-800">
            ...
            کپی فقط برای شناسه و کد صحت هر منبع فعال است.
          </div>
          <div className="rounded-2xl bg-white p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:ring-slate-800">
      ...
      {rows.length > 0 ? (
            ...
            {rows.map((row) => (
              <article key={row.source.key} className="grid grid-cols-12 gap-3 px-4 py-4 text-right">
                ...
                <div className="col-span-12 flex flex-wrap gap-2 md:col-span-2">
                  ...
                  <MlOperatorCopyButton value={row.source.latestChecksum} label="کپی کد صحت" compact />
                </div>
              </article>
            ))}
          </div>
        ...
        <div className="mt-4 rounded-[22px] border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
          ...
          <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">با تغییر فیلترها، مقایسه منابع بر اساس اطلاعات فعلی به‌روزرسانی می‌شود.</p>
        </div>
      )}
    </section>
  );
