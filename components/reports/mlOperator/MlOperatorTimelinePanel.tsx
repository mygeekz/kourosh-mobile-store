  ...
  return sources
    ...
    .flatMap((source) => {
      ...
      return baseItems.map((item, index) => {
        ...
        const description = source.items.length > 0 ? `رکورد ${toFaNumber(index + 1)} از ${sourceLabel}` : `آخرین وضعیت ${sourceLabel}`;
        return {
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500 dark:text-slate-400">
            این بخش ترتیب زمانی اطلاعات قابل مشاهده را برای بررسی مدیریتی نمایش می‌دهد.
          </p>
        </div>
      ...
      {visibleItems.length > 0 ? (
            ...
            <article key={event.key} className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/35">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2">
                  ...
                  <MlOperatorCopyButton value={event.checksum} label="کپی کد صحت" />
                  <button
                    type="button"
        ...
        <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center dark:border-slate-700 dark:bg-slate-950/35">
          <div className="text-sm font-black text-slate-800 dark:text-slate-100">رویدادی برای نمایش وجود ندارد</div>
