...
const routeLabels: Record<MlOperatorOverviewRouteKey, string> = {
  ...
  packageSnapshots: 'سوابق بسته‌ها',
};
...
const sourceDescription = (source: MlOperatorRouteResult): string => {
  if (source.state === 'ready') return 'این منبع داده دارد و برای بررسی مدیریتی آماده است.';
  if (source.state === 'empty') return 'منبع پاسخ معتبر دارد اما رکورد خروجی یا بسته‌ای برای مرور برنگشته است.';
  if (source.state === 'unauthorized') return 'دسترسی این منبع باید پیش از بررسی مدیریتی کنترل شود.';
  if (source.state === 'unavailable') return 'این منبع فعلاً در دسترس نیست و برای تصمیم‌گیری قابل اتکا نیست.';
...
const itemDescription = (item: unknown): string => {
  ...
  if (errors > 0) return `${toFaNumber(errors)} خطا در اطلاعات این ردیف ثبت شده است.`;
  if (warnings > 0) return `${toFaNumber(warnings)} هشدار برای بررسی مدیریتی وجود دارد.`;
  if (!safeItemHash(item)) return 'کد صحت این ردیف برای تطبیق موجود نیست.';
  if (!safeItemId(item)) return 'شناسه این ردیف برای پیگیری موجود نیست.';
  if (!statusLooksReady(status)) return status ? `وضعیت ردیف برای آمادگی نیازمند مرور است: ${status}` : 'وضعیت این ردیف در اطلاعات ثبت‌شده مشخص نیست.';
  return 'شناسه، کد صحت و وضعیت این ردیف برای بررسی مدیریتی آماده است.';
};
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500 dark:text-slate-400">
            این بخش وضعیت رسیدها و بسته‌های خروجی ثبت‌شده را برای بررسی آمادگی نمایش می‌دهد.
          </p>
        </div>

        <div className="grid min-w-full gap-2 sm:grid-cols-5 xl:min-w-[780px]">
          <MetricCard label="میانگین آمادگی" value={toFaNumber(summary.averageScore)} helper="بر اساس اطلاعات فعلی" />
          ...
          <MetricCard label="پوشش کد صحت" value={`${toFaNumber(summary.safeHashRows)} / ${toFaNumber(readinessRows.length)}`} helper="برای تطبیق اطلاعات" />
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-3">
        ...
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
          ...
          شناسه و کد صحت برای پیگیری قابل کپی است.
        </div>
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-6 text-slate-600 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-300 dark:ring-slate-800">
      ...
      {readinessRows.length > 0 ? (
        <div className="mt-4 overflow-hidden rounded-[22px] border border-slate-200 dark:border-slate-800">
          <div className="grid grid-cols-12 gap-2 bg-slate-100 px-4 py-3 text-[11px] font-black text-slate-500 dark:bg-slate-950/55 dark:text-slate-400">
            ...
            <div className="col-span-6 md:col-span-2">اطلاعات صحت</div>
            ...
            {readinessRows.map((row) => (
              <article key={row.key} className="grid grid-cols-12 gap-3 px-4 py-4 text-right">
                <div className="col-span-12 min-w-0 md:col-span-3">
                  ...
                  <div className="mt-2 text-[11px] font-bold text-slate-400 dark:text-slate-500">وضعیت: {row.status || 'نامشخص'}</div>
                </div>
                ...
                <div className="col-span-6 space-y-2 md:col-span-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <MlOperatorCopyButton value={row.safeHash} label="کپی کد صحت" compact />
                  </div>
                </div>
