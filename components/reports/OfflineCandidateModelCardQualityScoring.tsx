...
const statusLabel = (value?: string | null) => {
  ...
  if (!text) return '—';
  if (text.includes('ready')) return 'اطلاعات مدل کامل است';
  if (text.includes('warning')) return 'اطلاعات مدل نیازمند تکمیل است';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت امتیاز کیفیت مدل');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت امتیاز کیفیت مدل');
      } finally {
  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/70" aria-label="Offline candidate model card quality scoring">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-emerald-500">بررسی کیفیت اطلاعات مدل</span>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">امتیاز کیفیت و مستندات مدل</h3>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش کامل بودن اطلاعات و الزامات ایمنی مدل را ارزیابی می‌کند و هیچ تغییری در عملیات فروشگاه ایجاد نمی‌کند.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 rounded-2xl bg-white px-3 py-2 text-xs font-bold text-slate-500 dark:bg-slate-950 dark:text-slate-400">این امتیاز فقط کیفیت اطلاعات ثبت‌شده را نشان می‌دهد و به‌معنی فعال‌شدن خودکار مدل نیست.</p>
        </article>
      </div>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این صفحه کیفیت اطلاعات مدل را ارزیابی می‌کند و هیچ تغییر خودکاری در داده‌های تجاری انجام نمی‌شود.'}
      </p>
    </section>
  );
}
