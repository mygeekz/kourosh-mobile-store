...
const money = (v: unknown) => formatReportMoneyText(num(v));
const pct = (v: unknown) => formatExactPercentText(num(v));
...
export default function ReportsDecisionEngine({ compact = false }: { compact?: boolean }) {
  ...
  const score = Math.max(0, Math.min(100, (num(brain.score))));
  ...
  const auditScore = Math.max(0, Math.min(100, (num(audit?.score ?? brain.confidence ?? 100))));
  const issueCount = (num(audit?.counts?.total));
  const criticalCount = (num(audit?.counts?.critical));
  const warningCount = (num(audit?.counts?.warning));
  ...
  return (
    <section className="reports-ai-minibar reports-premium-panel print:hidden" dir="rtl" aria-label="هوش مالی گزارش" data-ui-report-ai-bar="trust" data-ui-report-surface="true">
      ...
      >
        ...
        <span className="reports-ai-minibar__meta">{formatExactNumberText(issueCount)} هشدار</span>
        <span className="reports-ai-minibar__meta">{formatExactNumberText(actions.length)} اقدام</span>
        ...
        <div className="reports-ai-minibar__details" data-ui-report-ai-details="true">
          <div className="reports-ai-minibar__grid">
            ...
            <div>
              ...
              <span>{formatExactNumberText(criticalCount)} بحرانی / {formatExactNumberText(warningCount)} هشدار</span>
