  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت اطلاعات آمادگی مدل');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت اطلاعات آمادگی مدل');
      } finally {
  ...
  return (
    <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-black text-slate-900 dark:text-white">آمادگی مدل برای استفاده</h3>
          <p className="text-xs font-bold text-slate-500">خلاصه وضعیت و کامل‌بودن اطلاعات مدل</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">امتیاز آمادگی</small>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">کامل‌بودن / ایمنی</small>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">پوشش اطلاعات</small>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">پایداری / محدودیت‌ها</small>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <article className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-900">
          <strong className="text-xs font-black text-slate-700 dark:text-slate-200">کنترل‌های آمادگی</strong>
        ...
        <article className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-900">
          <strong className="text-xs font-black text-slate-700 dark:text-slate-200">نشانه‌های آمادگی</strong>
          <ul className="mt-2 space-y-1 text-xs font-bold text-slate-500">
      ...
      <p className="mt-4 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">جزئیات آمادگی مدل در سوابق تحلیل هوشمند نگه‌داری می‌شود.</p>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این بخش صرفاً برای بررسی آمادگی مدل است و تغییری در اطلاعات فروشگاه ایجاد نمی‌کند.'}
      </p>
    </section>
  );
}
