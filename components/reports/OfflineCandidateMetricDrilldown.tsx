  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت جزئیات مدل');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت جزئیات مدل');
      } finally {
  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-slate-50/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70" aria-label="Offline candidate metric drilldown">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-cyan-500">جزئیات شاخص‌های مدل</span>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">جزئیات شاخص‌ها و توضیح‌پذیری مدل</h3>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش شاخص‌ها و توضیحات ثبت‌شده مدل را نمایش می‌دهد و هیچ اقدام خودکاری روی اطلاعات فروشگاه انجام نمی‌دهد.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
          ...
          <strong className="mt-1 block text-sm font-black text-slate-900 dark:text-white">فقط مشاهده</strong>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="mt-3 space-y-2">
            ...
            {!warnings.length && !limitations.length ? <p className="text-xs font-bold text-slate-500">هشدار یا محدودیتی ثبت نشده است.</p> : null}
          </div>
        </article>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              )) : <p className="text-xs font-bold text-slate-500">اطلاعات ورودی‌های مدل وجود ندارد.</p>}
            </div>
            <div className="space-y-2">
              ...
              )) : <p className="text-xs font-bold text-slate-500">اطلاعات اعتبارسنجی فایل وجود ندارد.</p>}
            </div>
          </div>
        </article>
      </div>
      ...
      <p className="mt-3 rounded-2xl bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-950 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این صفحه جزئیات ارزیابی مدل را نمایش می‌دهد و هیچ تغییر خودکاری در اطلاعات تجاری اعمال نمی‌کند.'}
      </p>
    </section>
  );
}
