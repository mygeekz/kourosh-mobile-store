  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-white/95 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/80" aria-label="Offline candidate threshold scenario metadata">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-indigo-500">بررسی سناریوهای آستانه</span>
          <h4 className="mt-1 text-base font-black text-slate-950 dark:text-white">Threshold Scenario Metadata</h4>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش سناریوهای آستانه و شاخص‌های دقت، بازیابی و F1 را برای مقایسه و تصمیم‌گیری نمایش می‌دهد.</p>
        </div>
      ...
      <p className="mt-4 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">جزئیات این بررسی در سوابق تحلیل هوشمند نگه‌داری می‌شود.</p>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این بخش صرفاً برای مقایسه سناریوهای آستانه است و تغییری در اطلاعات فروشگاه ایجاد نمی‌کند.'}
      </p>
    </section>
  );
}
