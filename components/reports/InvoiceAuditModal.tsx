...
export default function InvoiceAuditModal({
  ...
  const indicatorCards = [
    { label: 'فاکتورهای قابل کنترل', value: metricValue('فاکتورهای قابل کنترل', formatExactNumberText(auditRows.length)), icon: 'fa-file-invoice', tone: 'blue' },
    { label: 'سود منفی', value: metricValue('سود منفی', formatExactNumberText(reasonCountBy((text) => text.includes('سود منفی')))), icon: 'fa-arrow-trend-down', tone: 'rose' },
    { label: 'اختلاف جمع', value: metricValue('اختلاف جمع', formatExactNumberText(reasonCountBy((text) => text.includes('اختلاف') || text.includes('عدم تطابق')))), icon: 'fa-chart-line', tone: 'amber' },
    { label: 'تخفیف مشکوک', value: metricValue('تخفیف مشکوک', formatExactNumberText(reasonCountBy((text) => text.includes('تخفیف')))), icon: 'fa-tags', tone: 'emerald' },
  ];
  const topCards = [
    { label: 'فاکتورهای قابل کنترل', value: metricValue('فاکتورهای قابل کنترل', formatExactNumberText(auditRows.length)), icon: 'fa-file-lines', tone: 'blue' },
    { label: 'بالاترین ریسک', value: metricValue('بالاترین ریسک', `${formatExactNumberText(num(auditRows[0]?.riskScore))} از ۱۰۰`), icon: 'fa-shield-halved', tone: 'indigo' },
          ...
          <article className="acm193-panel acm193-memory">
            ...
            <div className="acm193-memory-grid">
              ...
              <div className="acm193-memory-card">
                ...
                <strong>{formatExactNumberText(num(selected.decision?.occurrenceCount))}</strong>
              </div>
            </div>
