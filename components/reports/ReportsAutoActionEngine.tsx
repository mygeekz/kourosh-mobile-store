  ...
  const actions = useMemo<AutoAction[]>(() => {
    ...
    if (issues > 0 || auditScore < 96) {
      list.push({
        ...
        summary: `${formatExactNumberText(issues)} مورد اختلاف/ریسک روی همین بازه دیده شده؛ قبل از ارسال رسمی گزارش بهتر است Drill-down بررسی شود.`,
        tone: auditScore < 85 ? 'critical' : 'warning',
        confidence: Math.max(70, Math.min(99, 100 - ((100 - auditScore) / 2))),
    ...
    if (score >= 75 && auditScore >= 96) {
      list.push({
        ...
        tone: 'positive',
        confidence: Math.min(99, ((score + auditScore) / 2)),
  ...
  return (
    <section className="reports-ai-minibar reports-ai-minibar--actions reports-premium-orbit reports-premium-panel print:hidden" dir="rtl" aria-label="اقدام‌های خودکار گزارش" data-ui-report-ai-bar="actions" data-ui-report-surface="true">
      ...
      >
        ...
        <span className="reports-ai-minibar__title">اقدام‌های پیشنهادی: {loading ? 'در حال تحلیل' : `${formatExactNumberText(actions.length)} مورد`}</span>
