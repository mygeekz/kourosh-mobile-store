...
function MetadataImportSavedViewUsagePanel({ savedViewUsagePayload, savedViewUsageRows, savedViewUsageLoading, savedViewUsageError, onApplySavedView }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/60" aria-label="خلاصه استفاده از نماهای ذخیره‌شده">
      <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
        <div>
          <strong className="text-xs font-black text-slate-800 dark:text-white">خلاصه استفاده از نماهای ذخیره‌شده</strong>
          <p className="mt-1 text-[11px] font-bold text-slate-500">خلاصه میزان استفاده از نماهای ذخیره‌شده برای دسترسی سریع‌تر؛ بدون ثبت جزئیات رفتاری کاربران.</p>
        </div>
      ...
      ) : !savedViewUsageLoading ? <p className="mt-2 text-xs font-bold text-slate-500">هنوز اطلاعات کافی برای نمایش میزان استفاده از نماهای ذخیره‌شده وجود ندارد.</p> : null}
    </div>
  );
}
