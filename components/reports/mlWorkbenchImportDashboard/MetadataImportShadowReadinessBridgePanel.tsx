...
const readinessLabel = (value?: MetadataToShadowReadinessBridgeRow['shadowCandidateReadiness']) => {
  ...
  if (value === 'needs_metadata_review_before_shadow_observation') return 'نیازمند بازبینی اطلاعات';
  ...
  if (!open) {
    return (
      <button type="button" onClick={onOpen} className="mlwb-v212-preview mt-4" aria-expanded={false} data-ml-readiness-bridge-anchor={SHADOW_READINESS_BRIDGE_TECHNICAL_GUARD_ANCHORS_VALUE}>
        <span className="mlwb-v212-preview__eyebrow text-sky-500">آمادگی برای پایش کنترل‌شده</span>
        <strong className="mlwb-v212-preview__title">آمادگی مدل برای پایش کنترل‌شده</strong>
        <span className="mlwb-v212-preview__desc">این بخش میزان آمادگی مدل‌ها برای پایش کنترل‌شده و ارزیابی تکمیلی را نمایش می‌دهد.</span>
        <span className="mlwb-v212-preview__meta">
          ...
          <em className="mlwb-v212-chip not-italic"><i className="fa-solid fa-eye-slash" /> بدون تغییر خودکار</em>
        </span>
      </button>
    );
  }
  ...
  return (
    <div className="mlwb-v212-shell mt-4 rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/80" data-ml-readiness-bridge-anchor={SHADOW_READINESS_BRIDGE_TECHNICAL_GUARD_ANCHORS_VALUE}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-extrabold text-slate-900 dark:text-white">آمادگی مدل‌ها برای پایش کنترل‌شده</p>
          <p className="mt-1 text-xs leading-6 text-slate-500 dark:text-slate-400">این بخش فقط وضعیت آمادگی مدل‌ها را نمایش می‌دهد و تغییری در اطلاعات فروشگاه ایجاد نمی‌کند.</p>
        </div>
