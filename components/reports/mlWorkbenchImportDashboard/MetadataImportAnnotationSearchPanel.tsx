  ...
  return (
    <form className="grid gap-2 rounded-2xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/60" onSubmit={props.onSubmit}>
      <div className="flex flex-col gap-1">
        <strong className="text-xs font-black text-slate-800 dark:text-white">جستجو و فیلتر یادداشت‌ها</strong>
        <span className="text-[11px] font-bold text-slate-500">یادداشت‌ها را بر اساس مدل، متن، نوع، سطح اهمیت و بازه زمانی جستجو و فیلتر کنید.</span>
      </div>
      <div className="grid gap-2 md:grid-cols-3">
        ...
        <SelectField controlOnly unstyled showChevron={false} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" value={props.annotationFilterScope} onChange={(event) => props.setAnnotationFilterScope(event.target.value as ReviewAnnotationScope | '')}>
          <option value="">همه بخش‌ها</option><option value="metadata_result">نتیجه تحلیل</option><option value="trend_signal">نشانه روند</option><option value="offline_metrics_comparison">شاخص‌های تحلیلی</option><option value="dashboard">داشبورد</option>
        </SelectField>
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <span className="text-xs font-bold text-slate-500">Filters: {nf.format(props.annotationFilterCount)} · Results: {nf.format(props.annotationResultCount)} · {props.activeSavedViewId ? `Preset: ${props.activeSavedViewId}` : 'جستجوی یادداشت‌ها'}</span>
