  ...
  if (!props.open) {
    return (
      <button type="button" className="mlwb-v212-preview mt-4" aria-label="پیش‌نمایش فضای یادداشت و مرور" aria-expanded={false} onClick={props.onOpen}>
        <span className="mlwb-v212-preview__eyebrow text-emerald-500">یادداشت‌ها و مرور</span>
        <strong className="mlwb-v212-preview__title">فضای یادداشت و مرور</strong><span className="mlwb-v212-preview__desc">یادداشت‌ها، جستجو و نماهای ذخیره‌شده برای مرور نتایج تحلیلی در همین بخش مدیریت می‌شوند.</span><span className="mlwb-v212-preview__meta"><em className="mlwb-v212-chip not-italic"><i className="fa-solid fa-note-sticky" /> اطلاعات تحلیلی</em><em className="mlwb-v212-chip not-italic"><i className="fa-solid fa-magnifying-glass" /> جستجو و فیلتر</em></span>
      </button>
    );
  }

  return (
    <div className="mt-4 rounded-[24px] border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900" aria-label="فضای یادداشت و مرور">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black tracking-[0.08em] text-emerald-500">فضای یادداشت و مرور</span>
          <span className="sr-only">یادداشت‌ها و مرور کارشناسی</span>
          <h4 className="mt-1 text-sm font-black text-slate-950 dark:text-white">فضای فشرده برای یادداشت، جستجو و نماهای ذخیره‌شده</h4>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش یادداشت‌ها، جستجو و نماهای ذخیره‌شده را برای مرور نتایج در یک فضای یکپارچه نمایش می‌دهد.</p>
        </div>
      ...
      <div className="mt-3 grid gap-2 rounded-2xl border border-slate-100 bg-white p-2 text-[11px] font-black text-slate-500 md:grid-cols-4 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400" aria-label="خلاصه یادداشت‌ها">
        ...
        <span><i className="fa-solid fa-user-shield ml-1" /> بدون ثبت رفتار کاربر</span>
      </div>
      ...
      <div className="mt-3 grid gap-3 xl:grid-cols-[0.95fr_1.05fr]" aria-label="نماهای ذخیره‌شده">
      ...
      <div className="mt-3 grid gap-3 xl:grid-cols-[1.08fr_0.92fr]" aria-label="فرم یادداشت">
        ...
        <form className="grid gap-2 rounded-2xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/60" onSubmit={props.onSaveAnnotation}>
          <div className="grid gap-2 md:grid-cols-3">
            <SelectField controlOnly unstyled showChevron={false} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" value={props.noteScope} onChange={(event) => props.setNoteScope(event.target.value as ReviewAnnotationScope)}><option value="metadata_result">نتیجه تحلیل</option><option value="trend_signal">سیگنال روند</option><option value="offline_metrics_comparison">متریک‌های آفلاین</option><option value="dashboard">داشبورد</option></SelectField>
