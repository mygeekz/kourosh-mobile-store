...
function MetadataImportTrendRegressionPanel({ open, trendPayload, trendLoading, trendError, onOpen }: Props) {
  ...
  if (!open) {
    return (
      <button type="button" className="mlwb-v212-preview mt-4" aria-label="پیش‌نمایش پنل روند و رگرسیون" aria-expanded={false} onClick={onOpen}>
        <span className="mlwb-v212-preview__eyebrow text-rose-500">روند تغییر شاخص‌ها</span>
  ...
  return (
    <div className="mlwb-v212-shell mt-4 rounded-[24px] border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900" aria-label="روند تغییر شاخص‌ها">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black tracking-[0.08em] text-rose-500">خلاصه روند تغییر شاخص‌ها</span>
          <h4 className="mt-1 text-sm font-black text-slate-950 dark:text-white">سیگنال‌های روند از تاریخچه ذخیره‌شده</h4>
          <p className="mt-1 text-xs font-bold leading-6 text-slate-500 dark:text-slate-400">این بخش روند نتایج ثبت‌شده، افت شاخص‌ها و افزایش هشدارها یا خطاهای مهم را در طول زمان نمایش می‌دهد.</p>
        </div>
