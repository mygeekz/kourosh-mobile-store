...
function MetadataImportSavedViewsPanel({ savedViewsPayload, savedViews, savedViewsLoading, savedViewsError, activeSavedViewId, annotationLoading, onApplySavedView }: Props) {
  return (
    <div className="rounded-2xl border border-sky-100 bg-white p-3 dark:border-sky-900/50 dark:bg-slate-950/60" aria-label="نماهای ذخیره‌شده یادداشت‌ها">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          <strong className="text-xs font-black text-slate-800 dark:text-white">نماهای ذخیره‌شده و دسترسی سریع</strong>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">نماهای سریع فقط شیوه نمایش و فیلتر اطلاعات را تغییر می‌دهند و روی اطلاعات ثبت‌شده اثری ندارند.</p>
        </div>
        <span className="text-xs font-black text-slate-500">{savedViewsLoading ? 'در حال دریافت نماها…' : `${nf.format(savedViewsPayload?.summary?.savedViewCount || savedViews.length)} نمای ذخیره‌شده`}</span>
      </div>
