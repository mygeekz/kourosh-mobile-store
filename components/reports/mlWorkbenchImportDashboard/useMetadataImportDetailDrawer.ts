  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت جزئیات نتیجه تحلیل مدل');
        if (active) setDetailPayload(json?.data || null);
      } catch (err) {
        if (active) setDetailError(err instanceof Error ? err.message : 'خطا در دریافت جزئیات نتیجه تحلیل مدل');
      } finally {
