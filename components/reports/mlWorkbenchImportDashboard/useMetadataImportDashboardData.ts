  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت سوابق نتایج تحلیل مدل');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت سوابق نتایج تحلیل مدل');
      } finally {
