...
function OfflineCandidateTrainingPackageQualitySnapshot({ metadataImportId, onClear }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || json?.success === false) throw new Error(json?.message || 'خطا در دریافت کیفیت داده آموزشی');
        if (active) setPayload(json?.data || null);
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'خطا در دریافت کیفیت داده آموزشی');
      } finally {
  ...
  return (
    <section className="mt-4 rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/70" aria-label="خلاصه کیفیت داده آموزشی">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <span className="text-[11px] font-black uppercase tracking-[0.24em] text-cyan-500">بررسی کیفیت داده‌های آموزش</span>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">نمای کیفیت داده آموزشی</h3>
          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">این بخش نتایج ارزیابی کیفیت داده آموزشی را خلاصه می‌کند و هیچ تغییری در عملیات فروشگاه ایجاد نمی‌کند.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <small className="text-xs font-bold text-slate-500">ردیف‌ها / ویژگی‌ها</small>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          <h4 className="text-sm font-black text-slate-900 dark:text-white">خلاصه داده‌های آموزش</h4>
          <dl className="mt-3 grid gap-2 text-xs">
            ...
            <div className="rounded-2xl bg-white px-3 py-2 dark:bg-slate-950">
              <dt className="font-black text-slate-700 dark:text-white">ویژگی / هدف</dt>
        ...
        <article className="rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {!errors.length && !warnings.length ? <span className="rounded-2xl bg-white px-3 py-2 text-xs font-black text-emerald-600 dark:bg-slate-950 dark:text-emerald-300">در اطلاعات ثبت‌شده هشدار یا خطای مهمی وجود ندارد.</span> : null}
          </div>
          <p className="mt-3 rounded-2xl bg-white px-3 py-2 text-xs font-bold text-slate-500 dark:bg-slate-950 dark:text-slate-400">این خلاصه فقط کیفیت داده آموزشی را نشان می‌دهد و باعث فعال‌شدن خودکار مدل نمی‌شود.</p>
        </article>
      </div>
        ...
        <Table layout="managed" density="comfortable" className="min-w-full divide-y divide-slate-100 text-right text-xs dark:divide-slate-800">
          ...
          <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-950">
            {checks.length ? checks.map((check) => (
              ...
              <tr>
                <td colSpan={5} className="px-3 py-4 text-center font-bold text-slate-500">هنوز اطلاعات ارزیابی داده‌های آموزش برای این مدل ثبت نشده است.</td>
              </tr>
            )}
          </tbody>
        </Table>
      ...
      <p className="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-slate-900 dark:text-slate-300">
        {summary?.recommendedNextAction || 'این صفحه کیفیت داده آموزشی را خلاصه می‌کند و هیچ تغییر خودکاری در اطلاعات تجاری اعمال نمی‌کند.'}
      </p>
    </section>
  );
}
