...
export default function useSmartInsightPayloadSlices({
  ...
  return useMemo(() => {
    ...
    const profitRealizedShare = num(profitSummary.realProfit) > 0
      ? ((num(profitSummary.recognizedProfit) / Math.max(1, num(profitSummary.realProfit))) * 100)
      : 0;
    const profitRiskShare = num(profitSummary.realProfit) > 0
      ? ((num(profitSummary.profitAtRisk) / Math.max(1, num(profitSummary.realProfit))) * 100)
      : 0;
    ...
    const customerAvgRisk = customerIntelligence.length
      ? (customerIntelligence.reduce((sum, customer) => sum + num(customer.riskScore), 0) / customerIntelligence.length)
      : 0;
    ...
    const executiveTone = executiveBrain.status === 'excellent'
      ? 'emerald'
      : executiveBrain.status === 'healthy'
        ? 'indigo'
        : executiveBrain.status === 'watch'
          ? 'amber'
          : executiveBrain.status === 'no_data'
            ? 'slate'
            : 'rose';
