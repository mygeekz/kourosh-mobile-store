...
export default function AlertManagementModal({
  ...
  const getReasonText = (reason: AlertReason) => {
    ...
    return String(record.text || record.label || record.value || 'دلیل ثبت‌شده در سامانه');
  };
  ...
  const confidenceAvg = safeAlertItems.length
    ? (safeAlertItems.reduce((sum, item) => sum + num(item.confidence), 0) / safeAlertItems.length)
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      value: formatExactNumberText(alertCounts.all),
      hint: `${formatExactNumberText(alertCounts.done)} مورد بسته‌شده`,
      icon: 'fa-file-circle-plus',
      tone: 'blue',
    },
    {
      label: 'پیشنهادهای باز',
      value: formatExactNumberText(alertCounts.action),
      hint: `${formatExactNumberText(alertCounts.critical)} مورد فوری/مهم`,
      icon: 'fa-bolt',
      tone: 'orange',
    },
  ];
          ...
          <>
            ...
            <div className="sib211-detail-grid sib217-detail-grid">
              ...
              <div>
                ...
                <small>اولویت</small>
                <strong>{formatExactNumberText(selectedBoardItem.priority)}</strong>
              </div>
              <div>
            ...
            {selectedBoardItem.metrics?.length ? (
              <section className="sib211-metrics">
                <h4><i className="fa-solid fa-chart-simple" /> شاخص‌های ثبت‌شده</h4>
            ...
            <section className="sib211-reasons">
              ...
              {!selectedBoardItem.reasons?.length ? <p className="sib211-empty"><i className="fa-solid fa-database" /> دلیل تکمیلی برای این مورد ثبت نشده است.</p> : null}
            </section>
      ...
      <section className="sib211-list" aria-label="لیست پیشنهادهای هوشمند">
        <div className="sib211-list__toolbar">
          <div>
            ...
            <span>{formatExactNumberText(visibleItems.length)} مورد قابل نمایش</span>
          </div>
          <div className="sib211-tabs">
            {filterTabs.map((tab) => (
              ...
              >
                ...
                {tab.label}
                <b>{formatExactNumberText(tab.count)}</b>
              </button>
            ))}
          </div>
        </div>
