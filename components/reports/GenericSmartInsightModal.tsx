...
function GenericSmartInsightModal({
  ...
  const modal = (
    <div className="smart-insight-modal-v185 smart-insight-modal-v185--sidebar-safe smart-insight-modal-v185--generic-root" style={modalStyle} onClick={() => onClose()}>
      <div className="smart-insight-modal-v185__surface w-full overflow-hidden rounded-[30px] border border-slate-200/95 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950" onClick={(e) => e.stopPropagation()}>
        <div className="smart-insight-modal-v185__header flex items-start gap-4 border-b border-slate-100 p-6 dark:border-slate-800">
          <button type="button" className="smart-insight-modal-v185__close" onClick={() => onClose()} aria-label="بستن پنجره تحلیل">
            <i className="fa-solid fa-xmark" />
          </button>
          <div className="smart-insight-modal-v185__header-icon" aria-hidden="true">
            <i className="fa-solid fa-wand-magic-sparkles" />
          </div>
          <div className="smart-insight-modal-v186__title min-w-0">
            <div className="smart-insight-modal-v185__eyebrow">
              <span className={`smart-insight-modal-v185__severity inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-black ${severityMeta[selected.severity]?.badge || severityMeta.medium.badge}`}>
                <i className="fa-solid fa-circle-dot" />
                {severityMeta[selected.severity]?.label || selected.severity}
              </span>
              <span className="smart-insight-modal-v185__type-chip"><i className="fa-solid fa-sparkles" /> تحلیل اجرایی</span>
            </div>
        ...
        <div className="smart-insight-modal-v185__body overflow-y-auto p-6">
          <div className="smart-insight-modal-v186__hero-grid grid gap-3 sm:grid-cols-3">
            <div className="smart-insight-modal-v186__hero-card rounded-[20px] border border-slate-200 bg-slate-50/90 p-4 dark:border-slate-800 dark:bg-slate-900">
              <span className="smart-insight-modal-v186__hero-icon" aria-hidden="true"><i className="fa-solid fa-bolt" /></span>
              <div className="text-[11px] font-black text-slate-500">امتیاز</div>
              <div className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{formatExactNumberText(num(selected.score))}</div>
            </div>
            <div className="smart-insight-modal-v186__hero-card rounded-[20px] border border-slate-200 bg-slate-50/90 p-4 dark:border-slate-800 dark:bg-slate-900">
              <span className="smart-insight-modal-v186__hero-icon" aria-hidden="true"><i className="fa-solid fa-shield-halved" /></span>
              <div className="text-[11px] font-black text-slate-500">اعتماد تحلیل</div>
              <div className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{percent(selected.confidence)}</div>
            </div>
            <div className="smart-insight-modal-v186__hero-card rounded-[20px] border border-slate-200 bg-slate-50/90 p-4 dark:border-slate-800 dark:bg-slate-900">
              <span className="smart-insight-modal-v186__hero-icon" aria-hidden="true"><i className="fa-solid fa-layer-group" /></span>
              <div className="text-[11px] font-black text-slate-500">دسته</div>
              <div className="mt-2 text-sm font-black text-slate-900 dark:text-white">{selected.category}</div>
            </div>
          </div>
          ...
          <div className="smart-insight-modal-v186__content-grid mt-4 grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
            ...
            <section className="smart-insight-modal-v186__panel smart-insight-modal-v186__panel--memory rounded-[24px] border border-indigo-200 bg-indigo-50/45 p-5 dark:border-indigo-500/25 dark:bg-indigo-500/10">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-3">
                <div className="rounded-2xl bg-white/90 p-3 dark:bg-slate-950/70"><div className="text-[11px] font-black text-slate-500">دفعات تکرار</div><div className="mt-1 text-sm font-black text-slate-900 dark:text-white">{formatExactNumberText(num(selected.decision?.occurrenceCount))}</div></div>
