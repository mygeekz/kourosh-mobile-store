      ...
      <div className="sales-summary-card rounded-[20px] border border-slate-200 bg-white/92 p-3 dark:border-slate-800 dark:bg-slate-950/70" data-ui-sales-summary-card="settings">
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <SearchableSelectField<number>
                ...
                options={customers.map((customer) => {
                  const name = (customer as any).fullName || (customer as any).name || `مشتری #${customer.id}`;
                  const phone = (customer as any).phoneNumber || (customer as any).phone || (customer as any).mobile || '';
                  return {
                    value: Number(customer.id),
                    label: `${name}${phone ? ` — ${phone}` : ''}`,
                    searchText: `${name} ${phone} ${customer.id}`,
                  };
                })}
          ...
          <div>
            ...
            <ShamsiDatePicker selectedDate={salesDate} onDateChange={onDateChange} preview="انتخاب تاریخ" hideIcon />
          </div>
