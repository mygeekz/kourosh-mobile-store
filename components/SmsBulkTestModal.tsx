...
const SmsBulkCheckModal: React.FC<Props> = ({ isOpen, onClose, patterns, defaultSelectedKeys, getBodyId }) => {
  ...
  useEffect(() => {
    ...
    fetchAllowedSmsTestRecipients(token)
      .then(({ items }) => {
        ...
        setRecipient((current) => items.some((item) => item.phone === current) ? current : (items[0]?.phone || ''));
      })
      .catch((error: any) => {
        if (cancelled) return;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/sms/bulk-check', {
        method: 'POST',
        headers: {
      ...
      setResult({ ok: false, data: { message: e?.message || 'ارتباط با سرور برای بررسی گروهی ارسال برقرار نشد.' } });
    } finally {
      setIsSending(false);
    }
  };
      ...
      <div className="sms-bulk-test-modal">
        <div className="sms-bulk-test-modal__intro rounded-xl border border-gray-200 dark:border-gray-700 bg-white/60 dark:bg-gray-800/40 p-3">
          ...
          <div className="app-subtle mt-1 text-xs">
            ابتدا قالب‌های پیامک را انتخاب کن؛ سپس یک شماره مجاز تست را انتخاب و متغیرها را برای ارسال نمونه تکمیل کن.
          </div>
        </div>
        ...
        {step === 1 ? (
              ...
              {patterns.map((p) => {
                ...
                return (
                  <Button
                    ...
                    className="w-full justify-start rounded-2xl p-3 text-right"
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="inline-flex items-center gap-1 rounded-lg bg-black/5 px-2 py-1 text-xs text-current dark:bg-white/10"
                      >
          ...
          <>
            <div className="sms-bulk-test-modal__recipient rounded-2xl border border-slate-200 bg-white/75 p-3 dark:border-slate-700 dark:bg-slate-900/40">
              <label className="app-label">شماره مجاز گیرنده برای بررسی ارسال</label>
              <SelectField controlOnly className="app-input" value={recipient} onChange={(e) => setRecipient(e.target.value)} disabled={recipientsLoading || allowedRecipients.length === 0}>
              ...
              <div className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">ارسال گروهی نیز فقط به Allowlist ذخیره‌شده روی سرور انجام می‌شود.</div>
            ...
            {result ? (
              <div className={`sms-bulk-test-modal__result rounded-xl border p-3 text-sm ${result.ok ? 'border-green-200 bg-green-50 text-green-800 dark:border-green-900/30 dark:bg-green-900/20 dark:text-green-200' : 'border-red-200 bg-red-50 text-red-800 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-200'}`}>
                <div className="font-semibold">{result.ok ? 'نتیجه بررسی گروهی' : 'خطا در بررسی گروهی ارسال'}</div>
                <div className="mt-1">{result.data?.message || (result.ok ? 'بررسی گروهی ارسال انجام شد.' : humanizeSmsError(result.data?.message || result.data).message)}</div>
            ...
            <div className="sms-bulk-test-modal__footer flex items-center justify-between gap-2">
              ...
              <Button
                type="button"
                disabled={!recipient.trim() || selected.length === 0 || recipientsLoading || allowedRecipients.length === 0}
                onClick={submit}
                variant="primary"
                loading={isSending}
