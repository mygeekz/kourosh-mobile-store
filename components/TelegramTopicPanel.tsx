...
const TelegramTopicPanel: React.FC<Props> = ({ topic, title, allowedTypes }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/telegram/topic-config/${encodeURIComponent(topic)}`, {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res2 = await apiFetch(`/api/telegram/topic-config/${encodeURIComponent(topic)}/templates`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(getAuthHeaders(token) as any),
  ...
  const preview = async () => {
    try {
      const res = await apiFetch(`/api/telegram/topic-config/${encodeURIComponent(topic)}/preview`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(getAuthHeaders(token) as any),
  ...
  const sendCheck = async () => {
    ...
    try {
      const json: any = await runWithFeedback(
        apiFetch(`/api/telegram/topic-config/${encodeURIComponent(topic)}/check`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(getAuthHeaders(token) as any),
          ...
          body: JSON.stringify({ type: activeType, template: tplText }),
        }).then((response) => parseApiResult(response, { endpoint: '/api/telegram/topic-config/check', action: 'بررسی ارسال تلگرام' })),
        {
          kind: 'send',
          loading: 'در حال بررسی ارسال تلگرام…',
          success: 'بررسی ارسال تلگرام با موفقیت انجام شد.',
          endpoint: '/api/telegram/topic-config/check',
        }
      );
      const sent = json.data?.sent ?? 0;
      const total = json.data?.total ?? 0;
      setNotification({ type: 'success', message: `بررسی ارسال انجام شد. موفق: ${sent} از ${total}` });
    } catch (e: any) {
      setNotification({ type: 'error', message: humanizeErrorMessage(e?.message || 'بررسی ارسال انجام نشد', { endpoint: '/api/telegram/topic-config/check', action: 'بررسی ارسال تلگرام' }) });
    } finally {
