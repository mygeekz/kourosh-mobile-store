...
export default function DataTableShell<T = never>({
  ...
  return (
    ...
    >
      ...
      <TableViewport
        ariaLabel={typeof title === 'string' ? title : 'جدول داده'}
        className="ux-table-shell__body [&&]:!bg-transparent"
        data-ui-table-scroll="true"
      >
        {hasStructuredRows ? (
          <Table
            className={cn('report-table ux-data-table', tableClassName)}
            density="comfortable"
            layout="managed"
          >
            <thead>
      ...
      {pagination ? <TablePagination {...pagination} /> : footer ? (
