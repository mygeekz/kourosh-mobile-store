...
const ModalField: React.FC<ModalFieldProps> = ({ label, iconClass, required, error, hint, children, className }) => {
  ...
  const trailingAction = isElement ? childProps.trailingAction : undefined;
  ...
  if (isElement) {
    ...
    if (primaryChild.type === TextField) {
      ...
      const { size: _nativeSelectSize, trailingAction: _selectTrailingAction, ...selectChildProps } = childProps;
      ...
      const { trailingAction: _textareaTrailingAction, ...textareaChildProps } = childProps;
      renderedChild = React.createElement(TextareaField, {
        ...(textareaChildProps as React.TextareaHTMLAttributes<HTMLTextAreaElement>),
        ...commonControlProps,
        controlOnly: true,
        icon: controlIcon,
        error: error || undefined,
  ...
  if (trailingAction && renderedChild) {
    renderedChild = (
      <>
        {renderedChild}
        <span className="app-form-field__trailing-action modal-field-premium__trailing-action">{trailingAction}</span>
      </>
    );
  }
