...
const inferContent = (title: string, actionLabel?: string): InferredEmptyState => {
  ...
  if (/تعمیر/.test(normalized)) {
    return {
      ...
      description: 'هنوز پرونده تعمیری ثبت نشده است. با ایجاد نخستین پذیرش، وضعیت، هزینه و تحویل دستگاه از همین مرکز مدیریت می‌شود.',
      actionLabel: actionLabel || 'ثبت پذیرش تعمیر',
      tone: 'violet',
...
const normalizeFontAwesomeIconClass = (iconClassName: string): string => {
  const normalized = iconClassName.trim();
  if (!normalized) return 'fa-solid fa-folder-open';
  if (/\b(?:fa-solid|fa-regular|fa-brands|fas|far|fab)\b/.test(normalized)) return normalized;
  if (/^fa-[a-z0-9-]+$/i.test(normalized)) return `fa-solid ${normalized}`;
  return 'fa-solid fa-folder-open';
};

const renderEmptyStateIcon = (icon: React.ReactNode): React.ReactNode => {
  if (typeof icon === 'string') {
    return <i className={cn(normalizeFontAwesomeIconClass(icon), 'text-2xl')} aria-hidden="true" />;
  }
  return icon;
};
  ...
  return (
    <div className={cn('empty-state-modern premium-no-result premium-empty-state-card', `premium-empty-state-card--${resolvedTone}`, className)} data-ui-surface="empty-state" data-ui-card="true" dir="rtl">
      <div className={cn('premium-empty-state-card__icon empty-state-modern__icon', `premium-empty-state-card__icon--${resolvedTone}`)}>
        {renderEmptyStateIcon(resolvedIcon)}
      </div>
