...
const EventField: React.FC<EventFieldProps> = ({ label, children, valueClassName = '' }) => (
  <div className="min-w-0 bg-slate-50/95 px-2.5 py-2.5 sm:px-3 dark:bg-slate-900/75">
...
const FinancialTimelineEvent: React.FC<FinancialTimelineEventProps> = ({
  ...
  const eventNode = (
    ...
    <div
      className="mt-3 grid min-w-0 grid-cols-[repeat(auto-fit,minmax(10rem,1fr))] gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200 dark:border-slate-800 dark:bg-slate-800"
      data-ui-financial-event-fields="adaptive"
    >
      <EventField label={sourceLabel} valueClassName="break-words text-slate-800 dark:text-slate-100 [overflow-wrap:anywhere]">{source}</EventField>
      <EventField label={amountLabel} valueClassName={`whitespace-nowrap break-normal tabular-nums [overflow-wrap:normal] ${valueToneClassMap[amountTone]}`}>{amount}</EventField>
      <EventField label={dateLabel} valueClassName="whitespace-nowrap break-normal [overflow-wrap:normal] text-slate-800 dark:text-slate-100">{date}</EventField>
      ...
      <EventField label={deepLinkLabel}>
        {resolvedDeepLinks.length > 0 ? (
          <div className="flex min-w-0 flex-col items-stretch gap-1.5 sm:flex-row sm:flex-wrap sm:items-center">
