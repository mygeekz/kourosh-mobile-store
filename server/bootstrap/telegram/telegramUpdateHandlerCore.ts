      ...
      if (startPayload.startsWith("plink_") || startPayload.startsWith("staff_")) {
        ...
        try {
          const result = securityLinking
            ...
            : { ok: false };
          if (!result.ok) {
            if (isPartner && result.code === "REBIND_REJECTED") {
              await sendSecurityMessage(chatId, "این پرونده قبلاً به حساب تلگرام دیگری متصل است. از پروفایل همکار، «اتصال مجدد تلگرام» را انتخاب و لینک جدید صادر کنید.");
              return;
            }
            if (isPartner && result.code === "REBIND_STALE") {
              await sendSecurityMessage(chatId, "وضعیت اتصال این پرونده بعد از صدور لینک تغییر کرده است. برای امنیت، یک لینک اتصال مجدد جدید از پروفایل همکار بسازید.");
              return;
            }
            ...
            await sendBotMessage(
              ...
              telegramCard(
                ...
                linked.reason === "rebind_rejected"
                  ? "این پرونده قبلاً به حساب تلگرام دیگری متصل شده است. برای تغییر حساب، مدیر باید از پروفایل همکار «اتصال مجدد تلگرام» را انتخاب کند."
