...
const roundMoney = (value: number): number => Number.isFinite(value) ? value : 0;
...
export function buildFinancialAudit(input: FinancialAuditInput) {
  ...
  for (const sale of installmentRows) {
    ...
    const isCheckSale = String(sale.saleType || '').trim() === 'check' || n(sale.numberOfInstallments) === 0;
    const manualReceiptAmount = Math.max(0, n(sale.manualReceiptAmount));
    const scheduledDebt = isCheckSale
      ? n(sale.checkScheduledAmount) + manualReceiptAmount
      : n(sale.scheduledAmount);
    const scheduledTotal = n(sale.downPayment) + scheduledDebt;
    const totalMismatch = diff(scheduledTotal, actualSalePrice);
    if (isMeaningful(totalMismatch, 0.00001)) {
      pushIssue(issues, {
        id: `installment-total-${id}`,
        severity: 'warning',
        area: 'payments',
        title: 'اختلاف مبلغ فروش اقساط با برنامه پرداخت',
        description: isCheckSale ? 'جمع پیش‌پرداخت و چک‌های قرارداد با قیمت فروش واقعی برابر نیست.' : 'جمع پیش‌پرداخت و برنامه واقعی اقساط با قیمت فروش واقعی برابر نیست.',
        entityType: 'installment_sale',
        entityId: id,
        expected: actualSalePrice,
        actual: scheduledTotal,
        difference: totalMismatch,
        actionHint: isCheckSale ? 'مبلغ چک‌ها یا قیمت فروش واقعی را اصلاح کن.' : 'برنامه اقساط ذخیره‌شده یا قیمت فروش واقعی را بررسی کن.',
      });
    }
    const collected = n(sale.downPayment) + n(sale.paidAmount) + n(sale.cashedCheckRemainder) + manualReceiptAmount;
    if (collected - actualSalePrice > 0.00001) {
      pushIssue(issues, {
