 ...
 * - totalSales: مجموع فروش فعال (SUM(grandTotal))
 *
...
export async function salesSummary(): Promise<{ totalOrders: number; totalSales: number | null }> {
  ...
  return new Promise((resolve, reject) => {
    db.get(
      "SELECT COUNT(*) as totalOrders, COALESCE(SUM(grandTotal), 0) as totalSales FROM sales_orders WHERE (status IS NULL OR status = 'active')",
      [],
      (err: any, row: any) => {
