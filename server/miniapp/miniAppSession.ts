...
export interface MiniAppSessionStore {
  ...
  revokeTelegramUser(telegramUserId: string): number;
  cleanup(): void;
  size(): number;
}
...
export const createMiniAppSessionStore = (clock: Clock = Date.now): MiniAppSessionStore & { revokeStaff(userId: number): number } => {
  ...
  const revokeTelegramUser = (telegramUserId: string): number => {
  ...
  return { create, get, revokeToken, revokeStaff, revokeIdentity, revokeTelegramUser, cleanup, size: () => sessions.size };
};
...
export const revokeMiniAppTelegramUserSessions = defaultStore.revokeTelegramUser;
