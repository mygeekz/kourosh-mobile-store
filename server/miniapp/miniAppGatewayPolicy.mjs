...
export const isAllowedMiniAppManagerMutationPath = (pathname) => /^\/api\/miniapp\/manager\/notifications\/(?:[1-9]\d*\/read|read-all)$/.test(String(pathname || ""));
...
export const isAllowedMiniAppApiPath = (pathname) => pathname === "/api/miniapp/auth"
  ...
  || pathname.startsWith("/api/miniapp/manager/");

export const isAllowedMiniAppApiRequest = (method, pathname) => {
  const normalized = String(method || "").toUpperCase();
  if (pathname === "/api/miniapp/auth") return normalized === "POST";
  if (isAllowedMiniAppManagerMutationPath(pathname)) return normalized === "POST";
...
export const classifyMiniAppGatewayRequest = ({ method, pathname, hasBody = false, contentLength = 0, allowOptions = false }) => {
  ...
  if (isAnyApiPath(pathname)) {
    ...
    if (isAllowedMiniAppManagerMutationPath(pathname) && hasBody) return { allowed: false, status: 400, code: "MUTATION_BODY_NOT_ALLOWED", kind: "rejected" };
