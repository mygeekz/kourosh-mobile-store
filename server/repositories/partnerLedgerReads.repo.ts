...
import { getPartnerLedgerCanonicalBalance } from "../db/ledgerBalanceConsistency";
...
export const getPartnerLedgerProfileSnapshotFromDb = async (
  ...
  const [items, summary, canonicalBalance] = await Promise.all([
    allAsync(
      `${ledgerCte}
       SELECT id, partnerId, transactionDate, createdAt, updatedAt, description, debit, credit, balance,
        ...
        ORDER BY datetime(COALESCE(transactionDate, createdAt, updatedAt)) DESC, id DESC
        LIMIT ?`,
      [safePreviewSize],
    ),
    getAsync(
    ...
    getPartnerLedgerCanonicalBalance(partnerId),
  ]);
  return {
    items: items || [],
    summary: {
      ...
      latestBalance: Number(canonicalBalance || 0),
    },
  };
};
...
export const listPartnerLedgerDirectoryFromDb = async (
  ...
  const [rows, countRow, overallSummary, filteredSummary, systemRows, batchRows, canonicalBalance] = await Promise.all([
    allAsync(
      `${ledgerCte}
       SELECT id, partnerId, transactionDate, createdAt, updatedAt, description, debit, credit, balance,
              referenceType, referenceId, settlementBatchId, changeHistoryJson
    ...
    query.includeMeta ? getAsync(
      `${ledgerCte}
       SELECT COALESCE(SUM(COALESCE(debit, 0)), 0) AS totalDebit,
              COALESCE(SUM(COALESCE(credit, 0)), 0) AS totalCredit,
              COALESCE((SELECT balance FROM display_partner_ledger d2 WHERE ${where.sql} ORDER BY datetime(COALESCE(d2.transactionDate, d2.createdAt, d2.updatedAt)) DESC, d2.id DESC LIMIT 1), 0) AS latestBalance
         FROM display_partner_ledger
        WHERE ${where.sql}`,
    ...
    query.includeMeta ? getPartnerLedgerCanonicalBalance(partnerId) : Promise.resolve(0),
  ]);
...
export const getPurchasedItemsFromPartnerDb = async (
  ...
  const phones = includePhones ? await allAsync(
    `SELECT ph.id, ph.model as name, ph.imei as identifier,
            ...
            COALESCE((
              SELECT SUM(MAX(0, COALESCE(cl.credit, 0)))
              FROM customer_ledger cl
              WHERE cl.referenceType = 'installment_manual_receipt'
                AND cl.referenceId = ${LATEST_PHONE_INSTALLMENT_SALE_ID_SQL}
            ), 0) as installmentSaleManualReceiptAmount,
            COALESCE(
...
WITH partner_phones AS (
  ...
  JOIN installment_transactions it ON it.installment_payment_id = ip.id
  GROUP BY ia.phoneId
), installment_manual_receipt_totals AS (
  SELECT ia.phoneId, COALESCE(SUM(MAX(0,COALESCE(cl.credit,0))),0) AS paidAmount
  FROM installment_active ia
  JOIN customer_ledger cl
    ON cl.referenceType = 'installment_manual_receipt'
   AND cl.referenceId = ia.saleId
  GROUP BY ia.phoneId
  ...
  SELECT
    ...
    COALESCE(imr.paidAmount,0) AS installmentSaleManualReceiptAmount,
  ...
  LEFT JOIN installment_manual_receipt_totals imr ON imr.phoneId = ph.id
  ...
  SELECT raw_phone_settlement.*,
    (COALESCE(installmentSaleDownPayment,0) + COALESCE(installmentSaleTransactionPaidAmount,0) + COALESCE(installmentSaleCheckPaidAmount,0) + COALESCE(installmentSaleManualReceiptAmount,0)) AS installmentCollectedAmount,
...
export const getPartnerPhoneSettlementTimelineFromDb = async (
    ...
    summary = await getAsync(
      `${PARTNER_SOLD_PHONE_SETTLEMENT_STATE_FOR_PHONE_CTE}
       SELECT
         ...
         installmentSaleTransactionPaidAmount, installmentSaleCheckPaidAmount, installmentSaleManualReceiptAmount, installmentCustomerRemainingAmount,
  ...
  const normalizedSummary = summary
    ? {
        ...
        installmentSaleManualReceiptAmount: Number(summary.installmentSaleManualReceiptAmount || 0),
