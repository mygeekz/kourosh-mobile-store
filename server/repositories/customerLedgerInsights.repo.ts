...
export const getCustomerLedgerInsightsFromDb = async (
  ...
  const totals = await getAsync(
    `SELECT 
        COALESCE(SUM(cl.debit),0) AS totalDebit,
        COALESCE(SUM(cl.credit),0) AS totalCredit,
        COALESCE((SELECT eb.currentBalance FROM v_customer_effective_balance eb WHERE eb.customerId = ?),0) AS currentBalance
