...
type MiniAppIdentityRepositoryDependencies = {
  ...
  resolveUserAccess?: (userId: number) => Promise<{ permissions?: readonly string[] } | null>;
};
...
const defaultDependencies: MiniAppIdentityRepositoryDependencies = {
  ...
  resolveUserAccess: async (userId) => {
    const module = await loadRuntimeModule("../db/domains/accessControl.db");
    return (module.resolveUserTenantAuthorization as (id: number) => Promise<{ permissions?: readonly string[] } | null>)(userId);
  },
};
...
export const createMiniAppIdentityRepository = (
  ...
  findStaffIdentities: async (telegramUserId: string): Promise<MiniAppIdentityRecord[]> => {
    await dependencies.ensureDatabase();
    const rows = uniqueRecords((await dependencies.readRows(
      ...
      [telegramUserId],
    )) as MiniAppIdentityRecord[]);
    if (!dependencies.resolveUserAccess) return rows;
    return Promise.all(rows.map(async (row) => {
      const authorization = await dependencies.resolveUserAccess!(row.id);
      return { ...row, permissions: authorization ? [...(authorization.permissions || [])] : [] };
    }));
  },
});
