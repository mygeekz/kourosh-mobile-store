...
import { accountingLedgerBalanceSumSql } from "../accounting/accountingEngine";
...
export type CustomerLedgerDirectorySummary = {
  ...
  legacyCashedCheckAdjustment?: number;
  ...
  if (query.includeSummary) {
    const summaryRow = await getAsync(
      `SELECT
         ...
         ${accountingLedgerBalanceSumSql("customer")} AS currentBalance,
         MAX(COALESCE(updatedAt, createdAt, transactionDate)) AS latestTransaction
       FROM customer_ledger
       WHERE customerId = ?`,
      [safeCustomerId],
    );
    const effectiveRow = await getAsync(
      `SELECT currentBalance, rawLedgerBalance, legacyCashedCheckAdjustment
    ...
    summary = {
      ...
      currentBalance: Number(effectiveRow?.currentBalance ?? summaryRow?.currentBalance ?? 0),
      rawLedgerBalance: Number(effectiveRow?.rawLedgerBalance ?? summaryRow?.currentBalance ?? 0),
      legacyCashedCheckAdjustment: Math.max(0, Number(effectiveRow?.legacyCashedCheckAdjustment || 0)),
