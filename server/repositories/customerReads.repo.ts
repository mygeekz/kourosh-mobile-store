...
export const searchCustomersWithBalanceFromDb = async (
  ...
  return await allAsync(`
    SELECT
      ...
      COALESCE(eb.currentBalance, 0) AS currentBalance,
      COALESCE(eb.rawLedgerBalance, 0) AS rawLedgerBalance,
      COALESCE(eb.legacyCashedCheckAdjustment, 0) AS legacyCashedCheckAdjustment
    FROM customers c
...
const getCustomerDirectorySummaryFromDb = async (): Promise<CustomerDirectorySummary> => {
  const row = await getAsync(`
    WITH ledger AS (
      SELECT customerId, currentBalance AS balance
      FROM v_customer_effective_balance
    ), followups AS (
...
const getCustomerDirectoryRowsByIds = async (ids: number[], sort: CustomerDirectorySort): Promise<any[]> => {
  ...
  return await allAsync(`
    ...
    ledger_activity AS (
      ...
      SELECT eb.customerId, eb.currentBalance, eb.rawLedgerBalance, eb.legacyCashedCheckAdjustment,
             COALESCE(la.ledgerEntryCount,0) AS ledgerEntryCount,
             la.lastLedgerAt
      FROM v_customer_effective_balance eb
      LEFT JOIN ledger_activity la ON la.customerId = eb.customerId
      WHERE eb.customerId IN (${placeholders})
    ), followups AS (
      SELECT customerId,
  ...
  if (fastPath) {
    ...
    const enrichedCte = `
      WITH ledger_activity AS (
        SELECT customerId, MAX(COALESCE(updatedAt,createdAt,transactionDate)) AS lastLedgerAt
        FROM customer_ledger GROUP BY customerId
      ), ledger AS (
        SELECT eb.customerId, eb.currentBalance, la.lastLedgerAt
        FROM v_customer_effective_balance eb
        LEFT JOIN ledger_activity la ON la.customerId = eb.customerId
