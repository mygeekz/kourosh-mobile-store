...
const PARTNER_LEDGER_DELTA_L_SQL = accountingLedgerDeltaSql("partner", "l");
const PARTNER_LEDGER_DELTA_SQL = accountingLedgerDeltaSql("partner");
...
export const getAllPartnersWithBalanceFromDb = async (
  ...
  let sql = `
    SELECT
      p.*,
      COALESCE((
        SELECT SUM(${PARTNER_LEDGER_DELTA_L_SQL})
        FROM partner_ledger l
      ...
      CASE WHEN COALESCE((SELECT SUM(${PARTNER_LEDGER_DELTA_L_SQL}) FROM partner_ledger l WHERE l.partnerId = p.id), 0) > 0
        THEN COALESCE((SELECT SUM(${PARTNER_LEDGER_DELTA_L_SQL}) FROM partner_ledger l WHERE l.partnerId = p.id), 0)
        ELSE 0
      ...
      (
        COALESCE((SELECT SUM(${PARTNER_LEDGER_DELTA_L_SQL}) FROM partner_ledger l WHERE l.partnerId = p.id), 0)
...
const getPartnerDirectorySummaryFromDb = async () => {
  const balanceRow = await getAsync(`
    WITH ledger AS (
      SELECT partnerId, SUM(${PARTNER_LEDGER_DELTA_SQL}) AS balance
      FROM partner_ledger
      GROUP BY partnerId
    )
    SELECT
...
export const listPartnersDirectoryFromDb = async (query: PartnerDirectoryQuery = {}): Promise<PartnerDirectoryResult> => {
  ...
  const ledgerCte = needsBalance ? `WITH ledger AS (SELECT partnerId, SUM(${PARTNER_LEDGER_DELTA_SQL}) AS currentBalance FROM partner_ledger GROUP BY partnerId)` : "";
