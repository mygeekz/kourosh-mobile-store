...
export interface OldMobilePhonePayload {
  // For the deprecated mobile phone structure
  purchasePrice: number;
  sellingPrice: number;
  brand: string;
  model: string;
...
export interface UserUpdatePayload {
  // For updating user's role/profile
  roleId?: number;
...
export const runAsync = (
  ...
  return new Promise((resolve, reject) => {
    if (!db)
      return reject(
        new Error("Database not initialized. Call getDbInstance first."),
      );
...
const getOrCreateMobilePhoneCategory = async (): Promise<{
  id: number;
  name: string;
}> => {
  let category = await getAsync(
...
const seedInitialRolesAndAdmin = async (): Promise<void> => {
  ...
  const adminUser = await getAsync("SELECT id FROM users WHERE username = ?", [
    DEFAULT_ADMIN_USERNAME,
  ]);
  if (!adminUser && adminRole?.id) {
    // check adminRole.id to ensure role was created
    const hashedPassword = await bcryptjs.hash(DEFAULT_ADMIN_PASSWORD, 10);
    await runAsync(
      "INSERT INTO users (username, passwordHash, roleId) VALUES (?, ?, ?)",
      [DEFAULT_ADMIN_USERNAME, hashedPassword, adminRole.id],
    );
    console.log(`Default admin user "${DEFAULT_ADMIN_USERNAME}" created.`);
  } else if (!adminRole?.id) {
    console.error(
      `Could not create default admin user because Admin role ID is missing.`,
    );
  }
};
...
const initializeDatabaseInternal = async (): Promise<void> => {
  ...
  try {
    ...
    const productCols: any[] = (await allAsync(
      "PRAGMA table_info(products);",
    )) as any[];
    const colNames = new Set(
      (productCols || []).map((c: any) => c?.name).filter(Boolean),
    );
    ...
    `);
    try {
      const storePartnerCols: any[] = (await allAsync(
        "PRAGMA table_info(store_partners);",
      )) as any[];
      if (
        !storePartnerCols.some((col) => String(col?.name || "") === "isStore")
      ) {
        await runAsync(
    ...
    const phoneCols: any[] = (await allAsync(
      "PRAGMA table_info(phones);",
    )) as any[];
    const phoneColNames = new Set(
      (phoneCols || []).map((c: any) => c?.name).filter(Boolean),
    );
...
`);
    ...
    try {
      ...
      await runAsync(
        `UPDATE installment_checks SET status='نقد شد' WHERE status='وصول شده'`,
      );
      await runAsync(
        `UPDATE installment_checks SET status='برگشت خورد' WHERE status='برگشت خورده'`,
      );
      await runAsync(
...
export const getDbInstance = (
  ...
  dbInitializationPromise = new Promise<sqlite3.Database | null>(
    (resolveConnection, rejectConnection) => {
      ...
      if (db && forceNew) {
        db.close((closeErr: Error | null) => {
          ...
          dbInstance = null;
          console.log(
            "Existing DB connection closed (or attempted to close) for re-initialization.",
          );
          connect();
        });
      } else {
...
const PHONE_PURCHASE_LEDGER_REFERENCE_TYPES = [
  "phone_purchase",
  "phone_purchase_edit",
  "phone_purchase_reversal_on_edit",
] as const;
const PRODUCT_PURCHASE_LEDGER_REFERENCE_TYPES = [
  "product_purchase",
  "product_purchase_edit",
] as const;
const PURCHASE_LEDGER_REFERENCE_TYPE_SET = new Set<string>([
...
const buildPhonePurchaseDescription = (phone: {
  model?: string | null;
  imei?: string | null;
  id?: number | null;
...
const fetchLatestPurchaseLedgerRowForReference = async (
  referenceId: number,
...
export const addCategoryToDb = async (name: string): Promise<any> => {
  ...
  try {
    ...
    return await getAsync("SELECT * FROM categories WHERE id = ?", [
      result.lastID,
    ]);
  } catch (err: any) {
    if (err.message.includes("UNIQUE constraint failed")) {
...
export const updateCategoryInDb = async (
  ...
  await getDbInstance();
  try {
    const existing = await getAsync("SELECT id FROM categories WHERE id = ?", [
      id,
    ]);
    if (!existing) {
...
export const deleteProductFromDb = async (
  ...
  try {
    ...
    await execAsync("ROLLBACK;").catch((rbErr) =>
      console.error("Rollback failed in deleteProductFromDb:", rbErr),
    );
...
const resolveHistoryWindow = (filters?: {
  days?: number;
  startDate?: string;
  endDate?: string;
}) => {
...
const getPhoneHistoryEventClass = (
  row: any,
...
// --- Sales ---
export const getSellableItemsFromDb = async (): Promise<{
  phones: any[];
  inventory: any[];
  services: any[];
}> => {
...
const inferCustomerLedgerSourceCandidate = (
  row: any,
): { kind: CustomerLedgerSourceInfo["sourceKind"]; id: number | null } => {
...
const resolveCustomerLedgerSourceInfo = async (
  row: any,
  customerId: number,
  ...
  if (candidate.kind === "installment_sale") {
    ...
    if (
      String(row?.referenceType || "")
        .trim()
        .toLowerCase() === "installment_payment_tx"
    ) {
  ...
  if (candidate.kind === "sales_order") {
    ...
    return {
      ...
      sourceLabel: order?.id
        ? `فاکتور فروش #${formatFaSourceNumber(orderId)}`
        : `فاکتور فروش #${formatFaSourceNumber(orderId)} یافت نشد`,
  ...
  if (candidate.kind === "repair") {
    ...
    return {
      ...
      sourceLabel: repair?.id
        ? `پرونده تعمیر #${formatFaSourceNumber(repairId)}`
        : `پرونده تعمیر #${formatFaSourceNumber(repairId)} یافت نشد`,
...
const getLatestCustomerLedgerSourceForReport = async (
  customerId: number,
): Promise<CustomerLedgerSourceInfo> => {
  const fallback: CustomerLedgerSourceInfo = {
    sourceKind: null,
...
const resolvePartnerLedgerSourceInfo = async (
  row: any,
  partnerId: number,
...
const getLatestPartnerLedgerSourceForReport = async (
  partnerId: number,
): Promise<PartnerLedgerSourceInfo> => {
  const fallback: PartnerLedgerSourceInfo = {
    sourceKind: null,
...
// --- Settings ---
export const getAllSettingsAsObject = async (): Promise<
...
export const updateMultipleSettings = async (
  ...
  try {
    ...
    await execAsync("ROLLBACK;");
    throw new Error(
      `خطا در عملیاتی پایگاه داده در به‌روزرسانی تنظیمات: ${err.message}`,
    );
  }
};
...
export const updateUserInDb = async (
  ...
  if (
    user.username === "admin" &&
    data.roleId &&
    (await getAsync("SELECT name FROM roles WHERE id = ?", [data.roleId]))
  ...
  if (fieldsToUpdate.length === 0) {
    const role = await getAsync("SELECT name FROM roles WHERE id = ?", [
      user.roleId,
    ]);
    return {
      id: user.id,
...
export const changePasswordInDb = async (
  ...
  await getDbInstance();
  const user = await getAsync("SELECT passwordHash FROM users WHERE id = ?", [
    userId,
  ]);
  if (!user) throw new Error("کاربر یافت نشد.");
...
export const resetUserPasswordInDb = async (
  ...
  await getDbInstance();
  const user = await getAsync("SELECT id, username FROM users WHERE id = ?", [
    userId,
  ]);
...
export const getDashboardKPIs = async (): Promise<FrontendDashboardKPIs> => {
  ...
  const totalInstallmentSalesRes = await getAsync(
    "SELECT COALESCE(SUM(actualSalePrice),0) AS total FROM installment_sales",
  );
...
export const addInstallmentSaleToDb = async (
  ...
  try {
    ...
    for (const pid of phoneIds) {
      await runAsync(
        "UPDATE phones SET status = 'فروخته شده (قسطی)', saleDate = ? WHERE id = ?",
        [saleDateISO, pid],
      );
    }
    ...
    try {
      ...
      console.warn(
        "Installment sale saved, but customer ledger update failed:",
...
> => {
  ...
  for (const saleDb of salesFromDb) {
    ...
    let remainingAmount =
      Number(saleDb.totalInstallmentPrice || 0) -
      Number(saleDb.downPayment || 0);

    let nextDueDate: string | null = null;
    let overallStatus: FrontendInstallmentSale["overallStatus"] =
      "در حال پرداخت";
    ...
    if (isCheckSale) {
      ...
      const unsettled = checks.filter(
        (c: any) =>
          c.status !== "نقد شد" && c.status !== "به مشتری برگشت داده شده",
      );
      ...
      for (const payment of payments) {
        if (payment.status !== "پرداخت شده") {
          ...
          try {
            if (
              moment(payment.dueDate, "jYYYY/jMM/jDD").isBefore(moment(), "day")
            ) {
              hasOverdue = true;
            }
          } catch {}
...
export const getInstallmentSaleByIdFromDb = async (
  ...
  for (const c of checks) {
    ...
    (c as any).cashRemaining = Math.max(
      0,
  ...
  if (isCheckSale) {
    ...
    const unsettled = checks.filter(
      (c: any) =>
        c.status !== "نقد شد" &&
        Number((c as any).cashRemaining ?? c.amount ?? 0) > 0,
    );
...
export const getProfitabilityReportFromDb = async (): Promise<
  ProfitabilityAnalysisItem[]
> => {
  await getDbInstance();
  const query = `
...
const deleteInstallmentTransactionCustomerLedger = async (
  txId: number,
  paymentId: number,
) => {
  const meta = await getInstallmentPaymentLedgerMeta(paymentId);
...
const _toNumber = (v: any) =>
  Number(String(v ?? "0").replace(/[^\d.-]/g, "")) || 0;

const assertInstallmentPaymentAmountIsValid = async (
  paymentId: number,
  amount: number,
  excludeTxId?: number,
): Promise<void> => {
...
export const getAllStockCountsFromDb = async () => {
  await getDbInstance();
  return await allAsync(
    `SELECT * FROM stock_counts ORDER BY datetime(createdAt) DESC, id DESC`,
  );
};

export const getStockCountByIdFromDb = async (stockCountId: number) => {
  await getDbInstance();
  const sc = await getAsync(`SELECT * FROM stock_counts WHERE id = ?`, [
    stockCountId,
  ]);
  if (!sc) return null;
  const items = await allAsync(
...
export const upsertStockCountItemInDb = async (
  ...
  await getDbInstance();
  const sc = await getAsync(`SELECT status FROM stock_counts WHERE id=?`, [
    stockCountId,
  ]);
  if (!sc) throw new Error("انبارگردانی یافت نشد.");
  if (String(sc.status) !== "open")
    throw new Error("این انبارگردانی بسته شده است.");

  const pr = await getAsync(`SELECT stock_quantity FROM products WHERE id=?`, [
    productId,
  ]);
  if (!pr) throw new Error("محصول یافت نشد.");
...
export const completeStockCountInDb = async (
  ...
  const items = await allAsync(
    `SELECT * FROM stock_count_items WHERE stockCountId=?`,
    [stockCountId],
  );
  await execAsync("BEGIN TRANSACTION;");
  try {
...
export const linkCustomerTelegramByPhone = async (opts: {
  ...
  const telegramUserId = String(opts.telegramUserId || "").trim();
  if (!phone || !chatId || !telegramUserId)
    throw new Error("Invalid link args");

  const c = await getAsync(
    `SELECT id FROM customers WHERE phoneNumber=? LIMIT 1`,
    [phone],
  );
...
export const linkPartnerTelegramByPhone = async (opts: {
  ...
  const p = await getAsync(
    `SELECT id FROM partners WHERE phoneNumber=? LIMIT 1`,
    [phone],
  );
...
export const linkCustomerTelegramById = async (opts: {
  ...
  const chatId = String(opts.chatId || "").trim();
  const telegramUserId = String(opts.telegramUserId || "").trim();
  if (!customerId || !chatId || !telegramUserId)
    throw new Error("Invalid link args");
...
export const createStorePartnerFromDb = async (payload: {
  ...
  if (payload.isStore) {
    await runAsync(
      `UPDATE store_partners SET isStore = 0 WHERE isStore = 1`,
    ).catch(() => undefined);
  }
  const result = await runAsync(
...
export const updateStorePartnerFromDb = async (
  ...
  const current = await getAsync(`SELECT * FROM store_partners WHERE id = ?`, [
    id,
  ]);
  if (!current) throw new Error("شریک موردنظر پیدا نشد.");
  if (payload.isStore) {
    await runAsync(
      `UPDATE store_partners SET isStore = 0 WHERE isStore = 1 AND id <> ?`,
      [id],
    ).catch(() => undefined);
  }
  await runAsync(
...
export const listProfitShareProfilesFromDb = async (): Promise<any[]> => {
  ...
  for (const profile of profiles as any[]) {
    result.push({
      ...profile,
      items: await getProfileItems("profit_share_profile_items", profile.id),
    });
  }
  return result;
};
...
export const createProfitShareProfileFromDb = async (payload: {
  ...
  if (!payload.items?.length)
    throw new Error("حداقل یک شریک برای پروفایل سهم سود لازم است.");
  const total = payload.items.reduce(
    (sum, item) => sum + normalizePercent(item.sharePercent),
    0,
  );
  if (Math.abs(total - 100) > 0.05)
    throw new Error("جمع درصدهای سهم سود باید 100 باشد.");
  ...
  return {
    ...(await getAsync(`SELECT * FROM profit_share_profiles WHERE id = ?`, [
      id,
    ])),
    items: await getProfileItems("profit_share_profile_items", id),
  };
};
...
export const listOwnershipProfilesFromDb = async (): Promise<any[]> => {
  ...
  for (const profile of profiles as any[]) {
    result.push({
      ...profile,
      items: await getProfileItems("ownership_profile_items", profile.id),
    });
  }
  return result;
};
...
export const createOwnershipProfileFromDb = async (payload: {
  ...
  if (!payload.items?.length)
    throw new Error("حداقل یک شریک برای پروفایل مالکیت لازم است.");
  const total = payload.items.reduce(
    (sum, item) => sum + normalizePercent(item.sharePercent),
    0,
  );
  if (Math.abs(total - 100) > 0.05)
    throw new Error("جمع درصدهای مالکیت باید 100 باشد.");
  if (payload.isDefault) {
    await runAsync(
  ...
  const result = await runAsync(
    ...
    [
      String(payload.title || "").trim(),
      payload.ownershipType || "shared",
      payload.notes || null,
      payload.isDefault ? 1 : 0,
      payload.profitShareProfileId || null,
    ],
  );
  const id = Number(result.lastID);
  ...
  return {
    ...profile,
    items: await getProfileItems("ownership_profile_items", id),
  };
};
...
export const saveStoreOwnershipConfigurationFromDb = async (payload: {
  ...
  if (!items.length)
    throw new Error("حداقل یک سهم برای تنظیمات فروشگاه لازم است.");
  ...
  if (!normalizedItems.length)
    throw new Error("حداقل یک شریک معتبر برای تنظیمات فروشگاه لازم است.");
  const total = normalizedItems.reduce(
    (sum, item) => sum + item.sharePercent,
    0,
  );
  if (Math.abs(total - 100) > 0.05)
    throw new Error("جمع درصد سهم شرکا باید 100 باشد.");

  const ids = normalizedItems.map((item) => item.storePartnerId);
  const partners = await allAsync(
    `SELECT * FROM store_partners WHERE id IN (${ids.map(() => "?").join(",")})`,
    ids,
  );
  if ((partners as any[]).length !== ids.length)
  ...
  await runAsync(`UPDATE store_partners SET isStore = 0`).catch(
    () => undefined,
  );
  ...
  if (!defaultProfitShare) {
    const created = await createProfitShareProfileFromDb({
    ...
    await runAsync(
      `UPDATE profit_share_profiles SET title = ?, updatedAt = strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc') WHERE id = ?`,
      ["پروفایل پیش‌فرض سود فروشگاه", Number(defaultProfitShare.id)],
    );
    await replaceProfitShareProfileItems(
    ...
    (defaultProfitShare as any).items = await getProfileItems(
      "profit_share_profile_items",
      Number(defaultProfitShare.id),
    );
  }
  ...
  if (!storeOwnership) {
    ...
    await runAsync(
      `UPDATE ownership_profiles SET title = ?, ownershipType = 'store', isDefault = 1, profitShareProfileId = ?, updatedAt = strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc') WHERE id = ?`,
      [
    ...
    await replaceOwnershipProfileItems(
      Number(storeOwnership.id),
    ...
    (storeOwnership as any).items = await getProfileItems(
      "ownership_profile_items",
      Number(storeOwnership.id),
    );
  }
...
const createDefaultOwnershipCore = async (
  legacyPartnerIds: number[],
): Promise<any> => {
  ...
  for (const partner of legacyPartners) {
    ...
    const created = await createStorePartnerFromDb({
  ...
  if (!defaultProfitShare) {
    ...
    const items = createdStorePartners.map((sp, index) => ({
      ...
      sharePercent:
        index === count - 1 ? normalizePercent(100 - base * (count - 1)) : base,
      sortOrder: index,
    }));
    defaultProfitShare = await createProfitShareProfileFromDb({
...
const resolveLegacyPartnerOwnershipMap = async (): Promise<
  Map<number, number>
> => {
  const map = new Map<number, number>();
  const links = await allAsync(
...
export const applyStoreOwnershipBackfillFromDb = async (): Promise<any> => {
  ...
  return {
    phonesUpdated,
    productsUpdated,
    coverage: await getStoreOwnershipCoverageFromDb(),
  };
};
...
export const assignStoreOwnershipReviewItemsFromDb = async (payload: {
  ...
  const notes =
    String(payload?.notes || "").trim() || "انتساب دستی از صف بازبینی مالکیت";
  let updated = 0;
  if (targetType === "phones") {
    for (const id of ids) {
      const row = await getAsync(
        `SELECT id, supplierId FROM phones WHERE id = ?`,
        [id],
      );
      if (!row?.id) continue;
      await runAsync(`UPDATE phones SET ownershipProfileId = ? WHERE id = ?`, [
    ...
    for (const id of ids) {
      const row = await getAsync(
        `SELECT id, supplierId FROM products WHERE id = ?`,
        [id],
      );
      if (!row?.id) continue;
      await runAsync(
        `UPDATE products SET ownershipProfileId = ? WHERE id = ?`,
...
const getProfitShareLinesByProfileId = async (
...
const getOwnershipLinesByProfileId = async (
...
const resolveOwnershipContextByProfileId = async (
  ownershipProfileId: number | null | undefined,
...
const allocateAmountAcrossShares = (
  amount: number,
...
const purgeProfitSnapshotsForSource = async (
  sourceKind: "sales_order" | "installment_sale",
  sourceId: number,
): Promise<void> => {
...
const persistSaleProfitSnapshotItem = async (
...
export const snapshotInstallmentSaleProfitAllocations = async (
  ...
  const sale = await getAsync(
    `SELECT installmentsStartDate FROM installment_sales WHERE id = ?`,
    [saleId],
  );
  const saleDate = sale?.installmentsStartDate
    ? (() => {
        ...
        const m = moment(
          raw,
          ["YYYY-MM-DD", "jYYYY/jMM/jDD", moment.ISO_8601],
          true,
        );
...
const buildSaleProfitSnapshotResponse = async (
  sourceKind: "sales_order" | "installment_sale",
  sourceId: number,
): Promise<any> => {
  const snapshots = await allAsync(
...
const getLegacyPartnerContextForReports =
...
const resolveLegacyOwnershipShares = async (
...
const getLegacySaleRowsForReports = async (
...
const buildLegacyComputedSales = async (
  range: PartnerReportRange = {},
): Promise<any[]> => {
...
const buildLegacyAccessoriesReportFromDb = async (
...
const buildLegacyPhonesReportFromDb = async (
  range: PartnerReportRange & { partnerId: number },
): Promise<any> => {
  const context = await getLegacyPartnerContextForReports();
  const partner = context.partners.find(
...
const buildLegacySettlementReportFromDb = async (
  range: PartnerReportRange = {},
): Promise<any> => {
