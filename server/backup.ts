...
const checksumPathFor = (databasePath: string) => `${databasePath}.sha256`;

const writeBackupChecksum = (databasePath: string, sha256: string) => {
  const checksumPath = checksumPathFor(databasePath);
  fs.writeFileSync(checksumPath, `${sha256}  ${path.basename(databasePath)}\n`, 'utf8');
  return checksumPath;
};

const readStoredBackupChecksum = (databasePath: string): string | null => {
  const checksumPath = checksumPathFor(databasePath);
  if (!fs.existsSync(checksumPath)) return null;
  const match = fs.readFileSync(checksumPath, 'utf8').trim().match(/^([a-f0-9]{64})(?:\s|$)/i);
  return match ? match[1].toLowerCase() : null;
};

export const verifyBackupChecksum = (databasePath: string) => {
  const sha256 = computeFileSha256(databasePath);
  const storedSha256 = readStoredBackupChecksum(databasePath);
  return {
    sha256,
    storedSha256,
    checksumVerified: Boolean(storedSha256 && storedSha256 === sha256),
    checksumPath: checksumPathFor(databasePath),
  };
};
...
export const createDbBackup = async (options?: { prefix?: string }) => {
  ...
  const sha256 = computeFileSha256(dst);
  const checksumPath = writeBackupChecksum(dst, sha256);
  const verification = verifyBackupChecksum(dst);
...
export const listBackups = (): BackupListItem[] => {
  ...
  return fs
    ...
    .map((fileName) => {
      ...
      const verification = verifyBackupChecksum(backupPath);
      return {
        fileName,
        size: stat.size,
        mtime: stat.mtime.toISOString(),
...
export const deleteBackup = (fileName: string) => {
  ...
  try { fs.unlinkSync(checksumPathFor(backupPath)); } catch {}
};
