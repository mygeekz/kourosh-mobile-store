...
export const TELEGRAM_IDENTITY_WRITE_POLICY: readonly TelegramIdentityWriterPolicy[] = [
  ...
  { writer: "telegramIdentitySecurity.redeemStaffTelegramLink", entity: "staff", fields: ["user_telegram_links.telegram_user_id", "user_telegram_links.chat_id"], fieldClass: "authentication", authorization: "latest active opaque Staff token, active tenant management permission, private chat", allowed: true },
  ...
  { writer: "telegramIdentitySecurity.revokeManagerTelegramBinding", entity: "staff", fields: ["user_telegram_links"], fieldClass: "lifecycle", authorization: "managers.manage plus active current-tenant management target; tokens and MiniApp sessions revoked", allowed: true },
  { writer: "telegramIdentitySecurity.revokeStaffTelegramBinding", entity: "staff", fields: ["user_telegram_links"], fieldClass: "lifecycle", authorization: "central lifecycle unlink compatibility path; tokens and MiniApp sessions revoked", allowed: true },
  { writer: "telegramIdentitySecurity.unlinkStaffTelegram", entity: "staff", fields: ["user_telegram_links"], fieldClass: "lifecycle", authorization: "compatibility wrapper around revokeStaffTelegramBinding", allowed: true },
