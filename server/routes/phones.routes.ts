...
import {
  ...
  buildPhoneSensitiveUpdateImpact,
...
export const registerPhonesRoutes = (
  ...
  app.post(
    '/api/phones/:id/sensitive-update/prepare',
    authorizeRole(PHONE_ROLES),
    async (req, res, next) => {
      try {
        const phoneId = Number(req.params.id);
        if (!Number.isInteger(phoneId) || phoneId <= 0) return res.status(400).json({ success: false, message: 'شناسه گوشی نامعتبر است.' });
        const payload = { ...(req.body || {}) };
        delete (payload as any).confirmationToken;
        const impact = await buildPhoneSensitiveUpdateImpact(phoneId, payload);
        if (!impact) return res.json({ success: true, data: { required: false } });
        const actor = actorFromRequestUser((req as any).user);
        const data = prepareSensitiveAccountingOperation({ action: 'phone_sensitive_update', scopeKey: `phone:${phoneId}`, actor, payload, impact });
        return res.json({ success: true, data: { required: true, ...data } });
      } catch (e) { next(e); }
    },
  );
  ...
  app.put(
    ...
    async (req, res, next) => {
      try {
        ...
        const impact = await buildPhoneSensitiveUpdateImpact(phoneId, payload);
        if (impact) {
          consumeSensitiveAccountingOperation({ token: confirmationToken, action: 'phone_sensitive_update', scopeKey: `phone:${phoneId}`, actor, payload });
        }
        const data = await phonesService.updatePhone(
          phoneId,
          payload as PhoneEntryUpdatePayload,
          getPhoneActor((req as any).user),
        );
        if (impact) {
          await recordSensitiveAccountingMutation({ action: 'phone_sensitive_update', entityType: 'phone', entityId: phoneId, actor, reason: 'ویرایش بهای خرید/تأمین‌کننده/مالکیت', before: impact, after: data });
        }
        res.json({ success: true, data, message: 'گوشی با موفقیت ویرایش شد.' });
      } catch (e) {
        next(e);
      }
    },
