...
import { buildMeliPayamakPatternTokens, buildTelegramTemplatePreset } from '../../shared/messages';
import {
  addAuditLog,
  addPartToRepairInDb,
  createRepairInDb,
...
const renderPreparedTelegramTemplate = (template: string, vars: Record<string, unknown>) =>
  String(template || '').replace(/\{([a-zA-Z0-9_]+)\}/g, (_m, key) => esc(vars?.[key] ?? ''));
        ...
        try {
          ...
          const sendRepairEvent = async (
            ...
            if (eventType === 'REPAIR_RECEIVED') {
              messageVars = { name: r.customerFullName, deviceModel: r.deviceModel, repairId: String(r.id) };
              tokens = [String(messageVars.name || ''), String(messageVars.deviceModel || ''), String(messageVars.repairId || '')];
              meliPatternKey = 'meli_payamak_repair_received_pattern_id';
              ...
              messageVars = { name: r.customerFullName, deviceModel: r.deviceModel, finalCost: formatPriceForSms(r.finalCost) };
              tokens = [String(messageVars.name || ''), String(messageVars.deviceModel || ''), String(messageVars.finalCost || '')];
              meliPatternKey = 'meli_payamak_repair_ready_pattern_id';
            ...
            if (provider === 'telegram') {
              ...
              const templateKey = eventType === 'REPAIR_RECEIVED'
                ? 'telegram_repair_received_message'
                : 'telegram_repair_ready_message';
              const msg = renderPreparedTelegramTemplate(
                buildTelegramTemplatePreset(templateKey, 'manager', 'formal'),
                messageVars,
              );
            ...
            if (provider === 'meli_payamak') {
              ...
              const meliTokens = buildMeliPayamakPatternTokens(meliPatternKey, messageVars);
