...
import { hasAnyManagementPermission } from "../security/managementAccessPolicy";
...
export const registerUsersRoutes = (
  ...
  app.put("/api/users/:id", authorizeRole(["Admin"]), async (req, res, next) => {
    try {
      ...
      let stillHasManagementAccess = selectedRole === "Admin" || selectedRole === "Manager";
      try {
        const authorization = await resolveUserTenantAuthorization(userId);
        stillHasManagementAccess = hasAnyManagementPermission(authorization?.permissions || []);
      } catch (error) {
        const message = String((error as any)?.message || error || "");
        const schemaMissing = message === "ACCESS_CONTROL_SCHEMA_UNAVAILABLE" || /no such table:\s*(?:tenant_memberships|access_roles|access_role_permissions|tenant_membership_roles|access_permissions)/i.test(message);
        if (!schemaMissing) throw error;
      }
      if (!stillHasManagementAccess) await unlinkStaffTelegram(userId, req.user!);
      res.json({
        success: true,
        data,
      });
