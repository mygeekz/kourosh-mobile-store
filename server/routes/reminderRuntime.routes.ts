...
import { buildMeliPayamakPatternTokens, buildTelegramRuntimeDefault } from "../../shared/messages";
import {
  allAsync,
  getAsync,
    ...
    try {
      ...
      if (eventType === "INSTALLMENT_REMINDER") {
        ...
        messageVars = {
          ...
          amount: formatPriceForSms(p.amountDue),
          dueDate: String(p.dueDate || ""),
          saleId: String(p.saleId || ""),
        };
        const legacyReminderTokens = [messageVars.name, messageVars.amount, messageVars.dueDate];
        tokens = provider === "meli_payamak" && settings.meli_payamak_installment_due_notice_pattern_id
          ? buildMeliPayamakPatternTokens("meli_payamak_installment_due_notice_pattern_id", messageVars)
          : legacyReminderTokens;
        ...
        telegramTemplate = settings.telegram_installment_reminder_message;
        ...
        const settlementVars = {
          name: String(s.customerFullName || ""),
          saleId: String(s.saleId),
          total: formatPriceForSms(s.totalPrice),
        };
        messageVars = settlementVars;
        const usesCurrentSettlementPattern = provider === "meli_payamak" && Boolean(settings.meli_payamak_installment_settlement_pattern_id);
        tokens = usesCurrentSettlementPattern
          ? buildMeliPayamakPatternTokens("meli_payamak_installment_settlement_pattern_id", settlementVars)
          : [settlementVars.name, settlementVars.saleId, settlementVars.total];
        meliBodyId =
        ...
        telegramTemplate = (settings as any).telegram_installment_settlement_message || settings.telegram_installment_completed_message;
        ...
        messageVars = {
          ...
          repairId: String(r.id),
        };
        tokens = provider === "meli_payamak"
          ? buildMeliPayamakPatternTokens("meli_payamak_repair_received_pattern_id", messageVars)
          : [messageVars.name, messageVars.deviceModel, messageVars.repairId];
        ...
        telegramTemplate = settings.telegram_repair_received_message;
        ...
        messageVars = {
          ...
          deviceModel: String(r.deviceModel || ""),
          repairId: String(r.id),
          estimatedCost: formatPriceForSms(r.estimatedCost),
        };
        const usesCurrentRepairCostPattern = provider === "meli_payamak" && Boolean((settings as any).meli_payamak_repair_cost_notice_pattern_id);
        tokens = usesCurrentRepairCostPattern
          ? buildMeliPayamakPatternTokens("meli_payamak_repair_cost_notice_pattern_id", messageVars)
          : [messageVars.name, messageVars.deviceModel, messageVars.estimatedCost];
        meliBodyId = Number(
          (settings as any).meli_payamak_repair_cost_notice_pattern_id ||
        ...
        messageVars = {
          ...
          deviceModel: String(r.deviceModel || ""),
          repairId: String(r.id),
          finalCost: formatPriceForSms(r.finalCost),
        };
        tokens = provider === "meli_payamak"
          ? buildMeliPayamakPatternTokens("meli_payamak_repair_ready_pattern_id", messageVars)
          : [messageVars.name, messageVars.deviceModel, messageVars.finalCost];
        ...
        telegramTemplate = settings.telegram_repair_ready_message;
      } else if (
        ...
        tokens = [messageVars.name, messageVars.amount, messageVars.dueDate];
        ...
        if (eventType === "INSTALLMENT_DUE_7") {
          ...
          if (provider === "meli_payamak" && (settings as any).meli_payamak_installment_due_notice_pattern_id) {
            tokens = buildMeliPayamakPatternTokens("meli_payamak_installment_due_notice_pattern_id", messageVars);
          }
        ...
        messageVars = {
          ...
          amount: formatPriceForSms(c.amount),
        };
        tokens = [messageVars.name, messageVars.checkNumber, messageVars.dueDate, messageVars.amount];
      ...
      try {
        switch (provider) {
          ...
          case "telegram": {
            ...
            telegramTemplate = telegramTemplate || buildTelegramRuntimeDefault(String(eventType));
            if (!telegramTemplate) throw new Error("قالب پیام تلگرام یافت نشد.");
            const values: Record<string, string> = messageVars;
