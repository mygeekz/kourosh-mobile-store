...
import { kickStableTunnelRuntimeAfterSettingsSave } from "../connectivity/stableTunnelRuntimeKick";
import {
...
import { SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY, sanitizeSmsTestRecipients, serializeSmsTestRecipientAllowlist } from "../../shared/smsTestRecipients";
import { syncTelegramMenuButton } from "../services/telegramMenuSync.service";
import { runMiniAppDiagnostic, searchMiniAppDiagnosticSubjects } from "../services/miniAppDiagnostics.service";
import {
...
export const registerSettingsRoutes = (
  ...
  app.get("/api/settings/miniapp-diagnostics/subjects", authorizeRole(["Admin"]), async (req, res, next) => {
    try {
      const kind = String(req.query?.kind || "").trim() as "customer" | "partner";
      const query = String(req.query?.q || "").trim();
      const items = await searchMiniAppDiagnosticSubjects({ kind, query });
      res.setHeader("Cache-Control", "no-store");
      return res.json({ success: true, data: { items } });
    } catch (error: any) {
      if (String(error?.code || "") === "MINIAPP_DIAGNOSTIC_SUBJECT_INVALID") return res.status(400).json({ success: false, code: error.code, message: String(error?.message || "نوع پرونده نامعتبر است.") });
      return next(error);
    }
  });

  app.post("/api/settings/miniapp-diagnostics/run", authorizeRole(["Admin"]), async (req, res, next) => {
    try {
      const kind = String(req.body?.kind || "").trim() as "customer" | "partner";
      const subjectId = Number(req.body?.subjectId);
      const report = await runMiniAppDiagnostic({ kind, subjectId });
      res.setHeader("Cache-Control", "no-store");
      return res.json({ success: true, data: report });
    } catch (error: any) {
      const code = String(error?.code || "MINIAPP_DIAGNOSTIC_FAILED");
      if (code === "MINIAPP_DIAGNOSTIC_SUBJECT_INVALID") return res.status(400).json({ success: false, code, message: String(error?.message || "پرونده نامعتبر است.") });
      if (code === "MINIAPP_DIAGNOSTIC_SUBJECT_NOT_FOUND") return res.status(404).json({ success: false, code, message: String(error?.message || "پرونده یافت نشد.") });
      return next(error);
    }
  });
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        if (!screenshotPath) {
          return res.status(404).json({ success: false, message: "تصویر گزارش نصب برنامه پیدا نشد." });
        }
        res.setHeader("Cache-Control", "no-store");
        res.setHeader("X-Content-Type-Options", "nosniff");
        return res.type("png").sendFile(screenshotPath);
  ...
  const relayEnrollHandler: RequestHandler = async (req, res, next) => {
    try {
      ...
      if (!relay.controlUrl || (relay.provider === "custom" && !relay.connectorUrl)) return res.status(409).json({ success: false, code: "RELAY_CONTROL_PLANE_NOT_CONFIGURED", message: "نشانی مدیریت و اتصال رله انتخاب‌شده کامل تنظیم نشده‌اند." });
      ...
      if (String(current.kourosh_cloud_provisioned || "") === "1" && currentAssignmentProvider === relay.provider) return res.status(409).json({ success: false, code: "RELAY_ALREADY_PROVISIONED", message: "این نصب قبلاً برای همین سرویس رله فعال شده است؛ برای تعویض کلید از کد بازیابی استفاده کنید." });
      ...
      if (code === "TENANT_EXISTS") return res.status(409).json({ success: false, code, message: "این نصب از قبل در سرویس رله ثبت شده است و جایگزینی خودکار اتصال مجاز نیست." });
      return next(e);
    }
  };
  ...
  const relayRotateHandler: RequestHandler = async (req, res, next) => {
    try {
      ...
      const current = await getAllSettingsAsObject();
      const relay = resolveRelaySettingsFromRequest(current, req.body);
      if (!relay.controlUrl || (relay.provider === "custom" && !relay.connectorUrl)) return res.status(409).json({ success: false, code: "RELAY_CONTROL_PLANE_NOT_CONFIGURED", message: "نشانی مدیریت رله تنظیم نشده است." });
      const installationId = String(current.installation_id || "").trim();
      if (String(current.kourosh_cloud_provisioned || "") !== "1") return res.status(409).json({ success: false, code: "RELAY_NOT_PROVISIONED", message: "این نصب هنوز برای سرویس رله فعال نشده است." });
      ...
      initializeCloudConnectorRuntime(nextSettings, { updateSetting: async (key, value) => { await persistScopedSettings({ [key]: value }); } });
      initializeMiniAppSnapshotRuntime(nextSettings);
      return res.json({ success: true, message: "کلید اتصال رله با موفقیت به‌روزرسانی شد.", data: { provider: relay.provider, assignedPublicUrl: rotated.assignedPublicUrl, status: getCloudConnectorRuntimeStatus() } });
    } catch (e: any) {
      ...
      if (code === "TENANT_NOT_FOUND") return res.status(409).json({ success: false, code, message: "اطلاعات این نصب در سرویس رله پیدا نشد." });
      return next(e);
    }
  };
  ...
  app.post("/api/settings/cloud-connector/prepare", authorizeRole(["Admin"]), async (_req, res, next) => {
    try {
      if (!isDevelopmentCloudProvisioningAvailable()) return res.status(409).json({ success: false, code: "CLOUD_PROVISIONING_CONTROL_PLANE_UNAVAILABLE", message: "فعال‌سازی خودکار این سرویس در این نسخه در دسترس نیست." });
      const current = await getAllSettingsAsObject();
      const installationId = String(current.installation_id || "").trim();
      ...
      initializeCloudConnectorRuntime(nextSettings, { updateSetting: async (key, value) => { await persistScopedSettings({ [key]: value }); } });
      initializeMiniAppSnapshotRuntime(nextSettings);
      return res.json({ success: true, message: "رله مدیریت‌شده با موفقیت برای این نصب فعال شد.", data: getCloudConnectorRuntimeStatus() });
    } catch (e) { return next(e); }
  });
    ...
    try {
      ...
      if (relayProvider === "custom") {
        ...
        if ((transportMode === "relay" || miniAppMode === "relay") && (!customControl || !customConnector)) {
          return res.status(400).json({ success: false, code: "CUSTOM_RELAY_NOT_CONFIGURED", message: "برای رله شخصی، نشانی HTTPS مدیریت و نشانی امن WSS اتصال لازم است." });
        }
      ...
      if (telegramRelayNeedsValidation || miniAppRelayNeedsValidation) {
        ...
        if (telegramRelayNeedsValidation) {
          ...
          if (!relayHealthy) {
            initializeCloudConnectorRuntime(current, { updateSetting: async (key, value) => { await persistScopedSettings({ [key]: value }); } });
            return res.status(409).json({ success: false, code: "RELAY_NOT_READY", message: "فعال‌سازی رله تلگرام فقط پس از آماده‌شدن سرویس، برقراری اتصال امن و تأیید سلامت تلگرام مجاز است." });
          }
        }
        ...
        if (miniAppRelayNeedsValidation) {
          ...
          if (!miniAppHealthy) {
            initializeCloudConnectorRuntime(current, { updateSetting: async (key, value) => { await persistScopedSettings({ [key]: value }); } });
            return res.status(409).json({ success: false, code: "RELAY_MINIAPP_NOT_READY", message: "Mini App رله فقط پس از آماده‌شدن نشانی عمومی و تأیید سلامت مسیر اتصال قابل فعال‌سازی است." });
          }
        }
      }
      ...
      if (miniAppMode === "self_hosted" || miniAppMode === "external_tunnel" || miniAppMode === "stable_tunnel") {
        ...
        if (!normalizedPublicUrl) return res.status(400).json({ success: false, code: miniAppMode === "stable_tunnel" ? "INVALID_TELEGRAM_STABLE_MINIAPP_URL" : "INVALID_TELEGRAM_MINIAPP_PUBLIC_URL", message: miniAppMode === "stable_tunnel" ? "برای انتشار اصلی، یک نشانی ثابت HTTPS روی /miniapp.html لازم است؛ نشانی‌های موقت مجاز نیستند." : miniAppMode === "external_tunnel" ? "برای تونل، یک نشانی عمومی HTTPS معتبر لازم است." : "برای میزبانی مستقل Mini App، یک نشانی عمومی HTTPS معتبر لازم است." });
      ...
      if (miniAppMode === "stable_tunnel") {
        ...
        if (!liveOriginUrl) return res.status(400).json({ success: false, code: "INVALID_MINIAPP_LIVE_ORIGIN_URL", message: "برای دسترسی زنده فروشگاه، یک نشانی HTTPS ثابت و معتبر لازم است." });
        const publicUrl = validateTelegramStableMiniAppCanonicalUrl(proposed.telegram_miniapp_public_url);
        if (!publicUrl) return res.status(400).json({ success: false, code: "INVALID_TELEGRAM_STABLE_MINIAPP_URL", message: "برای انتشار اصلی، یک نشانی ثابت HTTPS روی /miniapp.html لازم است." });
        if (new URL(publicUrl).origin === new URL(liveOriginUrl).origin) {
          return res.status(400).json({
            success: false,
            code: "MINIAPP_PUBLIC_EDGE_LIVE_ORIGIN_MUST_DIFFER",
            message: "نشانی عمومی Mini App باید با نشانی زنده فروشگاه متفاوت باشد تا هنگام قطع ارتباط فروشگاه، آخرین اطلاعات همگام‌شده همچنان در دسترس بماند.",
          });
        }
      ...
      try {
        ...
        if (hasTransportInput && resolveTelegramTransportMode(savedSettings) !== transportMode) {
          throw Object.assign(new Error("روش اتصال تلگرام به‌درستی ذخیره نشد."), { code: "TELEGRAM_SETTINGS_PERSISTENCE_MISMATCH" });
        }
        if (hasMiniAppModeInput && resolveMiniAppPublicAccessMode(savedSettings) !== miniAppMode) {
          throw Object.assign(new Error("روش دسترسی Mini App به‌درستی ذخیره نشد."), { code: "MINIAPP_SETTINGS_PERSISTENCE_MISMATCH" });
        }
        if (hasRelayProviderInput && resolveRelayProvider(savedSettings) !== relayProvider) {
          throw Object.assign(new Error("سرویس رله انتخاب‌شده به‌درستی ذخیره نشد."), { code: "RELAY_SETTINGS_PERSISTENCE_MISMATCH" });
        }
      ...
      // The Windows startup coordinator may have skipped the stable tunnel when
      // these settings were empty (for example after a Settings reset). Saving
      // valid values while the app is already running must reconcile the named
      // tunnel now instead of requiring a full Kourosh/Windows restart.
      const stableTunnelKick = kickStableTunnelRuntimeAfterSettingsSave(savedSettings);
      ...
      if (miniAppMode === "stable_tunnel") {
        telegramMenuSync = await syncTelegramMenuButton(savedSettings).catch((error) => ({
          state: "error",
          attempts: 0,
          message: error instanceof Error ? error.message : String(error || "همگام‌سازی منوی تلگرام انجام نشد."),
        }));
      }
      return res.json({ success: true, message: "تنظیمات اتصال تلگرام و Mini App ذخیره شد.", data: { savedKeys, transportMode, miniAppMode, relayProvider, gatewayRuntimeConfig, gatewayRestartRequired: false, stableTunnelKick, telegramMenuSync } });
    } catch (e) { return next(e); }
  });
  ...
  app.post("/api/settings", authorizeRole(["Admin"]), async (req, res, next) => {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(config, SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY)) {
        const raw = String(config[SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY] || "").trim();
        if (!raw) {
          config[SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY] = "[]";
        } else {
          let parsed: unknown;
          try { parsed = JSON.parse(raw); } catch {
            return res.status(400).json({ success: false, message: "فهرست شماره‌های مجاز تست پیامک ساختار معتبری ندارد." });
          }
          if (!Array.isArray(parsed)) {
            return res.status(400).json({ success: false, message: "فهرست شماره‌های مجاز تست پیامک باید به‌صورت لیست ذخیره شود." });
          }
          const sanitized = sanitizeSmsTestRecipients(parsed);
          if (sanitized.length !== parsed.length) {
            return res.status(400).json({ success: false, message: "حداقل یکی از شماره‌های مجاز تست نامعتبر، تکراری یا بیشتر از سقف مجاز است." });
          }
          config[SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY] = serializeSmsTestRecipientAllowlist(sanitized);
        }
      }
