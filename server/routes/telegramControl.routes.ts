...
import { getSystemTelegramTransportStatus } from "../telegram/SystemTelegramTransport";
    ...
    async (req, res, next) => {
      try {
        ...
        if (result?.success) {
          ...
          const systemRoute = transportMode === "system" ? getSystemTelegramTransportStatus().preferred : null;
          return res.json({
            success: true,
            message: systemRoute?.label
              ? `اتصال تلگرام از مسیر «${systemRoute.label}» برقرار است.`
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        // Proxy OK is only meaningful when Proxy is the selected transport; Direct/System/Relay do not consume app proxy config.
        const proxyOk: boolean | null = transportMode === "proxy" ? (proxyConfigured ? botApiOk : false) : null;
        const systemTransport = transportMode === "system" ? getSystemTelegramTransportStatus() : null;
