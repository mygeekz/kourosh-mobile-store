    ...
    try {
      ...
      if (!customerId)
        return res
          .status(400)
          .json({ success: false, message: "customerId نامعتبر است." });
      // مانده اعلان باید با همان مانده موثر UI/گزارش‌ها یکسان باشد.
      const c = (await getAsync(
        `SELECT c.id, c.fullName, c.phoneNumber,
              COALESCE(eb.currentBalance, 0) AS currentBalance,
              COALESCE(c.telegram_opted_out,0) AS telegram_opted_out,
              COALESCE(c.telegram_chat_id, c.telegramChatId) AS tg_chat_id
         FROM customers c
