...
export const registerInstallmentsRoutes = (
  ...
  app.post(
    '/api/installment-sales/:id/cancel/prepare',
    authorizeRole(['Admin']),
    async (req, res, next) => {
      try {
        ...
        if (reason.length < 3) {
          return res.status(400).json({ success: false, message: 'برای فسخ، دلیل حداقل سه حرفی الزامی است.' });
        }
        const mode = req.body?.mode === 'full_reversal' ? 'full_reversal' : 'review_required';
        const payload = {
          reason,
          mode,
          returnPhysicalItems: Boolean(req.body?.returnPhysicalItems),
          returnUnusedChecks: Boolean(req.body?.returnUnusedChecks),
        };
        const preview: any = await installmentsService.getInstallmentCancellationPreview(saleId, mode);
        const impact = {
          title: `فسخ قرارداد اقساطی #${saleId}`,
          summaryText: [
            `مانده قبل از فسخ: ${Math.round(Number(preview?.receivable?.remaining || 0)).toLocaleString('fa-IR')} تومان`,
            `وصول پس از پیش‌پرداخت: ${Math.round(Number(preview?.receivable?.collectedAfterDownPayment || 0)).toLocaleString('fa-IR')} تومان`,
            `برگشت مالی قرارداد: ${Math.round(Number(preview?.effects?.reversalCredit || 0)).toLocaleString('fa-IR')} تومان`,
            `مبلغ قابل استرداد: ${Math.round(Number(preview?.effects?.expectedRefundDue || 0)).toLocaleString('fa-IR')} تومان`,
            `${Number(preview?.inventory?.physicalQuantity || 0).toLocaleString('fa-IR')} قلم فیزیکی`,
            `${Number(preview?.checks?.unused || 0).toLocaleString('fa-IR')} چک استفاده‌نشده`,
            `${Number(preview?.reconciliation?.issueCount || 0).toLocaleString('fa-IR')} مغایرت نیازمند بررسی`,
          ].join('؛ '),
          severity: 'danger' as const,
          money: {
            remainingBeforeCancellation: Number(preview?.receivable?.remaining || 0),
            collectedAfterDownPayment: Number(preview?.receivable?.collectedAfterDownPayment || 0),
            reversalCredit: Number(preview?.effects?.reversalCredit || 0),
            expectedRefundDue: Number(preview?.effects?.expectedRefundDue || 0),
          },
          counts: {
            physicalQuantity: Number(preview?.inventory?.physicalQuantity || 0),
            unusedChecks: Number(preview?.checks?.unused || 0),
            reconciliationIssues: Number(preview?.reconciliation?.issueCount || 0),
          },
          details: { saleId, mode, preview },
        };
        const confirmation = prepareSensitiveAccountingOperation({
          action: 'installment_sale_cancel',
          scopeKey: `installment-sale:${saleId}`,
          actor: actorFromRequestUser(req.user),
          payload,
          impact,
        });
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        consumeSensitiveAccountingOperation({
          token: req.body?.confirmationToken,
          action: 'installment_sale_cancel',
          scopeKey: `installment-sale:${saleId}`,
          actor: actorFromRequestUser(req.user),
          payload,
        ...
        await recordSensitiveAccountingMutation({
          action: 'installment_sale_cancel',
          entityType: 'installment_sale',
          entityId: saleId,
          actor: actorFromRequestUser(req.user),
          reason,
          before: { preparedPayload: payload },
          after: result.data,
          accountType: 'customer',
          accountId: Number((result.data as any)?.customerId || 0) || null,
        });
        res.json({
          success: true,
          data: result.data,
