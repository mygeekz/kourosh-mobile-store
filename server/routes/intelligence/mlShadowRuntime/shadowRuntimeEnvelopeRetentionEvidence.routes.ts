...
export const registerShadowruntimeenveloperetentionevidenceRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-readiness-pack/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeArtifactEnvelopeRetentionEvidenceExportReadinessPackContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-readiness-pack/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeArtifactEnvelopeRetentionEvidenceExportReadinessPackSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-readiness-pack/exports",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data =
        ...
        res.json({
          ...
          data: {
            ...
            upstreamRetentionPolicyReadinessSnapshot:
              data.upstreamRetentionPolicyReadinessSnapshot,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-readiness-pack/exports/:exportKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
          getShadowRuntimeArtifactEnvelopeRetentionEvidenceExportDetail(
            req.params.exportKey,
          );
        if (!data) {
          res
            .status(404)
            .json({
              success: false,
              message:
                "Artifact envelope retention evidence export readiness record not found.",
            });
          return;
        }
