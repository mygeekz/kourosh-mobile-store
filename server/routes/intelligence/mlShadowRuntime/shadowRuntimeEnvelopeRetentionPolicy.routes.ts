...
export const registerShadowruntimeenveloperetentionpolicyRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-policy-readiness-pack/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeArtifactEnvelopeRetentionPolicyReadinessPackContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-policy-readiness-pack/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeArtifactEnvelopeRetentionPolicyReadinessPackSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-policy-readiness-pack/policy",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data =
        ...
        res.json({
          ...
          data: {
            ...
            upstreamEnvelopeStorageReadinessSnapshot:
              data.upstreamEnvelopeStorageReadinessSnapshot,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-policy-readiness-pack/policies/:policyKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data = getShadowRuntimeArtifactEnvelopeRetentionPolicyDetail(
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message:
                "Artifact envelope retention policy readiness record not found.",
            });
          return;
        }
