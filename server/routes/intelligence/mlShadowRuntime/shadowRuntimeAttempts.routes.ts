...
export const registerShadowruntimeattemptsRoutesRoutes = (
  app: Express,
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        const attempts = await listShadowRuntimeAttempts(req.query.limit);
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/attempts/:id",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const attempt = await getShadowRuntimeAttemptById(req.params.id);
        if (!attempt) {
          res
            .status(404)
            .json({
              success: false,
              message: "Shadow runtime attempt not found.",
            });
          return;
        }
