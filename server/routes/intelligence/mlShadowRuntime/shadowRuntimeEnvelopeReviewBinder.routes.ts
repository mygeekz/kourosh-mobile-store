...
export const registerShadowruntimeenvelopereviewbinderRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-review-binder-design/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeArtifactEnvelopeRetentionEvidenceExportReviewBinderDesignContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-review-binder-design/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeArtifactEnvelopeRetentionEvidenceExportReviewBinderDesignSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-review-binder-design/binders",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data =
        ...
        res.json({
          ...
          data: {
            ...
            readinessRows: data.readinessRows,
            upstreamEvidenceExportReadinessSnapshot:
              data.upstreamEvidenceExportReadinessSnapshot,
            total: data.binders.length,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-retention-evidence-export-review-binder-design/binders/:binderKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message:
                "Artifact envelope retention evidence export review binder design record not found.",
            });
          return;
        }
