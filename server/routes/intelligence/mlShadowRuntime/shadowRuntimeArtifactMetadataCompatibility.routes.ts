...
export const registerShadowruntimeartifactmetadatacompatibilityRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/artifact-metadata-compatibility-matrix/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeArtifactMetadataCompatibilityMatrixContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-metadata-compatibility-matrix/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeArtifactMetadataCompatibilityMatrixSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-metadata-compatibility-matrix/matrix",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = buildShadowRuntimeArtifactMetadataCompatibilityMatrix();
        res.json({
          success: true,
          data: {
            matrixRows: data.matrixRows,
            manifestValidations: data.manifestValidations,
            total: data.matrixRows.length,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-metadata-compatibility-matrix/manifests/:manifestKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message:
                "Artifact metadata compatibility matrix manifest not found.",
            });
          return;
        }
