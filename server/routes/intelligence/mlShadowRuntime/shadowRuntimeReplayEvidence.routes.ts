...
export const registerShadowruntimereplayevidenceRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/replay-delta-trend-evidence-pack/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeReplayDeltaTrendEvidencePackContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/replay-delta-trend-evidence-pack/summary",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data = await getShadowRuntimeReplayDeltaTrendEvidencePackSummary({
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/replay-delta-trend-evidence-pack/batches",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data = await getShadowRuntimeReplayDeltaTrendEvidencePackSummary({
        ...
        res.json({
          success: true,
          data: {
            trendTimeline: data.trendTimeline,
            batchEvidence: data.batchEvidence,
            total: data.batchEvidence.length,
            evidencePack: data.currentShadowRuntimeReplayDeltaTrendEvidencePack,
          },
        });
      } catch (err) {
        next(err);
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/replay-delta-trend-evidence-pack/batches/:id",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
          await getShadowRuntimeReplayDeltaTrendEvidencePackBatchDetail(
            req.params.id,
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message:
                "Shadow runtime replay delta trend evidence batch not found.",
            });
          return;
        }
        res.json({ success: true, data });
