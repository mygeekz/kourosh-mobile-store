...
export const registerShadowruntimecandidateoutputRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-fixture-pack/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeCandidateOutputFixturePackContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-fixture-pack/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeCandidateOutputFixturePackSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-fixture-pack/fixtures",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
  ...
  app.post(
    "/api/brain/ml-shadow-runtime/candidate-output-fixture-pack/validate",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const candidateOutput =
          ((req.body || {}) as Record<string, unknown>).candidateOutput ??
          req.body;
        res.json({
          success: true,
          data: validateShadowRuntimeCandidateOutputFixture(candidateOutput),
        });
      } catch (err) {
        next(err);
      }
