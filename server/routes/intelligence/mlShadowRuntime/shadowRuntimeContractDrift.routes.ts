...
export const registerShadowruntimecontractdriftRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/candidate-contract-drift-review-pack/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeCandidateContractDriftReviewPackContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-contract-drift-review-pack/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeCandidateContractDriftReviewPackSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-contract-drift-review-pack/drift",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = buildShadowRuntimeCandidateContractDriftReviewPack();
        res.json({
          success: true,
          data: {
            driftRows: data.driftRows,
            snapshots: data.snapshots,
            total: data.driftRows.length,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-contract-drift-review-pack/versions/:versionKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message: "Candidate contract drift review version not found.",
            });
          return;
        }
