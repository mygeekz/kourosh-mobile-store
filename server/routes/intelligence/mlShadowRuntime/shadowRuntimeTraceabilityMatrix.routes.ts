...
export const registerShadowruntimetraceabilitymatrixRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-review-binder-traceability-matrix/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeArtifactEnvelopeReviewBinderTraceabilityMatrixContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-review-binder-traceability-matrix/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeArtifactEnvelopeReviewBinderTraceabilityMatrixSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-review-binder-traceability-matrix/matrix",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data =
        ...
        res.json({
          ...
          data: {
            ...
            traceabilityRows: data.traceabilityRows,
            upstreamReviewBinderDesignSnapshot:
              data.upstreamReviewBinderDesignSnapshot,
            total: data.traceabilityRecords.length,
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/artifact-envelope-review-binder-traceability-matrix/binders/:binderKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
        ...
        if (!data) {
          res
            ...
            .json({
              success: false,
              message:
                "Artifact envelope review binder traceability matrix record not found.",
            });
          return;
        }
