...
export const registerShadowruntimecomparisonmatrixRoutesRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-comparison-matrix/contract",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: buildShadowRuntimeCandidateOutputComparisonMatrixContract(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-comparison-matrix/summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        res.json({
          success: true,
          data: getShadowRuntimeCandidateOutputComparisonMatrixSummary(),
        });
      } catch (err) {
        next(err);
      }
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-comparison-matrix/matrix",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = buildShadowRuntimeCandidateOutputComparisonMatrix();
        res.json({
          success: true,
          data: {
  ...
  app.get(
    "/api/brain/ml-shadow-runtime/candidate-output-comparison-matrix/fixtures/:fixtureKey",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const data =
          getShadowRuntimeCandidateOutputComparisonMatrixFixtureDetail(
            req.params.fixtureKey,
          );
        if (!data) {
          res
            .status(404)
            .json({
              success: false,
              message: "Candidate output comparison matrix fixture not found.",
            });
          return;
        }
