...
export const registerOfflineArtifacts07ArtifactRetentionGovernanceReviewRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-reviews/:retentionGovernanceReviewId/archives",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const retentionGovernanceArchives = await listOfflineArtifactRetentionGovernanceReviewArchiveReadinessForRetentionGovernanceReview(req.params.retentionGovernanceReviewId, req.query.limit);
        res.json({ success: true, data: { retentionGovernanceReviewId: req.params.retentionGovernanceReviewId, retentionGovernanceArchives, total: retentionGovernanceArchives.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-reviews/:retentionGovernanceReviewId/archive-readiness",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await prepareOfflineArtifactRetentionGovernanceReviewArchiveReadiness({
          retentionGovernanceReviewId: req.params.retentionGovernanceReviewId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact retention governance review archive readiness validation failed; no retention job was scheduled, no deletion or purge was enabled, no archive file was created, no artifact bytes were included, and no artifact was executed.",
