...
export const registerOfflineArtifacts09ArtifactRetentionGovernanceArchiveRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-archives/:retentionGovernanceArchiveId/finalizations",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const finalizations = await listOfflineArtifactGovernanceArchiveChainFinalizationReadinessForRetentionGovernanceArchive(req.params.retentionGovernanceArchiveId, req.query.limit);
        res.json({ success: true, data: { retentionGovernanceArchiveId: req.params.retentionGovernanceArchiveId, finalizations, total: finalizations.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-archives/:retentionGovernanceArchiveId/finalization-readiness",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await prepareOfflineArtifactGovernanceArchiveChainFinalizationReadiness({
          retentionGovernanceArchiveId: req.params.retentionGovernanceArchiveId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact governance archive chain finalization readiness validation failed; no retention job was scheduled, no deletion or purge was enabled, no file was created, no artifact bytes were included, and no artifact was executed.",
