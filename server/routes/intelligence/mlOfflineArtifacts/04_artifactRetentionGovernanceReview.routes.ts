...
export const registerOfflineArtifacts04ArtifactRetentionGovernanceReviewRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-review-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactRetentionEvidenceGovernanceReviewSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-reviews",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const retentionGovernanceReviews = await listOfflineArtifactRetentionEvidenceGovernanceReviewReadinessManifests(req.query.limit);
        res.json({ success: true, data: { retentionGovernanceReviews, total: retentionGovernanceReviews.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
