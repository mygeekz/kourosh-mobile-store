...
export const registerOfflineArtifacts21ArtifactIntakeRoutes = (
  app: Express,
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        const artifacts = await listQuarantinedArtifacts(req.query.limit);
        res.json({ success: true, data: { artifacts, total: artifacts.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        const artifact = await getQuarantinedArtifactById(req.params.id);
        if (!artifact) {
          res.status(404).json({ success: false, message: "Offline artifact intake record not found." });
          return;
        }
