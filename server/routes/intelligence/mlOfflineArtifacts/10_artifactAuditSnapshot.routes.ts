...
export const registerOfflineArtifacts10ArtifactAuditSnapshotRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshot-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactFinalizationChainAuditSnapshotSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshots",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditSnapshots = await listOfflineArtifactFinalizationChainAuditSnapshotReadinessManifests(req.query.limit);
        res.json({ success: true, data: { auditSnapshots, total: auditSnapshots.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
