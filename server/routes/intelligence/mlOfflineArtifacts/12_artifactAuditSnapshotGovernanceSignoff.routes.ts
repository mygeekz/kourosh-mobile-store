...
export const registerOfflineArtifacts12ArtifactAuditSnapshotGovernanceSignoffRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshot-governance-signoff-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactAuditSnapshotGovernanceSignoffSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshot-governance-signoffs",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditSnapshotGovernanceSignoffs = await listOfflineArtifactAuditSnapshotGovernanceSignoffReadinessManifests(req.query.limit);
        res.json({ success: true, data: { auditSnapshotGovernanceSignoffs, total: auditSnapshotGovernanceSignoffs.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
