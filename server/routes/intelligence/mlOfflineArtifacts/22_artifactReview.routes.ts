...
export const registerOfflineArtifacts22ArtifactReviewRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/:id/reviews",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const reviews = await listOfflineArtifactQuarantineReviewEvidenceForArtifact(req.params.id, req.query.limit);
        res.json({ success: true, data: { artifactId: req.params.id, reviews, total: reviews.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
