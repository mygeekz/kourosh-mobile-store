...
export const registerOfflineArtifacts08ArtifactFinalizationRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/finalization-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactGovernanceArchiveChainFinalizationSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/finalizations",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const finalizations = await listOfflineArtifactGovernanceArchiveChainFinalizationReadinessManifests(req.query.limit);
        res.json({ success: true, data: { finalizations, total: finalizations.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
