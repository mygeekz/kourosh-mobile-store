...
export const registerOfflineArtifacts13ArtifactAuditSnapshotRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshots/:auditSnapshotId/governance-signoffs",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditSnapshotGovernanceSignoffs = await listOfflineArtifactAuditSnapshotGovernanceSignoffsForAuditSnapshot(req.params.auditSnapshotId, req.query.limit);
        res.json({ success: true, data: { auditSnapshotId: req.params.auditSnapshotId, auditSnapshotGovernanceSignoffs, total: auditSnapshotGovernanceSignoffs.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshots/:auditSnapshotId/governance-signoff",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await signoffOfflineArtifactAuditSnapshotGovernanceReadiness({
          auditSnapshotId: req.params.auditSnapshotId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact audit snapshot governance signoff validation failed; no retention job was scheduled, no deletion or purge was enabled, no file was created or exported, no artifact bytes were included, and no artifact was executed.",
