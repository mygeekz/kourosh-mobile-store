...
export const registerOfflineArtifacts23ArtifactBinderRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/:id/binders",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const binders = await listOfflineArtifactReviewBinderReadinessManifestsForArtifact(req.params.id, req.query.limit);
        res.json({ success: true, data: { artifactId: req.params.id, binders, total: binders.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/:id/binder-readiness",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await prepareOfflineArtifactReviewBinderExportReadiness({
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact review binder readiness validation failed; no file was exported, no artifact bytes were included, and no artifact was executed.",
