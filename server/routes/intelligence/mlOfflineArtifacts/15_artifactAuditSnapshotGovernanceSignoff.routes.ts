...
export const registerOfflineArtifacts15ArtifactAuditSnapshotGovernanceSignoffRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshot-governance-signoffs/:auditSnapshotGovernanceSignoffId/archives",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditGovernanceArchives = await listOfflineArtifactAuditSnapshotGovernanceArchiveReadinessForAuditSnapshotGovernanceSignoff(req.params.auditSnapshotGovernanceSignoffId, req.query.limit);
        res.json({ success: true, data: { auditSnapshotGovernanceSignoffId: req.params.auditSnapshotGovernanceSignoffId, auditGovernanceArchives, total: auditGovernanceArchives.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/audit-snapshot-governance-signoffs/:auditSnapshotGovernanceSignoffId/archive-readiness",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await prepareOfflineArtifactAuditSnapshotGovernanceArchiveReadiness({
          auditSnapshotGovernanceSignoffId: req.params.auditSnapshotGovernanceSignoffId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact audit snapshot governance archive readiness validation failed; no retention job was scheduled, no deletion or purge was enabled, no file was created or exported, no artifact bytes were included, and no artifact was executed.",
