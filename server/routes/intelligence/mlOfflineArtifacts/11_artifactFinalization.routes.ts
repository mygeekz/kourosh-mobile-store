...
export const registerOfflineArtifacts11ArtifactFinalizationRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/finalizations/:finalizationId/audit-snapshots",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditSnapshots = await listOfflineArtifactFinalizationChainAuditSnapshotReadinessForFinalization(req.params.finalizationId, req.query.limit);
        res.json({ success: true, data: { finalizationId: req.params.finalizationId, auditSnapshots, total: auditSnapshots.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/finalizations/:finalizationId/audit-snapshot-readiness",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await prepareOfflineArtifactFinalizationChainAuditSnapshotReadiness({
          finalizationId: req.params.finalizationId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact finalization chain audit snapshot readiness validation failed; no retention job was scheduled, no deletion or purge was enabled, no file was created or exported, no artifact bytes were included, and no artifact was executed.",
