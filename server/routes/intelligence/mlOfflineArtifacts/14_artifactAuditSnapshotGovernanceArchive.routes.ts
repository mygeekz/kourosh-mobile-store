...
export const registerOfflineArtifacts14ArtifactAuditSnapshotGovernanceArchiveRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-governance-archive-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactAuditSnapshotGovernanceArchiveSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/audit-governance-archives",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const auditGovernanceArchives = await listOfflineArtifactAuditSnapshotGovernanceArchiveReadinessManifests(req.query.limit);
        res.json({ success: true, data: { auditGovernanceArchives, total: auditGovernanceArchives.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
