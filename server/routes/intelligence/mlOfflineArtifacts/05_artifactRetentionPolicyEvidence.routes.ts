...
export const registerOfflineArtifacts05ArtifactRetentionPolicyEvidenceRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-policy-evidence/:retentionPolicyEvidenceId/governance-reviews",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const retentionGovernanceReviews = await listOfflineArtifactRetentionEvidenceGovernanceReviewsForRetentionPolicyEvidence(req.params.retentionPolicyEvidenceId, req.query.limit);
        res.json({ success: true, data: { retentionPolicyEvidenceId: req.params.retentionPolicyEvidenceId, retentionGovernanceReviews, total: retentionGovernanceReviews.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    "/api/brain/ml-artifacts/offline-intake/retention-policy-evidence/:retentionPolicyEvidenceId/governance-review",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const result = await reviewOfflineArtifactRetentionEvidenceGovernanceReadiness({
          retentionPolicyEvidenceId: req.params.retentionPolicyEvidenceId,
          request: req.body || {},
        ...
        if (!result.accepted) {
          res.status(400).json({
            success: false,
            message: "Offline artifact retention evidence governance review validation failed; no retention job was scheduled, no deletion or purge was enabled, no file was created, no artifact bytes were included, and no artifact was executed.",
