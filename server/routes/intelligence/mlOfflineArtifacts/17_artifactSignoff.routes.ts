...
export const registerOfflineArtifacts17ArtifactSignoffRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/governance-signoff-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactReviewBinderGovernanceSignoffSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/governance-signoffs",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const signoffs = await listOfflineArtifactReviewBinderGovernanceSignoffEvidence(req.query.limit);
        res.json({ success: true, data: { signoffs, total: signoffs.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
