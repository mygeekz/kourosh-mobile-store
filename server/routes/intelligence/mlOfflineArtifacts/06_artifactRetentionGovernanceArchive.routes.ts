...
export const registerOfflineArtifacts06ArtifactRetentionGovernanceArchiveRoutes = (
  app: Express,
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-archive-summary",
    authorizeRole(["Admin", "Manager"]),
    async (_req, res, next) => {
      try {
        const data = await buildOfflineArtifactRetentionGovernanceReviewArchiveSummary();
  ...
  app.get(
    "/api/brain/ml-artifacts/offline-intake/retention-governance-archives",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const retentionGovernanceArchives = await listOfflineArtifactRetentionGovernanceReviewArchiveReadinessManifests(req.query.limit);
        res.json({ success: true, data: { retentionGovernanceArchives, total: retentionGovernanceArchives.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
