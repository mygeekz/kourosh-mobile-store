...
export const registerOfflineArtifacts20ArtifactBinderRoutes = (
  app: Express,
  { authorizeRole }: IntelligenceRouteDeps,
): void => {

  app.get(
    "/api/brain/ml-artifacts/offline-intake/binders",
    authorizeRole(["Admin", "Manager"]),
    async (req, res, next) => {
      try {
        const binders = await listOfflineArtifactReviewBinderReadinessManifests(req.query.limit);
        res.json({ success: true, data: { binders, total: binders.length, safety: getOfflineArtifactIntakeSafetyGate() } });
      } catch (err) {
        next(err);
      }
    },
