  ...
  async (req, res, next) => {
    try {
      ...
      if (eventType === "INSTALLMENT_REMINDER") {
        ...
        template = (settings as any).telegram_installment_settlement_message || settings.telegram_installment_completed_message;
      ...
      // Default templates come from the shared message catalog.
      if (!template) template = buildTelegramRuntimeDefault(eventType);
