...
import {
  SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY,
  authorizeSmsTestRecipient,
  getEnabledSmsTestRecipients,
...
app.get(
  "/api/sms/test-recipients",
  authorizeRole(["Admin", "Manager"]),
  async (_req, res, next) => {
    try {
      const settings = await getAllSettingsAsObject();
      const items = getEnabledSmsTestRecipients(settings[SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY]);
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const settings = await getAllSettingsAsObject();
      const recipientAuth = authorizeSmsTestRecipient(settings[SMS_TEST_RECIPIENT_ALLOWLIST_SETTING_KEY], to);
      if (!recipientAuth.ok) {
        return res.status(recipientAuth.status).json({ success: false, code: recipientAuth.code, message: recipientAuth.message });
      }
      const recipient = recipientAuth.recipient;
      ...
      // خطاهای برگشتی از سرویس پیامک (مثل BodyId نامعتبر یا خطای اعتبارسنجی) خطای سرور ما محسوب نمی‌شوند.
      ...
      for (const t of arr) {
        ...
        results.push({
          ...
          message: smsResult?.success
            ? "ارسال شد"
            : smsResult?.message || "ارسال پیامک ناموفق بود.",
          data: smsResult,
        });
      }
