...
import {
  ...
  buildProductSensitiveUpdateImpact,
...
export const registerProductsRoutes = (
  ...
  app.post(
    "/api/products/:id/sensitive-update/prepare",
    authorizeRole(PRODUCT_ROLES),
    async (req, res, next) => {
      try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ success: false, message: "شناسه محصول نامعتبر است." });
        const payload: any = { ...(req.body || {}) };
        delete payload.confirmationToken;
        const impact = await buildProductSensitiveUpdateImpact(id, payload);
        if (!impact) return res.json({ success: true, data: { required: false } });
        const actor = actorFromRequestUser(req.user);
        const data = prepareSensitiveAccountingOperation({ action: "product_sensitive_update", scopeKey: `product:${id}`, actor, payload, impact });
  ...
  app.put(
    ...
    async (req, res, next) => {
      try {
        ...
        const impact = await buildProductSensitiveUpdateImpact(id, payload);
        if (impact) {
          consumeSensitiveAccountingOperation({ token: confirmationToken, action: "product_sensitive_update", scopeKey: `product:${id}`, actor, payload });
        }
        ...
        if (impact) {
          await recordSensitiveAccountingMutation({ action: "product_sensitive_update", entityType: "product", entityId: id, actor, reason: "ویرایش بهای خرید/تأمین‌کننده/مالکیت", before: impact, after: result });
        }
        // Log update
        if (req.user) {
          await addAuditLog(
