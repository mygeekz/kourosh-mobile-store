...
const getSaleReceiptLedgerTotals = async (saleId: number) => {
  const row = await getAsync(
    `SELECT
       ...
       COALESCE(SUM(CASE WHEN cl.referenceType = 'installment_manual_receipt' THEN cl.credit ELSE 0 END),0) AS manualCredits
    ...
    WHERE (cl.referenceType = 'installment_payment_tx' AND cl.referenceId IN (
       ...
       OR (cl.referenceType = 'installment_manual_receipt' AND cl.referenceId = ?)`,
  ...
  const manualCredits = Math.max(0, Number(row?.manualCredits || 0));
  return { paymentCredits, checkCredits, manualCredits, total: paymentCredits + checkCredits + manualCredits };
};
