...
import { updateSaleProfitSnapshotSourceStatus, persistSaleProfitSnapshotItem, snapshotInstallmentSaleProfitAllocations, buildSaleProfitSnapshotResponse } from "./profitSnapshots.db";
  ...
  try {
    ...
    if (customerId && totalPrice > 0) {
      ...
      await addCustomerLedgerEntryInternal(
        ...
        { referenceType: "sales_transaction_charge", referenceId: Number(saleResult.lastID) },
      );
    }
  ...
  for (const row of rows as any[]) {
    ...
    await persistSaleProfitSnapshotItem({
      ...
      supplierIdAtSale: row.itemType === "phone"
        ? Number(row.phoneSupplierId || 0) || null
        : row.itemType === "inventory"
          ? Number(row.productSupplierId || 0) || null
          : null,
      supplierNameAtSale: row.itemType === "phone"
        ...
        : row.itemType === "inventory"
          ...
          : null,
      sourceCostBasis: "sales_order_item_buy_price",
