...
export const syncInstallmentTransactionCustomerLedger = async (
  ...
  if (existing?.id) {
    await createLedgerReversal({
      ...
      reason: `اصلاح دریافت تراکنش قسط #${txId} از سند منبع`,
      replacement: {
        description,
        debit: 0,
        credit: amount,
        transactionDate: isoDate,
        referenceType: "installment_payment_tx",
        referenceId: txId,
      },
    });
...
export const reconcileInstallmentCustomerLedger = async (
  ...
  for (const sale of saleRows) {
    ...
    if (primary?.id && chargeNeedsUpdate) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(primary.id),
        reason: `اصلاح بدهی فروش اقساطی #${saleId} از سند منبع`,
        replacement: {
          description,
          debit: debt,
          credit: 0,
          transactionDate,
          referenceType: "installment_charge",
          referenceId: saleId,
        },
      });
    ...
    for (const duplicate of ledgerRows.slice(1)) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(duplicate.id),
        reason: `خنثی‌سازی رکورد تکراری بدهی فروش اقساطی #${saleId}`,
      });
  ...
  for (const tx of transactionRows) {
    ...
    if (primary?.id && receiptNeedsUpdate) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(primary.id),
        reason: `اصلاح دریافت قسط #${txId} از سند منبع`,
        replacement: {
          description,
          debit: 0,
          credit: amount,
    ...
    for (const duplicate of ledgerRows.slice(1)) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(duplicate.id),
        reason: `خنثی‌سازی رکورد تکراری دریافت قسط #${txId}`,
      });
      result.removedDuplicates += 1;
      touchedCustomers.add(saleCustomerId);
    }
  ...
  for (const ledgerRow of receiptLedgerRows) {
    ...
    await createLedgerReversal({
      kind: "customer",
      originalLedgerId: Number(ledgerRow.id),
      reason: `خنثی‌سازی دریافت قسط یتیم #${txId} بدون تراکنش منبع`,
    });
...
export const deleteInstallmentTransactionCustomerLedger = async (
  ...
  await createLedgerReversal({
    kind: "customer",
    originalLedgerId: Number(existing.id),
    reason: `حذف تراکنش قسط #${txId} از طریق سند برگشتی`,
  });
