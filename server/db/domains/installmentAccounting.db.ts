...
import { calculateInstallmentContractAccountingState } from "../../accounting/accountingEngine";
import { createLedgerReversal } from "../../accounting/accountingReversal";
...
export const getInstallmentSaleReceivableState = async (
  ...
  const contractBase = { actualSalePrice: sale.actualSalePrice, downPayment: sale.downPayment };
  ...
  const manualReceiptRow = await getAsync(
    `SELECT COALESCE(SUM(COALESCE(credit,0)),0) AS paid
       FROM customer_ledger
      WHERE referenceType = 'installment_manual_receipt'
        AND referenceId = ?`,
    [saleId],
  );
  const manualLedgerReceipt = Math.max(0, Number(manualReceiptRow?.paid || 0));
  const collectedAfterDownPayment = transactionPaid + cashedCheckRemainder + manualLedgerReceipt;
  const canceled = ["canceled", "cancelled"].includes(String(sale.status || "").trim().toLowerCase());
  const canonical = calculateInstallmentContractAccountingState({
    ...contractBase,
  ...
  return {
    saleId,
    contractDebt: canonical.contractDebt,
    transactionPaid,
    cashedCheckRemainder,
    manualLedgerReceipt,
    collectedAfterDownPayment: canonical.collectedAfterDownPayment,
    remaining: canonical.remaining,
    overpayment: canonical.overpayment,
  };
};
...
export const syncInstallmentCheckCustomerLedger = async (
  ...
  if (normalizedStatus !== "نقد شد") {
    if (existing?.id) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(existing.id),
        reason: `تغییر وضعیت چک #${checkId} از وصول‌شده به ${normalizedStatus}`,
      });
  ...
  if (ledgerCredit <= MONEY_EPSILON) {
    if (existing?.id) {
      await createLedgerReversal({
        kind: "customer",
        originalLedgerId: Number(existing.id),
        reason: `صفر شدن مانده قابل ثبت چک #${checkId} پس از تطبیق بازیافت`,
      });
      await recalcCustomerBalancesInternal(Number(check.customerId));
    }
    return;
  ...
  if (existing?.id) {
    await createLedgerReversal({
      kind: "customer",
      originalLedgerId: Number(existing.id),
      reason: `همگام‌سازی اثر وصول چک #${checkId} از سند منبع`,
      replacement: {
        description,
        debit: 0,
        credit: ledgerCredit,
...
export const removeInstallmentSaleCustomerLedger = async (
  ...
  for (const row of rows as any[]) {
    await createLedgerReversal({
      kind: "customer",
      originalLedgerId: Number(row.id),
      reason: `حذف اثر دفتر فروش اقساطی #${saleId} از طریق سند برگشتی`,
    });
  }
