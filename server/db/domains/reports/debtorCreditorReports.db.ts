...
export const getDebtorsList = async (): Promise<FrontendDebtorReportItem[]> => {
  ...
  // Current customer balance comes from the effective-balance view: raw ledger movements
  // plus only provable legacy cashed-check adjustments whose historical cash date is unknown.
  // This avoids both stale cached balances and false debt from legacy check imports.
  const rows = await allAsync(`
    SELECT c.id, c.fullName, c.phoneNumber,
           COALESCE(eb.currentBalance, 0) AS balance
      FROM customers c
      LEFT JOIN v_customer_effective_balance eb ON eb.customerId = c.id
     WHERE COALESCE(eb.currentBalance, 0) > 0.00001
...
> => {
  ...
  const rows = await allAsync(`
    SELECT p.id, p.partnerName, p.partnerType,
           ROUND(COALESCE(SUM(${accountingLedgerDeltaSql("partner", "pl")}), 0), 2) AS balance
      FROM partners p
