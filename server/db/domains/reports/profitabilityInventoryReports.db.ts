  ...
  const query = `
    WITH lines AS (
      SELECT
        ...
        CASE
          WHEN st.itemType = 'inventory' THEN COALESCE(NULLIF(st.buyPrice, 0), 0) * st.quantity
          WHEN st.itemType = 'phone' THEN COALESCE(NULLIF(st.buyPrice, 0), 0) * st.quantity
          ELSE 0
        END AS totalCost
      ...
      SELECT
        ...
        CASE
          WHEN soi.itemType = 'inventory' THEN COALESCE(NULLIF(soi.buyPrice, 0), 0) * soi.quantity
          WHEN soi.itemType = 'phone' THEN COALESCE(NULLIF(soi.buyPrice, 0), 0) * soi.quantity
          ELSE 0
        END AS totalCost
      ...
      SELECT
        ...
        CASE
          WHEN isi.itemType = 'inventory' THEN COALESCE(NULLIF(isi.buyPrice, 0), 0) * isi.quantity
          WHEN isi.itemType = 'phone' THEN COALESCE(NULLIF(isi.buyPrice, 0), 0) * isi.quantity
          ELSE 0
        END AS totalCost
