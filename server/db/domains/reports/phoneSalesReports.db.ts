...
export const getPhoneSalesReport = async (
  ...
  await getDbInstance();
  // v338: سود تاریخی فقط از buyPrice منجمدشده در سند فروش محاسبه می‌شود.
  // نبود buyPrice یعنی evidence تاریخی کافی نداریم؛ هرگز از currentPurchasePrice امروز fallback نمی‌گیریم.
  const query = `
    SELECT
    ...
    FROM (
      SELECT
          ...
          ph.imei,
          COALESCE(NULLIF(soi.buyPrice, 0), 0) as purchasePrice,
          soi.totalPrice as totalPrice,
          (soi.totalPrice - (COALESCE(NULLIF(soi.buyPrice, 0), 0) * COALESCE(soi.quantity, 1))) as profit
      FROM sales_order_items soi
      JOIN sales_orders so ON so.id = soi.orderId
      ...
      SELECT
          ...
          ph.model as phoneModel,
          ph.imei,
          COALESCE(NULLIF(st.buyPrice, 0), 0) as purchasePrice,
          st.totalPrice as totalPrice,
          (st.totalPrice - (COALESCE(NULLIF(st.buyPrice, 0), 0) * COALESCE(st.quantity, 1))) as profit
      FROM sales_transactions st
...
export const getPhoneInstallmentSalesReport = async (
  ...
  await getDbInstance();
  // v338: فروش اقساطی نیز فقط از buyPrice سند استفاده می‌کند؛ legacy بدون line evidence با cost=0/unknown می‌ماند و از قیمت امروز حدس زده نمی‌شود.
  const query = `
    SELECT
      saleId,
      dateCreated,
    ...
    FROM (
      SELECT
        ...
        ph.model as phoneModel,
        ph.imei,
        COALESCE(NULLIF(isi.buyPrice, 0), 0) as purchasePrice,
        COALESCE(isi.totalPrice, isale.actualSalePrice, 0) as actualSalePrice,
        (COALESCE(isi.totalPrice, isale.actualSalePrice, 0) - (COALESCE(NULLIF(isi.buyPrice, 0), 0) * COALESCE(isi.quantity, 1))) as totalProfit
