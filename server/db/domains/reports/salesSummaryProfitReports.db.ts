...
export const getSalesSummaryAndProfit = async (
  ...
  // v338 historical freeze: COGS مالی فقط از buyPrice ذخیره‌شده در سند فروش می‌آید.
  // قیمت فعلی کارت کالا/گوشی یک Live valuation است و نباید سود تاریخی را بازنویسی کند.
  const ordersCogs = await getAsync(
    `SELECT COALESCE(SUM(
        CASE
          WHEN soi.itemType = 'inventory' THEN COALESCE(NULLIF(soi.buyPrice, 0), 0) * COALESCE(soi.quantity, 0)
          WHEN soi.itemType = 'phone' THEN COALESCE(NULLIF(soi.buyPrice, 0), 0) * COALESCE(soi.quantity, 0)
          ELSE 0
        END
      ), 0) as cogs
  ...
  const legacyCogs = await getAsync(
    `SELECT COALESCE(SUM(
        CASE
          WHEN st.itemType = 'inventory' THEN COALESCE(NULLIF(st.buyPrice, 0), 0) * COALESCE(st.quantity, 0)
          WHEN st.itemType = 'phone' THEN COALESCE(NULLIF(st.buyPrice, 0), 0) * COALESCE(st.quantity, 0)
          ELSE 0
        END
      ), 0) as cogs
