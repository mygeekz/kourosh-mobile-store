...
export const getAbcReport = async (
  ...
  // v338 historical freeze: COGS uses the sale-line buyPrice only; current product purchasePrice is live inventory data.
  const rows: any[] = await allAsync(
    `
    SELECT
      x.productId,
    ...
    FROM (
      SELECT
        ...
        (COALESCE(soi.quantity,0) * COALESCE(NULLIF(soi.buyPrice,0),0)) as cogs
      FROM sales_order_items soi
      JOIN sales_orders so ON so.id = soi.orderId
      ...
      SELECT
        ...
        (COALESCE(isi.quantity,0) * COALESCE(NULLIF(isi.buyPrice,0),0)) as cogs
      FROM installment_sale_items isi
      JOIN installment_sales ins ON ins.id = isi.saleId
