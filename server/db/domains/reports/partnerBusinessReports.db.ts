...
import { accountingLedgerDeltaSql } from "../../../accounting/accountingEngine";
...
export const getPartnerAccessoriesReportFromDb = async (
  ...
  const salesRows = await allAsync(
    `SELECT sps.id as snapshotId, sps.saleDate, sps.sourceKind, sps.sourceId, sps.sourceItemId,
            ...
            COALESCE(spca.sharePercent, 0) as ownershipSharePercent,
            SUM(CASE WHEN spa.allocationType = 'owner_gain' THEN COALESCE(spa.amount,0) ELSE 0 END) as partnerOwnerGainAmount,
            SUM(CASE WHEN spa.allocationType = 'shared_profit' THEN COALESCE(spa.amount,0) ELSE 0 END) as partnerSharedProfitAmount,
            SUM(COALESCE(spa.amount,0)) as partnerTotalProfitAmount
       FROM sale_profit_snapshots sps
       JOIN sale_profit_capital_allocations spca ON spca.snapshotId = sps.id AND spca.storePartnerId = ? AND spca.sourceStatus = 'active'
       LEFT JOIN sale_profit_allocations spa ON spa.snapshotId = sps.id AND spa.storePartnerId = ? AND spa.sourceStatus = 'active'
      ...
      GROUP BY sps.id, sps.saleDate, sps.sourceKind, sps.sourceId, sps.sourceItemId, sps.itemDescription,
               sps.quantity, sps.saleAmount, sps.initialCostAmount, sps.ownerGainAmount, sps.sharedProfitAmount, sps.totalProfitAmount, spca.sharePercent
...
export const getPartnerSettlementReportFromDb = async (
  ...
  try {
    ...
    const lifetimeLedgerRows = await allAsync(
      `SELECT spl.storePartnerId,
              SUM(${accountingLedgerDeltaSql("partner", "pl")}) as partnerDetailCurrentBalance,
  ...
  const salesRows = await allAsync(
    `SELECT sps.id as snapshotId, sps.saleDate, sps.sourceKind, sps.sourceId, sps.sourceItemId,
      ...
      GROUP BY sps.id, sps.saleDate, sps.sourceKind, sps.sourceId, sps.sourceItemId, sps.itemId, sps.itemDescription,
               ...
               sps.sharedProfitAmount, sps.totalProfitAmount, ph.model, ph.imei, spca.sharePercent
      ORDER BY sps.saleDate DESC, sps.id DESC`,
    [partnerId, partnerId].concat(salesFilter.params as any),
  );
