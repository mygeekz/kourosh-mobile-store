...
import { allAsync, getAsync, runAsync, type SqliteBindValue } from "../query";
import { allocateAccountingAmountByShares, calculateSaleProfitAccountingState } from "../../accounting/accountingEngine";
import {
  buildDateRangeSql,
...
export type SaleProfitSnapshotItemInput = {
  ...
  ownershipProfileId: number | null;
  supplierIdAtSale?: number | null;
  supplierNameAtSale?: string | null;
  sourceCostBasis?: string | null;
  snapshotState?: "frozen_at_sale" | "legacy_reconstructed";
...
export const allocateAmountAcrossShares = (
  ...
  return allocateAccountingAmountByShares(Number(amount), normalized);
};
...
export const purgeProfitSnapshotsForSource = async (
  ...
  if (ids.length) {
    ...
    await runAsync(
      `DELETE FROM sale_profit_capital_allocations WHERE snapshotId IN (${ids.map(() => "?").join(",")})`,
      ids,
    ).catch(() => undefined);
  }
  await runAsync(
...
export const updateSaleProfitSnapshotSourceStatus = async (
  ...
  await runAsync(
    `UPDATE sale_profit_capital_allocations
        SET sourceStatus = ?, updatedAt = (strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc'))
      WHERE snapshotId IN (SELECT id FROM sale_profit_snapshots WHERE sourceKind = ? AND sourceId = ?)`,
    [sourceStatus, sourceKind, sourceId],
  ).catch(() => undefined);
};
...
export const persistSaleProfitSnapshotItem = async (
  ...
  const accounting = calculateSaleProfitAccountingState({
    quantity: input.quantity,
    saleAmount: input.saleAmount,
    initialCostPerUnit: input.initialCostPerUnit,
    marketCostPerUnit: input.marketCostPerUnit,
    ownershipType: ctx.ownershipType,
  });
  const { quantity, saleAmount, initialCostPerUnit, marketCostPerUnit, initialCostAmount, marketCostAmount, ownerGainAmount, sharedProfitAmount, totalProfitAmount } = accounting;
  ...
  const snapshotValues: readonly SqliteBindValue[] = [
    input.sourceKind,
    input.sourceId,
    ...
    totalProfitAmount,
    "active",
    Number(input.supplierIdAtSale || 0) || null,
    String(input.supplierNameAtSale || "").trim() || null,
    JSON.stringify(ctx.ownershipItems),
    JSON.stringify(ctx.profitShareItems),
    2,
    input.snapshotState || "frozen_at_sale",
    String(input.sourceCostBasis || "sale_line_buy_price").trim() || "sale_line_buy_price",
    new Date().toISOString(),
    notes || null,
  ];
  if (snapshotColumns.length !== snapshotValues.length) {
    throw new Error(
      `sale_profit_snapshots insert contract mismatch: ${snapshotColumns.length} columns / ${snapshotValues.length} values`,
    );
  }

  const snapshotInsert = await runAsync(
    `INSERT INTO sale_profit_snapshots (${snapshotColumns.join(", ")}) VALUES (${snapshotColumns.map(() => "?").join(",")})`,
    snapshotValues,
  );
  const snapshotId = Number(snapshotInsert.lastID || 0);

  const capitalAllocations = allocateAmountAcrossShares(initialCostAmount, ctx.ownershipItems);
  for (const allocation of capitalAllocations) {
    await runAsync(
      `INSERT INTO sale_profit_capital_allocations (
...
export const snapshotInstallmentSaleProfitAllocations = async (
  saleId: number,
  options?: { legacyBackfill?: boolean },
): Promise<void> => {
  const existing = await getAsync(
    `SELECT COUNT(*) AS count FROM sale_profit_snapshots WHERE sourceKind = 'installment_sale' AND sourceId = ?`,
    [saleId],
  );
  if (Number(existing?.count || 0) > 0) return;
  const sale = await getAsync(
    `SELECT saleDateISO, saleDate, installmentsStartDate, dateCreated, status FROM installment_sales WHERE id = ?`,
    [saleId],
  );
  const saleDate = (() => {
  ...
  const rows = await allAsync(
    `SELECT isi.id as sourceItemId, isi.saleId as sourceId, isi.itemType, isi.itemId, isi.description,
            ...
            ph.purchasePrice as phonePurchasePrice, ph.ownershipProfileId as phoneOwnershipProfileId, ph.supplierId as phoneSupplierId, php.partnerName as phoneSupplierName,
            pos.ownershipProfileId as phoneHistoricalOwnershipProfileId, pos.sourceLegacyPartnerId as phoneHistoricalSupplierId, phps.partnerName as phoneHistoricalSupplierName,
            pr.purchasePrice as productPurchasePrice, pr.ownershipProfileId as productOwnershipProfileId, pr.supplierId as productSupplierId, prp.partnerName as productSupplierName,
            prs.ownershipProfileId as productHistoricalOwnershipProfileId, prs.sourceLegacyPartnerId as productHistoricalSupplierId, prps.partnerName as productHistoricalSupplierName
       FROM installment_sale_items isi
       LEFT JOIN phones ph ON isi.itemType = 'phone' AND ph.id = isi.itemId
       LEFT JOIN partners php ON php.id = ph.supplierId
       LEFT JOIN phone_ownership_snapshots pos ON isi.itemType = 'phone' AND pos.phoneId = isi.itemId
       LEFT JOIN partners phps ON phps.id = pos.sourceLegacyPartnerId
       LEFT JOIN products pr ON isi.itemType = 'inventory' AND pr.id = isi.itemId
       LEFT JOIN partners prp ON prp.id = pr.supplierId
       LEFT JOIN product_ownership_snapshots prs ON isi.itemType = 'inventory' AND prs.productId = isi.itemId
       LEFT JOIN partners prps ON prps.id = prs.sourceLegacyPartnerId
      WHERE isi.saleId = ?
      ORDER BY isi.id ASC`,
    [saleId],
  );
  ...
  for (const row of rows as any[]) {
    const isLegacyBackfill = Boolean(options?.legacyBackfill);
    const initialCostPerUnit = isLegacyBackfill
      ...
      : row.itemType === "phone"
        ...
        : row.itemType === "inventory"
          ...
          : 0;
    const ownershipProfileId = isLegacyBackfill
      ? row.itemType === "phone"
        ? Number(row.phoneHistoricalOwnershipProfileId) || null
        : row.itemType === "inventory"
          ? Number(row.productHistoricalOwnershipProfileId) || null
          : null
      : row.itemType === "phone"
    ...
    await persistSaleProfitSnapshotItem({
      ...
      ownershipProfileId,
      supplierIdAtSale: isLegacyBackfill
        ? row.itemType === "phone"
          ? Number(row.phoneHistoricalSupplierId || 0) || null
          : row.itemType === "inventory"
            ? Number(row.productHistoricalSupplierId || 0) || null
            : null
        : row.itemType === "phone"
          ? Number(row.phoneHistoricalSupplierId || 0) || null
          : row.itemType === "inventory"
            ? Number(row.productHistoricalSupplierId || 0) || null
            : null,
      supplierNameAtSale: isLegacyBackfill
        ? row.itemType === "phone"
          ? row.phoneHistoricalSupplierName || null
          : row.itemType === "inventory"
            ? row.productHistoricalSupplierName || null
            : null
        : row.itemType === "phone"
          ? row.phoneSupplierName || null
          : row.itemType === "inventory"
            ? row.productSupplierName || null
            : null,
      sourceCostBasis: isLegacyBackfill ? "legacy_sale_document_buy_price" : "installment_sale_item_buy_price",
      snapshotState: isLegacyBackfill ? "legacy_reconstructed" : "frozen_at_sale",
      fallbackNotes: ownershipProfileId
        ? null
        : "مالکیت مشخص نبود؛ تلاش شد پروفایل پیش‌فرض اعمال شود.",
    });
  }
  const sourceStatus = String(sale?.status || "active").toLowerCase();
  if (sourceStatus === "canceled" || sourceStatus === "cancelled") {
    await updateSaleProfitSnapshotSourceStatus("installment_sale", saleId, "canceled");
  }
};



export const rebuildSalesOrderProfitSnapshots = async (orderId: number): Promise<void> => {
  const existing = await getAsync(
    `SELECT COUNT(*) AS count FROM sale_profit_snapshots WHERE sourceKind = 'sales_order' AND sourceId = ?`,
    [orderId],
  );
  if (Number(existing?.count || 0) > 0) return;
  const order = await getAsync(`SELECT transactionDate, status FROM sales_orders WHERE id = ?`, [orderId]);
  if (!order) return;
  const rows = await allAsync(
    `SELECT soi.id as sourceItemId, soi.orderId as sourceId, soi.itemType, soi.itemId, soi.description,
       ...
       LEFT JOIN partners php ON php.id = ph.supplierId
       LEFT JOIN phone_ownership_snapshots pos ON soi.itemType = 'phone' AND pos.phoneId = soi.itemId
       LEFT JOIN partners phps ON phps.id = pos.sourceLegacyPartnerId
       LEFT JOIN products pr ON soi.itemType = 'inventory' AND pr.id = soi.itemId
       LEFT JOIN partners prp ON prp.id = pr.supplierId
       LEFT JOIN product_ownership_snapshots prs ON soi.itemType = 'inventory' AND prs.productId = soi.itemId
       LEFT JOIN partners prps ON prps.id = prs.sourceLegacyPartnerId
      WHERE soi.orderId = ?
      ORDER BY soi.id ASC`,
    [orderId],
  ...
  for (const row of rows as any[]) {
    const initialCostPerUnit = Number(row.buyPrice) || 0;
    ...
    await persistSaleProfitSnapshotItem({
      ...
      sourceItemId: Number(row.sourceItemId),
      saleDate: String(order.transactionDate || "").slice(0, 10) || null,
      itemType: row.itemType,
      itemId: Number(row.itemId) || null,
      ...
      marketCostPerUnit: Number(row.buyPrice) || initialCostPerUnit || 0,
      ownershipProfileId,
      supplierIdAtSale: row.itemType === "phone"
        ? Number(row.phoneHistoricalSupplierId || 0) || null
        : row.itemType === "inventory"
          ? Number(row.productHistoricalSupplierId || 0) || null
          : null,
      supplierNameAtSale: row.itemType === "phone"
        ? row.phoneHistoricalSupplierName || null
        : row.itemType === "inventory"
          ? row.productHistoricalSupplierName || null
          : null,
      sourceCostBasis: "legacy_sale_document_buy_price",
      snapshotState: "legacy_reconstructed",
...
/**
 * Legacy-compatible name, v338 semantics: backfill missing snapshots only.
 * Asset price/supplier/ownership edits must never rewrite an existing sale snapshot.
 */
export const rebuildProfitSnapshotsForItem = async (
  itemType: "phone" | "inventory",
  itemId: number,
): Promise<{ salesOrders: number; installmentSales: number }> => {
  const orderRows = await allAsync(
    `SELECT DISTINCT orderId AS id FROM sales_order_items WHERE itemType = ? AND itemId = ?`,
    [itemType, itemId],
  ).catch(() => [] as any[]);
  const installmentRows = await allAsync(
    `SELECT DISTINCT saleId AS id FROM installment_sale_items WHERE itemType = ? AND itemId = ?`,
    [itemType, itemId],
  ).catch(() => [] as any[]);
  for (const row of orderRows as any[]) await rebuildSalesOrderProfitSnapshots(Number(row.id));
  for (const row of installmentRows as any[]) await snapshotInstallmentSaleProfitAllocations(Number(row.id), { legacyBackfill: true });
  return { salesOrders: orderRows.length, installmentSales: installmentRows.length };
};

/** v338: source reconciliation only backfills missing snapshots; existing historical facts are immutable. */
export const rebuildAllProfitSnapshotsFromSources = async (): Promise<{ salesOrders: number; installmentSales: number }> => {
  const orderRows = await allAsync(`SELECT id FROM sales_orders ORDER BY id ASC`).catch(() => [] as any[]);
  const installmentRows = await allAsync(`SELECT id FROM installment_sales ORDER BY id ASC`).catch(() => [] as any[]);
  for (const row of orderRows as any[]) await rebuildSalesOrderProfitSnapshots(Number(row.id));
  for (const row of installmentRows as any[]) await snapshotInstallmentSaleProfitAllocations(Number(row.id), { legacyBackfill: true });
  return { salesOrders: orderRows.length, installmentSales: installmentRows.length };
};
...
export const getLegacySaleRowsForReports = async (
  ...
  const salesOrderRows = await allAsync(
    `SELECT 'sales_order' as sourceKind, so.id as sourceId, soi.id as sourceItemId, so.transactionDate as saleDate,
            ...
            CASE
              WHEN soi.itemType IN ('phone','inventory') THEN COALESCE(NULLIF(soi.buyPrice, 0), 0)
              ELSE COALESCE(soi.buyPrice, 0)
            END as marketUnitBuyPrice,
            ph.model as phoneModel, ph.imei as phoneImei,
            pos.sourceLegacyPartnerId as phoneHistoricalSupplierId,
            pr.name as productName, prs.sourceLegacyPartnerId as productHistoricalSupplierId
       FROM sales_order_items soi
  ...
  const installmentRows = await allAsync(
    `SELECT 'installment_sale' as sourceKind, ins.id as sourceId, isi.id as sourceItemId, COALESCE(ins.saleDateISO, ins.dateCreated) as saleDate,
            ...
            CASE
              WHEN isi.itemType IN ('phone','inventory') THEN COALESCE(NULLIF(isi.buyPrice, 0), 0)
...
export const buildLegacyComputedSales = async (
  ...
  for (const row of sourceRows as any[]) {
    ...
    const initialUnitCost = itemType === "service" ? 0 : Number(row.marketUnitBuyPrice) || 0;
    ...
    const legacySupplierId =
      itemType === "phone"
        ? Number(row.phoneHistoricalSupplierId || 0)
        : itemType === "inventory"
          ? Number(row.productHistoricalSupplierId || 0)
          : null;
...
export const getPartnerProfitReportFromDb = async (
  ...
  const hasSnapshots =
    ...
    (await tableExists("sale_profit_capital_allocations"));
  if (!hasSnapshots) {
  ...
  const capitalRows = await allAsync(
    `SELECT sps.sourceKind, sps.sourceId, sps.itemType, sps.saleDate, sps.initialCostAmount,
            spca.storePartnerId, sp.name as partnerName, sp.colorTag, spca.sharePercent
       FROM sale_profit_snapshots sps
       JOIN sale_profit_capital_allocations spca ON spca.snapshotId = sps.id AND spca.sourceStatus = 'active'
       JOIN store_partners sp ON sp.id = spca.storePartnerId
      WHERE sps.sourceStatus = 'active'${dateFilter.sql}
      ORDER BY sps.saleDate DESC, sps.id DESC, spca.id DESC`,
    dateFilter.params,
  ).catch(() => [] as any[]);
