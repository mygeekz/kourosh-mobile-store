...
export const deleteInstallmentSaleFromDb = async (
  ...
  throw new Error("قرارداد اقساطی سند مالی تاریخی است و Hard Delete نمی‌شود؛ فقط عملیات فسخ غیرمخرب مجاز است.");
};
...
manual_receipt_by_sale AS (
  SELECT
    cl.referenceId AS saleId,
    COALESCE(SUM(MAX(0, COALESCE(cl.credit, 0))), 0) AS manualLedgerReceipt
  FROM customer_ledger cl
  JOIN directory_scope scoped_sale ON scoped_sale.id = cl.referenceId
  WHERE cl.referenceType = 'installment_manual_receipt'
  GROUP BY cl.referenceId
),
sale_receivable AS (
  SELECT
    isale.id AS saleId,
    ...
    MAX(0, COALESCE(mr.manualLedgerReceipt, 0)) AS manualLedgerReceipt,
    MAX(0, COALESCE(ts.transactionPaid, 0) + COALESCE(cr.cashedCheckRemainder, 0) + COALESCE(mr.manualLedgerReceipt, 0)) AS collectedAfterDownPayment,
    CASE
      ...
      ELSE MAX(
        ...
        MAX(0, COALESCE(isale.actualSalePrice, 0) - COALESCE(isale.downPayment, 0))
          - MAX(0, COALESCE(ts.transactionPaid, 0) + COALESCE(cr.cashedCheckRemainder, 0) + COALESCE(mr.manualLedgerReceipt, 0))
      )
  ...
  LEFT JOIN manual_receipt_by_sale mr ON mr.saleId = isale.id
),
...
latest_collection_events AS (
  ...
  SELECT
    cl.referenceId AS saleId,
    -1000000000 - cl.id AS eventId,
    cl.credit AS amount,
    cl.transactionDate AS eventDate,
    'manual_receipt' AS source
  FROM customer_ledger cl
  JOIN directory_scope scoped_sale ON scoped_sale.id = cl.referenceId
  WHERE cl.referenceType = 'installment_manual_receipt'
    AND COALESCE(cl.credit, 0) > 0
...
export const getInstallmentSaleByIdFromDb = async (
  ...
  const manualReceipts = await allAsync(
    `SELECT id,
            COALESCE(credit, 0) AS amountPaid,
