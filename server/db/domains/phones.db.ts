...
import { resolvePhoneCostBasisAmount } from "../phoneCostBasis";
import { syncSupplierAutoOwnership } from "../ownershipConsistency";
import {
...
const insertPhoneEntryWithinTransaction = async (
  ...
  await syncSupplierAutoOwnership("phone", Number(newPhoneId), supplierId || null);
  ...
  try {
    ...
    if (shouldSyncPurchaseLedger) {
      ...
      if (previousLedgerRow) {
        // v341: never mutate the original financial ledger row. Preserve the
        // purchase document and append an exact reversal + replacement.
        const existingReversal = await getAsync(
          `SELECT reversalLedgerId, replacementLedgerId FROM accounting_reversal_links WHERE ledgerKind='partner' AND originalLedgerId=?`,
          [Number(previousLedgerRow.id)],
        );
        if (!existingReversal) {
          const reversalDescription = `برگشت سند خرید گوشی #${Number(previousLedgerRow.id)} — اصلاح اطلاعات خرید گوشی #${phoneId}`;
          const reversal = await runAsync(
            `INSERT INTO partner_ledger
              (partnerId, transactionDate, createdAt, updatedAt, description, debit, credit, balance, referenceType, referenceId, changeHistoryJson)
             VALUES (?, ?, ?, ?, ?, ?, ?, 0, 'accounting_reversal', ?, ?)`,
            [
              Number(previousLedgerRow.partnerId || existingPhone.supplierId || syncPartnerId),
              nowIso,
              nowIso,
              nowIso,
              reversalDescription,
              Number(previousLedgerRow.credit || 0),
              Number(previousLedgerRow.debit || 0),
              Number(previousLedgerRow.id),
              stringifyLedgerChangeHistory((previousLedgerRow as any)?.changeHistoryJson, {
                changedAt: nowIso,
                reason: 'phone_purchase_reversal_on_edit',
                actor: actor ? { userId: actor.userId ?? null, username: actor.username ?? null, displayName: actor.displayName ?? null } : null,
                before: beforeSnapshot,
                after: { reversalOfLedgerId: Number(previousLedgerRow.id), phoneId },
              }),
            ],
          );
          const replacement = await runAsync(
            `INSERT INTO partner_ledger
              (partnerId, transactionDate, createdAt, updatedAt, description, debit, credit, balance, referenceType, referenceId, changeHistoryJson)
             VALUES (?, ?, ?, ?, ?, 0, ?, 0, 'phone_purchase', ?, ?)`,
            [syncPartnerId, nowIso, nowIso, nowIso, nextDescription, newPurchasePrice, phoneId, historyJson],
          );
          await runAsync(
            `INSERT INTO accounting_reversal_links
              (ledgerKind, originalLedgerId, reversalLedgerId, replacementLedgerId, reason, createdAt)
             VALUES ('partner', ?, ?, ?, ?, ?)`,
            [Number(previousLedgerRow.id), Number(reversal.lastID), Number(replacement.lastID), `اصلاح اطلاعات خرید گوشی #${phoneId}`, nowIso],
          );
        }
      } else {
        await addPartnerLedgerEntryInternal(
    ...
    let updatedPhone = await getAsync(
      "SELECT ph.*, pa.partnerName as supplierName FROM phones ph LEFT JOIN partners pa ON ph.supplierId = pa.id WHERE ph.id = ?",
      [phoneId],
    );
    const ownershipSync = await syncSupplierAutoOwnership(
      "phone",
      Number(phoneId),
      Number(updatedPhone?.supplierId || 0) || null,
    );
    if (ownershipSync.changed) {
      updatedPhone = await getAsync(
        "SELECT ph.*, pa.partnerName as supplierName FROM phones ph LEFT JOIN partners pa ON ph.supplierId = pa.id WHERE ph.id = ?",
        [phoneId],
      );
    }
