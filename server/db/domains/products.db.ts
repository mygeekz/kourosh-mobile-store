...
export const addProductToDb = async (
  ...
  try {
    ...
    await syncSupplierAutoOwnership("product", Number(newProductId), supplierId || null);
...
export const updateProductInDb = async (
  ...
  try {
    ...
    const ownershipSync = await syncSupplierAutoOwnership(
      "product",
      Number(productId),
      supplierId === undefined ? Number(product.supplierId || 0) || null : (Number(supplierId || 0) || null),
    );
    return await getTypedAsync<ProductRecord>(
      `SELECT p.*, c.name as categoryName, pa.partnerName as supplierName
          FROM products p
