...
export const getAuditLogs = async (limit: number = 100, offset: number = 0) => {
  return allAsync(
    `SELECT id, userId, username, role, action, entityType, entityId, description, tenantId, source, requestId, beforeJson, afterJson, metadataJson, createdAt
     FROM audit_logs
...
export const getAuditLogReport = async (filters: AuditLogReportFilters = {}) => {
  ...
  if (query) {
    ...
    where.push(`(
      ...
      COALESCE(beforeJson, '') LIKE ? COLLATE NOCASE OR
      COALESCE(afterJson, '') LIKE ? COLLATE NOCASE OR
      COALESCE(metadataJson, '') LIKE ? COLLATE NOCASE
    )`);
