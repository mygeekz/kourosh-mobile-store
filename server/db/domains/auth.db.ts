...
import { syncLegacyUserAccessGrant } from "./accessControl.db";
...
export const addUserToDb = async (
  ...
  const legacyRole = await getTypedAsync<RoleNameRow>(
    "SELECT name FROM roles WHERE id = ?",
    [roleId],
  );
  const legacyRoleName = requireRoleName(legacyRole);
  ...
  try {
    ...
    await syncLegacyUserAccessGrant(result.lastID, legacyRoleName);
...
export const updateUserInDb = async (
  ...
  let requestedLegacyRoleName: string | null = null;
  if (data.roleId !== undefined) {
    const legacyRole = await getTypedAsync<RoleNameRow>(
      "SELECT name FROM roles WHERE id = ?",
      [data.roleId],
    );
    requestedLegacyRoleName = requireRoleName(legacyRole);
  }

  params.push(userId);
  if (requestedLegacyRoleName !== null) await runAsync("BEGIN IMMEDIATE");
  try {
    await runAsync(
      `UPDATE users SET ${fieldsToUpdate.join(", ")} WHERE id = ?`,
      params,
    );
    if (requestedLegacyRoleName !== null) {
      await syncLegacyUserAccessGrant(userId, requestedLegacyRoleName);
      await runAsync("COMMIT");
    }
  } catch (error) {
    if (requestedLegacyRoleName !== null) await runAsync("ROLLBACK").catch(() => undefined);
    throw error;
  }
