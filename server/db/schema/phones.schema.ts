...
export const createPhonesSchema = async (): Promise<void> => {
  ...
  await runAsync(`
    CREATE TABLE IF NOT EXISTS sale_profit_snapshots (
      ...
      supplierIdAtSale INTEGER,
      supplierNameAtSale TEXT,
      ownershipSharesJson TEXT NOT NULL DEFAULT '[]',
      profitShareSharesJson TEXT NOT NULL DEFAULT '[]',
      snapshotVersion INTEGER NOT NULL DEFAULT 2,
      snapshotState TEXT NOT NULL DEFAULT 'frozen_at_sale',
      sourceCostBasis TEXT,
      frozenAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc')),
      notes TEXT,
      createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc')),
      updatedAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now', 'utc')),
  ...
  const profitSnapshotCols = await allTypedAsync<TableInfoRow>("PRAGMA table_info(sale_profit_snapshots);");
  const profitSnapshotColNames = new Set(profitSnapshotCols.map((column) => column.name).filter(Boolean));
  const profitSnapshotColumnUpgrades: Array<[string, string]> = [
    ["supplierIdAtSale", "INTEGER"],
    ["supplierNameAtSale", "TEXT"],
    ["ownershipSharesJson", "TEXT NOT NULL DEFAULT '[]'"],
    ["profitShareSharesJson", "TEXT NOT NULL DEFAULT '[]'"],
    ["snapshotVersion", "INTEGER NOT NULL DEFAULT 1"],
    ["snapshotState", "TEXT NOT NULL DEFAULT 'legacy_baseline'"],
    ["sourceCostBasis", "TEXT"],
    ["frozenAt", "TEXT"],
  ];
  for (const [name, definition] of profitSnapshotColumnUpgrades) {
    if (!profitSnapshotColNames.has(name)) {
      await runAsync(`ALTER TABLE sale_profit_snapshots ADD COLUMN ${name} ${definition}`);
    }
  }
  await runAsync(`
  ...
  `);
  await runAsync(`CREATE INDEX IF NOT EXISTS idx_sale_profit_capital_allocations_snapshot ON sale_profit_capital_allocations(snapshotId);`);
  await runAsync(`CREATE INDEX IF NOT EXISTS idx_sale_profit_capital_allocations_partner ON sale_profit_capital_allocations(storePartnerId);`);
  await runAsync(`
