...
export const createMlCandidatePackageHumanSignoffArchivePacksSchema = async (): Promise<void> => {
  await runAsync(`
    CREATE TABLE IF NOT EXISTS ml_candidate_package_human_signoff_archive_packs (
      ...
      archive_pack_json TEXT NOT NULL,
      retention_policy_json TEXT NOT NULL,
      safety_policy_json TEXT NOT NULL,
      summary_json TEXT NOT NULL,
      signed_archive_hash TEXT NOT NULL,
      archive_pack_is_production_approval INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_load_package_bytes INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_persist_artifact_bytes INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_execute_model INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_invoke_runtime INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_expose_inference_endpoint INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_activate_artifact INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_deploy_artifact INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_production_score INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_schedule_retention_jobs INTEGER NOT NULL DEFAULT 0,
      archive_pack_can_delete_or_purge INTEGER NOT NULL DEFAULT 0,
      archive_pack_metadata_only INTEGER NOT NULL DEFAULT 1,
  ...
  await runAsync(
    `CREATE INDEX IF NOT EXISTS idx_ml_candidate_package_human_signoff_archive_packs_signoff_id ON ml_candidate_package_human_signoff_archive_packs(signoff_id, created_at)`,
  );

  await runAsync(
    `CREATE INDEX IF NOT EXISTS idx_ml_candidate_package_human_signoff_archive_packs_key ON ml_candidate_package_human_signoff_archive_packs(archive_pack_key, created_at)`,
  );

  await runAsync(
    `CREATE INDEX IF NOT EXISTS idx_ml_candidate_package_human_signoff_archive_packs_hash ON ml_candidate_package_human_signoff_archive_packs(signed_archive_hash)`,
  );
};
