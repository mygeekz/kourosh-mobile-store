...
export const createTelegramIdentitySecuritySchema = async (): Promise<void> => {
  ...
  await addPartnerColumn("ALTER TABLE telegram_partner_link_tokens ADD COLUMN allow_rebind INTEGER NOT NULL DEFAULT 0 CHECK(allow_rebind IN (0,1))");
  await addPartnerColumn("ALTER TABLE telegram_partner_link_tokens ADD COLUMN previous_telegram_user_id TEXT");
