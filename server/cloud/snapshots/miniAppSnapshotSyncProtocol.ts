...
export type MiniAppSnapshotSyncBodyV1 = {
  ...
  /**
   * Current stable Live Origin for this installation. It is signed together with
   * the snapshot body so the Edge can safely self-heal a stale D1 live_origin
   * after local settings are restored/reset.
   */
  liveOrigin?: string;
};
...
export const createSignedMiniAppSnapshotSyncRequest = (input: {
  ...
  if (input.liveOrigin !== undefined) {
    try {
      const parsed = new URL(String(input.liveOrigin || ""));
      if (parsed.protocol !== "https:" || parsed.username || parsed.password || parsed.search || parsed.hash || (parsed.pathname !== "/" && parsed.pathname !== "")) throw new Error("invalid");
      liveOrigin = parsed.origin;
    } catch {
      throw Object.assign(new Error("Mini App live origin is invalid for snapshot sync."), { code: "MINIAPP_SNAPSHOT_SYNC_LIVE_ORIGIN_INVALID" });
    }
  }
