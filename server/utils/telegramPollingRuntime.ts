...
export const createTelegramPollingRuntime = (deps: TelegramPollingRuntimeDeps) => {
  ...
  const autoConfigureTelegramUpdateMode = async () => {
    ...
    // v356: never trust a persisted `webhook` mode blindly. MiniApp can stay
    // healthy while a stale Telegram webhook makes the conversational bot
    // completely silent. Telegram exposes delivery failures through
    // getWebhookInfo, so validate the actual remote state on every startup.
    ...
    const webhookResult = webhookInfo?.data?.result || webhookInfo?.result || {};
    const webhookUrl = String(webhookResult?.url || "").trim();
    const webhookIsPublic = isTelegramWebhookUrlPublic(webhookUrl);
    const lastErrorUnix = Number(webhookResult?.last_error_date || 0);
    const lastErrorMessage = String(webhookResult?.last_error_message || "").trim();
    const lastWebhookAtRaw = String((settings as any).telegram_last_webhook_at || "").trim();
    const lastWebhookUnix = lastWebhookAtRaw ? Math.floor(Date.parse(lastWebhookAtRaw) / 1000) : 0;
    const hasUnrecoveredWebhookError =
      lastErrorUnix > 0 && (!Number.isFinite(lastWebhookUnix) || lastWebhookUnix <= 0 || lastErrorUnix > lastWebhookUnix);

    if (webhookIsPublic && !hasUnrecoveredWebhookError) {
      return {
        changed: false,
        mode: "webhook",
        reason: explicitMode === "webhook" ? "explicit-webhook-healthy" : "public-webhook",
        webhookUrl,
      };
    }

    if (explicitMode === "webhook") {
      deps.telegramLog("stale webhook detected; falling back to polling", {
        webhookUrl: webhookUrl || null,
        webhookIsPublic,
        lastErrorUnix: lastErrorUnix || null,
        lastErrorMessage: lastErrorMessage || null,
        lastWebhookAt: lastWebhookAtRaw || null,
      });
    }
