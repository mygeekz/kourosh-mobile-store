...
import { markdownishToHtml, renderTplHtml } from "./messagingFormatters";
import { buildTelegramRuntimeDefault } from "../../shared/messages";
...
const buildInstallmentText = (
  ...
  const defaults: Record<string, string> = {
    due_7: buildTelegramRuntimeDefault("INSTALLMENT_DUE_7"),
    due_3: buildTelegramRuntimeDefault("INSTALLMENT_DUE_3"),
    due_today: buildTelegramRuntimeDefault("INSTALLMENT_DUE_TODAY"),
    overdue: buildTelegramRuntimeDefault("INSTALLMENT_OVERDUE_NOTICE"),
  };
...
const buildRepairReadyText = (item: any, settings: any) => {
  ...
  const defaults = buildTelegramRuntimeDefault("REPAIR_READY_FOR_PICKUP");
  const tplRaw =
...
const buildRepairStatusChangedText = (item: any, settings: any) => {
  ...
  const defaults = buildTelegramRuntimeDefault("REPAIR_STATUS_UPDATED");
  const tplRaw =
    String(
