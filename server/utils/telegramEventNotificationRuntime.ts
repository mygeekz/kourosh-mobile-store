...
import { buildMeliPayamakPatternTokens, buildTelegramRuntimeDefault, buildTelegramTemplatePreset } from "../../shared/messages";
...
type TelegramEventNotificationRuntimeDeps = {
  ...
  publishManagerEvent: (input: { topic: TelegramTopic; sourceEventType: string; text: string; meta?: { entityType?: string; entityId?: number; data?: Record<string, unknown> } }) => Promise<any>;
};
...
export const createTelegramEventNotificationRuntime = ({
  ...
  const enqueueTelegramToTopicTargets = async (
    ...
    try {
      // Manager delivery is independent from legacy topic chat filters.
      // The manager engine applies its own per-user preferences and permission gate.
      try {
        await publishManagerEvent({ topic, sourceEventType: typeKey, text, meta });
      } catch {
        // Per-manager notifications must never break the originating business flow.
      }
  ...
  const enqueueEventNotifications = async (
    ...
    if (eventType.startsWith("INSTALLMENT_")) {
      ...
      const saleMeta = eventType === "INSTALLMENT_COMPLETED"
        ? await getAsync(`SELECT actualSalePrice AS totalPrice FROM installment_sales WHERE id=? LIMIT 1`, [Number((p as any)?.saleId || 0)])
        : null;
      const installmentVars = {
        name: p.customerFullName,
        amount: formatPriceForSms(p.amountDue),
        dueDate: p.dueDate,
        saleId: String((p as any)?.saleId || ""),
        total: formatPriceForSms(Number((saleMeta as any)?.totalPrice || 0)),
      };
      const tokens = eventType === "INSTALLMENT_COMPLETED"
        ? (settings.meli_payamak_installment_settlement_pattern_id
            ? buildMeliPayamakPatternTokens("meli_payamak_installment_settlement_pattern_id", installmentVars)
            : [installmentVars.name, installmentVars.saleId, installmentVars.total])
        : [installmentVars.name, installmentVars.amount, installmentVars.dueDate];
      // SMS identifiers
      ...
      const values: Record<string, any> = {
        name: installmentVars.name,
        amount: installmentVars.amount,
        dueDate: installmentVars.dueDate,
        total: installmentVars.total,
        link,
        saleId: installmentVars.saleId,
      ...
      const tgTemplate = String(
        (settings as any)[tgTemplateKey] ||
          (eventType === "INSTALLMENT_COMPLETED" ? (settings as any).telegram_installment_completed_message : "") ||
          "",
      ).trim();
      ...
      const tgText = build(
        tgTemplate ||
          (eventType === "INSTALLMENT_COMPLETED"
            ? buildTelegramTemplatePreset("telegram_installment_settlement_message", "customer", "formal")
            : buildTelegramRuntimeDefault(eventType)),
        values,
      );
    ...
    if (eventType.startsWith("CHECK_")) {
      ...
      const build = (tpl: string, vals: Record<string, any>) =>
        safeReplaceTemplate(tpl, vals);
      const tgText = build(
        tgTemplate || buildTelegramRuntimeDefault(eventType),
        values,
      );
      if (mode === "sms" || mode === "both") {
        await enqueueOutbox({
  ...
  const sendCustomCustomerNotification = async (
    ...
    if (
      ...
      fallbackTemplate = buildTelegramTemplatePreset(tgKey, "customer", "formal");
    } else if (
      ...
      const c = (await getAsync(
        `SELECT c.id, c.fullName, c.phoneNumber, COALESCE(eb.currentBalance,0) AS currentBalance
           FROM customers c
           LEFT JOIN v_customer_effective_balance eb ON eb.customerId = c.id
          WHERE c.id=?`,
        [targetId],
      )) as any;
      if (!c) return;
      ...
      vars = {
        ...
        status: bal > 0 ? "بدهی" : bal < 0 ? "طلب" : "تسویه",
    ...
    if (
      ...
      const tokens = buildMeliPayamakPatternTokens(smsKey, vars);
      await enqueueOutbox({
        channel: "sms",
        provider,
        eventType,
