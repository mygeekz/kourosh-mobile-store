...
import { MESSAGE_BRAND_NAME } from "../../shared/messages";
import {
...
export const buildCollectionCenterAutomation = (
  ...
  const callScript =
    `سلام وقت بخیر، از ${MESSAGE_BRAND_NAME} تماس می‌گیرم. بابت ` +
    docTitle +
    "، مانده حساب شما " +
    outstandingText +
    " است. " +
  ...
  const smsText =
    ...
    " شما " +
    outstandingText +
    ` است. لطفاً زمان پرداخت را اعلام بفرمایید. ${MESSAGE_BRAND_NAME}`;
  const telegramText =
    "سلام " +
    customerName +
    " عزیز 🌿\nبرای " +
