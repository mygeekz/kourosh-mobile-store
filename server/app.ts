...
import { createDbBackup, startDailyBackupJob } from "./backup";
import {
  closeDbConnection,
...
const runPendingMigrationsWithSafetyBackup = async (db: any) => runPendingMigrations(db, undefined, {
  beforeApplyBatch: async ({ pendingFiles }) => {
    const backup = await createDbBackup({ prefix: 'pre-migration' });
    if (!backup.checksumVerified) throw new Error('Pre-migration backup checksum verification failed.');
    console.log(`[migrations] safety backup ${backup.fileName} created before ${pendingFiles.length} pending migration(s)`);
    return { backupFileName: backup.fileName, backupSha256: backup.sha256, checksumVerified: backup.checksumVerified };
  },
});
...
export const startKouroshServer = createKouroshServerStarter({
  ...
  port,
  getDbInstance,
  runPendingMigrations: runPendingMigrationsWithSafetyBackup,
