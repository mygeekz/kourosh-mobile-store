...
import { accountingMovementDelta } from "../accounting/accountingEngine";
import { recordAccountingAuditEvent } from "../accounting/accountingAuditTrail";
...
const getAutoRecognizedPaidAmount = (row: any, settlementPurchasePrice: number): number => {
  ...
  if (isInstallmentSale) {
    ...
    const installmentManualReceiptPaid = normalizeNumber(row?.installmentSaleManualReceiptAmount);
    const reconstructedCollected = Math.max(0, installmentDownPayment + installmentTransactionPaid + installmentCheckPaid + installmentManualReceiptPaid);
    const canonicalCollected = Math.max(reconstructedCollected, normalizeNumber(row?.installmentCollectedAmount));
    return Math.min(
...
const assertIdempotencyCompatible = (
  ...
  if (
    ...
    return reject(
      "idempotency-conflict",
      "این درخواست با اطلاعات تسویه دیگری ثبت شده است. اطلاعات را تازه‌سازی و دوباره بررسی کنید.",
  ...
  if (existing.dryRunId && request.dryRunId && existing.dryRunId !== request.dryRunId) {
    return reject(
      "idempotency-conflict",
      "این درخواست با وضعیت دیگری از تسویه ثبت شده است. اطلاعات را تازه‌سازی و دوباره بررسی کنید.",
...
const validateRequiredRequest = (
  ...
  if (unsafeFields.length > 0) {
    return reject(
      "blocking-validation-errors",
      "اطلاعات ارسالی تسویه معتبر نیست. صفحه را تازه‌سازی و دوباره تلاش کنید.",
      { fields: unsafeFields },
    );
  }

  if (!normalizeText(request.idempotencyKey)) {
    return reject("missing-idempotency-key", "اطلاعات امنیتی درخواست کامل نیست. صفحه را تازه‌سازی و دوباره تلاش کنید.");
  }

  if (!normalizeText(request.settlementDraftId) || !normalizeText(request.dryRunId)) {
    return reject("dry-run-not-found", "اطلاعات آماده‌سازی تسویه کامل نیست. صفحه را تازه‌سازی و دوباره بررسی کنید.");
  }

  if (request.managerConfirmation?.confirmed !== true) {
    return reject("missing-confirmation", "تایید مدیر برای ثبت تسویه الزامی است.");
  }
...
const validateDryRunSnapshot = (
  ...
  if (!snapshot) {
    return reject("dry-run-not-found", "اطلاعات آماده‌سازی تسویه پیدا نشد. صفحه را تازه‌سازی کنید.");
  }

  if (snapshot.lines.length === 0 || snapshot.amount <= 0) {
    return reject("missing-settlement-data", "اطلاعات قابل تسویه برای ثبت نهایی وجود ندارد.");
  }

  if (snapshot.blockingErrors.length > 0) {
    return reject(
      "blocking-validation-errors",
      "اطلاعات تسویه نیازمند اصلاح است و در وضعیت فعلی قابل ثبت نیست.",
      { blockingErrors: snapshot.blockingErrors },
    );
  }

  if (normalizeText(request.dryRunId) !== snapshot.dryRunId) {
    return reject("dry-run-stale", "اطلاعات تسویه تغییر کرده است. صفحه را تازه‌سازی و دوباره بررسی کنید.", {
  ...
  if (
    ...
    return reject("dry-run-stale", "مبلغ تاییدشده با اطلاعات فعلی تسویه همخوان نیست. صفحه را تازه‌سازی کنید.", {
  ...
  if (Array.isArray(request.confirmedLineIds) && request.confirmedLineIds.length > 0) {
    ...
    if (requestedLineIds.join(",") !== expected) {
      return reject("dry-run-stale", "ردیف‌های تاییدشده با اطلاعات فعلی تسویه همخوان نیستند. صفحه را تازه‌سازی کنید.", {
...
const insertLedgerRows = async (
  ...
  // Canonical accounting is movement-based. Never chain a new settlement from
  // the cached balance of the last inserted row: a backdated/manual entry can
  // make that cache stale. Keep one canonical running total for this batch.
  let runningBalance = await getPartnerLedgerCanonicalBalance(partnerId);
  for (const line of snapshot.lines) {
    const debit = line.amount;
    const credit = 0;
    runningBalance += accountingMovementDelta("partner", { debit, credit });
    ...
    const result = await runAsync(
      ...
      [
        ...
        submittedAt,
        submittedAt,
        `تسویه مدیر برای ${line.label}${line.identifier ? ` (${line.identifier})` : ""}`,
        debit,
        credit,
        balance,
        REFERENCE_TYPE,
  ...
  // Rebuild chronological row caches inside the same transaction. This makes
  // settlement insertion safe even when submittedAt is not the newest ledger date.
...
export const submitPartnerSettlementAtomic = async (
  ...
  if (!PARTNER_SETTLEMENT_ALLOWED_ROLES.includes(roleName as PartnerSettlementSubmitterRole)) {
    return reject("forbidden", "ثبت تسویه فقط برای مدیر یا سرپرست مجاز است.");
  }
  ...
  try {
    ...
    if (transactionDryRunError || !transactionSnapshot) {
      await execAsync("ROLLBACK");
      return transactionDryRunError || reject("dry-run-not-found", "اطلاعات تسویه هنگام ثبت تغییر کرد. دوباره بررسی کنید.");
    }
    ...
    await recordAccountingAuditEvent({
      eventType: "partner_settlement_submit",
      entityType: "partner_settlement",
      entityId: partnerId,
      accountType: "partner",
      accountId: partnerId,
      actor: { userId: user.id ?? null, username: user.username ?? null, role: roleName || null },
      reason: "manager_confirmed_atomic_settlement",
      before: { dryRunId: transactionSnapshot.dryRunId, settlementDraftId: transactionSnapshot.settlementDraftId, lineIds: transactionSnapshot.lineIds, amount: transactionSnapshot.amount },
      after: { settlementId, ledgerEntryIds, idempotencyKey, settlementFingerprint, sourceFingerprint },
      source: "partner_settlement_atomic_v341",
    });
    ...
    return reject("transaction-rolled-back", "ثبت تسویه کامل نشد و هیچ تغییر نیمه‌کاره‌ای ذخیره نشد.", {
