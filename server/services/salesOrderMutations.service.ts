...
export async function removeSalesOrder(
  ...
  return deleteSalesOrderById(orderId, reason);
}
...
const normalizeSalesReturnItem = (
  ...
  // Invoice line description/price are intentionally not accepted from the client.
  // The repository derives them from the immutable persisted sales_order_items row.
  return { itemType, itemId, quantity };
};
...
export function normalizeSalesReturnPayload(
  ...
  if (!reason) {
    throw new Error('ثبت دلیل مرجوعی الزامی است.');
  }
