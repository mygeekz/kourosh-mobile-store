...
const syncTelegramMenu = async (settings: Record<string, unknown>, deps: SyncDeps) => {
  ...
  return updateStatus({ phase: "PUBLIC_URL_READY", telegramMenu: "error", message: result.message || "همگام‌سازی منوی تلگرام انجام نشد." });
};
...
export const createMiniAppPublicUrlSyncService = (overrides: Partial<SyncDeps> = {}) => {
  ...
  const sync = async (input: SyncInput) => {
    ...
    if (!normalizedPublicUrl) {
      updateStatus({ phase: "ERROR", provider, tunnel: "error", message: "نشانی عمومی Mini App معتبر نیست." });
    ...
    if (currentMode === "self_hosted" || currentMode === "relay" || currentMode === "stable_tunnel") {
      const protectedStatus = updateStatus({
        ...
        publicUrl: null,
        hostname: null,
        message: `تنظیمات فعلی Mini App در حالت ${currentMode} حفظ شد و با تونل موقت جایگزین نشد.`,
      });
    ...
    if (resolveMiniAppPublicAccessMode(saved) !== "external_tunnel" || validateTelegramMiniAppPublicUrl(saved.telegram_miniapp_public_url) !== normalizedPublicUrl) {
      ...
      updateStatus({ phase: "ERROR", gateway: "error", message: "تنظیمات نشانی عمومی Mini App به‌درستی ذخیره نشد." });
    ...
    try {
      ...
      updateStatus({ phase: "ERROR", gateway: "error", message: "همگام‌سازی تنظیمات اتصال Mini App انجام نشد." });
      throw error;
    }
    ...
    if (!publicHealth.ok) {
      const pendingStatus = updateStatus({
        ...
        message: `دسترسی عمومی Mini App هنوز آماده پاسخ‌گویی نیست${publicHealth.status ? ` (کد ${publicHealth.status})` : ""}.`,
      });
    ...
    if (diagnosticQuickTunnel) {
      const diagnosticStatus = updateStatus({
        ...
        telegramMenu: "idle",
        message: "تونل موقت فقط برای بررسی اتصال استفاده شد و نشانی اصلی Mini App یا منوی تلگرام تغییر نکرد.",
      });
      return {
        success: true,
        ready: true,
