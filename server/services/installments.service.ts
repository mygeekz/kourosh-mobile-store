...
import { buildMeliPayamakPatternTokens } from '../../shared/messages';
        ...
        if (unpaidAfter === 0) {
          ...
          if (s?.customerPhoneNumber) {
            ...
            const settlementVars = {
              name: s.customerFullName,
              saleId: String(s.saleId),
              total: formatPriceForSms(s.totalPrice),
            };
            const legacySettlementTokens = [settlementVars.name, settlementVars.saleId, settlementVars.total];
            const tokens = provider === 'meli_payamak' && settings.meli_payamak_installment_settlement_pattern_id
              ? buildMeliPayamakPatternTokens("meli_payamak_installment_settlement_pattern_id", settlementVars)
              : legacySettlementTokens;
            const meliBodyId =
            ...
            const telegramTemplate = (settings as any).telegram_installment_settlement_message || settings.telegram_installment_completed_message;
              ...
              case 'telegram': {
                ...
                if (botToken && chatId && telegramTemplate) {
                  ...
                  const values: Record<string, string> = {
                    name: settlementVars.name,
                    saleId: settlementVars.saleId,
                    total: settlementVars.total,
                    amount: settlementVars.total,
                  };
