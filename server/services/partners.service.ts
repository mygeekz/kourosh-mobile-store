...
export const partnersService = {
  ...
  createLedgerEntry: (partnerId: number, payload: LedgerEntryPayload, user?: any) =>
    partnersRepo.addPartnerLedgerEntry(
      partnerId,
      payload,
      user ? { userId: user.id, username: user.username, role: user.roleName || user.role } : null,
    ),


  createPhoneSettlementPayment: async (partnerId: number, phoneId: number, payload: LedgerEntryPayload, user?: any) => {
    const timeline = await partnersRepo.getPartnerPhoneSettlementTimeline(partnerId, phoneId, { page: 1, pageSize: 10, includeMeta: true });
    if (!timeline?.summary) throw new AppError("گوشی فروخته‌شده برای این همکار یافت نشد.", 404);
    const amount = Number(payload?.debit || 0);
    const remaining = Math.max(0, Number(timeline.summary.phoneSettlementBalance || 0));
    if (!Number.isFinite(amount) || amount <= 0) throw new AppError("مبلغ پرداخت باید مثبت باشد.", 400);
    if (amount > remaining + 0.00001) {
      throw new AppError(`مبلغ پرداخت از مانده سرمایه این گوشی بیشتر است. مانده: ${remaining}`, 409);
    }
    const description = String(payload?.description || "").trim();
    if (description.length < 3) throw new AppError("شرح پرداخت حداقل ۳ کاراکتر الزامی است.", 400);
    return partnersRepo.addPartnerPhoneSettlementLedgerEntry(
      partnerId,
      phoneId,
      { ...payload, debit: amount, credit: 0, referenceType: undefined, referenceId: undefined, referenceMode: undefined } as any,
      user ? { userId: user.id, username: user.username, role: user.roleName || user.role } : null,
    );
  },
  ...
  updateLedgerEntry: (
    ...
    user?: any,
  ) => partnersRepo.updatePartnerLedgerEntry(
    partnerId,
    entryId,
    payload,
    user ? { userId: user.id, username: user.username, role: user.roleName || user.role } : null,
  ...
  deleteLedgerEntry: (partnerId: number, entryId: number, user?: any) =>
    partnersRepo.deletePartnerLedgerEntry(
      partnerId,
      entryId,
      user ? { userId: user.id, username: user.username, role: user.roleName || user.role } : null,
    ),
