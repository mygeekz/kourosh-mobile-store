...
const assertExistingSignoffCompatible = (
  ...
  if (existing.settlementId && requestedSettlementId && existing.settlementId !== requestedSettlementId) {
    return reject(
      'signoff-idempotency-conflict',
      'این تایید مدیر قبلاً برای تسویه دیگری ثبت شده است.',
  ...
  if (existing.settlementFingerprint && requestedFingerprint && existing.settlementFingerprint !== requestedFingerprint) {
    return reject(
      'signoff-idempotency-conflict',
      'اطلاعات این تایید مدیر با تسویه فعلی همخوان نیست. صفحه را تازه‌سازی کنید.',
...
const validateRequest = (
  ...
  if (unsafeFields.length > 0) {
    return reject('missing-signoff-data', 'اطلاعات تایید مدیر معتبر نیست. صفحه را تازه‌سازی و دوباره تلاش کنید.', { fields: unsafeFields });
  }

  if (!normalizeText(request.settlementId) || !normalizeText(request.idempotencyKey) || !normalizeText(request.settlementFingerprint)) {
    return reject('missing-signoff-data', 'اطلاعات لازم برای ثبت تایید مدیر کامل نیست. صفحه را تازه‌سازی کنید.');
  }

  if (request.managerSignoff?.confirmed !== true) {
    return reject('missing-signoff-confirmation', 'ثبت تایید نیازمند تایید صریح مدیر است.');
  }
...
export const persistPartnerSettlementManagerSignoff = async (
  ...
  if (!ALLOWED_ROLES.includes(roleName as (typeof ALLOWED_ROLES)[number])) {
    return reject('forbidden', 'ثبت تایید تسویه فقط برای مدیر یا سرپرست مجاز است.');
  }
  ...
  try {
    ...
    if (submittedSettlementRows.length === 0) {
      await execAsync('ROLLBACK');
      return reject('settlement-not-found', 'تسویه ثبت‌شده مربوط به این تایید در دفتر همکار پیدا نشد.');
    }
    ...
    return reject('transaction-rolled-back', 'ثبت تایید مدیر کامل نشد و هیچ تغییر نیمه‌کاره‌ای ذخیره نشد.', {
