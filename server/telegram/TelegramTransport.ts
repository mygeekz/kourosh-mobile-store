export type TelegramTransportMode = "disabled" | "direct" | "system" | "proxy" | "relay";
...
export type TelegramTransportResult = {
  ...
  errorCode?:
    ...
    | "TELEGRAM_API_ERROR";
  details?: { httpStatus?: number; rawResponseText?: string; durationMs?: number; transportRoute?: string; transportRouteLabel?: string };
};
...
export const resolveTelegramTransportMode = (settings: Record<string, unknown>): TelegramTransportMode => {
  ...
  if (raw === "disabled" || raw === "direct" || raw === "system" || raw === "proxy" || raw === "relay") return raw;
