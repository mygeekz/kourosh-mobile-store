...
export const humanizeSmsError = (rawInput: unknown): SmsHumanError => {
  ...
  if (!raw) {
    return {
      ...
      action: 'یک بار دیگر بررسی ارسال را انجام بده؛ اگر خطا تکرار شد، گزارش خطاها را بررسی کن.',
  ...
  if (includesAny(text, ['bodyid', 'body id', 'pattern id', 'patternid', 'شناسه پترن', 'شناسه الگو', 'کد بدنه', 'الگو نامعتبر', 'bodyid نامعتبر', /(^|[^0-9])-108([^0-9]|$)/])) {
    return {
      ...
      action: 'شناسه قالب را مستقیم از پنل ملی‌پیامک کپی کن، مطمئن شو قالب تأیید و برای همان حساب یا خط خدماتی فعال است؛ سپس بررسی ارسال را انجام بده.',
      technical: raw,
      severity: 'danger',
    };
  }
  ...
  if (includesAny(text, ['credit', 'balance', 'شارژ', 'اعتبار', 'موجودی', 'insufficient', 'کافی نیست'])) {
    return {
      ...
      action: 'اعتبار پنل پیامک را افزایش بده و بعد از شارژ، بررسی ارسال را دوباره انجام بده.',
      technical: raw,
      severity: 'warning',
    };
  }

  if (includesAny(text, ['فهرست مجاز', 'allowlist', 'شماره فعالِ مجاز', 'شماره‌های مجاز'])) {
    return {
      title: 'مقصد تست در فهرست مجاز نیست',
      message: raw,
      action: 'از بخش «شماره‌های مجاز ارسال تست» در تنظیمات پیامک، شماره را اضافه و فعال کن؛ سپس تنظیمات را ذخیره و دوباره ارسال تست را انجام بده.',
      technical: raw,
      severity: 'warning',
    };
  }
  ...
  if (includesAny(text, ['recipient', 'mobile', 'phone', 'number', 'شماره', 'گیرنده', 'موبایل', 'invalid number', 'not valid'])) {
    return {
      ...
      action: 'شماره را با فرمت صحیح مثل 09123456789 بررسی کن و دوباره بررسی ارسال را انجام بده.',
      technical: raw,
      severity: 'warning',
    };
  }
  ...
  if (includesAny(text, ['500', '502', '503', '504', 'server error', 'internal', 'خطای سرور'])) {
    return {
      ...
      action: 'چند دقیقه بعد دوباره بررسی کن. اگر تکرار شد، وضعیت سرویس‌دهنده یا پنل پیامک را بررسی کن.',
      technical: raw,
      severity: 'danger',
    };
  }
