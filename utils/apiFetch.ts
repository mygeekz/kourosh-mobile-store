...
const DEV_BACKEND_ORIGIN = (typeof import.meta !== 'undefined' && (import.meta as any)?.env?.VITE_API_BASE_URL)
...
const shouldUseBackendFallback = (url: string) => {
  ...
  return ['5173', '4173'].includes(window.location.port) || window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
};
...
const createFallbackResponse = async (url: string, options: RequestInit = {}): Promise<Response | null> => {
  ...
  if (url.startsWith('/api/reports/expenses-summary')) {
    return createJsonResponse({ success: true, data: { total: 0, byCategory: [] } });
  }
...
export const apiFetch = async (url: string, options: RequestInit = {}): Promise<Response> => {
  ...
  const primaryUrl = shouldUseBackendFallback(url) ? buildAbsoluteUrl(DEV_BACKEND_ORIGIN, url) : url;
  const request = (requestUrl: string) => fetch(requestUrl, {
    ...options,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (shouldUseBackendFallback(url) && (response.status === 404 || response.status === 502 || response.status === 503)) {
        const retryResponse = await request(buildAbsoluteUrl(DEV_BACKEND_ORIGIN, url));
