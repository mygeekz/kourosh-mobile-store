...
export async function buildReportWorkbook({ title, element }: Args) {
  ...
  // ExcelJS is intentionally loaded on demand. This keeps the reports shell
  // lightweight and moves the export engine to a dedicated async vendor chunk.
  ...
  const sheet = wb.addWorksheet('Report', {
    ...
    const setFont = (cell: any, patch: Record<string, any> = {}) => {
      const prev = (cell.font || {}) as Record<string, any>;
      cell.font = {
        name: BASE_FONT,
    ...
    if (rows.length && numericCols.some(Boolean)) {
      ...
      totalRow.height = 20;
      totalRow.eachCell((cell: Cell, colNumber: number) => {
    ...
    sheet.eachRow({ includeEmpty: true }, (r: Row) => {
      r.eachCell({ includeEmpty: true }, (cell: Cell) => {
    ...
    for (let c = 1; c <= colCount; c++) {
      let max = 10;
      sheet.getColumn(c).eachCell({ includeEmpty: true }, (cell: Cell) => {
        const v = cell.value;
