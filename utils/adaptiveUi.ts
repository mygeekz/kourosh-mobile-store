...
export const getBalanceLabel = (state: BalanceState, subject: BalanceSubject = 'customer') => {
  ...
  if (state === 'positive') return 'این مشتری بدهکار است';
  if (state === 'negative') return 'این مشتری بستانکار است';
...
export const getBalanceActionLabel = (state: BalanceState, subject: BalanceSubject = 'customer') => {
  ...
  if (state === 'positive') return 'ثبت دریافت';
  if (state === 'negative') return 'ثبت پرداخت / شارژ حساب';
