...
// کتابخانه‌های سنگین مثل exceljs / jspdf / html2canvas فقط هنگام کلیک روی «خروجی»
...
const downloadCsvFallback = (filename: string, rows: Record<string, any>[], columns: ExcelColumn<any>[]) => {
  const csvName = filename.replace(/\.xlsx$/i, '.csv');
  const header = columns.map((column) => column.header);
  const body = rows.map((row) => header.map((key) => row[key]));
  const content = [header, ...body].map((row) => row.map(csvEscape).join(',')).join('\n');
  downloadBlob(new Blob([`\ufeff${content}`], { type: 'text/csv;charset=utf-8' }), csvName);
};
...
export function exportToExcel<T extends Record<string, any>>(
  ...
  void (async () => {
    ...
    try {
      ...
      downloadCsvFallback(outName, mappedRows, columns);
    }
  })();
}
