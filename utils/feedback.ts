...
const actionContextMap: Array<{ match: RegExp; title: string; hint?: string }> = [
  ...
  { match: /ذخیره پروفایل شخصی|ذخیره مشخصات حساب/i, title: 'ذخیره مشخصات حساب انجام نشد.', hint: 'نام و نام خانوادگی را بررسی کنید و دوباره تلاش کنید.' },
...
const endpointContextMap: Array<{ match: RegExp; title: string; hint?: string }> = [
  ...
  { match: /\/api\/me\/(profile|upload-avatar|change-password)/i, title: 'عملیات حساب کاربری انجام نشد.', hint: 'فایل انتخابی یا اطلاعات رمز عبور را بررسی کنید.' },
...
export function humanizeErrorMessage(raw: unknown, context?: { endpoint?: string; action?: string }): string {
  ...
  if (/openssl|certificate|pkcs|x509|گواهی محلی/i.test(text)) {
    if (/^ساخت گواهی محلی انجام نشد/.test(text)) {
      return sentence(text);
    }
    return 'ساخت گواهی محلی انجام نشد؛ PowerShell/OpenSSL، دسترسی ویندوز و نام دامنه را بررسی کنید.';
  }
